# nix/web.nix — X19 Web Dashboard (Vite/React) frontend build
{ x19NpmLib, ... }:
x19NpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @x19/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../x19_cli/web_dist for the
