...
#
# `services.x19` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  x19Home, programs.x19 (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "x19-config-attrs";
    description = "X19 YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $X19_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    }:
    {
      ...
      package = mkOption {
        ...
        description = "The x19 package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The X19 configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `x19 config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $X19_HOME/.env. X19 reads that file at each start, with
          load_x19_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."x19/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that X19 refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          X19 reads SOUL.md and memories/ from X19_HOME, so put those
          files in `x19HomeFiles`.
      ...
      x19HomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into X19_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          X19 reads SOUL.md and the memory files from X19_HOME and not
          from the working directory. Declare those files here, or X19
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the x19 plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. X19 discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          x19_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as x19 (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in x19's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `x19 gateway`.";
      };
      ...
      # ── The backend: `x19 serve` or `x19 dashboard` ──────────────
      # `x19 serve` and `x19 dashboard` are the same entry point,
      # x19_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that X19 Desktop
      ...
      # controls an external gateway, with `x19 gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that X19
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            X19_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. X19 Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."x19/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `x19` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.x19.installPackage was removed. X19 now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.x19.enable = ${lib.boolToString (value != false)};  # the x19 CLI, and X19_HOME for your shells
        programs.x19.desktop.enable = true;  # the desktop application

      `services.x19` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "x19-config.yaml" (
    ...
    pkgs.writeShellScript "x19-env-merge" ''
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "x19: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # X19_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and X19 make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${x19Home}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. X19
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${x19Home}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${x19Home}/config.yaml
            ${run}chmod ${modes.config} ${x19Home}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "x19-managed" managedSystem} ${x19Home}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${x19Home}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${x19Home}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${x19Home}/auth.json"
        else
          ''
            if [ ! -e ${x19Home}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${x19Home}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree x19Home cfg.x19HomeFiles}
      ...
      ${run}find ${x19Home}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "x19: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${x19Home}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  #
  # `exec` on the last line keeps x19 as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "x19-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "x19-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "x19-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          X19_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export X19_DASHBOARD_SESSION_TOKEN

          if [ -z "$X19_DASHBOARD_SESSION_TOKEN" ]; then
            echo "x19-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "x19-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "x19-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "x19-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "x19-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "x19-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "x19-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "X19 web dashboard and desktop backend"
    else
      "X19 backend for X19 Desktop";

  # The environment that each X19 process needs, from either module.
  #
  # managedSystem gives the value of X19_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give X19 an identity and a memory, use
          ${optionPath}.x19HomeFiles instead. Those files go to
          X19_HOME. X19 reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of X19_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
