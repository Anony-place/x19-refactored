...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # X19 derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            x19 = prev.x19.overrideAttrs (_old: {
              X19_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $X19_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$X19_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        x19 = prev.x19.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "x19-env" {
    x19 = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "x19-editable-env" {
    x19 = dependency-groups;
  };
}
