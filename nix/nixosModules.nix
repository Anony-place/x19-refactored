# nix/nixosModules.nix — the NixOS module for x19
#
...
#
# Container mode: x19 runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $X19_HOME/.env
# and read by x19 at startup — no container recreation needed for env changes.
#
# Tool resolution: the x19 wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.x19;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      x19 = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      x19Home = "${cfg.stateDir}/.x19";
      ...
      # X19_HOME via addToSystemPackages, so they can save settings through the
      ...
      # x19 user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "x19-container-entrypoint" ''
        set -eu

        X19_UID="''${X19_UID:?X19_UID must be set}"
        X19_GID="''${X19_GID:?X19_GID must be set}"

        # ── Group: ensure a group with GID=$X19_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$X19_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="x19"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$X19_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$X19_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$X19_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$X19_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="x19"
          TARGET_HOME="/home/x19"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$X19_UID" -g "$X19_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$X19_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$X19_UID:$X19_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure X19_HOME is owned by the target user.
        ...
        if [ -n "''${X19_HOME:-}" ] && [ -d "$X19_HOME" ]; then
          find "$X19_HOME" \! -user "$X19_UID" -exec chown "$X19_UID:$X19_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/x19-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/x19-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/x19 ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/x19
          chmod 0440 /etc/sudoers.d/x19
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$X19_UID" --regid="$X19_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $X19_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "x19-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        x19_bin=${containerDataDir}/current-package/bin/x19
      '';

      # Default: /var/lib/x19/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the x19 group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit x19Home; };
    ...
    in
    {
      options.services.x19 =
        common.sharedOptions {
          defaultPackage = x19;
          defaultPackageText = lib.literalExpression "x19.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/x19";
              description = "State directory. Contains .x19/ subdir (X19_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the x19 CLI to environment.systemPackages and export
                X19_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.x19 symlink to the service
                  stateDir. These users are automatically added to the x19 group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.x19.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the x19 CLI to system PATH and export X19_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.x19/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.X19_HOME = x19Home;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.x19";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.x19.workingDirectory;
                optionPath = "services.x19";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.x19: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the x19 user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.x19: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed x19
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure x19 is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${x19Home}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${x19Home}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."x19-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${x19Home} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${x19Home} ${cfg.workingDirectory}
                  ...
                  find ${x19Home} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${x19Home}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${x19Home}/$_subdir"
                    chmod 2770 "${x19Home}/$_subdir"
                    find "${x19Home}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg x19Home;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${x19Home}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${x19Home}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${x19Home}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.x19";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${x19Home}" ]; then
                                rm -f "${symlinkPath}"
                                echo "x19: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.x19 -> stateDir/.x19 for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.x19 = {
              ...
              # $X19_HOME/.env by the activation script. load_x19_dotenv()
              ...
              preStart = lib.mkIf lingerEnabled ''
                ...
                if [ ! -S "/run/user/$(id -u)/bus" ]; then
                  echo "x19: no user bus at /run/user/$(id -u)/bus after 10s;" \
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.x19 = {
              description = "X19 Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
                "network-online.target"
              ]
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  X19_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  X19_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env X19_UID="$X19_UID" \
                    --env X19_GID="$X19_GID" \
                    --env X19_HOME=${containerDataDir}/.x19 \
                    --env X19_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/x19 gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
