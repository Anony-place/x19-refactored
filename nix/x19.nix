...
let
  ...
  # lib.nix) — the wrapper exposes them via X19_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from X19_BUNDLED_PLUGINS.
  ...
  # at by X19_BUNDLED_LOCALES so the wrapped binary always resolves human
...
stdenv.mkDerivation (finalAttrs: {
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${x19Venv}/bin/${name} $out/bin/${name} \
          ...
          --set X19_BUNDLED_SKILLS $out/share/x19/skills \
          --set X19_OPTIONAL_SKILLS $out/share/x19/optional-skills \
          --set X19_BUNDLED_PLUGINS $out/share/x19/plugins \
          --set X19_BUNDLED_LOCALES $out/share/x19/locales \
          --set X19_OPTIONAL_MCPS $out/share/x19/optional-mcps \
          --set X19_WEB_DIST $out/share/x19/web_dist \
          --set X19_TUI_DIR $out/ui-tui \
          --set-default X19_BIN $out/bin/x19 \
          --set X19_PYTHON ${x19Venv}/bin/python3 \
          --set X19_NODE ${lib.getExe x19NpmLib.nodejs}${
  ...
  passthru =
    ...
    in
    {
      ...
      x19Desktop = callPackage ./desktop.nix {
        inherit x19NpmLib electron;
        x19Agent = finalAttrs.finalPackage;
      };

      devShellHook = ''
        export X19_PYTHON=${devPython}/bin/python3
      '';
