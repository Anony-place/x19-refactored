# nix/desktop.nix — X19 Desktop (Electron) app build + wrapper
#
# `x19Agent` is the fully-built `.#default` package — it ships the
# `x19` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `X19_DESKTOP_X19` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing X19 CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # X19_HOME and X19_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "x19-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "x19-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = x19NpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "x19-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe x19NpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/x19-desktop $out/bin
    cp -r ${renderer}/* $out/share/x19-desktop/
    ...
    substituteInPlace $out/share/x19-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/x19-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # X19_DESKTOP_X19 to the absolute path of the nix-built `x19`
    # binary so the desktop's resolver step 4 ("existing X19 CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/x19-desktop \
      --add-flags "$out/share/x19-desktop" \
      --set X19_DESKTOP_X19 "${lib.getExe x19Agent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/x19.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../x19_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/x19-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/x19.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/x19.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for X19";
    homepage = "https://github.com/Anony-place/x19-refactored";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "x19-desktop";
  };
}
