# nix/packages.nix — X19 package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./x19.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.x19Tui;
        web = full.x19Web;
        desktop = full.x19Desktop;

        update-npm-lockfile = full.x19NpmLib.updateNpmLockfile;
      };
    };
}
