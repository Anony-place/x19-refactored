...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.x19 = programs;
              services.x19 = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.x19. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.x19);
      ...
      configKeys = pkgs.runCommand "x19-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${x19Venv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "x19-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "x19-build-package" { } ''
          echo "PASS: package built at ${x19}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "x19-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.X19_TEST = "1";
                environmentFiles = [ "/run/secrets/x19-env" ];
                x19HomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "x19" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "x19" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.x19AgentSetup.data;

            failures =
              lib.optional (names != [
                "x19"
                "x19-backend"
              ]) "expected x19 + x19-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/x19 gateway" (argvOf "x19")
              ) "gateway process does not run `x19 gateway`: ${argvOf "x19"}"
              ++ lib.optional (
                !lib.hasInfix "bin/x19 serve" (argvOf "x19-backend")
              ) "backend process does not run `x19 serve`: ${argvOf "x19-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "x19-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/x19-check/.x19" (envOf n)) names
              ) "gateway and backend must share one X19_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.X19_HOME or null != "/home/x19-check/.x19"
              ) "programs.x19.enable must export X19_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "x19-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/x19-check/.x19/SOUL.md" activation
              ) "x19HomeFiles must install into X19_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads X19_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "X19_MANAGED=home-manager" (envOf n)) names
              ) "processes must report X19_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "x19-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "x19-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            cases = [
              ...
              {
                name = "x19HomeFiles needs no directory";
                ok = accepts { x19HomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "x19-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one X19_HOME ───────────────
        # `programs.enable` exports X19_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.x19
        # while the services use the X19_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/x19-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                x19Home = "/home/x19-check/.x19-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "x19-desktop") cfg.home.packages;
            ...
            # second X19 runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "x19") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.x19-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.x19-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one x19-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "X19_HOME" != "/home/x19-check/.x19-work"
              ) "the launcher must carry X19_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "X19_HOME")})"
              ++ lib.optional (
                setValue "X19_MANAGED" != "home-manager"
              ) "the launcher must report X19_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "X19_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "X19_DESKTOP_X19" != "${lib.head agentPackages}/bin/x19"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "X19_DESKTOP_X19")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "X19_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "X19_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "X19_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "X19_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "X19_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "x19-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares X19_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set X19_HOME" wrapper
              ) "the launcher must carry X19_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "X19_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "X19_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? x19-backend || cfg.launchd.agents ? x19-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "x19-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `x19`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.x19.enable = true;";
              }
              {
                value = false;
                expect = "programs.x19.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "x19-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "x19-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "x19" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."x19-setup".text;

            failures =
              lib.optional (names != [
                "x19"
                "x19-backend"
              ]) "expected x19 + x19-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/x19 gateway" (execOf "x19")
              ) "gateway unit does not run `x19 gateway`: ${execOf "x19"}"
              ++ lib.optional (
                !lib.hasInfix "bin/x19 dashboard" (execOf "x19-backend")
              ) "backend unit does not run `x19 dashboard`: ${execOf "x19-backend"}"
              ++ lib.optional (
                units.x19.environment.X19_HOME != units.x19-backend.environment.X19_HOME
              ) "gateway and backend must share one X19_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/x19/.x19/SOUL.md" activation
              ) "x19HomeFiles must install into X19_HOME";
          ...
          in
          pkgs.runCommand "x19-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs x19, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.x19-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/x19 serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec x19 directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "x19-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "x19-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec x19, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/x19 dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "x19-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.x19;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.x19;
          ...
          in
          pkgs.runCommand "x19-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $X19_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          ...
          in
          pkgs.runCommand "x19-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'X19_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--x19-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "x19-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "x19-package-contents" { } ''
          set -e
          ...
          ${x19}/bin/x19 --version 2>&1 | grep -qi "x19" || (echo "FAIL: version check"; exit 1)
        ...
        cli-commands = pkgs.runCommand "x19-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking x19 --help ==="
          ${x19}/bin/x19 --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${x19}/bin/x19 --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "x19-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${x19}/share/x19/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${x19}/share/x19/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "X19_BUNDLED_SKILLS" ${x19}/bin/x19 || \
            (echo "FAIL: X19_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: X19_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${x19}/share/x19/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${x19}/share/x19/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "X19_OPTIONAL_SKILLS" ${x19}/bin/x19 || \
            (echo "FAIL: X19_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, X19_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "x19-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${x19}/share/x19/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${x19}/share/x19/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "X19_BUNDLED_PLUGINS" ${x19}/bin/x19 || \
            (echo "FAIL: X19_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: X19_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "x19-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${x19}/share/x19/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${x19}/share/x19/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${x19}/share/x19/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "X19_BUNDLED_LOCALES" ${x19}/bin/x19 || \
            (echo "FAIL: X19_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: X19_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + X19_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && X19_BUNDLED_LOCALES=${x19}/share/x19/locales \
            ${x19Venv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with X19_BUNDLED_LOCALES set"; exit 1)
        ...
        # X19_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "x19-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${x19}/share/x19/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${x19}/share/x19/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "X19_OPTIONAL_MCPS" ${x19}/bin/x19 || \
            (echo "FAIL: X19_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: X19_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${x19}/bin/x19 mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: x19 mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "x19-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${x19}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${x19}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "X19_TUI_DIR" ${x19}/bin/x19 || \
            (echo "FAIL: X19_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: X19_TUI_DIR set in wrapper"
        ...
        # Verify X19_NODE is set in wrapper and points to Node 26+
        # (X19 pins its toolchain to Node 26 everywhere)
        x19-node = pkgs.runCommand "x19-node-version" { } ''
          set -e
          echo "=== Checking X19_NODE in wrapper ==="
          grep -q "X19_NODE" ${x19}/bin/x19 || \
            (echo "FAIL: X19_NODE not set in wrapper"; exit 1)
          echo "PASS: X19_NODE present in wrapper"

          X19_NODE=$(sed -n "s/^export X19_NODE='\(.*\)'/\1/p" ${x19}/bin/x19)
          test -x "$X19_NODE" || (echo "FAIL: X19_NODE=$X19_NODE not executable"; exit 1)
          echo "PASS: X19_NODE executable at $X19_NODE"

          NODE_MAJOR=$("$X19_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, X19 requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All X19_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify X19_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "x19-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(X19_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking X19_MANAGED guards ==="
          check_blocked "config set" ${x19}/bin/x19 config set model foo
          check_blocked "config edit" ${x19}/bin/x19 config edit
        ...
        extra-python-packages = let
          ...
          x19WithExtra = x19.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "x19-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${x19WithExtra}/bin/x19 || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${x19WithExtra}/bin/x19 || \
          ...
          if grep -q "PYTHONPATH" ${x19}/bin/x19; then
        ...
        extra-dependency-groups = let
          x19WithGroups = x19.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "x19-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${x19WithGroups}"
          echo "venv: ${x19WithGroups.x19Venv}"
        ...
        messaging-variant = pkgs.runCommand "x19-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.x19Venv}/bin/python3 -c \
        ...
        in pkgs.runCommand "x19-config-roundtrip" {
          ...
          merge_and_load() {
            local x19_home="$1"
            export X19_HOME="$x19_home"
            ${configMergeScript} ${nixSettings} "$x19_home/config.yaml"
            ${x19Venv}/bin/python3 -c '
import json, sys
