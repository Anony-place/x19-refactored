...
from x19_cli.config import atomic_config_write, cfg_get
...
_BUSY_MODE_BEHAVIOR = {
    "queue": ("queues for next turn", "Messages will be queued for the next turn while X19 is busy."),
    "steer": ("steers into current run (after next tool call)",
              "Messages will be steered into the current run (after the next tool call)."),
    "interrupt": ("interrupts current run", "Messages will interrupt the current run while X19 is busy."),
}
...
def _spawn_detached_update(x19_cmd, output_path, exit_code_path) -> None:
    """Spawn ``x19 update --gateway`` detached so it survives the gateway restart it may trigger.
    ...
    this interpreter (not venv\\Scripts\\x19.exe — that shim holds its own file open, and the
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "x19_cli.main", "update", "--gateway"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **windows_detach_popen_kwargs())
        return
    x19_cmd_str = " ".join(shlex.quote(part) for part in x19_cmd)
    update_cmd = (
        f"PYTHONUNBUFFERED=1 {x19_cmd_str} update --gateway"
...
class GatewaySlashCommandsMixin(
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """The prefix users can always type to reach X19 commands (adapter ``typed_command_prefix``,
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "pause":
            ...
            return f"✓ {name} paused. Resume with `/platform resume {name}` or `x19 gateway restart` to reset."
        if not queued:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _x19_home / name, build(), indent=None)
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running X19 version."""
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        # (NOT `x19 skills diff <name>`, which diffs a bundled skill against its stock version).
        if sub == "diff" and len(out) > 3000:
            pending_id = args[1] if len(args) > 1 else "<id>"
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.x19/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while X19 is working."""
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_x19_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return ("No skill bundles installed.\nCreate one on the host with:\n"
                    "  `x19 bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        protect privacy; ``x19 debug share`` from the CLI does full uploads."""
        from x19_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
        ...
        # ``get_x19_home()`` and uploads them to a public paste. Losing the contextvar override
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update — spawn ``x19 update`` detached (``setsid``) so it survives the gateway
        restart it may trigger; marker files let this or the next gateway process notify the user."""
        import json
        from gateway.run import _x19_home, _resolve_x19_bin
        ...
        if src.platform not in self._UPDATE_ALLOWED_PLATFORMS:
            try:
                ...
                return t("gateway.update.platform_not_messaging")
        if is_managed():
            return f"✗ {format_managed_message('update X19')}"
        if not (Path(__file__).parent.parent.resolve() / '.git').exists():
            return t("gateway.update.not_git_repo")
        x19_cmd = _resolve_x19_bin()
        if not x19_cmd:
            return t("gateway.update.x19_cmd_not_found")
        pending_path = _x19_home / ".update_pending.json"
        output_path = _x19_home / ".update_output.txt"
        exit_code_path = _x19_home / ".update_exit_code"
        pending = {
        ...
        try:
            _spawn_detached_update(x19_cmd, output_path, exit_code_path)
        except Exception as e:
