...
class GatewayStartupMixin:
    ...
    async def _drain_startup_restore_queue(self) -> int:
        ...
        while queue:
            ...
            with suppress(Exception):
                setattr(event, "_x19_startup_restore_replay", True)
            await adapter.handle_message(event)
            drained += 1
        return drained
    ...
    async def _run_boot_probe_in_launch_scope(self, fn):
        ...
        if getattr(self.config, "multiplex_profiles", False):
            ...
            try:
                async with _async_profile_runtime_scope(get_x19_home()):
    ...
    async def _recover_unclean_sessions(self) -> tuple[int, int]:
        ...
        with _log_suppressed(logging.WARNING, "Exact active-turn recovery on startup failed: %s"):
            agent_timeout = max(1.0, _float_env("X19_AGENT_TIMEOUT", 1800))
    ...
    def _open_faulthandler_log(self):
        ...
        log_dir = getattr(self.config, "log_dir", None) or os.path.join(str(get_x19_home()), "logs")
    ...
    def _start_log_startup_environment(self) -> None:
        ...
        if self._gateway_loop is not None:
            ...
            with _log_suppressed(logging.DEBUG, "Startup watchdog disarm failed", exc_info=True):
                from x19_startup_watchdog import disarm_startup_watchdog
        ...
        with suppress(Exception):
            logger.info(
                ...
                "or X19_MAX_ITERATIONS from .env, or default 500)",
                int(os.getenv("X19_MAX_ITERATIONS", "500")),
            )
        ...
        with suppress(Exception):
            ...
            if _redact_raw.lower() in {"1", "true", "yes", "on"}:
                ...
                logger.warning(
                    "Secret redaction: DISABLED (X19_REDACT_SECRETS=%s). API keys and tokens may appear "
        ...
        with _log_suppressed(logging.DEBUG, "security advisory check failed at gateway startup", exc_info=True):
            from x19_cli.security_advisories import detect_compromised, gateway_log_message
            _adv_msg = gateway_log_message(detect_compromised())
            if _adv_msg:
                logger.warning("%s", _adv_msg)
                logger.warning("Run `x19 doctor` on the gateway host for full remediation steps.")
        ...
        with _log_suppressed(logging.DEBUG, "check_systemd_timing_alignment failed: %s"):
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    ...
                    "mid-drain. Run `x19 gateway install --force` to regenerate the unit, or shorten "
    ...
    def _register_launch_profile_config_hooks() -> None:
        ...
        with _profile_runtime_scope(get_process_x19_home()):
            GatewayStartupMixin._register_config_hooks(
                "shell-hook/webhook registration failed at gateway startup", level=logging.WARNING)
    ...
    def _register_config_hooks(fail_fmt: str, *fail_args, level: int = logging.DEBUG) -> None:
        ...
        Gateway has no TTY, so consent must come from --accept-hooks, X19_ACCEPT_HOOKS, or
    ...
    def _recover_secondary_process_checkpoints(self, process_registry) -> int:
        ...
        from x19_constants import get_x19_home
        launch_home = get_x19_home().resolve()
        recovered = 0
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        _clean_marker = _x19_home / ".clean_shutdown"
    ...
    async def start(self) -> bool:
        ...
        logger.info("Starting X19 Gateway...")
        ...
        # The gateway is a boot owner of the Nous free tier, beside `cmd_chat` and `x19 serve`: every
