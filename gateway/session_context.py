"""Session-scoped context variables for the X19 gateway.

Replaces the old ``os.environ``-based ``X19_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "X19_SESSION_PLATFORM", "X19_SESSION_SOURCE", "X19_SESSION_CHAT_ID",
    "X19_SESSION_CHAT_TYPE", "X19_SESSION_CHAT_NAME", "X19_SESSION_THREAD_ID",
    "X19_SESSION_USER_ID", "X19_SESSION_USER_ID_ALT", "X19_SESSION_USER_NAME",
    "X19_SESSION_SCOPE_ID", "X19_SESSION_KEY", "X19_SESSION_ID",
    "X19_UI_SESSION_ID", "X19_SESSION_MESSAGE_ID", "X19_SESSION_PROFILE",
    "X19_BROWSER_CONTROL_PRINCIPAL", "X19_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "X19_CRON_SESSION", "X19_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("X19_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("X19_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("X19_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("X19_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("X19_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``X19_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``X19_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds X19_SESSION_PLATFORM, CLI/TUI/
# desktop bind X19_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``X19_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("X19_PLATFORM") or get_session_env("X19_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("X19_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See Anony-place/x19-refactored#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``X19_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
