...
``validate_media_delivery_path`` reads ``X19_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``X19_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``X19_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``x19 cron run``, ``x19 send``) calls
...
_FLAG_ENVS = (("strict", "X19_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "X19_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "X19_MEDIA_ALLOW_DIRS"
_TRUST_RECENT_SECONDS_ENV = "X19_MEDIA_TRUST_RECENT_SECONDS"
_TRUTHY = frozenset({"1", "true", "yes", "on"})


def _routed_gateway_cfg() -> Optional[Dict[str, Any]]:
    """``gateway`` section of the ROUTED profile's config when a X19_HOME override is active
