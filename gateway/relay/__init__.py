"""Relay/connector support package for the X19 gateway.
...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_x19_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from x19_cli.skin_engine import get_active_skin  # late import: boot-safe
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``x19 gateway enroll``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.x19/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'x19'}"
