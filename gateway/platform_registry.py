...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``x19 setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``x19 gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved X19_HOME and overlay the
        ...
        # `x19` invocation, so the import happens only when a lookup asks for it.
