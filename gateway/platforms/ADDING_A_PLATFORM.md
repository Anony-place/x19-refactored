...
There are two ways to add a platform to the X19 gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.x19/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `x19 gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `x19_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the X19 core.
...
"x19-your-platform": {
    ...
    "tools": _X19_CORE_TOOLS,
    "includes": []
},
```

And add it to the `x19-gateway` composite:

```python
"x19-gateway": {
    "includes": [..., "x19-your-platform"]
}
```
...
## 12. Status Display (`x19_cli/status.py`)
...
## 13. Gateway Setup Wizard (`x19_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ x19_cli/ toolsets.py \
  --include="*.py" -l | sort -u
