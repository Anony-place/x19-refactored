...
All hooks write atomic JSON payloads under ``<x19_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active X19_HOME."""
    from x19_constants import get_x19_home
    flush_dir = get_x19_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<x19_home>/pending_messages/``), so a runtime cap rotation no longer silently
