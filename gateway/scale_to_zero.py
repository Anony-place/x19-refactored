...
SCALE_TO_ZERO_ENV = "X19_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(x19_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under X19_HOME."""
    if x19_home is None:
        from x19_constants import get_x19_home
        x19_home = get_x19_home()
    return Path(x19_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
