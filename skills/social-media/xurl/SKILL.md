...
version: 1.1.3
author: xdevplatform + openclaw + X19
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by X19 tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official X19 Docker layout, `/opt/data` is `X19_HOME`, but X19 tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for X19-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, X19 tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the X19 subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- X19 adaptation: reformatted for X19 skill conventions; safety guardrails preserved verbatim.
