...
Ledger path resolution (first wins):
  --ledger PATH
  $X19_CITATION_LEDGER
  $X19_HOME/cache/citations/ledger.json
"""
...
from _x19_home import get_x19_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("X19_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_x19_home() / "cache" / "citations" / "ledger.json"
