#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for X19.
...
# Ensure sibling modules (_x19_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _x19_home import display_x19_home, get_x19_home

X19_HOME = get_x19_home()
TOKEN_PATH = X19_HOME / "google_token.json"
CLIENT_SECRET_PATH = X19_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = X19_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same X19 profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the X19 Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the X19 Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to X19 home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_x19_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for X19")
