#!/usr/bin/env python3
"""Google Workspace API CLI for X19.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing X19-facing JSON contract and falls back to the Python client
...
from _x19_home import get_x19_home
...
def _gws_binary() -> str | None:
    override = os.getenv("X19_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for X19")
