...
uv run python "${X19_HOME:-$HOME/.x19}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  export GITHUB_TOKEN=$(uv run python "${X19_HOME:-$HOME/.x19}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
