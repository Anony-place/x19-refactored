...
- A long-running process (x19 gateway, tui_gateway) misbehaves and you can't restart it
...
The x19 test runner and pytest both support this:
...
For long-lived processes: X19 gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <x19-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside X19 you have two practical options:
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<x19-repo>" }
  ]
}
```
...
## Debugging X19-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `x19` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `x19 --tui`)
...
debugpy.wait_for_client()
```
Start `x19 --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For X19 subagents, debug one process at a time.
