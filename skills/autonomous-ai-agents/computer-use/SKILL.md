...
version: 2.1.0
author: Francesco Bonacci (f-trycua), X19
license: MIT
...
`mode` decides what X19 hands back to you, not what the driver does.
...
rejects image tool results), X19 routes the screenshot through the
...
|---|---|
| `cua-driver not installed` | Run `x19 computer-use install`, or `x19 tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `x19 computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| "bare element_index is not accepted" / `snapshot_id_required` | The driver saw a raw index without its token. This is a wrapper defect, not something you fix by passing `snapshot_id` (X19 has no such argument). Re-capture once; if it repeats, tell the user to run `x19 update` and fall back to `coordinate=[x, y]` from the capture's bounds meanwhile |
| "tool 'capture' has no reviewed risk classification" / `Unknown tool` | Something called the driver's MCP vocabulary directly (`capture`, `screenshot`, `get_window_state`, `click` with raw args). Only the `computer_use(action=…)` vocabulary in this file exists on the X19 side |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `x19 computer-use doctor` |
...
| `x19 computer-use doctor` says "could not be started … Access is denied" (Windows) | The X19 venv interpreter can't execute a binary under `C:\Program Files\WindowsApps`; the tool itself may still work because the shell resolves another copy on PATH. Fix once: reinstall cua-driver with the upstream installer (lands under the user profile) or set `X19_CUA_DRIVER_CMD` to a copy outside `WindowsApps`. The same denial spams `errors.log` for any other `WindowsApps` binary X19 spawns (e.g. `bws.exe`) |
| Anything else weird | **First action: ask the user to run `x19 computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
X19 intentionally keeps THIS skill focused on the X19-side
...
links the pack into `~/.x19/skills/cua-driver` (X19 is a detected
...
calling the X19 actions from this file — the wrapper does the translation.
