---
name: x19
description: "Use, configure, theme, extend, and orchestrate X19."
version: 3.2.0
author: X19 + Teknium
license: MIT
...
metadata:
  x19:
    ...
    homepage: https://github.com/Anony-place/x19-refactored
    related_skills: [claude-code, codex, opencode]
---

# X19

X19 is an open-source AI agent framework by X19 that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. X19 works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes X19 different:

- **Self-improving through skills** — X19 learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent X19 instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every X19 feature. If an X19 feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.
...
# Other surfaces
x19 desktop                 # launch the native desktop app (alias: x19 gui)
x19 dashboard               # web admin panel + embedded chat
x19 proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```

## Key Paths

```
~/.x19/config.yaml       Main configuration (settings — never secrets)
~/.x19/.env              API keys and secrets ONLY (under $X19_HOME if set)
$X19_HOME/skills/        Installed skills
~/.x19/skins/            Custom themes (see references/themes.md)
~/.x19/desktop-plugins/  Desktop app UI plugins (see references/desktop-plugins.md)
~/.x19/tui-widgets/      TUI widget apps (see references/tui-widgets.md)
~/.x19/pets/             Installed pet mascots (see references/petdex.md)
~/.x19/state.db          Canonical session store (SQLite + FTS5)
~/.x19/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
~/.x19/logs/             Gateway and error logs
~/.x19/auth.json         OAuth tokens and credential pools
~/.x19/x19/     Source code (if git-installed)
```

Profiles use `~/.x19/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$X19_HOME` — never hardcode `~/.x19`.
...
need more than their docs page. For everything else X19 ships, fetch
`llms.txt` and it maps the question to the page that answers it.

Two theming rules that hold even without loading the reference: **you apply skins yourself** (`x19 config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`x19 skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.

## Spawning Additional X19 Instances

Run additional X19 processes as fully independent subprocesses — separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `x19` process |
|-|-----------------|--------------------------|
...
X19 uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `x19 chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in X19; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Desktop app** (`x19 desktop` / `x19 gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`x19 dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `x19 --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`x19 --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`x19 proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
