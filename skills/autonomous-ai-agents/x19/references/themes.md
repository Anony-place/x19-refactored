...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for X19 itself.
...
- Write access to the X19 home dir — `~/.x19` by default, or `$X19_HOME`
  / the active profile's dir. Skins live in `<x19-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `x19 config set`).
...
3. `write_file` it to `<x19-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `x19 config set display.skin default` (or they can `/skin default`).
...
```
x19 skin set <key> <hex>      # e.g. x19 skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `x19 skin set background "#08201f"` changes
only the background; `x19 skin use <name>` / `x19 skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.x19`** when a profile is active — resolve the real home
  from `$X19_HOME` first, falling back to `~/.x19`.
...
- **Never hand-edit `config.yaml` to activate.** Use `x19 config set
...
- **You apply it, not the user.** `x19 config set display.skin <name>` is
...
- `read_file` the written `<x19-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `x19 config get display.skin` and confirm it reports `<name>`.
