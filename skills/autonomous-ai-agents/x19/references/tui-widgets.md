...
Author widget apps for the X19 TUI (`x19 --tui`): glanceable ambient
...
- The TUI must be in use (`x19 --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.x19/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.x19/logs/tui_gateway_crash.log` if a widget never appears.
