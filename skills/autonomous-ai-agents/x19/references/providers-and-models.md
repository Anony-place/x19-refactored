...
| nous | OAuth device code | `x19 auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `x19 auth add openai-codex` |
| qwen-oauth | OAuth | `x19 auth add qwen-oauth` |
| minimax-oauth | OAuth | `x19 auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`x19 auth`).
Fallback chain when the primary fails: `x19 fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`x19_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   x19 config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
