...
X19 runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`x19 config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually X19's env scrubber dropping
...
X19-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every X19 tool and most Windows APIs. Prefer forward slashes in code
