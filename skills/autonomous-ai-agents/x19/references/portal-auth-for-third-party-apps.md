...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the X19 process**. Its LLM calls go through X19's already-configured provider. | Already uses Portal if user's X19 is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside X19. That LLM call is already authenticated
via X19's provider config. The plugin's own server auth (e.g.
...
What X19 has that *feels* like OAuth — `x19 login --provider nous`
opening a browser, user signs in, token lands in `~/.x19/auth.json` — is a
**X19-specific browser flow**. Under the hood it produces a credential
X19 uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read X19's existing Portal credential out of `~/.x19/auth.json`.
...
rides on the credential X19 already holds.

Where this could live in X19:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for X19 itself.
...
1. First decide: X19 plugin (runs inside X19) or separate app? If plugin,
   it already uses Portal via X19's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   X19 itself") is fully supported. X19 treats agent
...
pitch in the Tool Gateway announcement is specifically about X19's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when X19 is configured to use Portal as its provider. It is
