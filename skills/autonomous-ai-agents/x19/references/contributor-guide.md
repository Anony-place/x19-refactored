...
For occasional contributors and PR authors. Full developer docs: https://anony-place.github.io/x19-refactored/docs/developer-guide/
...
Config: `~/.x19/config.yaml` (settings), `~/.x19/.env` (API keys) — both under `$X19_HOME` when it is set.
...
`_X19_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_x19_home()` for paths,
never hardcode `~/.x19`. For custom/local-only tools, write a plugin in
`~/.x19/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `x19_cli/commands.py`
...
- Tests auto-redirect `X19_HOME` to temp dirs — never touch real `~/.x19/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/x19_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_x19_home()` from `x19_constants` for all paths (profile-safe)
