...
Write plugins for the X19 desktop app: statusbar items, layout panes,
...
system (`~/.x19/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$X19_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$X19_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.x19-package.json` marker) — the ONLY root the renderer loads from — so
...
- The X19 desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$X19_HOME/desktop-plugins/` (usually
  `~/.x19/desktop-plugins/`).

## How to Run

1. Create `$X19_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.x19/...` by default, or `~/.x19/profiles/<profile>/...` under a
...
The ONLY import surface is `@x19/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from X19
  ...
  (`x19://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.x19/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@x19/plugin-sdk`, `react`, and
