...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    x19_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__x19_dashboard_no_matching_source__";
    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id, rowProfile(id)), {
          ...
          headers: {
            "X-X19-Session-Token":
              (window as unknown as { __X19_SESSION_TOKEN__?: string })
                .__X19_SESSION_TOKEN__ ?? "",
          },
        });
        if (!res.ok) {
