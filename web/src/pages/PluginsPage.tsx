...
const MEMORY_PROVIDER_BUILTIN = "__x19_memory_builtin__";
  ...
  if (!hasDetails || !setup) {
    return (
      <p className="border border-destructive/50 px-3 py-2 text-xs text-destructive">
        This provider is installed but unavailable. It may need local dependencies or a manual setup step before X19 can activate it.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <p className={isBlocked ? "text-destructive" : "text-muted-foreground"}>
        {needsDependencySetup
          ? "Finish these setup steps before X19 can activate this provider."
      ...
      {setup.required_env.length && needsDependencySetup ? (
        <div className="grid gap-2">
          <p className="text-muted-foreground">
            Required environment values. Fill the matching fields below, or set them in the X19 environment.
          </p>
          <div className="flex flex-wrap gap-2">
                ...
                <div className="flex flex-col gap-4 min-w-0">
                  ...
                  {!selectedMemoryName && (
                    <p className="text-xs text-muted-foreground">
                      X19 will use the built-in MEMORY.md and USER.md files.
                    </p>
                  )}
        ...
        <div className="flex flex-col gap-3" data-testid="plugin-catalog-section">
          ...
          {catalogLoading ? (
            ...
            <p className="text-xs text-text-tertiary">
              ...
              <a
                className="underline"
                href="https://anony-place.github.io/x19-refactored/docs/plugins"
                target="_blank"
                rel="noreferrer"
              >
                {t.pluginsPage.catalogEmptyDocsLink ?? "Learn about X19 plugins"}
              </a>
            </p>
          ) : (
    ...
    <Card className={cn(busy ? "opacity-70" : undefined)}>
      <CardContent className="flex flex-col gap-3 px-6 py-4">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-text-tertiary">
          ...
          {entry.requires_x19 ? (
            <span>x19 {entry.requires_x19}</span>
          ) : null}
