...
export const API_UNREACHABLE_MESSAGE =
  "X19 dashboard cannot reach the X19 service. Is `x19 dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The X19 service hit an internal error.",
  502: "The dashboard proxy could not reach the X19 service.",
  503: "The X19 service is not ready yet. Try again in a moment.",
  504: "The X19 service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The X19 service answered with an unexpected error (${status}).`;
}
