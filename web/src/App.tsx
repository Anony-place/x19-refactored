...
const SIDEBAR_COLLAPSED_KEY = "x19-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="x19-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="x19-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateX19 = status?.can_update_x19 === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'x19 update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "x19 update";
    return (
      t.status.updateX19ConfirmMessage ??
      `This will run 'x19 update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateX19ConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateX19) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateX19,
      runningLabel: t.status.updatingX19,
      spin: false,
    });
  }
  ...
  return (
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the X19 gateway process. Connected channels and active sessions will reconnect afterward.")
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateX19ConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateX19ConfirmTitle ?? `${t.status.updateX19}?`}
    />
    </>
  );
}
