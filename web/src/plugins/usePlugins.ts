...
import { api, X19_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "x19:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${X19_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${X19_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?x19_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-x19-plugin", manifest.name);
