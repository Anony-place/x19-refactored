...
interface X19ConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function X19ConsoleModal({ open, onClose }: X19ConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("x19> ");
  const inputPromptRef = useRef("x19> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "x19> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to X19 Console...\x1b[0m");
    ...
    void (async () => {
      try {
        ...
        ws.onclose = (ev) => {
          ...
          const message =
            ev.code === 1006 && !hasReadyFrameRef.current
              ? "Console could not connect to the dashboard server. Check that `x19 dashboard` is running, then click Reconnect."
              : ev.code === 1000
                ? "Console closed."
        ...
        writeLine(
          term,
          "\x1b[31mConsole could not connect to the dashboard server. Check that `x19 dashboard` is running, then click Reconnect.\x1b[0m",
        );
      }
    })();
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="x19-console-title"
    >
      <div
        className={cn(
          themedBody,
