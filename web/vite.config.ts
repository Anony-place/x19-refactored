...
const BACKEND = process.env.X19_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `x19 dashboard` server injects a one-shot
 * session token into `index.html` (see `x19_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__X19_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function x19DevToken(): Plugin {
  const TOKEN_RE = /window\.__X19_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__X19_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "x19:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[x19] Could not find session token in ${BACKEND} — ` +
              `is \`x19 dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__X19_SESSION_TOKEN__="${match[1]}";` +
              `window.__X19_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[x19] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`x19 dashboard\` or set X19_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@x19/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `x19 dashboard` must serve these; Vite has no
