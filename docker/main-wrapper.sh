#!/bin/sh
# shellcheck shell=sh
# /opt/x19/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching X19_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `x19` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `x19 <args>` (subcommand passthrough)
#
# Drop to x19 via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${X19_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${X19_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export X19_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset X19_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid x19 "$@"; exec "$@"; }
...
# an arbitrary non-root, non-x19 UID, the bootstrap was skipped and the
# baked image dirs (owned by the x19 build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u x19)" ]; then
    cat >&2 <<EOF
[x19] ERROR: container started with --user $cur_uid (an arbitrary, non-x19 UID) — not supported.
...
# to the x19 user's home before dropping privileges so libraries that
...
_x19_orig_cwd="${X19_ORIG_CWD:-$PWD}"
...
# the user's command so `x19 chat` starts in the Docker -w
