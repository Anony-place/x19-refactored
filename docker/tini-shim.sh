...
# X19 WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: X19_TINI_SHIM_TARGET overrides the `/init` path (and
# X19_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${X19_TINI_SHIM_TARGET:-/init}"
WRAPPER="${X19_TINI_SHIM_WRAPPER:-/opt/x19/docker/main-wrapper.sh}"
