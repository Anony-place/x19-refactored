...
if [ "$$" -eq 1 ]; then
    exec /init /opt/x19/docker/main-wrapper.sh "$@"
fi

echo "[x19] WARNING: container entrypoint is not PID 1; skipping s6-overlay /init and falling back to direct bootstrap. Supervised services are unavailable in this runtime, but the requested command will still run." >&2
...
/opt/x19/docker/stage2-hook.sh
exec /opt/x19/docker/main-wrapper.sh "$@"
