...
Thank you for contributing to X19! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want X19 to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo Anony-place/x19-refactored "<your terms>"
  gh search prs --repo Anony-place/x19-refactored --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/Anony-place/x19-refactored/issues?q=) · [PRs (all states)](https://github.com/Anony-place/x19-refactored/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — X19 itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every X19 install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `x19 skills browse` (labeled "official") and install it with `x19 skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `x19 skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.x19/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(x19_home, config)` for setup-wizard integration
...
- Integrate with `x19 memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. X19 ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.x19/plugins/`, project `.x19/plugins/`, or a pip entry point) — see [Build an X19 Plugin](https://anony-place.github.io/x19-refactored/docs/developer-guide/plugins)
...
The installer creates the X19 venv, wires the `x19` command, stamps the
install method for `x19 update`, and clones the full git project into
`$X19_HOME/x19` (usually `~/.x19/x19`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.sh | bash
cd "${X19_HOME:-$HOME/.x19}/x19"
...
Use this only if you intentionally do not want X19' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `x19` entrypoint from this venv; running the
system `python3 -m x19_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.x19/venvs/x19-dev --python 3.11
...
```bash
mkdir -p ~/.x19/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.x19/config.yaml
touch ~/.x19/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.x19/.env
```

### Run

```bash
# The standard installer already put `x19` on PATH.
x19 doctor
x19 chat -q "Hello"
```

If you used the manual clone fallback, run `./x19` from the checkout or
...
├── cli.py                    # X19CLI class — interactive CLI orchestrator (~4.6k LOC + x19_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (x19-cli, x19-telegram, etc.)
├── x19_state.py           # SessionDB facade (~1.4k LOC); implementation in x19_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── x19_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # X19CLI mixins (slash commands, display, session, ...)
...
│
├── skills/                   # Bundled skills (copied to ~/.x19/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (anony-place.github.io/x19-refactored)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.x19/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.x19/`)

| Path | Purpose |
|------|---------|
| `~/.x19/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.x19/.env` | API keys and secrets |
| `~/.x19/auth.json` | OAuth credentials (Nous Portal) |
| `~/.x19/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.x19/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.x19/state.db` | SQLite session database |
| `~/.x19/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.x19/cron/` | Scheduled job data |
| `~/.x19/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`x19_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `x19 sessions export` for an explicit export.
...
(for example `_X19_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.x19`:
...
The user may skip setup and keep loading the skill. X19 only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `x19 setup` or update `~/.x19/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native X19 tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "X19" is the secondary collaborator. If the contributor's commit shows "X19" as author because they used X19 to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing X19 tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `x19 --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

X19 uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.x19/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `x19_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `x19_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

X19 runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the x19 wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `x19_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `x19_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `x19_cli/gateway_windows.py::
...
X19 has terminal access. Security matters.
...
2. **Test manually**: Run `x19` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Anony-place/x19-refactored/issues)
- Include: OS, Python version, X19 version (`x19 --version`), full error traceback
