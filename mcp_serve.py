"""
X19 MCP Server — expose messaging conversations as MCP tools (`x19 mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the X19-specific
channels_list. Client config: {"mcpServers": {"x19": {"command": "x19", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("x19.mcp_serve")
...
def _x19_home() -> Path:
    try:
        ...
        return Path(os.environ.get("X19_HOME", Path.home() / ".x19"))


def _get_sessions_dir() -> Path:
    return _x19_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_x19_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_x19_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the X19 analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the X19 MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("x19", instructions=(
        "X19 messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the X19 MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
