 ...
 * the three user-facing uninstall modes to the `x19 uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m x19_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `x19_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'x19_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/X19.app/Contents/MacOS/X19  → …/X19.app
 *   Windows: …\X19\X19.exe                 → …\X19  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/X19.app/Contents/MacOS/X19 → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/X19.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs X19.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]X19$/i.test(dir) || /[\\/]x19-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/x19
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import x19_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, x19Home }) {
 ...
 * modes + `pythonPath`=agentRoot so `import x19_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but X19 install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "X19_HOME=${String(x19Home).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
