 ...
 *   - the browser sees only a minimal "return to X19" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to X19.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('x19:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('x19:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('x19:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
