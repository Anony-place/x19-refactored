...
export function createIntroRevealWindowController({
  ...
  function spawnIntroRevealWindow() {
    ...
    win.on('closed', () => {
      ...
      if (main && !main.isDestroyed()) {
        main.webContents.send('x19:intro-reveal:closed')
      }
    })
  ...
  ipcMain.handle('x19:intro-reveal:open', (_event, payload?: IntroRevealOpenPayload) => openIntroReveal(payload))
  ipcMain.handle('x19:intro-reveal:close', (_event, payload?: IntroRevealClosePayload) => closeIntroReveal(payload))
  ipcMain.on('x19:intro-reveal:ready', event => {
    if (event.sender === introRevealWindow?.webContents) {
      introRevealShow?.reveal()
    }
  })
  ipcMain.on('x19:intro-reveal:skip', event => {
    const main = mainWindow()

    if (event.sender === introRevealWindow?.webContents && main && !main.isDestroyed()) {
      main.webContents.send('x19:intro-reveal:skip')
    }
  })
