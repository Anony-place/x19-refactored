// Profile-delete routing logic for the `x19:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_x19_home() and
...
/** Parse a `x19:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its X19_HOME.
 *
...
/**
 * Route the next `x19:api` request away from the primary/window backend
 ...
 * ensure_x19_home() recreates the directory the delete just removed.
 */
