...
const READY_RE = /^X19_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitX19Path = '') {
  const explicit = psLiteral(explicitX19Path)
  ...
  const script = [
    ...
    '$x19Home=$env:X19_HOME',
    'if(-not $x19Home){$x19Home=Join-Path $env:LOCALAPPDATA "x19"}',
    'Assert-NoReparse $x19Home $true',
    '$candidate=[IO.Path]::Combine($x19Home, "x19\\venv\\Scripts\\x19.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "x19\\.venv\\Scripts\\x19.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $x19Home "x19\\venv\\Scripts\\x19.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "x19\\.venv\\Scripts\\x19.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command x19.exe -ErrorAction SilentlyContinue',
    ...
    '$x19=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$x19=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $x19){throw "X19 is not installed on the remote Windows host."}',
    'Assert-NoReparse $x19 $false',
    'if($explicit -and $x19 -ne $explicit){throw "The configured X19 path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($x19), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;x19Home=$x19Home;x19Path=$x19;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(x19Home) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class X19MarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(x19Home)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".x19-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[X19MarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, x19Home) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(x19Home)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitX19Path = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitX19Path)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'x19_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'x19_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'x19_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.x19Home)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".x19-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};x19Path=${psLiteral(reservation.x19Path)};x19Home=${psLiteral(reservation.x19Home)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.x19Path === 'string' &&
    typeof lock.x19Home === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.x19Path === runtime.x19Path &&
    lock.x19Home === runtime.x19Home
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteX19Path)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.x19Home)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.x19Path])

  if (!inspection.supported) {
    const error: any = new Error('Update X19 on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.x19Path = inspection.path
  const x19Version = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located x19 at ${runtime.x19Path}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, x19Path: runtime.x19Path }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForX19(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="X19 SSH"', 'powershell.exe -NoLogo')
