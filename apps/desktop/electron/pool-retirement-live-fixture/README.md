...
X19 Python environment (including the serve dependencies):

```sh
X19_TEST_REAL_SERVE=1 \
X19_TEST_PYTHON=/absolute/path/to/venv/bin/python \
X19_TEST_ELECTRON=/absolute/path/to/native/electron \
node ../../node_modules/vitest/vitest.mjs run \
  --config electron/pool-retirement-live-fixture/vitest.config.ts
```

`X19_TEST_ELECTRON` is optional when Electron resolves from the desktop/root
...
- A hidden, sandboxed native renderer imports the real `src/store/gateway.ts` and
  `X19Gateway`. It opens four actual WebSockets (legacy and registry-local scopes
...
- Cleanup waits for every owned backend exit and verifies the coordinator has zero
  live leases. The temporary HOME, per-child X19_HOME, Electron userData,
