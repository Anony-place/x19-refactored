...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'x19-auth-recovery-'))
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForX19Ready(f.baseUrl, {
    ...
    const mediaRequest = { url: 'x19-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
