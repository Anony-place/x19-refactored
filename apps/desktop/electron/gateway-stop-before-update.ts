 ...
 *
 * So we delegate to `x19 gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `x19 update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(x19CliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(x19CliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(x19CliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(x19CliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(x19CliPath, args)
  }

  if (!existsSync(x19CliPath)) {
    return false
  }

  try {
    exec(x19CliPath, args, {
