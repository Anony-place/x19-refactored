...
const SCAN_MODULE = 'x19_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/X19 processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'x19_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another X19 process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the X19 installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other X19 windows and terminals, then retry.  If the problem\n' +
    'persists, run `x19 update` in a terminal for detailed diagnostics.'
  )
}
