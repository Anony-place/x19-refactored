export type ParentWatchdogEnv = {
  X19_PARENT_PID: string
  X19_PARENT_START_MARKER?: string
  X19_PARENT_NONCE?: string
  /** Spawn tag consumed by x19_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { X19_PARENT_PID: String(pid) }
  env.X19_SPAWN = spawnTag(pid, startMarker)
  ...
  env.X19_PARENT_START_MARKER = startMarker
  env.X19_PARENT_NONCE = nonce

  return env
}

/** Build the `X19_SPAWN` tag mirrored by x19_cli/process_identity.py.
