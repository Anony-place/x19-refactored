 ...
 *   1. X19_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\x19\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect X19_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.X19_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'x19', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'x19', 'git', 'usr', 'bin', 'bash.exe'))
  }
