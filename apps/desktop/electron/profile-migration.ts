...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.x19`). Default's state.db and gateway.pid live here. */
  x19Home: string
  /** Named-profile root (`~/.x19/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isX19Process: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `x19Home`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, x19Home: string, profilesRoot: string): string {
  return name === 'default' ? `${x19Home}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, x19Home: string, profilesRoot: string): string {
  return name === 'default' ? `${x19Home}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveX19Home(profilesRoot: string, x19Home?: string): string {
  if (x19Home) {
    return x19Home
  }

  // Tests that predate x19Home pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live x19 process.
 ...
 *
 * `x19Home` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isX19Process'> & { x19Home?: string }
): string[] {
  const x19Home = resolveX19Home(profilesRoot, deps.x19Home)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, x19Home, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.x19Home, deps.profilesRoot))
...
/** Default is always a candidate; it is `$X19_HOME`, not `$X19_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `x19 --profile default` and is worse than writing nothing
