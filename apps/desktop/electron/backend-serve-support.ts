...
// spawns `x19 serve`; runtimes older than serve only have `dashboard`, so
...
// (covers a bare `x19` resolved from PATH with no known source root). Result
...
export function createBackendServeSupportResolver(x19Home: string, rememberLog: (message: string) => void) {
    ...
    const pending = (async () => {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'x19_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          // `x19 --version` (~10.5s measured cold), and a false negative here
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, X19_HOME: x19Home, ...(backend.env || {}) },
            timeout: PROBE_TIMEOUT_MS,
            stdio: 'ignore',
            // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
            // (see resolveX19Backend step 4); execFileSync of a .cmd without
