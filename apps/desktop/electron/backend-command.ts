// Backend subcommand routing for the desktop-managed X19 process.
//
// The desktop app launches its own headless backend via `x19 serve` — it
...
// app hasn't updated yet, or an older `x19` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional X19 profile to pin via `--profile`.
 */
 ...
 * `-m x19_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `x19_cli/subcommands/dashboard.py` source registers
