...
test('primary SSH reuse rejects a descriptor with a different remote X19 path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteX19Path: '/srv/x19', user: 'alice' },
    profiles: {}
  })
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'x19-vps', kind: 'ssh', label: 'X19 VPS', host: 'x19-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'x19-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'x19-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'x19-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteX19Path: '/opt/x19' },
  ...
  assert.equal(resolve({ ...base, remoteX19Path: '/opt/x19' }), 'ssh-path')
...
// The electron and @x19/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@x19/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'x19-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('token only persists on token-auth remotes; oauth/cloud drop it', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'C', url: 'https://c.x19.cloud', authMode: 'oauth', token: { enc: 'x' } },
    registry
  )
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteX19Path, '/opt/x19')
...
test('remote input normalizes URL and auth mode; cloud keeps org', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'Cloud', url: 'https://foo.x19.cloud', authMode: 'oauth', org: 'nous' },
    registry
  )
...
test('migrate: v1 cloud keeps cloud provenance + org', () => {
  const registry = migrateV1ToRegistry({
    mode: 'cloud',
    remote: { url: 'https://a.x19.cloud', authMode: 'oauth', org: 'nous' }
  })
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'x19-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'x19-alex')
  assert.equal(applied.lastUsed, 'x19-alex')
})
