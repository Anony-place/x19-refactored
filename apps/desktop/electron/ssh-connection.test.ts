...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env X19_DASHBOARD_SESSION_TOKEN=abc123deadbeef X19_DESKTOP=1 x19 dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /X19_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /X19_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-X19-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-X19-Session-Token: hdr_888'), /X-X19-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-X19-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('x19', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v x19', 15000)
  assert.equal(args[args.length - 1], 'command -v x19')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `x19-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `x19-ssh-test-${process.pid}-${Date.now()}`)
...
test('withRemoteTimeout kills a hung probe remotely instead of orphaning it (#110478)', async () => {
  ...
  const wrapped = withRemoteTimeout('x19 --version 2>&1', 15)
