...
export const ZOOM_STORAGE_KEY = 'x19:desktop:zoomLevel'
 ...
 * but forgot to emit 'x19:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('x19:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `x19:zoom:changed`, which never fired —
