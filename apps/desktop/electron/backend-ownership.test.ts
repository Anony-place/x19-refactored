...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'x19 serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/x19 serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m x19_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/x19 --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\X19 Runtime\\x19.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('x19 chat --query serve'), false)
