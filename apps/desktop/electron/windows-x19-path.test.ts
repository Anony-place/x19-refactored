// Unit tests for the pure Windows `x19` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvX19Command(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `x19` shim shadows
//      the real x19.cmd/x19.exe.
...
//   3. resolveVenvX19Command() — must probe the venv python via
//      canImportX19Cli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvX19: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvX19: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvX19: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvX19: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvX19: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvX19: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvX19Command>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportX19Cli: async () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    x19Home: '/fake/x19-home',
...
test('resolveVenvX19Command: returns null off Windows', async () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(await resolveVenvX19Command('/root/venv/Scripts/x19.exe', [], deps), null)
})

test('resolveVenvX19Command: returns null for a .cmd/.bat script command', async () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(await resolveVenvX19Command('/root/venv/Scripts/x19.cmd', [], deps), null)
})

test('resolveVenvX19Command: returns null when the basename is not x19/x19.exe', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvX19Command('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvX19Command: returns null when the parent dir is not Scripts', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvX19Command('/root/venv/bin/x19.exe', [], deps), null)
})

test('resolveVenvX19Command: returns null when the venv python does not exist on disk', async () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(await resolveVenvX19Command('/root/venv/Scripts/x19.exe', [], deps), null)
})

test('resolveVenvX19Command: probes the venv python before trusting it (returns null on failed probe)', async () => {
  let probed = false

  const deps = makeDeps({
    canImportX19Cli: async (python: string) => {
      probed = true
  ...
  const result = await resolveVenvX19Command('/root/venv/Scripts/x19.exe', ['serve'], deps)
...
test('resolveVenvX19Command: returns the resolved python backend descriptor when the probe passes', async () => {
  const deps = makeDeps()
  const result = await resolveVenvX19Command('/root/venv/Scripts/x19.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'x19_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvX19Command: is case-insensitive on x19.exe and the Scripts dir name', async () => {
  const deps = makeDeps()

  assert.ok(await resolveVenvX19Command('/root/venv/Scripts/X19.EXE', [], deps))
  assert.ok(await resolveVenvX19Command('/root/venv/SCRIPTS/x19.exe', [], deps))
})
