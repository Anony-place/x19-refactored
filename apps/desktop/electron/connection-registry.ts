 ...
 * runtime, remote gateways, X19 Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@x19/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteX19Path || '').trim() !== String(activeSsh.remoteX19Path || '').trim() ||
...
/** Turn `ls ~/.x19/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. X19 Cloud
 * instances are platform-managed — we never run `x19 update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteX19Path: input.remoteX19Path,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/X19-managed names; an empty result stores nothing.
 ...
 * `remoteX19Path`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'X19 Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'X19 Cloud' : 'Remote gateway')
