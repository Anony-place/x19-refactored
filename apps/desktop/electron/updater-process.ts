 ...
 *
 * The staged Tauri `x19-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `x19 update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `x19 update`, swaps/relaunches
 * the app, and writes .x19-update-result.json. With the app gone before
 * the update starts, the X19_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `x19 update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into X19_HOME on *every* platform
 * (`x19-setup.exe` on Windows, `x19-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_x19_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `x19 update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(x19Home, 'x19-setup.exe')
 ...
 *
 * `copy_self_to_x19_home` deliberately no-ops during `--update`
 ...
 * "Another X19 update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `x19 update`, which
 ...
 * the installer is written to X19_HOME at install/repair time, making mtime
 ...
 * The lead sentence is plain: nothing changed and X19 keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and X19 keeps running as before. " +
