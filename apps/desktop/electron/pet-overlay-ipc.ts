...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('x19:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('x19:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('x19:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('x19:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('x19:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('x19:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('x19:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('x19:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('x19:pet-overlay:control', payload)
  })
}
