// The standalone desktop-plugin root (`<X19_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.x19-package.json` marker
...
export const PACKAGE_MARKER = '.x19-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.x19-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every x19 home the app knows about locally: the default plus each profile. */
export async function localHomes(x19Home: string): Promise<string[]> {
  const profiles = await listDirs(path.join(x19Home, 'profiles'))

  return [x19Home, ...profiles.map(name => path.join(x19Home, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(x19Home: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(x19Home, 'profiles'))) {
    const scopedRoot = path.join(x19Home, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(x19Home: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(x19Home)) {
