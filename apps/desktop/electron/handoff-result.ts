 ...
 * console. It writes X19_HOME/.x19-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen X19 within 30 minutes.
...
export function handoffResultPath(x19Home: string): string {
  return path.join(x19Home, '.x19-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(x19Home)
  let raw: string
