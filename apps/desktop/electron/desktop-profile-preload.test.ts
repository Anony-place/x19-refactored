...
test('the native preload exposes routed peer opening and default preference events with unsubscribe', async () => {
  ...
  assert.equal(name, 'x19Desktop')
  const route = { connectionId: 'remote-work', profile: 'work' }

  await bridge.openWindow(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['x19:window:openInstance', route])
  await bridge.openWindow()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['x19:window:openInstance', undefined])
  await bridge.profile.setDefault(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['x19:profile:default:set', route])
  await bridge.profile.getDefault()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['x19:profile:default:get'])
  ...
  assert.equal(channel, 'x19:profile:default:changed')
  listener({}, route)
  listener({}, null)
