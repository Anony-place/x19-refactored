 ...
 * `iter_x19_node_dirs()` in x19_constants.py, which the Electron main
 * process cannot import.
 */
function x19ManagedNodePathEntries(
  x19Home,
  ...
  const root = pathModule.join(x19Home, 'node')
...
function buildDesktopBackendPath({
  ...
  const x19NodeDirs = x19ManagedNodePathEntries(x19Home, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([x19NodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeX19HomeRoot(x19Home, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!x19Home) {
    return x19Home
  }

  const resolved = pathModule.resolve(String(x19Home))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). x19_bootstrap sets this inside the child too, but only
