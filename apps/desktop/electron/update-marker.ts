...
export function markerPath(x19Home) {
  return path.join(x19Home, '.x19-update-in-progress')
}
 ...
 *
 * The Tauri-based x19-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(x19Home)
  const nowMs = now()
  const owner = readLiveUpdateMarker(x19Home, { kill, maxAgeMs, now: () => nowMs })
 ...
 * X19 to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(x19Home, opts)
