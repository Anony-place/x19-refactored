...
beforeAll(async () => {
  fixtureDir = await mkdtemp(path.join(tmpdir(), 'x19-wslg-process-'))
  await build({
    entryPoints: [
  ...
  await writeFile(
    ...
    const fd = Number(process.env.X19_DESKTOP_READY_FD)
    ...
    console.log(JSON.stringify({ kind: 'launcher', pid: process.pid,
      ...
      readyEnv: process.env.X19_DESKTOP_READY_FD
    }))
