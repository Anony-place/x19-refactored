 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `x19 update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.x19Home, 'X19 home')
  const x19Path = validateRemoteValue(target.x19Path, 'launcher path')
  ...
  const launcherWord = expandRemotePath(x19Path)

  const updateCommand =
    `env X19_HOME=${homeWord} ` +
    `X19_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'X19_UPDATE_ORIGIN_PROFILE=default ' +
    `X19_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `X19_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:X19_HOME=${psLiteral(home)}`,
    `$env:X19_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:X19_UPDATE_ORIGIN_PROFILE="default"',
    `$env:X19_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:X19_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:X19_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:X19_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:X19_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(x19Path)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.x19-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote X19 updated and every managed SSH profile is ready.'
        : restoreOk
