...
export interface X19ReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForX19Ready`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote X19 gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForX19Ready().
 *
...
/**
 * True when the backend URL points at a Nous-managed X19 Cloud instance
 ...
 * `isReauthRequired` so startX19 latches; a bare `needsOauthLogin` (the
...
export async function waitForX19Ready(baseUrl: string, options: X19ReadyOptions): Promise<void> {
  ...
  throw new Error(`X19 backend did not become ready: ${detail}`)
}
