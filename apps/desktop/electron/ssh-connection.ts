...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `x19 --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(X19_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-X19-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.x19/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'x19-desktop-ssh')
  }

  return path.join(os.homedir(), '.x19', 'desktop-ssh')
}
