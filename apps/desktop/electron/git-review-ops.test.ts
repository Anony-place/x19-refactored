...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'x19-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'x19-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'X19 Test'], { cwd: dir })
