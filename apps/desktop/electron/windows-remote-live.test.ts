...
//   X19_WIN_SSH_HOST   ssh alias/host of the Windows box
//   X19_WIN_SSH_USER   remote user
//   X19_WIN_SSH_X19 absolute path to the remote x19.exe under test
const liveHost = process.env.X19_WIN_SSH_HOST || ''
const liveUser = process.env.X19_WIN_SSH_USER || ''
const configuredX19 = process.env.X19_WIN_SSH_X19 || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-X19-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredX19)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredX19.replace('x19.exe', 'python.exe')}' -m x19_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredX19.replace('x19.exe', 'python.exe')
        const terminate = `& '${python}' -m x19_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.x19Path}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m x19_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m x19_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
