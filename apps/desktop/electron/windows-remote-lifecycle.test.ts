...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command x19.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          x19Home: 'C:\\Users\\alice\\.x19',
          x19Path: 'C:\\X19\\x19.exe',
          python: 'C:\\X19\\python.exe'
        })
      }

      if (script.includes('.x19-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('x19_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.x19\\profiles\\research')
  assert.match(script, /\.x19-update-in-progress/)
...
test('Windows probe validates X19 and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $x19Home')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('X19 is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /X19 is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\X19's\\python.exe" }, 'inspect', [
    'C:\\x y\\x19.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'x19_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /X19''s/)
  assert.match(script, /C:\\x y\\x19\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { x19Path: lock.x19Path, x19Home: lock.x19Home }
