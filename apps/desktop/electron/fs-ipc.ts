...
// hardening, X19_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('x19:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('x19:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('x19:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('x19:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<X19_HOME>/desktop-plugins`,
  // resolved from the main-process X19_HOME (see resolveX19Home) — NOT from
  // the connected backend. A remote backend reports its own `x19_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(x19Home, 'profiles', profile) : x19Home
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(x19Home, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(x19Home, root)
    await reconcileUnifiedDesktopHalves(x19Home, root)

    return root
  }

  ipcMain.handle('x19:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('x19:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(x19Home, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(x19Home, root)
  })

  // The LOCAL logs root (`<X19_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where X19_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('x19:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('x19:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('x19:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('x19:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('x19:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('x19:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
