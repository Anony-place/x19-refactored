...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'X19_DESKTOP_REMOTE_URL is set but X19_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote X19 backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `x19 serve` the desktop never
