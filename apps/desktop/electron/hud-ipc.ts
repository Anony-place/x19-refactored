...
export function registerHudIpc({
  ...
  ipcMain.on('x19:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('x19:hud:windowing', event => {
  ...
  ipcMain.on('x19:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('x19:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('x19:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('x19:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('x19:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('x19:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('x19:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('x19:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('x19:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('x19:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('x19:hud:close', async () => {
    closeHudWindow()
