...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "X19". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `x19 profile rename
  ...
  // never generic "X19" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as X19 (the agent it is)
...
/** The profile id the backend accepts (`x19_cli.profiles._PROFILE_ID_RE`
