/**
 * X19 Bot Mode — a "one chat per agent" roster for the X19 desktop.
 *
 * Left pane "Bots": one row per X19 profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (X19 cron jobs) scoped to the
 ...
 * bot-initiated sends use `x19 -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@x19/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@x19/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @x19.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.x19Desktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.x19Desktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is X19' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['x19-bots']?.title || bot.title || ''
            ).trim()

            // message_agent only resolves canonical identities: the relay
            // matches a roster row's handle/profile (± @connection-id), the
            // local path a bare profile name or 'x19'. botHandle() prefers
