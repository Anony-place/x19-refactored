...
describe('routing', () => {
  ...
  it('resolves @x19 to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@x19 take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @x19', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @x19,/)
    ...
    expect(peer).toMatch(/group chat with @x19/)
  })

  // #89720: a renamed primary is @bobby to the roster, autocomplete and the
  // mention resolver; introducing it to itself as @x19 made it treat
