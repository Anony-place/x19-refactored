...
import { forgetSessionUnread, host, queryClient } from '@x19/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['x19-bots']
...
/** Permanently delete a bot's X19 profile, then remove plugin-local state
 ...
 * directory (x19#52279). That is the "can't delete a bot" error. */
