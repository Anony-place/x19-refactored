...
import { atom, host } from '@x19/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'x19-bots-groups'
...
/** The v3 envelope stored under the default profile's `x19-bots-groups`
 ...
 *  clients (X19 Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "X19"
 *  (community report, Aug 21 2026: renamed default still read "X19 is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as X19 (matching displayName and the
 *  @x19 handle) so the main agent never loses its name in rooms.
 *
 ...
 *  failing `default`s are never one anonymous "X19" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'X19' : profile)
  ...
  // title, then the local row's display_name, then default → X19.
