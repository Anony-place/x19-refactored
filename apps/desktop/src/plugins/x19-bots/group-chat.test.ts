...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['x19-bots-groups']
}
...
describe('speaker labels', () => {
  it('relabels X19 control-frame openers only in member-authored transcript lines', async () => {
  ...
  it('the default profile speaks as X19 in transcripts, not @default', async () => {
    ...
    expect(line).toBe('X19: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale X19', async () => {
    const { chat, rounds } = await loadRoom()
    const { formatGroupChatLine } = await import('./group-round-prompt')
    const data = await import('./data')

    // A renamed default (core display_name via `x19 profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "X19 is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → X19, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('X19')
  ...
  it('qualifies twins per room and never renders a raw key when the roster row is missing', async () => {
    ...
    expect(chat.groupSpeakerLabel('local::reviewer')).toBe('reviewer')
    expect(chat.groupSpeakerLabel('spark::default')).toBe('X19')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['x19-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['x19-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['x19-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'x19-bots-groups': 1 } } }
      }
