...
import { host, LruCache } from '@x19/plugin-sdk'
...
// to (local, remote URL, SSH, X19 Cloud, docker) must be able to find
...
// x19_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['x19-bots']?.title || profile?.display_name || ''),
