 ...
 * the raw Cloud hostname, or as generic "X19" when Cloud was the only
 * source.
 *
...
vi.mock('@x19/plugin-sdk', () => ({ host: hostMock }))
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "X19"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "X19".
    expect(displayName({ name: 'default' }, null)).toBe('X19')
  })
