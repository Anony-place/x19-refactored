...
import * as sdk from '@x19/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['x19-bots'].chat) were
 ...
 *  click path's tile-staleness probe (x19#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This X19 Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (x19#89617) — clicking Retry
  ...
  // until an app restart (x19#93604). A resume is cheap and
...
export function notifyBotOpenFailure(error: unknown, bot: RosterRow, step: BotOpenStep, botName?: string) {
  ...
  if (botModeGatewayNeedsUpdate(error)) {
    const connectionLabel = bot.connectionLabel || bot.connectionId || 'X19'
    host.notify?.({
      kind: 'error',
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update X19 Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['x19-bots']
