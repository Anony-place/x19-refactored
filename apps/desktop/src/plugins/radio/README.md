...
Bundled X19 Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@x19/plugin-sdk`, `react`, and
...
`$X19_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use X19 theme
...
Stream availability, regional restrictions, and metadata vary. X19 is not
