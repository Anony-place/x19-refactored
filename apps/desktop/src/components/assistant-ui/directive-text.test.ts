...
import { formatRefValue, x19DirectiveFormatter } from './directive-text'
...
describe('x19DirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = x19DirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = x19DirectiveFormatter.parse('@file:src/main.tsx the entry point')
  ...
  it('parses session links with profile/id values', () => {
    const segments = x19DirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...x19DirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = x19DirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...x19DirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
