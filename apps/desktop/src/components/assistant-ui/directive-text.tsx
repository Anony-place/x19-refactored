...
const X19_REF_TYPES = WIRE_REFERENCE_KINDS
type X19RefType = (typeof X19_REF_TYPES)[number]
...
const X19_DIRECTIVE_RE = referenceRe()
...
export const x19DirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(X19_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...x19DirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing X19 directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.x19Desktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.x19Desktop?.readFileDataUrl(id)
