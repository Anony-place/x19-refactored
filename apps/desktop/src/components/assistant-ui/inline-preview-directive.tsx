 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.x19.send(prompt)` (or declarative
 * `data-x19-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'x19-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'x19-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `x19.send(prompt)`.
 ...
 *  wires `data-x19-send` so declarative HTML works with zero script:
 *  `<button data-x19-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.x19={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-x19-send]"):null;' +
    'if(el)send(el.getAttribute("data-x19-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `x19 serve` — renders live.)
