...
function sessionsBotsTree() {
  ...
  registerPane('x19-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'x19-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('x19-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('x19-bots:pane')
    expect($hiddenStripTabs.get()).toContain('x19-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('x19-bots:pane')

    expect(setStripTabHidden('x19-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('x19-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('x19-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('x19-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('x19.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('x19-bots:pane')
    ...
    revealTreePane('x19-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('x19-bots:pane')
    expect(window.localStorage.getItem('x19.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'x19-bots:pane', title: 'x19-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'x19-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
