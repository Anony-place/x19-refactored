 ...
 *    a `X19Plugin` registers automatically (vite glob — drop a folder in).
 *    `x19-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `x19-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<x19 home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<x19 home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: X19Plugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default X19Plugin export — skipped`)
