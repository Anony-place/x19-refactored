...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to X19.
 *
 ...
 * that into an ordinary X19 turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * X19 keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (X19) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await x19Api<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by X19; late results for an
  ...
  /** Quiet progress for the live model ("X19 is running the tests…"). */
