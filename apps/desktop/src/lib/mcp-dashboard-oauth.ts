...
const UPDATE_BACKEND = 'Update the X19 backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.x19Desktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update X19 Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.x19Desktop.openExternal(authUrl)
    let pollFailures = 0
