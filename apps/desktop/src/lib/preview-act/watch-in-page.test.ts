...
beforeEach(() => {
  ...
  document.querySelector('x19-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__x19Watch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('x19-watch')
...
const drawn = () =>
  (window as unknown as { __x19Watch: { parts: Record<string, HTMLElement> & { shadow: ShadowRoot } } })
    .__x19Watch.parts
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('x19-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__x19WatchTag = value
    }
