...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __x19Watch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__x19Watch || (win.__x19Watch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('x19-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('x19-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __x19WatchTag?: number }).__x19WatchTag
