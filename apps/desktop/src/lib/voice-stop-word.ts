...
//
// When someone is in a hands-free "Hey X19" voice chat, the natural way to
...
// phrase (optionally addressed to X19), so a real turn that merely contains
...
// Optional address prefixes so "x19 stop" / "ok stop" / "hey x19, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey x19', 'hey x19,', 'x19', 'x19,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("x19") is not a stop command on its own.
      continue
    }
 ...
 * to X19). Returns false for anything that merely contains "stop" as part of
