...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|x19):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.x19Desktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.x19Desktop?.openExternal?.(href)
  }
}
 ...
 * doesn't cost a context switch out of X19, and it is the surface the agent
