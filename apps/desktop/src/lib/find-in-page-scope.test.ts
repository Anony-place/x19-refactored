...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>X19 X19</p>')
    performScopedFind(surface, 'x19', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'x19', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>x19</p>')
    performScopedFind(surface, 'x19', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">X19</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>x19</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'x19')).toBe(true)
  })
