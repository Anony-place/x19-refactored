...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> X19 Desktop')).toBe('no X19 Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>X19')).toBe('no X19')
  })
