...
// X19 is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.x19Mode
 ...
 * the only carriers of X19' palette — are dropped so the paste target's
