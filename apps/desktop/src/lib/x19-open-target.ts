 ...
 * `x19://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `x19://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `x19://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type X19OpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const X19_PROTOCOL = 'x19:'
...
export function normalizeX19OpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('x19://') || trimmed.startsWith(`${X19_PROTOCOL}//`)) {
    try {
      ...
      // x19://index-network/intent/1 → /index-network/intent/1
...
export function resolveX19OpenPath(target: X19OpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeX19OpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeX19OpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeX19OpenString(target.path)
 ...
 * (`x19://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromX19DeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveX19OpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveX19OpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `x19://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromX19DeepLink('open', name, params)
}
