import { x19Api } from '@/api/client'
import type {
  X19Connection,
  X19ReadDirResult,
  X19ReadFileTextResult,
  X19SelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: X19Connection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: X19Connection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<X19ReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<X19ReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<X19ReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<X19ReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.x19Desktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: X19SelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
