 ...
 * (`x19:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.x19Desktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.x19Desktop?.dataUrlReadMax) {
