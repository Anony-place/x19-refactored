...
import type { X19GitWorktree, X19RepoStatus } from '@/global'
...
const EMPTY_WORKTREES: X19GitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, X19RepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, X19GitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<X19RepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<X19GitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<X19RepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<X19GitWorktree[]>>()
const $noRepoStatus = atom<X19RepoStatus | null>(null)
const $noWorktrees = atom<X19GitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<X19RepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<X19GitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: X19RepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: X19GitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<X19RepoStatus | null>
  seq: number
  target: string
}
