/**
 * The text X19 sends during the guided first run: the runbook handed to the model at session.create, its persona,
...
export function buildChatOnboardingPrompt(suggestedName?: string | null, signedIn = false, capabilities = ''): string {
  ...
  return [
    "You are X19, and this is a brand-new user's very first conversation with you. Your job right now is to get the app arranged around them and their first real job started.",
    ...PERSONA,
    FIRST_USE_GUIDANCE,
    capabilities,
    ...
    ...(language
      ...
      : []),
    'Never call yourself "Setup", "the setup assistant", "the onboarding guide", or anything like it, and never say you are "not the agent" — you are X19, one thing, talking to them.',
