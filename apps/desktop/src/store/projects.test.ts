...
import type { ProjectInfo } from '@/types/x19'
...
const x19 = await import('@/x19')
const getX19Config = vi.mocked(x19.getX19Config)
...
describe('project scope', () => {
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('x19.desktop.projectScope')).toBe('p_abc')
  })
})
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the X19 backend.'
        )
      })
    } as never)
...
describe('createProject', () => {
  ...
  it.each(['default', 'coder'])('creates in the active %s profile without leaving All profiles', async profile => {
    const created = { folders: [], id: 'p_new', name: 'X19', primary_path: '/srv/x19' }
    ...
    vi.mocked(x19.x19Api).mockResolvedValue({ projects: [tree], active_id: created.id })
    $activeGatewayProfile.set(profile)
    setShowAllProfiles(true)

    await expect(createProject({ folders: ['/srv/x19'], name: created.name, use: true })).resolves.toEqual(created)
    ...
    expect(x19.x19Api).toHaveBeenCalledWith(
  ...
  it('does not retarget a project create when the profile changes during reconnect', async () => {
    ...
    const pending = createProject({ folders: ['/srv/x19'], name: 'X19' })
    const rejection = expect(pending).rejects.toThrow('Active X19 profile changed while connecting')
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getX19Config.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getX19Config).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getX19Config).not.toHaveBeenCalled()
    ...
    // `scan: true`) — repos with zero X19 sessions must still surface —
