import type { ConnectionState } from '@x19/shared'
...
import type { SessionInfo, UsageStats } from '@/types/x19'
...
const COMPOSER_MODEL_KEY = 'x19.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'x19.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'x19.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'x19.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'x19.desktop.composer.fast'
...
function composerScopeForConnection(connection: X19Connection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'x19.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'x19.desktop.lastRoute'
...
function workspaceCwdKey(connection: X19Connection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.x19Desktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.x19Desktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<X19Connection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'x19.desktop.sessionOwnerHints.v1'
 ...
 * X19Connection lookup is unavailable. */
...
export const setConnection = (next: Updater<X19Connection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useX19Config.
