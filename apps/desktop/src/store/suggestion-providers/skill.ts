 ...
 *  Working in `~/www/x19`, the draft says "x19" constantly
 *  — a "Use skill: x19" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`x19-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/x19-dev`, `plugin:skill`). */
