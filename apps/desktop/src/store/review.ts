...
import type { X19ReviewFile, X19ReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — X19' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'x19.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'x19.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'x19.desktop.reviewTreeMode'
const SELECTED_KEY = 'x19.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<X19ReviewFile[]>([])
...
export const $reviewShipInfo = atom<X19ReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['x19Desktop']>['git']>['review']>
...
export async function selectReviewFile(file: X19ReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly X19ReviewFile[], path: string): X19ReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.x19Desktop?.openExternal?.(existing.url)
