 ...
 * `@x19/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-x19-glass]`
 * block in styles.css).
 */
 ...
 * sets `x19Desktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.x19Desktop?.glassSupported === 'boolean'
    ? window.x19Desktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.x19Desktop?.translucencySupported === 'boolean'
    ? window.x19Desktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-x19-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-x19-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-x19-glass-on', glassLive)
  root.toggleAttribute('data-x19-glass', glassOn)
  root.toggleAttribute('data-x19-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-x19-glass-scope', scope)
  } else {
    root.removeAttribute('data-x19-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.x19Desktop?.setTranslucency?.(state)
  })
