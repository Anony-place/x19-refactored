...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@x19/shared'
...
const TILES_KEY = 'x19.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'x19.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'x19.desktop.botChatSessions.v1'
 ...
 *  (x19#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_x19_home() re-creates
 * the profile directory the delete just removed (x19#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (x19#94235). Tiles persisted
...
// Dev hook for automation (mirrors __X19_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__X19_SESSION_TILES__ = {
