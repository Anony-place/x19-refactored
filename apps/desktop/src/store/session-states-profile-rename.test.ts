...
describe('migrateTilesForProfile', () => {
  ...
  it('moves tabs, bot tiles, cached tails, remembered ids and owner hints to the new profile name', async () => {
    ...
    storage.writeJson('x19.desktop.sessionTiles.v2', {
      webdesign_bhp: [
    ...
    storage.writeJson('x19.desktop.sessionOwnerHints.v1', [
    ...
    const tiles = storage.readJson<Record<string, Tile[]>>('x19.desktop.sessionTiles.v2')
