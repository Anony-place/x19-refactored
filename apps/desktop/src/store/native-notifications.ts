...
import { type X19OpenTarget, resolveX19OpenPath } from '@/lib/x19-open-target'
...
const STORAGE_KEY = 'x19:native-notifications'
...
// "Backgrounded" = the user isn't on X19. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `x19://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `x19://index-network/intent/1`). */
  activate?: X19OpenTarget
 ...
 *  user is away from X19 — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveX19OpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveX19OpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.x19Desktop
