 ...
 *
 * The archive is the CLI's own `x19 profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/x19'
...
import type { ProfileDesktopOverlay } from '@/types/x19'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'X19 profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.x19Desktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.x19Desktop?.selectPaths?.({
