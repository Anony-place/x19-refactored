 ...
 * app's bounds and stays visible while X19 is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'x19.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'x19.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.x19Desktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.x19Desktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.x19Desktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.x19Desktop?.petOverlay?.close()
}
