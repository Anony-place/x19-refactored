 ...
 * composer with the reply scrolling above it, so X19 can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.x19Desktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.x19Desktop?.hud
...
export function resetHudLayout(): void {
  void window.x19Desktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.x19Desktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.x19Desktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
