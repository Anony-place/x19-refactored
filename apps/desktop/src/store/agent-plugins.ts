...
/**
 * Feature store for backend (agent) plugins — the native X19 plugins plus
 ...
 * primitives `x19 plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native X19. */
  portable?: boolean
 ...
 *  X19_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's X19_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
