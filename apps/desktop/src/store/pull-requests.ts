...
import type { X19BranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/x19'
...
export const $pullRequestsByBranch = atom<Record<string, X19BranchPullRequest>>({})
...
const $prScannedSessions = persistentAtom<string[]>('x19.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: X19BranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
