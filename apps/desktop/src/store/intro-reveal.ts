 ...
 *
 * This store is the only writer of x19-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'x19-intro-reveal-seen-v1'
...
/** The film will never play for this identity (X19_SKIP_INTRO). Record it
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.x19Desktop?.skipIntro !== true
}

/** The guided flow is on but its intro film was turned off at launch
 * (X19_SKIP_INTRO). Distinct from isIntroRevealEnabled(): false there also
...
export function startIntroReveal(): void {
  ...
  void window.x19Desktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.x19Desktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.x19Desktop?.introReveal
