...
// (#73044). Exercises the REAL x19 + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/x19')
...
const conn = (over: Partial<X19Connection> = {}): X19Connection =>
  ({
    authMode: 'oauth',
