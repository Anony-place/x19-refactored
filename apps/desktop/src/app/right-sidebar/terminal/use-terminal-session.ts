...
const X19_PATHS_MIME = 'application/x-x19-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(X19_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(X19_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.x19Desktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (x19 --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.x19Desktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[x19-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.x19Desktop?.terminal?.write(sessionId, `${command}\r`)
