...
import { disconnectOAuthProvider, listOAuthProviders } from '@/x19'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/x19'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.x19Desktop?.terminal)
...
//      in x19_cli/provider_catalog.py) — the SAME provider identity
//      `x19 model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // X19 can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider X19 can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // X19 never deletes creds another tool owns behind a silent API call.
