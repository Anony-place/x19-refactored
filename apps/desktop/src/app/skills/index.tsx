...
import type { SkillInfo, ToolsetInfo } from '@/types/x19'
...
export function SkillsView({
  ...
  const registryBridge = window.x19Desktop?.connections
  const rosterBridge = window.x19Desktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'X19 (default)' : p.name,
      value: p.name
    }))
