...
import { useX19Config } from './use-x19-config'

vi.mock('@/x19', () => ({
  getX19Config: vi.fn(),
  getX19ConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'x19.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getX19Config).mockResolvedValue(config as Awaited<ReturnType<typeof getX19Config>>)

describe('useX19Config refreshX19Config', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to X19' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useX19Config({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshX19Config()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useX19Config({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getX19Config>>>()
    vi.mocked(getX19Config).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useX19Config({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshX19Config(true)
    })
    expect(getX19Config).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getX19Config>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getX19Config>>>()
    vi.mocked(getX19Config).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useX19Config({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshX19Config(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getX19Config>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getX19Config>>>()
    vi.mocked(getX19Config).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshX19Config(true)
      refreshC = result.current.refreshX19Config(true)
    })
