...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'setup.ready') {
    // The boot bootstrap (x19_cli/free_tier_bootstrap.py) resolved the
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (X19 activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as X19Skin | undefined, { apply: true })
    }
