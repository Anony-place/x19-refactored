import { type GatewayEvent, registryBackendScopeKey } from '@x19/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshX19Config, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshX19Config — two REST calls (config + defaults) per event, per
