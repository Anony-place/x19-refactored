import type { BillingBlock } from '@x19/shared'
 ...
 * billing-specific toast — never the generic "X19 error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'X19 reported an error',
