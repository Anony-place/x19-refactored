...
import { getX19Config, getX19ConfigDefaults } from '@/x19'
...
interface X19ConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useX19Config({ activeSessionIdRef }: X19ConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshX19Config = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getX19Config(), getX19ConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying X19' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshX19Config, sttEnabled, voiceMaxRecordingSeconds }
}
