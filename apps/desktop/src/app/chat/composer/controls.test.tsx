...
describe('HUD mode', () => {
  it('keeps the mic inline, fans the toggles on hover, and offers no exit in the docked composer', async () => {
    ...
    expect(screen.getByLabelText('Wake word "hey x19"')).toBeTruthy()
...
describe('wake-word ear visibility', () => {
  ...
  const findEar = async () => {
    ...
    return screen.findByLabelText('Wake word "hey x19"')
  }

  it('stays reachable during a busy agent turn', async () => {
    applyWakeStatus({ available: true, enabled: true, listening: true, phrase: 'hey x19' })
  ...
  it('stays reachable (enabled in config) even when a start was refused', async () => {
    applyWakeStatus({ available: true, enabled: true, listening: false, phrase: 'hey x19' })
  ...
  it('stays reachable (never hides) even when unavailable and not enabled', async () => {
    applyWakeStatus({ available: false, enabled: false, listening: false, phrase: 'hey x19' })
    applyWakeStartResult({ hint: 'run `x19 tools` (Voice section)', reason: 'unavailable', started: false })
    renderControls()
  ...
  it('shows a disabled paused ear inside the voice-conversation pill', () => {
    ...
    const ear = screen.getByLabelText('Wake word: "hey x19" — paused during voice chat')
