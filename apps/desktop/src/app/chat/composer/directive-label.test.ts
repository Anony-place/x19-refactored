...
import { x19DirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
describe('one label per reference, on every surface', () => {
  ...
  it('an agent mention inserts as plain @name text, never an @simple: chip', () => {
    ...
    expect(x19DirectiveFormatter.serialize(item)).toBe('@mr-tester')
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/Anony-place/x19-refactored/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/Anony-place/x19-refactored/pull/74533'
