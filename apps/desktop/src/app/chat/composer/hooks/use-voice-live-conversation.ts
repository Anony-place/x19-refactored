...
interface VoiceLiveConversationOptions {
  ...
  enabled: boolean
  onFatalError?: () => void
  /** Interrupt the in-flight X19 turn (Stop-button seam). Fired when a new
...
/** Turn transcript fragments into the X19 turn: `prompt` is what the user
 ...
 * X19. There is no `transcribing` phase: the voice model owns speech.
 */
export function useVoiceLiveConversation({
    ...
    const session = new VoiceLiveSession({
      ...
      onDelegation: (delegationId, context) => {
        ...
        void Promise.resolve(latest.current.onSubmit(prompt, voiceContext)).catch(error => {
          ...
          session.speak(delegationId, 'Sorry, I could not reach X19 for that request.')
          setDelegation(null)
          refreshStatus()
        })
      },
  ...
  // Drive the reply back into the voice: stream commentary as X19 writes
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (tool && tool !== lastToolLabelRef.current) {
        ...
        session.think(delegationId, `X19 is working: ${tool}. Not done yet.`)
      }
      ...
      if (
        ...
        if (spokenLengthRef.current === 0) {
          session.think(delegationId, 'X19 finished that request without a spoken result.')
        }
