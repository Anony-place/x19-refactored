...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/x19'
...
// highlight markers (sqlite snippet() delimiters — see x19_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('x19:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('x19:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask X19 to
