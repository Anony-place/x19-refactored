...
import type { SessionInfo } from '@/x19'
...
// zero X19 sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, X19GitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, X19GitWorktree[]>>({})
