...
import { X19_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'x19:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.x19Desktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(X19_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
