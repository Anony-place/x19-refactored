...
const preamble = () => `  var w = window;
  ...
  w.__x19ActHolder = w.__x19ActHolder || {};
  w.__x19Act = ${actEngineSource()};
  w.__x19Watch_fn = (${watchInPage.toString()});
  w.__x19WatchTag = ${WATCH_TAG};
  var holder = w.__x19ActHolder;
  var act = function (a) { return w.__x19Act(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__x19Watch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__x19Hit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'X19-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
