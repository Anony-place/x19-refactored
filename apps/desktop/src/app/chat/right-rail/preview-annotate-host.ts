...
export function overlayInstallScript(source: string): string {
  return '(function(){var api=' + source + ';window.__x19Annotate=api;api.install();})()'
}
...
export async function teardownAnnotateOverlay(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    (function () {
      if (window.__x19Annotate && window.__x19Annotate.teardown) {
        window.__x19Annotate.teardown();
      }
      window.__x19Annotate = null;
    })()
  `)
}
...
export async function waitAnnotateEvent(guest: PreviewAnnotateGuest): Promise<AnnotatePageEvent> {
  const event = await guest.executeJavaScript(`
    window.__x19Annotate ? window.__x19Annotate.wait() : Promise.resolve({ type: 'end' })
  `)
...
export async function syncAnnotatePins(guest: PreviewAnnotateGuest, pins: AnnotatePinChrome[]): Promise<void> {
  await guest.executeJavaScript(`
    window.__x19Annotate && window.__x19Annotate.showPins(${JSON.stringify(pins)})
  `)
}
...
export async function showAnnotateDraft(
  ...
  await guest.executeJavaScript(`
    window.__x19Annotate && window.__x19Annotate.showDraft(${JSON.stringify(rect)}, ${number})
  `)
}

export async function hideAnnotateDraft(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    window.__x19Annotate && window.__x19Annotate.hideDraft()
  `)
}
...
export async function captureAnnotateCrop(
  ...
  await tryGuest(guest, 'window.__x19Annotate ? window.__x19Annotate.beginCapture() : null')

  try {
    return await guest.capture(padRect(rect))
  } finally {
    await tryGuest(guest, 'window.__x19Annotate && window.__x19Annotate.endCapture()')
  }
}
