...
function setWorkspaceTransfer(transferring: boolean): void {
  window.x19Desktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.x19Desktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.x19Desktop?.hud?.beginMove?.()
}
 ...
 * (see x19:hud:move-by — a transparent frameless window drifts wider on
