import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@x19/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as X19Gateway | null
  const gatewayRef = useRef<X19Gateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.x19Desktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<X19Gateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as X19Gateway | null
      }),
    []
  )
