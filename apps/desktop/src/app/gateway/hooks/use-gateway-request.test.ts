...
vi.mock('@/x19', async importOriginal => {
  const actual = await importOriginal<typeof X19Module>()

  class FakeX19Gateway {
  ...
  return { ...actual, X19Gateway: FakeX19Gateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as X19Gateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as X19Gateway, 'default')
  $gateway.set(primary as unknown as X19Gateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.x19Desktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as X19Gateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { x19Desktop: unknown }).x19Desktop = { getConnection }
    $gateway.set(dropped)
