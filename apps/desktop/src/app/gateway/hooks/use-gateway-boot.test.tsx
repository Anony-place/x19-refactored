...
// bug that drives the REAL useGatewayBoot hook + REAL X19Gateway through a
...
function Harness({
  ...
  refreshX19Config = async () => undefined,
  refreshSessions
}: {
  beforeConnectionSwitch?: () => void
  refreshX19Config?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
beforeEach(() => {
  ...
  ;(window as { x19Desktop?: unknown }).x19Desktop = fakeDesktop()
...
afterEach(() => {
  ...
  window.localStorage.removeItem('x19.desktop.workspace-cwd')
  $currentCwd.set('')
  $busy.set(false)
  $awaitingResponse.set(false)
})
...
describe('default-route profile adoption', () => {
  it.each([null, 'coder-remote'])(
    ...
    async connectionId => {
      ...
      ;(window as { x19Desktop?: unknown }).x19Desktop = desktop
      render(<Harness />)
      await flushAsync()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  ...
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForX19) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startX19()'s remote branch awaits waitForX19() for 45s before it
    ...
    // After ~45s waitForX19 gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('X19 backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshX19Config = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshX19Config={refreshX19Config} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForX19, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting X19…" forever; the renderer needs to own its own bound
  ...
  it('manual reconnect replaces only the active secondary route', async () => {
    ;(window as { x19Desktop?: unknown }).x19Desktop = {
      ...fakeDesktop(),
      getConnectionFor: vi.fn(async () => coderConn),
      getGatewayWsUrlFor: vi.fn(async () => coderConn.wsUrl)
    }
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote X19 gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "X19 couldn't start" overlay — the socket is open
  ...
  it('a backend exit while the boot overlay is up fails the overlay and does not add a dead-button toast', async () => {
    ...
    // X19" toast here would do nothing when clicked; the overlay's own
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\X19')
    expect($currentCwd.get()).toBe('C:\\X19')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startX19() rejects with a
  ...
  it('RETRY CONTRACT: a post-connect failure stays terminal even when its socket closes before boot catches it — a closed socket after a good dial is not a dial failure', async () => {
    ...
    const refreshX19Config = vi.fn(async () => {
    ...
    expect(refreshX19Config).toHaveBeenCalledTimes(1)
    expect($desktopBoot.get().error).toBeTruthy()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
    await advanceBackoff()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
  ...
  it('a failed cold boot keeps its recovery surface while main replays cold-boot progress behind it (#112899)', async () => {
    // Main keeps startX19() available after the renderer's boot concluded
    ...
    const desktop = fakeDesktop()
    desktop.getConnection = vi.fn(async () => {
      throw new Error('X19 backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example')
    })
    desktop.getBootProgress = vi.fn(async () => ({
      error: 'X19 backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example',
      fakeMode: false,
      message: 'Desktop boot failed',
      phase: 'backend.error',
      progress: 24,
    ...
    for (let replay = 0; replay < 2; replay += 1) {
      act(() => {
        ...
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Connecting to remote X19 backend at https://gateway.tailnet.example:8443',
          phase: 'backend.remote',
          progress: 24,
          running: true,
          timestamp: Date.now()
