import type { McpCatalogResponse, McpServerSummary } from '@/types/x19'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.x19Desktop.api<McpTestResult>({
 ...
 *  `saveX19Config`, this REPLACES rather than deep-merges, so deletes,
...
// `x19 mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveX19Config.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return x19Api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(
  ...
  return window.x19Desktop.api<McpServerSummary>({
    ...capabilityScoped(profile),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope, detectApps = false): Promise<McpCatalogResponse> {
  return window.x19Desktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.x19Desktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),
