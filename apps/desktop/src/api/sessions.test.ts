...
const x19Api = vi.mocked(client.x19Api)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    x19Api.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'x19-pi' })

    await deleteSession('sess-1', { connectionId: 'x19-pi', profile: 'tommy' })

    expect(x19Api.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((x19Api.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('getSession dial priority', () => {
  it('does not dial an explicitly scoped session read foreground', async () => {
    ...
    x19Api.mockResolvedValue({ id: 'sess-5' } as never)
    ...
    expect(x19Api.mock.calls[0][0]).toMatchObject({ profile: 'tommy', connectionId: 'local' })
    expect(x19Api.mock.calls[0][0]).not.toHaveProperty('priority')
  })
})
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = x19Api.mock.calls[0][0] as { body: Record<string, unknown> }
