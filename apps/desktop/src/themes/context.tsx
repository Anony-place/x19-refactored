...
import { ensureContrast, mix, parseColor } from '@x19/shared/color'
...
const SKIN_KEY = 'x19-desktop-theme-v2'
const MODE_KEY = 'x19-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'x19-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'x19-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'x19-desktop-active-profile-v1'
...
// mix / ensureContrast live in @x19/shared/color (shared with the TUI);
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark', chatFontFamily = $chatFontFamily.get()) {
  ...
  root.dataset.x19Theme = skinName
  root.dataset.x19Mode = rendered
  ...
  window.x19Desktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('x19-boot-background', chromeBg)
    window.localStorage.setItem('x19-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.x19ThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.x19Desktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (X19 authoring/activating a skin from a
