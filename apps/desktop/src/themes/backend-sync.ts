/**
 * Live skin sync from the X19 backend.
 *
 * The backend resolves the active skin (built-in or `$X19_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (X19
...
const BACKEND_THEMES_KEY = 'x19-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: X19Skin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as X19Skin)
