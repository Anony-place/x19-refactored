...
vi.mock('@/x19', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), x19Api: vi.fn() }))
...
const { deleteProfile, x19Api } = await import('@/x19')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its X19_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { x19Desktop: unknown }).x19Desktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(x19Api).toHaveBeenNthCalledWith(
      1,
    ...
    expect(x19Api).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(x19Api).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
