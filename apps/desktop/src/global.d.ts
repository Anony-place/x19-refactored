import type { GatewayWsUrlResult } from '@x19/shared'
import type { TranslucencyState } from '@x19/shared/translucency'

import type { ScreenshotApi } from '../electron/command-screenshot-types'
import type { X19Notification } from '../electron/notification-types'
...
declare global {
  interface Window {
    x19Desktop: {
      ...
      // Resume this session in the user's own terminal emulator (`x19 --tui
      ...
      // over x19:window-control; Electron/OS chrome owns them elsewhere.
      windowControls: {
        custom: boolean
        minimize: () => void
        toggleMaximize: () => void
      ...
      // X19 Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new X19_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: X19ApiRequest) => Promise<T>
      notify: (payload: X19Notification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<X19ReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<X19ReadFileTextResult>
      selectPaths: (options?: X19SelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<X19PreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<X19PreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<X19PreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: X19ActiveWork) => void
      setTitleBarTheme?: (payload: X19TitleBarTheme) => void
       ...
       *  (X19_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (X19_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh X19_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      readDir: (path: string) => Promise<X19ReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<X19_HOME>/desktop-plugins),
      ...
      /** LOCAL `<X19_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<X19GitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<X19GitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<X19GitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<X19RepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: X19ReviewScope, baseRef?: null | string) => Promise<X19ReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: X19ReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<X19ReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<X19RepoPullRequests>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: X19TerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<X19TerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: X19WindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: X19PreviewFileChanged) => void) => () => void
...
export interface X19TerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface X19TerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `x19 update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  x19Root?: string
...
export interface X19Connection {
  baseUrl: string
...
export interface X19TitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface X19ActiveWork {
  count: number
  titles: string[]
}

export interface X19WindowState {
  customWindowControls?: boolean
  darwinMajor?: number
  isFullscreen: boolean
  isMaximized?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is an X19 Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted X19 Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected X19 Cloud org (slug or id) to
...
// A discovered X19 Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface X19ApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface X19PreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface X19ReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface X19PreviewWatch {
  id: string
  path: string
}
...
export interface X19GitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
// preselect it.
export interface X19GitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface X19RepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface X19RepoStatus {
  branch: null | string
...
// turn began.
export type X19ReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface X19ReviewFile {
  path: string
  added: number
  removed: number
...
export interface X19ReviewList {
  files: X19ReviewFile[]
...
export interface X19BranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface X19RepoPullRequests {
  ghReady: boolean
  prs: X19BranchPullRequest[]
}
...
export interface X19ReviewShipInfo {
  ghReady: boolean
  pr: X19ReviewPr | null
}

export interface X19ReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface X19ReadDirResult {
  entries: X19ReadDirEntry[]
  error?: string
}

export interface X19PreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface X19SelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
