import type { ConnectionRequestPayload } from '@x19/shared'
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when X19 doesn't know how to remove it. */
...
export interface FreeTierStatus {
   ...
   *  (`x19_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in x19_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `x19 model` uses — instead of
...
export type X19ConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['x19-bots'].title) — the name the
...
// non-desktop) X19 simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (x19.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors x19_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with X19, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with X19 but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `x19 tools` active-provider detection). */
  is_active: boolean
 ...
 *  not X19); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** x19-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** x19-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/x19/update/check` — the backend's own update state.
