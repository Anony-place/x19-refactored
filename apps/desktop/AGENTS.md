...
How to build X19 Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another X19 surface can also
...
- A **runtime home change** (switching the underlying `X19_HOME` profile) is
...
previews) and the preview pane's `<webview>` (`persist:x19-preview`). Neither
...
- `main.ts` installs `electron/preview-guest-preload-entry.ts` via
  `will-attach-webview`, keyed on the `persist:x19-preview` partition only.
...
- `PreviewPane` admits `http:`/`https:` only (`src/lib/preview-external.ts`)
  and hands the URL to the existing `x19:openExternal` IPC, which applies
...
its shape. "Plugin" means several unrelated things across X19 — do not assume
...
`env -u NODE_ENV X19_GUEST_ONBOARDING=1 X19_HOME=<tmp>/.x19 X19_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`X19_PORTAL_BASE_URL=http://127.0.0.1:8765 X19_ANON_API_SECRET=test-secret X19_SHARED_AUTH_DIR=<tmp>/.x19/shared`
...
The free tier (a Nous identity with no account, `x19_cli/anon_auth.py`) reaches the renderer
