#!/usr/bin/env node
// set-exe-identity.mjs — stamp the X19 icon + version metadata onto the
// built X19.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked X19.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `x19 desktop`, the installer's
...
// Stamp the X19 icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(
  ...
  console.log('[set-exe-identity] done — X19 icon + identity stamped')
}
