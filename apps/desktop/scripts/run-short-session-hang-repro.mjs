...
const UPSTREAM_URL = 'https://github.com/Anony-place/x19-refactored.git'
...
const LEGACY_STORAGE_KEY = 'x19.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'x19.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.x19Desktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `x19-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const x19Home = join(sandbox, 'x19-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(x19Home, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(x19Home, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        X19_DESKTOP_APP_NAME: `X19ShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        X19_DESKTOP_X19_ROOT: target.targetRoot,
        X19_DESKTOP_IGNORE_EXISTING: '1',
        X19_DESKTOP_USER_DATA_DIR: userData,
        X19_HOME: x19Home,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'x19-short-session-ab-'))
  const prepared = []
