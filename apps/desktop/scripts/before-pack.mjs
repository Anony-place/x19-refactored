 ...
 * product name (`X19`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/X19'
 *
 * This is a hard failure with no obvious cause for the user — `x19 desktop`
 ...
 * (the mac-unpacked X19.app bundle) and Windows (win-unpacked), so we
 ...
 * X19.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `x19 desktop --build-only` (x19_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'X19.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'X19'}.exe`
