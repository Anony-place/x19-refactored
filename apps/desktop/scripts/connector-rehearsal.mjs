...
//   D=$(mktemp -d) && env -u X19_SHARED_AUTH_DIR X19_GUEST_ONBOARDING=1 X19_SKIP_INTRO=1 \
//     X19_HOME="$D/home" X19_DESKTOP_USER_DATA_DIR="$D/userdata" x19 desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'x19-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.X19_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set X19_DESKTOP_PYTHON to your installed X19 Python executable.')
...
for (const dir of ['.x19/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, X19_HOME: path.join(sandbox, '.x19'), X19_SHARED_AUTH_DIR: path.join(sandbox, '.x19/shared'),
  X19_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), X19_DESKTOP_CWD: path.join(sandbox, 'work'),
  X19_DESKTOP_X19_ROOT: root, X19_DESKTOP_PYTHON: python,
  X19_DESKTOP_IGNORE_EXISTING: '1', X19_DESKTOP_APP_NAME: 'X19 Connector Rehearsal',
  X19_GUEST_ONBOARDING: '1', X19_SKIP_INTRO: '1', X19_DESKTOP_DEV_SERVER: url, X19_DESKTOP_CDP_PORT: '9344'
})
