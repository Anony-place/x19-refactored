...
// no longer bundles the X19 Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'X19.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'X19'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'X19.exe'),
...
// Default X19_HOME for non-sandboxed runs -- matches main.ts's
// resolveX19Home(). On Windows it's %LOCALAPPDATA%\x19; elsewhere
// it's ~/.x19. The fresh-install sandbox launchFresh() sets its own
// X19_HOME and never touches this.
const DEFAULT_X19_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'x19')
  }
  return path.join(os.homedir(), '.x19')
})()
const VENV_ROOT = path.join(DEFAULT_X19_HOME, 'x19', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'x19-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.X19_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `X19-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `X19-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `X19-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'X19-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.X19_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.X19_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const x19Home = path.join(sandbox, 'x19-home')
  ...
  env.X19_DESKTOP_CWD = cwd
  env.X19_DESKTOP_IGNORE_EXISTING = '1'
  env.X19_DESKTOP_TEST_MODE = 'fresh-install'
  env.X19_DESKTOP_USER_DATA_DIR = userDataDir
  env.X19_HOME = x19Home
  delete env.X19_DESKTOP_X19
  delete env.X19_DESKTOP_X19_ROOT
  ...
  console.log(`  X19_HOME: ${x19Home}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(x19Home, 'x19', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The X19 Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of x19_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'x19', 'x19_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing X19
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + X19_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  X19_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
