 ...
 *
 * Stamps the X19 icon + identity onto the packed Windows X19.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `x19 desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'X19')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'X19'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); X19.exe keeps the stock Electron icon`)
  }
}
