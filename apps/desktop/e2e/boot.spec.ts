 ...
 * packaged binary) with a real `x19 serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → x19 serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with X19 title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('X19')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { x19Desktop?: Record<string, unknown> }).x19Desktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → x19 serve is spawned → WS connects → config
