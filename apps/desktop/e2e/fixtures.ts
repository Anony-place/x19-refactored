/**
 * Shared E2E fixtures for the X19 desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → x19 serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `x19 desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `x19-e2e-${prefix}-${Math.random()}`))
  const x19Home = path.join(root, 'x19-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(x19Home, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(x19Home, 'config.yaml')
 ...
 */
export function writeEnvFile(x19Home: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(x19Home, '.env')
 ...
 */
function writeEmptyConfig(x19Home: string): void {
  const configPath = path.join(x19Home, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - X19_HOME → sandbox x19-home (isolated config/sessions)
 *  - X19_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - X19_DESKTOP_IGNORE_EXISTING=1 → don't pick up `x19` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - X19_DESKTOP_X19_ROOT → REPO_ROOT (dev checkout resolution)
 *  - X19_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    X19_HOME: sandbox.x19Home,
    X19_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    X19_DESKTOP_IGNORE_EXISTING: '1',
    X19_DESKTOP_X19_ROOT: REPO_ROOT,
    X19_DESKTOP_APP_NAME: `X19E2E-${Date.now()}`,
    ...
    X19_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated X19_HOME + userData
 * @param env      - the process environment (already has X19_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.x19Home)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via X19_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`x19 serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.x19Home, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { X19_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to X19 backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'X19.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'X19.app', 'Contents', 'MacOS', 'X19')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'x19')
}
 ...
 * progress without spawning a real X19 backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * X19_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    X19_DESKTOP_BOOT_FAKE: '1',
    X19_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + x19-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).X19_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).X19_DESKTOP_X19
  delete (env as Record<string, string | undefined>).X19_DESKTOP_X19_ROOT
