 ...
 * second gateway, "Homelab", is a REAL second `x19 serve` this spec spawns
 * with its own X19_HOME, profiles and session token, registered in the v2
...
function findX19Binary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'x19')
  ...
  throw new Error('x19 binary not found: create the repo venv (uv sync) or put x19 on PATH')
}
...
/**
 * Spawn a second, fully real `x19 serve` as the remote gateway. Its
 * session token is pinned through X19_DASHBOARD_SESSION_TOKEN so the
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        X19_HOME: home,
        X19_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-X19-Session-Token': REMOTE_TOKEN },
      })
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.x19Home, ['research'])
