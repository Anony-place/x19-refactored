...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.x19Home, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.x19Home)
  const builder = await RealSessionBuilder.start(sandbox.x19Home)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.x19Home, mock.url)
  fs.appendFileSync(path.join(sandbox.x19Home, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
