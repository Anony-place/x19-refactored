 ...
 * AIAgent loop, using the E2E mock provider configured in `x19Home`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(x19Home: string) {
  ...
  static async start(x19Home: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(x19Home)
