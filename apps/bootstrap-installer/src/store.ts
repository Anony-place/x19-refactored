...
export const $x19Home = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.x19/logs/bootstrap-installer.log')
    $x19Home.set('~/.x19')
  ...
  try {
    const [logPath, x19Home, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_x19_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch X19" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (X19-Setup.exe --update);
...
export async function launchX19Desktop(): Promise<void> {
  ...
  await invoke('launch_x19_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'X19 repository' },
