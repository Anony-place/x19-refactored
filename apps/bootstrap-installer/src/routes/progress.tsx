...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating X19' : 'Setting up X19'

  const description = isUpdate
    ? 'X19 is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The X19 installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="x19-fade-in flex h-full flex-col">
