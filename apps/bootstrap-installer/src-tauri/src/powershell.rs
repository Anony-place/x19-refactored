...
/// read side does not see EOF until the last of them closes it. `x19 update`
...
/// stranded with it.
///
/// Same bound `Invoke-X19Step` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `x19_home_override` propagates to the child as $X19_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, x19_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = x19_home_override {
        cmd.env("X19_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, x19_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = x19_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_x19_home() {
    ...
    /// child. `x19 update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
