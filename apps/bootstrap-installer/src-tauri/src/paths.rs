...
//!
//! Mirrors `x19_constants.get_x19_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\x19
//!   macOS:   ~/.x19
//!   Linux:   ~/.x19  (override via $X19_HOME)
//!
//! NOTE (macOS): Python's get_x19_home(), scripts/install.sh, and the
//! Electron desktop's resolveX19Home() ALL use ~/.x19 on macOS — there
...
/// Returns the canonical X19 home directory, respecting $X19_HOME if set.
pub fn x19_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("X19_HOME") {
    ...
    {
        // %LOCALAPPDATA%\x19 — matches scripts/install.ps1's $X19Home.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("x19");
        }
    }

    // macOS + Linux + fallback: ~/.x19 (matches Python get_x19_home(),
    // install.sh, and the Electron desktop's resolveX19Home()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".x19");
    }
...
/// X19_HOME so it survives repo checkout deletion (unlike anything under
/// x19/).
///
/// On Windows this is `%LOCALAPPDATA%\x19\x19-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_x19`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under X19_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves X19_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    x19_home().join(".x19-update-in-progress")
}
...
pub fn copy_self_to_x19_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to X19_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_X19_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".x19-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under X19_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[x19-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("X19_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_x19_home() -> String {
    x19_home().to_string_lossy().into_owned()
}
