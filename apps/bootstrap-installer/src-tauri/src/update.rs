...
//!
//! Driven when the installer is launched as `X19-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old X19 desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `x19 update`
//!      or repair bootstrap can race locked files),
//!   2. run `x19 update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in x19_cli/main.py),
//!   3. run `x19 desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `x19 update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_x19) and
...
/// `x19 update` exit code meaning "another x19 process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// x19_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `x19 update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `x19 update` claims
/// the same file (see `x19_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `x19 update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in x19_cli/update_lock.py — all three read
...
/// a dashboard-spawned `x19 update`) still blocks.
...
/// the marker name our PID — because a `x19 update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let x19_home = crate::paths::x19_home();
    let install_root = x19_home.join("x19");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_x19`
    ...
    //
    // The same marker is the cross-process update lock (x19_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `x19 update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another X19 update is already running (PID {}, started {} ago). \
    ...
    let x19 = resolve_x19(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the x19 CLI under {}. Is X19 installed? \
    ...
    // async on Windows. If it still holds the venv shim, `x19 update`
    ...
    // ---- stage 2: x19 update -----------------------------------------
    // Pass --branch so `x19 update` targets the branch this installer was
    ...
    // without --branch, `x19 update` switches the checkout to `main` (a
    ...
    // --force skips `x19 update`'s Windows running-exe guard (which would
    ...
    // time `x19 update` runs there is no legitimate x19.exe to protect,
    // and the guard would only produce a false "X19 is still running" stop.
    //
    ...
    // probe only checks the x19.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all X19 windows" message.
    ...
    // Retry-once for the update-boundary crash. `x19 update` lazily imports
    ...
    // second `x19 update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close X19" state
    // a retry can't fix.
    ...
    // checkout predating the X19_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "X19 is still running. Close all X19 windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "x19 update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::x19_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: x19 desktop --build-only ----------------------------
    // `x19 update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `x19 update` already does above, and `x19 update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `x19 desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch X19 manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_x19_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for X19 to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // X19.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] X19 still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_x19(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_x19(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("X19.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("X19.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\x19.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also X19.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `x19 <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `x19 update` is precisely the shape that
...
/// Path to the venv x19 shim under an install root, regardless of existence.
fn venv_x19(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("x19.exe")
    } else {
        install_root.join("venv").join("bin").join("x19")
    }
}

/// Resolve the x19 CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `x19` on PATH.
fn resolve_x19(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_x19(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "x19.exe" } else { "x19" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "X19_HOME".to_string(),
        x19_home.as_os_str().to_os_string(),
    )];
    // `x19 update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `x19 update` child claims that SAME lock (x19_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("X19 is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in x19_cli/update_lock.py.
    envs.push((
        "X19_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        x19_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_x19_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no X19.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.x19-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.x19-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::x19_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_x19_is_under_install_root() {
        let root = Path::new("/x/x19");
        let shim = venv_x19(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/x19"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/x19"));
        assert!(
            envs.iter().any(|(k, v)| k == "X19_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the x19 update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_x19(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/x19");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.x19\x19\venv\scripts\X19.EXE"),
            Path::new(r"c:\users\tester\.x19\x19\venv\Scripts\x19.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.x19\x19\apps\desktop\X19.exe"),
            Path::new(r"C:\Users\tester\.x19\x19\venv\Scripts\x19.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".x19-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `x19 update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(x19Home, child.pid) races ahead
    ...
    // checkout's `x19 update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".x19-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running X19, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/X19.app"]),
            Some(PathBuf::from("/Applications/X19.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("X19.app");
        let tmp = base.join("X19.app.x19-update-new");
        let old = base.join("X19.app.x19-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("X19.app");
        let tmp = base.join("X19.app.x19-update-new"); // intentionally absent
        let old = base.join("X19.app.x19-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("X19.app");
        let tmp = base.join("X19.app.x19-update-new"); // absent
        let old = base.join("X19.app.x19-update-old");
        write_marker(&target, "OLD");
