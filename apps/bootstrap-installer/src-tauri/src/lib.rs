...
/// desktop app handing off via `X19-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under X19_HOME/logs/ so install
    ...
    // X19 is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "X19 installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // X19 is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "X19" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "X19" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_x19_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::x19_home().join("x19");
                if bootstrap::x19_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_x19_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "x19 already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running X19 Setup");
}
