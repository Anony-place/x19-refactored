...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update X19 Desktop and try again.')
    }
...
export interface X19WebSocketUrlOptions {
...
export function buildX19WebSocketUrl(options: X19WebSocketUrlOptions): string {
