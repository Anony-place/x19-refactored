...
The bug class: on Windows, when `x19.exe`/sibling shims could not be
...
import x19_cli._install_repair as ir
import x19_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("x19", "x19-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # x19.exe cannot be renamed; x19-gateway.exe can
    real_rename = Path.rename

    def deny_x19(self, target):
        if self.name == "x19.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_x19)
    monkeypatch.setattr(main_install_repair, "_x19_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "x19.exe" in exc_info.value.failed_shims
    # The successful rename (x19-gateway.exe) was rolled back.
    assert (scripts / "x19-gateway.exe").exists()
    assert not list(scripts.glob("x19-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("x19",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["x19.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "x19.exe" in out
