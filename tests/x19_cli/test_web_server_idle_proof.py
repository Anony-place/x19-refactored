"""A Desktop-pooled ``x19 serve`` child must PROVE it is idle before the Desktop may retire it
...
from x19_cli.web_server_idle_proof import idle_proof, pending_human_input
...
def _spawn_desktop_child(tmp_path: Path, name: str, *, busy: bool) -> subprocess.Popen:
    """Mirror the Desktop pool spawn: ``X19_DESKTOP=1`` (in-process cron ticker), a per-child
    X19_HOME, an ephemeral port, the session token the Desktop probes with. The busy child
    ...
    for k in ("X19_PARENT_PID", "X19_PARENT_START_MARKER", "X19_PARENT_NONCE"):
        env.pop(k, None)
    env.update(
        X19_HOME=str(home),
...
def _probe(port: int, token: str | None = TOKEN) -> tuple[int, dict]:
    req = urllib.request.Request(f"http://127.0.0.1:{port}/api/health/idle",
                                 headers={"X-X19-Session-Token": token} if token else {})
    try:
