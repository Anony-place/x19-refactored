...
A ``x19 update`` killed after git pull advanced HEAD but before the
...
- next ``x19 update`` with git already up to date still runs the
...
import x19_cli.main_web_build as main_web_build
import x19_cli.main_install_repair as main_install_repair
from x19_cli import update_cmd
import x19_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(x19_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(x19_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(x19_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(x19_main, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(
        x19_main,
        "_get_origin_url",
        lambda *a, **k: "https://github.com/Anony-place/x19-refactored.git",
    )
    monkeypatch.setattr(update_cmd, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        x19_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(x19_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        x19_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(x19_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        x19_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        x19_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(x19_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(x19_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(x19_main, "_build_web_ui", lambda *a, **k: None)
    ...
    import x19_cli.gateway as x19_gateway

    monkeypatch.setattr(
        x19_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(x19_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        x19_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "x19_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_x19_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_x19_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_x19_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "x19_cli.gateway.find_gateway_pids", lambda **k: []
    )
    ...
    monkeypatch.setattr("x19_cli.gateway_windows.is_installed", lambda: False)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``x19-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import x19_cli.update_inventory as ui
    ...
    import x19_cli.process_identity as pi
    ...
    latest = get_x19_home() / "logs" / "update_receipts" / "latest.json"
...
def test_clean_update_defers_desktop_owned_serve_and_clears_marker(
    ...
    x19_main.cmd_update(args)  # no SystemExit(1)
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(x19_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "x19 gateway restart" in err
...
#
# `systemctl --user restart x19-gateway` never runs this module's clear path, and an update
...
def test_startup_warn_kept_without_positive_evidence(monkeypatch, capsys, disk_sha, fleet):
    ...
    _patch_marker_sha(monkeypatch, disk_sha)
    monkeypatch.setattr("x19_cli.update_receipt.collect_fleet_versions", lambda **kwargs: fleet)
...
def test_startup_warn_silent_when_failed_receipt_already_restarted_fleet(monkeypatch, capsys):
    ...
    restart it performed; ``x19 update``'s catch-up still owes the fleet the checkout."""
        ...
        json.dumps(
            {
                ...
                "stop_reason": "AttributeError: module 'x19_cli.main_dashboard' has no attribute 'x'",
                "pre_update": {"sha": pre}, "post_update": {"sha": pulled},
                "gateway_restart": {
                    "restarted_services": ["x19-gateway"], "relaunched_profiles": [],
