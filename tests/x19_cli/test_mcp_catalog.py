"""Tests for x19_cli.mcp_catalog and x19_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("X19_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_x19_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp X19_HOME."""
    hh = tmp_path / "x19-home"
    hh.mkdir()
    monkeypatch.setenv("X19_HOME", str(hh))
    monkeypatch.setattr(
        "x19_cli.config.get_x19_home", lambda: hh
    )
    monkeypatch.setattr(
        "x19_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "x19_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_x19_home() lazily through x19_constants
    monkeypatch.setattr(
        "x19_constants.get_x19_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from x19_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from x19_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_onboarding_metadata_is_additive(self, catalog_dir):
        from x19_cli.mcp_catalog import _build_server_config, _parse_manifest
        from x19_cli.web_routers.mcp import _catalog_entry_json
    ...
    def test_suggest_discovery_metadata_is_bounded_data(self, catalog_dir):
        from x19_cli.mcp_catalog import CatalogError, _parse_manifest
        from x19_cli.web_routers.mcp import _catalog_entry_json
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from x19_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from x19_cli.mcp_catalog import _build_server_config
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from x19_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from x19_cli.config import load_config
        from x19_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from x19_cli.mcp_catalog import install_entry
        from x19_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from x19_cli.mcp_catalog import install_entry, uninstall_entry
        from x19_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from x19_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from x19_cli.mcp_picker import show_catalog
    ...
    def test_install_by_name_success(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from x19_cli.mcp_picker import install_by_name
        from x19_cli.config import load_config
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from x19_cli import mcp_catalog
        from x19_cli.mcp_catalog import _do_git_install
        ...
        from x19_cli import git_credentials
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "x19_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no X19_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("X19_OPTIONAL_MCPS", raising=False)
        from x19_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything X19 fetches/launches is pinned
        to an exact version.
