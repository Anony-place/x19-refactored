...
x19_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `x19 config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_x19_home = get_x19_home()`` at import time (module
    singleton), so monkeypatching X19_HOME after import doesn't move it.
    ...
    this only matters for tests that swap X19_HOME.)
    """
    import cli
