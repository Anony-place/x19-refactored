"""Tests for kanban board export / import (``x19_cli.kanban_transfer``).
...
from x19_cli import kanban_transfer as kt
from x19_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("X19_KANBAN_HOME", str(root))
        for var in ("X19_KANBAN_DB", "X19_KANBAN_WORKSPACES_ROOT",
                    "X19_KANBAN_ATTACHMENTS_ROOT", "X19_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this X19"):
