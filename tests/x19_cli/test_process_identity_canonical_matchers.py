...
from x19_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from x19_cli.update_cmd_windows import _x19_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m x19_cli.main` spawn shape —
# a user-launched `x19 serve` / `x19 dashboard` is refused on, never tree-killed.
CMDLINES = [
    ("python -m x19_cli.main serve " + LOOPBACK, "serve", True, True),
    ("python -m x19_cli.main dashboard", "dashboard", False, True),
    ("/venv/bin/x19 serve --isolated --host=127.0.0.1 --port=0 --ssh-owner-nonce abc", "serve", True, False),
    (r"C:\x19\.venv\Scripts\x19.exe serve --host 100.106.105.2 --port 9119", "serve", False, False),
    ("x19.exe dashboard", "dashboard", False, False),
    ("x19 --profile ops serve " + LOOPBACK, "serve", True, False),
    ("x19 -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m x19_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("x19 --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
    ("x19 gateway run --replace", "gateway", False, False),
    ("x19 chat --model serve", "chat", False, False),
...
def test_kill_and_relaunch_predicates_agree_with_the_canonical_holder_matcher(
    ...
    assert _x19_holder_subcommand(cmdline) == subcommand
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("x19 serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("x19 serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("x19 serve --host localhost --port 0")
