...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        with patch.object(X19CLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(X19CLI, "_is_session_yolo_active", return_value=False):
            text = X19CLI._build_status_bar_text(host, width=width)
