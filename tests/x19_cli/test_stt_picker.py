"""Tests for the Speech-to-Text category in `x19 tools` (tools_config).
...
from x19_cli.tools_config import (  # noqa: E402
...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from x19_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY
...
class TestModelPicker:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "x19_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from x19_cli.tools_config import _POST_SETUP_READY
