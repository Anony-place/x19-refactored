...
def test_presets_stale_when_a_staged_model_has_no_section(x19_home):
    from x19_cli.local_runtime.bootstrap import _presets_stale

    _stage(x19_home, "model-a")
    _stage(x19_home, "model-b")
    _write_presets(x19_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(x19_home):
    from x19_cli.local_runtime.bootstrap import _presets_stale
...
def test_legacy_presets_without_model_paths_are_regenerated(x19_home):
    from x19_cli.local_runtime.bootstrap import _presets_stale

    _stage(x19_home, "model-a")
    _write_presets(x19_home, "model-a")
    ini = x19_home / "runtimes/llamacpp/presets.ini"
    ini.write_text("[model-a]\nctx-size = 65536\n")
    assert _presets_stale()


def test_no_models_is_never_stale(x19_home):
    from x19_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(x19_home, monkeypatch):
    ...
    import x19_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "x19_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "x19_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(x19_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(x19_home, monkeypatch):
    import x19_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "x19_cli.local_runtime.endpoint._state_endpoint", lambda: None)
