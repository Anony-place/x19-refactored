"""Tests for the Ctrl+S prompt stash wiring inside X19CLI.

The state machine itself is covered by tests/x19_cli/test_prompt_stash.py. These
tests verify the cli.py side:
  - X19CLI.__init__ creates a PromptStash
...
tests/x19_cli/test_cli_extension_hooks.py.
"""
...
def _make_cli(**kwargs):
    """Create a X19CLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports x19_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``X19CLI.run()``'s local ``KeyBindings`` instance, so they are not
