...
the SAME provider universe as ``x19 model`` (the CLI/TUI picker).
...
def test_every_x19_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `x19 model` universe."""
    ...
    assert not missing, (
        "providers shown in `x19 model` but not configurable in the desktop "
