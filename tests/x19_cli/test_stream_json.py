"""``x19 chat -q … --format stream-json`` emits a parseable JSONL event stream and nothing else on stdout."""
...
def _run_stream_json_chat(monkeypatch, capsys, run_conversation, credentials_ok=True):
    ...
    import cli
    import x19_cli.main as cli_entry
    ...
    monkeypatch.setattr(cli, "X19CLI", FakeCLI)
    ...
    monkeypatch.setattr("x19_cli.free_tier_bootstrap.run_bootstrap", lambda **_k: None)
    monkeypatch.setattr("x19_cli.quiet_single_query.continue_quiet_notify_completions", lambda *_a, **_k: None)
