...
calls ``os._exit(0)`` ~5ms after ``X19_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            X19_PARENT_PID=str(parent.pid),
            X19_PARENT_START_MARKER=drifted_marker,
            X19_PARENT_NONCE="nonce-95693",
            X19_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("X19_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "X19_BACKEND_READY"), "backend never announced READY"
