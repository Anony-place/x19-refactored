...
board into `X19_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`x19 kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `x19 kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `X19_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("X19_KANBAN_BOARD")
    os.environ.pop("X19_KANBAN_BOARD", None)
    try:
        yield
    finally:
        if prev is None:
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("X19_KANBAN_BOARD") == "preset"
