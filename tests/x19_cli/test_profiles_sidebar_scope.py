...
def _uncached_sidebar_endpoints(monkeypatch):
    ...
    from x19_cli.web_routers import profiles as profiles_routes

    monkeypatch.setattr(profiles_routes, "_SIDEBAR_CACHE_TTL_SECONDS", 0.0)


@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_x19_home):
...
def client(monkeypatch, profiles_on_disk):
    ...
    import x19_state
    from x19_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    from x19_constants import get_x19_home
...
def _seed_project(home, name, folder):
    from x19_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_x19_home()``. If the fan-out failed to rebind
