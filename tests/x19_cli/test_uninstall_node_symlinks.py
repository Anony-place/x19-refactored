"""Tests for x19_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $X19_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the X19-managed node dir.
"""
...
def _make_x19_node(x19_home: Path) -> Path:
    """Create a fake $X19_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = x19_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    x19_home = fake_home / ".x19"
    _make_x19_node(x19_home)
    ...
    removed = uninstall.remove_node_symlinks(x19_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    x19_home = fake_home / ".x19"
    node_bin = _make_x19_node(x19_home)
