"""Tests for the update check mechanism in x19_cli.banner.
...
def git_repo(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "x19"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    monkeypatch.setenv("X19_HOME", str(tmp_path))
    monkeypatch.delenv("X19_REVISION", raising=False)
    ...
    monkeypatch.setattr("x19_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir


def _stub_git(monkeypatch, *, head=SHA_A, origin="https://github.com/Anony-place/x19-refactored.git"):
    calls = []
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with("Anony-place/x19-refactored", "main")
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``x19 update`` moving HEAD busts it at once;
