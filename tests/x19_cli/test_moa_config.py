...
from x19_cli.moa_config import (
...
def test_resolve_missing_moa_preset_has_actionable_error():
    ...
    assert "x19 moa list" in message
...
def test_validate_moa_payload_agrees_with_clean_slot():
    ...
    from x19_cli.moa_config import validate_moa_payload
...
def test_print_config_marks_aggregator_as_billed_and_warns_on_provider_mismatch(capsys):
    ...
    different provider than the main model, ``x19 moa list``/``configure`` say so."""
    from x19_cli import moa_cmd
