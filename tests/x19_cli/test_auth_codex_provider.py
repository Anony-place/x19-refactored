"""Tests for Codex auth — tokens stored in X19 auth store (~/.x19/auth.json)."""
...
def _setup_x19_auth(x19_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the X19 auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_x19_auth(x19_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (x19_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("X19_HOME", str(x19_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((x19_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``x19 auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``x19 auth add openai-codex``.
    ...
    (x19_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `x19 auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``x19 auth add openai-codex`` for accounts B and C
    ...
    (x19_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``x19 auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to X19 auth store, not ~/.codex/."""
    x19_home = tmp_path / "x19"
    ...
    _save_codex_tokens({"access_token": "x19-at", "refresh_token": "x19-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches X19 store
    assert not (codex_home / "auth.json").exists()

    # X19 auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "x19-at"


def test_resolve_returns_x19_auth_store_source(tmp_path, monkeypatch):
    x19_home = tmp_path / "x19"
    _setup_x19_auth(x19_home)
    monkeypatch.setenv("X19_HOME", str(x19_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "x19-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run x19 auth".
    """
    from x19_cli.auth import (
    ...
    assert "x19 auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from x19_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from x19_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch x19_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("x19_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
