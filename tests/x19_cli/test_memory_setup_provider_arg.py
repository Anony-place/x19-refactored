"""Tests for `x19 memory setup [provider]` routing.
...
``x19 memory setup honcho``) without the interactive picker — which matters
because the per-provider ``x19 <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "x19 memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets X19_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"X19_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("x19_cli.tools_config_cua.shutil.which", side_effect=which_side_effect), \
             patch("x19_cli.tools_config_cua.subprocess.run", fake_run):
