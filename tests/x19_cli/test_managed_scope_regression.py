...
def x19_home(tmp_path, monkeypatch):
    ...
    # /etc/x19 on the dev/CI box can't influence the test.
    monkeypatch.setenv("X19_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(x19_home, monkeypatch):
...
def test_env_expansion_in_user_config(x19_home, monkeypatch):
    from x19_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_x19_dotenv(x19_home=str(home))
