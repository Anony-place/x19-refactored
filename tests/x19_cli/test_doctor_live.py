"""Tests for ``x19 doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from x19_cli import doctor_live
from x19_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `x19 doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("x19_cli.doctor.X19_HOME", tmp_path / "home")
        monkeypatch.setattr("x19_cli.doctor.PROJECT_ROOT", tmp_path / "root")
