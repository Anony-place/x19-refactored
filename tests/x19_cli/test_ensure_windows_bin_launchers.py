"""``x19`` must survive git operations on the checkout (launcher layout).

The Windows ``x19`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``X19_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``x19\\bin``) was swept by ``x19 update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``x19`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``x19 update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``x19_constants.venv_bin_dir``), so these
...
from x19_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: X19_HOME/x19/venv/Scripts + launchers."""
    home = tmp_path / "x19"
    root = home / "x19"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under X19_HOME gains nothing anywhere."""
    home = tmp_path / "x19-home"
    home.mkdir()
    monkeypatch.setenv("X19_HOME", str(home))
    root = tmp_path / "src" / "x19"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``x19 -p <name>`` X19_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("X19_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `x19 update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "x19.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "x19.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\x19\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/X19/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``x19 update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/x19.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/x19.exe is not gitignored — x19 update's autostash "
