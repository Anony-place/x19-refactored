...
``x19 update`` exited 0 with zero rows — even when a gateway was
...
from x19_cli.main import _fleet_probe_expected_runtimes
from x19_cli.update_inventory import RuntimeRecord
...
class TestEmptySnapshotFailClosed:
    ...
    def test_incomplete_when_restart_phase_touched_gateways(self):
        ...
        assert (
            _fleet_probe_expected_runtimes(None, [], None, ["x19-gateway"], set())
            is True
        )
...
class TestCallSiteWiring:
    ...
    def test_settle_sleep_gated_on_expected_runtimes(self):
        ...
        from x19_cli import update_cmd_fleet
...
def test_unmapped_stops_are_not_expected_rows():
    ...
    from x19_cli.update_cmd_fleet import _GatewayRestartOutcome
