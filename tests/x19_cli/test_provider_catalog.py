"""Tests for the unified provider catalog (x19_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``x19 model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from x19_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
