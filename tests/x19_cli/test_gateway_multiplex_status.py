...
``x19 gateway status`` / ``gateway list`` / ``profile list`` keyed liveness
...
def _fake_multiplexer(monkeypatch, tmp_path, *, multiplex: bool, pid_file: bool = True):
    ...
    (tmp_path / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "kind": "x19-gateway", "gateway_state": "running",
        "start_time": status._get_process_start_time(os.getpid()), "x19_home": str(tmp_path),
    }))
    monkeypatch.setenv("X19_HOME", str(tmp_path / "profiles" / "beta"))
    monkeypatch.setattr(x19_constants, "_default_x19_root_memo", None)
    monkeypatch.setattr(
        status, "_read_process_cmdline", lambda pid: "python -m x19_cli.main gateway run --replace"
    )
