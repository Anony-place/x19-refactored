"""Tests for the `x19 memory reset` CLI command.
...
- Profile-scoped reset (uses X19_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake X19_HOME with memory files."""
    x19_home = tmp_path / ".x19"
    memories = x19_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("X19_HOME", str(x19_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nX19 repo is at ~/.x19/x19\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `x19 memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        x19_home = tmp_path / ".x19"
        (x19_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("X19_HOME", str(x19_home))
