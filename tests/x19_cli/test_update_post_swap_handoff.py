"""``x19 update`` finishes in an interpreter born on the pulled code (x19_cli/update_handoff.py).
...
stops at the swap and re-executes ``x19 update --post-swap <file>``; the child resumes the
...
from x19_cli import update_cmd, update_handoff, update_receipt
from x19_cli.update_inventory import RuntimeRecord, UpdatePlan
...
def test_parent_reexecs_tail_on_pulled_tree_and_relays_exit_code(monkeypatch, tmp_path):
    """The pre-swap interpreter runs NOTHING after the swap: it spawns ``python -m x19_cli.main
    ...
    import x19_cli.update_cmd_config as cfg
    ...
    assert spawned["cmd"][:4] == [sys.executable, "-m", "x19_cli.main", "update"]
    assert spawned["cmd"][4:] == ["--yes", "--no-gateway-restart", "--branch", "main", "--post-swap", spawned["cmd"][-1]]
    env = spawned["env"]
    assert env[update_handoff.POST_SWAP_ENV] == "1" and env["X19_UPDATE_REEXEC"] == "1"
...
def test_child_resumes_receipt_and_runs_git_tail_from_payload(monkeypatch, tmp_path):
    ...
    handoff.write_text(json.dumps({
        ...
        "receipt": {"schema": 1, "started_at": started, "outcome": "running", "steps": [{"name": "pre_update_backup", "ok": True}],
                    "skips": [], "gateway_restart": {}, "fleet": [], "pre_update": {"sha": "a" * 40}, "argv": ["x19", "update"]},
    }), encoding="utf-8")
    import x19_cli.update_cmd_config as cfg
    seen = {}
