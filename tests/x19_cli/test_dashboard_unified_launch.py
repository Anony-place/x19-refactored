...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "x19_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # X19_HOME.  For a standard install (X19_HOME unset) that root is
        # the platform-native default (~/.x19), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from x19_constants import get_default_x19_root
        assert env.get("X19_HOME") == str(get_default_x19_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (X19_DESKTOP=1) must NOT
