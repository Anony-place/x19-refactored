...
External secret backends are useful during normal X19 startup, but the
...
""",
    ...
    dispatch = (
        "x19_main.cmd_update = lambda _args: 0\n"
        "x19_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import x19_cli.main as x19_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "X19_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_update_startup_does_not_import_bitwarden_or_cryptography(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["x19", "update", "--check"])
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["x19", "chat"])
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_x19_dotenv(
        x19_home=home,
...
def test_update_probe_children_skip_external_secret_sources(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         "import sys; sys.argv = ['x19', 'update']\n"
         "from x19_cli.update_cmd_deps import _validate_critical_modules_import\n"
         "print('PROBE=' + repr(_validate_critical_modules_import(__import__('os').getcwd())))"],
        capture_output=True, text=True, timeout=180, cwd=REPO_ROOT,
        env={**os.environ, "X19_HOME": str(home)},
    )
    assert result.returncode == 0, result.stderr
