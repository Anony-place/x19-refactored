"""Tests for x19_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("x19_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("x19_cli.config.load_config")
    ...
    @patch("x19_cli.config.load_config")
    @patch.dict("os.environ", {"X19_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """X19_SESSION_PLATFORM should be used when X19_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_x19_platform_takes_precedence(self, tmp_path, monkeypatch):
        """X19_PLATFORM should win over X19_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from x19_cli.skills_config import _get_categories
        skills = [
