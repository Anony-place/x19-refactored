"""The `x19 profile purge-identity` retry path: dispatched, and honest about failure.

`x19 profile delete` reports a pending identity settlement when it cannot purge and names this
...
from x19_cli import profile_cmd, profiles
...
def test_purge_identity_reports_success(profile_env, monkeypatch, capsys):
    monkeypatch.setattr("x19_cli.profile_identity.purge_profile_identity", lambda name: True)
...
def test_purge_identity_exits_nonzero_when_settlement_stays_pending(
    ...
    monkeypatch.setattr("x19_cli.profile_identity.purge_profile_identity", lambda name: False)
...
def test_purge_identity_refuses_a_same_name_profile_created_after_the_delete(profile_env, capsys):
    ...
    scope = str(profile_env / ".x19" / "sessions")
    db = SessionDB(profile_env / ".x19" / "state.db")
    ...
    check = SessionDB(profile_env / ".x19" / "state.db")
    try:
