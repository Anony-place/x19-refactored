"""Regression test: `x19 dashboard --tui` must not hard-crash.

Older X19 desktop app shells (<= 0.15.x) spawn the backend as::

    x19 dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "X19 couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real x19_cli.main parser in a subprocess.
