"""Invariant tests for the plain-language startup messages of the ``x19`` CLI.

Contracts (not snapshots): a mistyped subcommand names the typo, offers the closest
match and ``x19 --help`` without dumping the 70-entry choice list; a rejected
profile name explains the rule; missing optional dependencies point at ``x19 update``.
"""
...
def test_unknown_subcommand_names_typo_suggests_closest_and_hides_choice_list():
    ...
    assert "'sesions' is not a `x19` command" in text
    assert "Did you mean: sessions" in text
    assert "x19 --help" in text
...
def test_unknown_subcommand_without_close_match_still_points_at_help():
    ...
    assert "'zzqx' is not a `x19` command" in text
...
def test_nested_group_typo_names_the_group_and_suggests():
    ...
    assert "'stat' is not a `x19 gateway` command" in text
...
def test_non_choice_errors_keep_argparse_usage_line():
    ...
    assert text.startswith("usage: x19")
...
def test_invalid_profile_flag_value_explains_rule_and_exits(monkeypatch):
    from x19_cli import main as _main

    monkeypatch.setattr(sys, "argv", ["x19", "-p", "Work Bot", "status"])
    err = io.StringIO()
    with redirect_stderr(err), pytest.raises(SystemExit) as exc:
    ...
    assert "x19 profile list" in text
...
def test_option_looking_dash_p_value_is_a_silent_skip_even_under_x19(monkeypatch):
    from x19_cli import main as _main

    # `-p no:xdist` reaching us through a differently named runner (tox, nox, python -m) must not exit.
    monkeypatch.setattr(sys, "argv", ["x19", "-p", "no:xdist", "tests/"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
    monkeypatch.setattr(sys, "argv", ["x19", "-p", "--flag"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_invalid_dash_p_after_a_subcommand_is_left_to_that_subcommand(monkeypatch):
    ...
    # A plugin/subcommand flag such as `x19 kanban serve -p "Work Bot"` is not our profile selector.
    monkeypatch.setattr(sys, "argv", ["x19", "kanban", "serve", "-p", "Work Bot"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_bare_continue_with_no_session_names_the_next_step(monkeypatch):
    ...
    monkeypatch.setattr(
        "x19_cli.terminal_breadcrumbs.resolve_breadcrumb_session", lambda: None, raising=False
    )
    out = io.StringIO()
    ...
    text = out.getvalue()
    assert "x19 sessions list" in text
...
def test_model_picker_does_not_warn_about_missing_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("x19_cli.auth.resolve_provider", _raise)
...
def test_missing_dashboard_deps_point_at_x19_update():
    ...
    assert "x19 update" in text
    assert "-m pip install -e '.[all]'" in text
    assert "metadata" not in text


def test_missing_node_message_names_cli_fallback(monkeypatch, capsys):
    from x19_cli import main_tui_launch as _tui

    monkeypatch.setattr("x19_constants.find_node_executable", lambda _b: None)
    monkeypatch.setattr("x19_cli.dep_ensure.ensure_dependency", lambda _n: False, raising=False)
    monkeypatch.delenv("X19_NODE", raising=False)
    with pytest.raises(SystemExit):
        _tui._tui_node_bin("node")
    text = capsys.readouterr().out
    assert "x19 --cli" in text
