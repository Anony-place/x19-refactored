"""Fixtures shared across x19_cli tests."""
...
def all_assignees_spawnable(monkeypatch):
    """Pretend every assignee maps to a real X19 profile.
...
def _suppress_concurrent_x19_gate(request, monkeypatch):
    """Default ``_detect_concurrent_x19_instances`` to ``[]`` for every test.

    The Windows update path now refuses to proceed when another
    ``x19.exe`` is detected (issue #26670). On a developer's Windows
    machine running the test suite via ``x19`` itself, this would
    flag the running agent as a concurrent instance and abort every
    ``cmd_update`` test. Tests that want to exercise the gate explicitly
    re-patch ``_detect_concurrent_x19_instances`` with their own
    ...
    if request.node.get_closest_marker("real_concurrent_gate"):
        return
    try:
        from x19_cli import main as _cli_main
    except Exception:
        return
    # raising=False: under pytest's per-test spawn isolation, a concurrent
    # xdist worker importing a module that transitively touches x19_cli.main
    # can briefly expose a partially-initialized module object here — one where
    # _detect_concurrent_x19_instances isn't defined yet. A bare setattr
...
def _inline_post_swap_handoff(request, monkeypatch):
    """Run the post-swap tail in-process instead of re-executing ``x19 update --post-swap``.
    ...
    if request.node.get_closest_marker("real_post_swap_handoff"):
        return
    try:
        from x19_cli import update_cmd, update_receipt
    except Exception:
        return
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from x19_cli import gateway, main, update_cmd, update_cmd_fleet
    from x19_cli import update_inventory, update_receipt
