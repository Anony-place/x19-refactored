...
from x19_cli.profiles import create_profile, rename_profile
...
def profile_env(tmp_path, monkeypatch):
    """Isolate profile paths and the process-level X19 root."""
...
def test_rename_preserves_profile_local_checkpoint_history(profile_env, tmp_path):
    ...
    token = set_x19_home_override(old_dir)
    try:
    ...
    token = set_x19_home_override(new_dir)
    try:
        manager = CheckpointManager(enabled=True, max_snapshots=5)
