...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.x19.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.x19.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("x19_constants.display_x19_home", lambda: "~/.x19-work")
    ...
    assert "~/.x19-work/logs/launchd-reload.log" in out
