...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_x19_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from x19_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """x19 -z bypasses X19CLI, but recall still needs SessionDB."""
    from x19_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "x19_state_registry", mod("x19_state_registry", acquire=fake_acquire))
    monkeypatch.setitem(
        sys.modules,
        "x19_cli.config",
        mod("x19_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "x19_cli.models",
        mod("x19_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "x19_cli.runtime_provider",
        mod(
            "x19_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "x19_cli.tools_config",
        mod("x19_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["X19_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/x19-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["X19_MODEL"] == "nous/x19-test"
    assert env["X19_INFERENCE_MODEL"] == "nous/x19-test"
    assert env["X19_TUI_PROVIDER"] == "nous"
    assert env["X19_INFERENCE_PROVIDER"] == "nous"
    assert env["X19_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["X19_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("x19-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_x19_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "x19-ink"
    ...
    monkeypatch.delenv("X19_TUI_DIR", raising=False)
