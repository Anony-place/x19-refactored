...
``x19 chat`` (which resolves the runtime directly) worked against the same config.
"""
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("X19_GUEST_ONBOARDING", raising=False)
    for var in ("OPENAI_API_KEY", "OPENROUTER_API_KEY", "ANTHROPIC_API_KEY", "OPENAI_BASE_URL",
                "OPENROUTER_BASE_URL", "X19_INFERENCE_PROVIDER", "NOUS_API_KEY"):
