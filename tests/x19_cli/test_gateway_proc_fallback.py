...
See: Anony-place/x19-refactored#7622
"""
...
_GATEWAY_CMD = "python -m x19_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m x19_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("x19_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("x19_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("x19_cli.gateway.is_macos", return_value=True),
            patch("x19_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "x19_cli.gateway.get_launchd_label",
                return_value="ai.x19.gateway.myprofile",
            ),
            patch(
                "x19_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.x19.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.x19.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.x19.gateway": 123,
            "ai.x19.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "x19_cli.gateway.get_launchd_label",
                return_value="ai.x19.gateway",
            ),
            patch(
                "x19_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.x19.gateway", "ai.x19.gateway-profile-b"],
            ),
            patch(
                "x19_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.x19.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("x19_cli.gateway.is_macos", return_value=True),
            patch("x19_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.x19.gateway.broken\n"
                    "  123  \t0\tai.x19.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every x19-gateway* unit unconditionally."""
        with (
