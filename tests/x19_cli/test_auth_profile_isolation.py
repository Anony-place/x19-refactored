...
A named profile (``X19_HOME`` under ``profiles/<name>``) resolves provider state and the
credential pool from ITS OWN ``auth.json`` only. The root ``~/.x19/auth.json`` is never a
...
def profile_env(tmp_path, monkeypatch):
    """Global root + an active named profile under Path.home()/.x19/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.x19            (has its own auth.json fixture)
    * Profile     -> tmp_path/.x19/profiles/coder   (active, X19_HOME points here)
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    global_root = tmp_path / ".x19"
    global_root.mkdir()
...
def test_named_profile_never_reads_the_root_store(profile_env):
    ...
    from x19_cli.auth import get_provider_auth_state, read_credential_pool, resolve_codex_runtime_credentials
    from x19_cli.auth_xai import _read_xai_oauth_tokens
    ...
    token = set_x19_home_override(own)
    try:
...
def test_profile_refresh_never_writes_through_to_the_root_store(profile_env):
    ...
    from x19_cli.auth_codex import _save_codex_tokens, clear_codex_pool_quota_cooldowns
    from x19_cli.auth_xai import _save_xai_oauth_tokens
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_x19_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (X19_HOME != ~/.x19, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    x19_home = tmp_path / "classic"
    x19_home.mkdir()
