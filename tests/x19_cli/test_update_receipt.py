"""Tests for x19_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated X19_HOME for receipt writes."""
    home = tmp_path / ".x19"
    home.mkdir()
    # ``_receipt_dir`` resolves through ``x19_constants.get_x19_home`` (env var), not
    # ``x19_cli.config`` — patch where production reads.
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["x19-gateway"],
        ...
        assert gr["restarted_services"] == ["x19-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another x19.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "x19.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            x19_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            x19_main,
        ...
        monkeypatch.setattr(
            x19_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import x19_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "x19_cli.profiles._get_default_x19_home", lambda: home
        )
        monkeypatch.setattr(
            "x19_cli.profiles._get_profiles_root",
    ...
    def test_identity_pending_row_gets_restart_aware_copy(self, capsys):
        ...
        assert ok is False
        out = capsys.readouterr().out
        assert "code identity not published yet" in out and "x19 gateway status" in out
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("x19_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
