...
Drives ``X19CLI.process_command("/reset now")`` against a minimal stand-in
and verifies:
...
def _make_cli_stub():
    """Build a minimal X19CLI-shaped object that can run ``process_command``
    ...
    self_._confirm_destructive_slash = X19CLI._confirm_destructive_slash.__get__(
        self_, type(self_)
    )
    self_._slash_handler = X19CLI._slash_handler
    self_._cmd_new = X19CLI._cmd_new.__get__(self_, type(self_))
    self_.process_command = X19CLI.process_command.__get__(self_, type(self_))
...
def test_new_without_skip_token_still_consults_modal():
    ...
    self_._split_destructive_skip = X19CLI._split_destructive_skip
    self_._normalize_slash_confirm_choice = X19CLI._normalize_slash_confirm_choice.__get__(
        self_, type(self_)
    )
    self_._confirm_destructive_slash = X19CLI._confirm_destructive_slash.__get__(
        self_, type(self_)
