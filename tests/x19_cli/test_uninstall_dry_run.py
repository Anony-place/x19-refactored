...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_x19_home", lambda: x19_home)
    monkeypatch.setattr(uninstall, "_is_default_x19_home", lambda home: False)
    ...
    assert str(x19_home) in output
    assert project_root.exists()
    assert x19_home.exists()
