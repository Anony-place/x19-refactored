"""Tests for the AI_AGENT / X19_AGENT harness-attribution env vars.
...
X19-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal X19' id in the public agent-harness
registry (``x19`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the X19 process env, and survives the
cross-session leak guard that strips ``X19_SESSION_*`` from subprocess
...
from x19_cli.main import _advertise_agent_env
...
class TestAdvertiseAgentEnv:
    def test_sets_both_vars_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("X19_AGENT", raising=False)
        _advertise_agent_env()
        assert os.environ["AI_AGENT"] == HARNESS_ID
        assert os.environ["X19_AGENT"] == "true"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__X19_CWD_testsession0__"
        env._snapshot_path = "/tmp/x19-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'X19_AGENT="${X19_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT X19=$X19_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "X19_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} X19=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", X19_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi X19=false" in out.stdout
