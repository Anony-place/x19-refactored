...
The canonical profile id ("default" for ~/.x19) is never touched —
...
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() and X19_HOME under tmp_path."""
...
class TestMetaAndValidation:
    ...
    def test_bot_title_read_from_x19_bots_ui_meta(self, profile_env):
        ...
        from x19_cli.web_routers.profiles import _profile_to_dict
...
class TestRenameDefault:
    ...
    def test_named_rename_still_real_and_keeps_display_name(
        ...
        monkeypatch.setattr(
            "x19_cli.profiles.check_alias_collision", lambda name: "skip"
        )
