...
from x19_cli.kanban import kanban_command
from x19_cli.kanban_parser import build_parser
...
def test_watch_names_resolved_board(tmp_path, monkeypatch, capsys, environment, explicit, expected):
    ...
    monkeypatch.setenv("X19_KANBAN_HOME", str(tmp_path))
    ...
    if environment:
        monkeypatch.setenv("X19_KANBAN_BOARD", environment)
