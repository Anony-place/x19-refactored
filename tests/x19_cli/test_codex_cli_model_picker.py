...
Covers:
 - Normal path (tokens already in X19 auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — X19
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `x19 auth openai-codex`. The old
...
@pytest.fixture()
def x19_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in X19 auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(x19_auth_only_env):
    """openai-codex appears when tokens are already in X19 auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or X19
    auth store."""
    x19_home = tmp_path / ".x19"
    x19_home.mkdir()
