"""Regression tests for Desktop-owned ``x19 serve`` lifecycle tracking."""

import pytest

from x19_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited X19_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from x19_cli import web_server_lifecycle

    monkeypatch.setenv("X19_PARENT_PID", "4242")
    monkeypatch.setenv("X19_PARENT_START_MARKER", "")
    monkeypatch.setenv("X19_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("X19_PARENT_PID", "4242")
    monkeypatch.setenv("X19_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("X19_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
        web_server_lifecycle.threading, "Thread", lambda *a, **k: started.append(1) or _NoThread()
    )
    with caplog.at_level(logging.WARNING, logger="x19_cli.web_server"):
        web_server_lifecycle._start_parent_death_watchdog()
