"""Tests for the Command Installation check in x19 doctor."""
...
import x19_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal X19_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".x19"
    ...
    x19_bin = venv_bin_dir / "x19"
    x19_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    x19_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, x19_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "x19"
        wrong_target = tmp_path / "wrong_x19"
        ...
        assert cmd_link.resolve() == x19_bin.resolve()
