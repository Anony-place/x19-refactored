"""Tests for x19_cli.security_advisories.
...
import x19_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect X19_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".x19"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full x19_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "x19_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
