...
A long-lived multiplex process (the Desktop's ``x19 serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    x19_logging.setup_logging(x19_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(x19_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_x19_home_override(profile)
    try:
        if adopt:
            x19_logging.setup_logging(x19_home=profile)
        else:
            assert x19_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in x19_logging._queued_file_handlers
            if isinstance(h, x19_logging._ProfileRoutingFileHandler)]
