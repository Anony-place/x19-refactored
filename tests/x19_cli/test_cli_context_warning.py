...
def _isolate(tmp_path, monkeypatch):
    """Isolate X19_HOME so tests don't touch real config."""
    home = tmp_path / ".x19"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal X19CLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("x19_cli.banner.build_welcome_banner"):
        from cli import X19CLI
        obj = X19CLI.__new__(X19CLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["x19-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below X19' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly X19' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above X19' minimum context length."""
