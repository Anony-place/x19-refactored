...
``x19_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``x19 --help``, ``x19 --version``,
``x19 logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from x19_cli._parser import build_top_level_parser, top_level_value_flag_sets
from x19_cli.main import (
...
def _live_subcommand_names() -> set[str]:
    """Run ``x19 --help`` in-process and parse the subcommand block.
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``x19 photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain x19-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level x19 <name> CLI command via PluginContext.
