...
from x19_cli.gateway import _windows_scheduled_task_state
...
def test_task_query_without_powershell_module_autoload():
    ...
    task_name = "X19_test_'" + uuid.uuid4().hex
    with patch("x19_cli.gateway.subprocess.run", side_effect=run_without_modules):
