...
#88251: ``x19 update`` treated a failed desktop pack as non-fatal, printed
...
import x19_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "x19_constants.with_x19_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "x19_constants.display_x19_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_x19_home", lambda: tmp_path)
