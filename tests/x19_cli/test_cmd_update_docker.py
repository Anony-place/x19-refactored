"""Tests for ``x19 update`` / ``--check`` inside the Docker container.
...
fix, ``x19 update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side X19, not an update to the running container, so the message
...
@patch("x19_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``x19 update`` inside Docker → friendly message + exit 2, no git calls.
    ...
    assert "docker pull Anony-place/x19-refactored:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    """
    from x19_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker pull Anony-place/x19-refactored:latest" in msg
    ...
    assert "X19_HOME" in msg or "/opt/data" in msg, (
