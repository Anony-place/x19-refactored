...
SessionDB.session_gateway_runtime (x19_state.py) — the round trip that
makes `x19 --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare X19CLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.X19CLI)
...
def test_restore_llamacpp_session_keeps_launch_base_url_for_same_provider(monkeypatch):
    """`x19 --provider llamacpp --base-url X --resume` is user intent for the SAME provider the
