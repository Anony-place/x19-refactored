...
from x19_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``x19_session_at`` / ``x19_session_rt``
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["x19_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
