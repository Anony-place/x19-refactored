...
vulnerable sqlite3 CLI at a live X19 database.
...
X19' own corruption banners used to instruct exactly that command
(`sqlite3 ~/.x19/state.db ".recover"`). The fix routes operators to
`x19 sessions recover --source ...`, whose lane snapshots the damaged
...
from x19_cli.session_lost_and_found import (
    _parse_sqlite3_cli_version,
    _wal_reset_vulnerable,
    find_sqlite3_cli,
    find_sqlite3_cli_refusal,
)
from x19_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable


LIVE_DB_SALVAGE_COMMAND = 'sqlite3 ~/.x19/state.db ".recover"'
...
class TestFindSqlite3CliRefusal:
    def test_missing_binary_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "x19_cli.session_lost_and_found.shutil.which", lambda _: None
        )
    ...
    def test_no_dbpage_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "x19_cli.session_lost_and_found.shutil.which",
            lambda _: "/usr/bin/sqlite3",
        )
        monkeypatch.setattr(
            "x19_cli.session_lost_and_found._cli_supports_recover",
            lambda _: False,
        )
        assert find_sqlite3_cli() is None
    ...
    def test_wal_reset_vulnerable_refusal(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "x19_cli.session_lost_and_found._parse_sqlite3_cli_version",
            lambda _: (3, 45, 1),
        )
        assert find_sqlite3_cli() is None
...
class TestParseSqlite3CliVersion:
    def test_parses_modern_output(self):
        ...
        with patch(
            "x19_cli.session_lost_and_found.subprocess.run",
            return_value=Probe(),
        ):
...
class TestGuidanceNeverNamesLiveDb:
    ...
    def test_repair_budget_error_names_safe_lane(self, tmp_path: Path):
        import x19_state_repair as x19_state  # helper lives in the split-out repair module

        message = x19_state._persistent_repair_exhausted_error(
            tmp_path / "state.db"
        )
    ...
    def test_forensic_backup_refusals_name_safe_lane(self):
        ...
        body = inspect.getsource(x19_state_repair._backup_db_file)
        assert ".recover\"`" not in body
        assert "sessions recover --source" in body
        assert "--inspect-only" in body

    def test_kanban_manual_recovery_warns_about_live_db(self):
        import x19_cli.kanban_ops as kanban  # ``_cmd_repair`` lives here (split from x19_cli.kanban)
...
# printed `x19 sessions recover --source <db>` — which cmd_sessions
...
class TestEmittedCommandsSatisfyCliContract:
    ...
    `x19 sessions recover --source <db>`, which cmd_sessions rejects
    ...
    the real `cmd_sessions` (the same function `x19` main() hands the
    ...
    def _namespace(source: Path, **overrides) -> "argparse.Namespace":
        """The namespace x19 main() produces for `sessions recover`.

        Mirrors the registrations in x19_cli/main.py (sessions_recover
    ...
    def test_old_v1_shape_is_still_rejected(self, tmp_path):
        ...
        import x19_cli.sessions_cmd as sc
