...
``x19 update`` / ``x19 dashboard --stop`` fall back to ``_kill_pids_posix`` for a
...
# Chat TUI): `x19 dashboard --stop` / `x19 update` must not SIGTERM their own process.
_STOPPER_CHILD = textwrap.dedent(
    """
    import os, pathlib, sys
    from x19_cli import dashboard_procs as dp
