...
def session_db(tmp_path):
    ...
    os.environ["X19_HOME"] = str(tmp_path / ".x19")
    os.makedirs(tmp_path / ".x19", exist_ok=True)
    from x19_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".x19" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal X19CLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        X19CLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        X19CLI._handle_branch_command(cli_instance, "/branch refactor approach")
