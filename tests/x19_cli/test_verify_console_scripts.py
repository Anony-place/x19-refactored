...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("x19", "x19-agent", "x19-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with patch("x19_cli.main_install_repair._is_windows", return_value=True), \
             patch("x19_cli.main_install_repair._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("x19_cli.main_install_repair._run_quarantined_install") as mock_install:
            from x19_cli.main_install_repair import _verify_console_scripts_installed
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("x19_cli.main_install_repair._is_windows", return_value=True):
            names = {path.name for path in main_install_repair._x19_exe_shims(fake_scripts_dir)}

        assert {"x19.exe", "x19-agent.exe", "x19-acp.exe"} <= names
