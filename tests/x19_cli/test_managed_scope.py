"""Unit tests for x19_cli.managed_scope (resolver + loaders + key helpers)."""
import textwrap
...
def test_managed_dir_env_scrubbed_by_default():
    """conftest must scrub X19_MANAGED_DIR so a dev-shell value can't leak in."""
    import os

    assert "X19_MANAGED_DIR" not in os.environ
