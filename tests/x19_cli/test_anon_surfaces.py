...
Contract (R-USR-1): wherever a free-tier identity renders (``x19 auth status nous``,
``x19 auth list``, ``x19 status``, ``x19 portal info``) the user sees the free-tier label
...
from x19_cli.auth import _load_auth_store  # noqa: F401  (store import name kept for parity with core tests)
...
def _account_state() -> dict:
    return {
        "auth_method": "oauth_device_code", "client_id": "x19-cli",
        "access_token": _jwt(sub="nas_user:real", client_id="x19-cli", account_tier="standard", paid_access=True),
...
def isolated_store(monkeypatch, tmp_path):
    monkeypatch.setenv("X19_SHARED_AUTH_DIR", str(tmp_path / "shared-store"))
    monkeypatch.setenv("X19_GUEST_ONBOARDING", "1")
...
def _render_all(capsys) -> dict[str, str]:
    ...
    out["x19 status"] = capsys.readouterr().out
...
def test_every_in_chat_free_tier_string_names_the_slash_command(monkeypatch):
    ...
    monkeypatch.setattr("x19_cli.auth.resolve_provider", lambda _requested: "nous")
    ...
    for text in (*command_copy, *refusal_copy):
        # The ruled refusal uses X19 as the grammatical subject; only that exact product-name
        # phrase is exempt from the broad top-level-command gate.
        assert "x19 " not in text.replace("this X19 can", "this product can").lower()
...
def test_no_chat_copy_of_any_sign_in_state_leaks_a_terminal_verb_or_a_forbidden_word():
    ...
    terminal_or_url = re.compile(r"x19 |https?://", re.IGNORECASE)
...
def test_terminal_only_strings_keep_the_terminal_verb(monkeypatch, capsys):
    assert "x19 " in nous_account.FREE_TIER_NEEDS_ACCOUNT
    assert "x19 " in anon_auth.UPGRADE_UNAVAILABLE
    assert "x19 " in anon_auth.FREE_TIER_NOT_SIGNED_IN

    monkeypatch.setattr("x19_cli.auth.get_provider_auth_state", lambda _provider: {"access_token": "token"})
    monkeypatch.setattr("x19_cli.model_switch_providers._free_tier_nous_row", lambda _provider: None)
    model_setup_flows._model_flow_nous({})
    assert "x19 " in capsys.readouterr().out
...
async def test_the_wait_line_is_only_composed_on_the_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "x19_cli.auth_device_flow._print_device_code_instructions", lambda *_args, **_kwargs: None
    )
    cli_copy = []
    ...
    for rel in ("gateway/slash_commands_login.py", "x19_cli/cli_commands_mixin.py"):
