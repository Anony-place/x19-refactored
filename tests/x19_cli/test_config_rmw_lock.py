...
@pytest.fixture
def client(monkeypatch, _isolate_x19_home):
    ...
    monkeypatch.setattr("x19_cli.model_cost_guard.expensive_model_warning", lambda *_a, **_k: None)
    cfg = load_config()
...
def _on_disk() -> dict:
    ...
    return yaml.safe_load((get_x19_home() / "config.yaml").read_text(encoding="utf-8"))
