"""Tests for the Kanban DB layer (x19_cli.kanban_db)."""
...
import x19_state
import x19_state_wal
from x19_cli import kanban_db as kb
from x19_cli import kanban_db_connect as kbc
from x19_cli import kanban_db_dispatch as kbd
from x19_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated X19_HOME with an empty kanban DB."""
    home = tmp_path / ".x19"
    home.mkdir()
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import x19_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import x19_cli.kanban_db as _kb
    from x19_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("X19_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("X19_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """X19' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` X19' DB/metadata/logs on task completion.
    """
...
# spawned with `x19 -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's X19_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's X19_HOME."""

    def _set_home(self, monkeypatch, tmp_path, x19_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("X19_HOME", str(x19_home))
        monkeypatch.delenv("X19_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.x19/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.x19/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.x19/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # X19_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile X19_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # X19_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is X19_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["X19_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["X19_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["X19_KANBAN_TASK"] == "t_dispatch_env"
        assert env["X19_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "X19_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see x19_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import x19_state_wal as _x19_state_wal
    monkeypatch.setattr(
        _x19_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _x19_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    x19_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("x19_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="x19_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        x19_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "x19_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from x19_cli import sqlite_safe_read
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'x19 kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from x19_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_x19_argv()
#
# Workers spawned by the dispatcher must use a `x19` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["x19", ...]`
...
# only falls back to the PATH shim when ``x19_cli`` is not importable.
# ---------------------------------------------------------------------------


def test_resolve_x19_argv_prefers_module_form_over_path_shim(monkeypatch):
    """A `x19` on PATH must not shadow the running install (#111569):
    the module argv wins whenever ``x19_cli`` is importable; only an
    explicit ``$X19_BIN`` overrides it."""
    import shutil
    import sys
    from x19_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("X19_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/tmp/planted/x19")
    monkeypatch.setattr(kbd, "_safe_which_no_cwd", lambda name: "/tmp/planted/x19")
    assert kbd._resolve_x19_argv() == [sys.executable, "-m", "x19_cli.main"]

    monkeypatch.setenv("X19_BIN", "/opt/x19/bin/x19")
    assert kbd._resolve_x19_argv() == ["/opt/x19/bin/x19"]


def test_resolve_x19_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m x19_cli.main`.

    Pins the correct module name (NOT `x19` — there is no top-level
    `x19` package). Regression for #23198: the original PR shipped
    `python -m x19` which fails with `No module named x19` on every
    invocation.
    """
    import shutil
    import sys
    import x19_cli.kanban_db as kb
    from x19_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("X19_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_x19_argv()
    assert argv == [sys.executable, "-m", "x19_cli.main"]


def test_resolve_x19_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `x19_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m x19_cli.main --version`
    ...
    assert "X19" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="x19_cli.kanban_db"):
        with kbc.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from x19_cli.kanban_db_connect import connect
    from x19_cli.sqlite_safe_read import file_length_matches_header
