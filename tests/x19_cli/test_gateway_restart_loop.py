...
Covers:
- Defense 1: gateway stop/restart refuse when _X19_GATEWAY=1
...
from x19_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "x19 gateway restart",
        "x19 gateway stop",
        "x19 gateway uninstall",
        "x19  gateway  restart",         # double spaces
        "x19 GaTeWaY rEsTaRt",           # mixed case
        "X19 GATEWAY RESTART",           # uppercase
        # The previous product's launcher no longer exists, so its command
        # form is deliberately NOT asserted here: branch A anchors on the
        # real `x19` executable.
        # executable. (This case used to be spelled via a .lower().replace()
        # trick that hid the legacy token from residue scans.)
    ])
    def test_x19_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.x19.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.x19/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l x19-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.x19.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.x19.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.x19/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            ...
            "'ai.x19.gateway-cronus:/c.plist' 'ai.x19.gateway-plutus:/p.plist' "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated x19 note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.x19.gateway` executes exactly as
        ...
        "launchctl dis\\able gui/501/ai.x19.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.x19."gateway"',
        # Same class on the systemctl and x19-CLI branches.
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.x19.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'systemctl re"start" x19-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "x19 cron list",
        "x19 update",
        "x19 config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `x19 gateway start` is benign — starting a gateway from inside a
        ...
        "x19 gateway start",
        "x19 gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway x19
        # services must not be falsely blocked (the old `.*x19` matched any
        # x19 token).
        "launchctl unload ai.x19.update-checker.plist",
        "launchctl restart ai.x19.daemon",
        ...
        "systemctl restart x19-meta.service",
        "systemctl restart x19-cron-helper",
        ...
        "echo after the x19 gateway restarted cleanly",
        "the x19 gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "x19 skill view gateway-notes && echo x19 gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `x19` is a path component
        # there, not a command.
        "cat '/docs/x19 gateway restart-notes.md'",
        "less /home/user/notes/x19 gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "x19 gateway restart",
        "x19 gateway restart; echo done",
        "x19 gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;x19 gateway restart",
        "true && x19 gateway stop",
        "echo $(x19 gateway restart)",
        "echo `x19 gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.x19.gateway"])',
        'subprocess.run(["x19", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.x19.gateway")',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: x19 gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `x19 -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry X19_HOME/X19_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("X19_PROFILE", "zeus")
        monkeypatch.delenv("X19_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "x19 -p zeus gateway stop",
        "x19 -p zeus gateway restart",
        "x19 --profile zeus gateway restart",
        "x19 --profile zeus gateway stop",
        "x19 --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "x19 -v -p zeus gateway restart",
        "x19 -p zeus -v gateway restart",
        "x19 --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "x19 -p venus gateway stop",
        "x19 -p venus gateway restart",
        "x19 --profile venus gateway restart",
        "x19 --profile=venus gateway stop",
        "x19 -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "x19 -p zeus gateway start",
        "x19 -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("x19 gateway restart")
        assert _contains_gateway_lifecycle_command("x19 gateway stop")

    def test_x19_home_derived_profile(self, monkeypatch):
        # Without X19_PROFILE the guard falls back to the X19_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("X19_PROFILE", raising=False)
        monkeypatch.delenv("X19_PROFILE_NAME", raising=False)
        import x19_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("x19 -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("x19 -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("x19 -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("x19 gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_x19_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade x19 then run x19 gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_x19_gateway_once.sh). The script must live under
        # X19_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("X19_HOME", str(tmp_path / ".x19"))
        scripts_dir = tmp_path / ".x19" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_X19_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _X19_GATEWAY=1.

    Issue #37453: systemctl --user restart x19-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart x19-gateway",
        "systemctl --user restart x19-gateway",
        "systemctl stop x19-gateway.service",
        "x19 gateway restart",
        "x19 gateway uninstall",
        "launchctl kickstart gui/501/ai.x19.gateway",
        "launchctl bootout gui/501/ai.x19.gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart x19-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.x19.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-x19 label: label-independent detection is the point
        # (#62891 second reproduction used `ai.x19.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.x19.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _X19_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _X19_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_X19_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="x19 gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["x19 gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.x19.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.x19.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-x19 systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run x19 gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("x19 gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/x19\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 x19 gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_x19_home()` falls back to Path.home(), which raises
        RuntimeError when neither X19_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart x19-gateway' .",
        "grep -rn 'x19 gateway restart' ./logs",
        "rg 'x19 gateway restart' ../archive",
        "grep -c 'systemctl stop x19-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM x19_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell x19 gateway restart"',
        'sqlite3 ./db ".system systemctl restart x19-gateway"',
        'psql ./x -c "\\! systemctl restart x19-gateway"',
        "grep -r 'x19 gateway restart' . | sh",
        "grep -r 'x19 gateway restart' ./logs | bash",
        "grep -r 'x19 gateway restart' . | sudo sh",
        "grep -r 'x' . ; x19 gateway restart",
        "grep -r 'x' . && systemctl restart x19-gateway",
        'grep -r "$(x19 gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart x19-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell x19 gateway restart"',
        'sqlite3 db ".system systemctl restart x19-gateway"',
        'psql -c "\\! systemctl restart x19-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    x19_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run x19 gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("X19_HOME", str(tmp_path / ".x19"))
        (tmp_path / ".x19").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run x19 gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop x19-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart x19-gateway' /var/log/syslog",
        "rg 'x19 gateway restart' /home/user/.x19/logs/",
        "journalctl -u x19-gateway --grep 'systemctl restart x19-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'x19_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell x19 gateway restart"',
        'psql -c "\\! systemctl restart x19-gateway"',
        "grep 'systemctl restart x19-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart x19-gateway",
        'grep "$(systemctl restart x19-gateway)" f',
        "grep 'restart' log; systemctl restart x19-gateway",
        'sqlite3 db "SELECT 1"; x19 gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "x19 gateway restart",
        "sudo systemctl stop x19-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart x19-gateway%'\""
        )
