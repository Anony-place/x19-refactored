"""``x19 update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``x19.exe`` gets quarantined,
...
from x19_cli.update_cmd import _editable_install_is_current
