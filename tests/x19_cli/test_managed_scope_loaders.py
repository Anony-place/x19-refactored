...
x19_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # x19_time checks an env override first; ensure it's unset so config wins.
    ...
    assert x19_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (X19_TIMEZONE, X19_REDACT_SECRETS, X19_MAX_ITERATIONS,
    ...
    # writing X19_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
