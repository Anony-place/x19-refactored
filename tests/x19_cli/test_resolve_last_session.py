"""Verify `x19 -c` picks the session the user most recently used."""
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = x19_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_cli_continues_a_oneshot(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("x19_cli.main._resolve_workspace_key", lambda: None)
    monkeypatch.setattr("x19_state.SessionDB", lambda **kw: real_db(db_path=state_db, **kw))
