...
from x19_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    X19_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def x19_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("x19_cli.config") or mod == "x19_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, x19_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, x19_home, capsys, monkeypatch):
