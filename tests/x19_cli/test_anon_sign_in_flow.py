...
Driven directly against the same fake account service ``x19 auth upgrade`` is tested with, so the
...
from x19_cli.auth_constants import AuthError
from tests.x19_cli.test_anon_upgrade import (  # noqa: F401  (fixtures used by name)
...
def test_already_signed_in_short_circuits_before_any_network(portal):
    from x19_cli.auth import _load_auth_store, _save_auth_store, _save_provider_state
