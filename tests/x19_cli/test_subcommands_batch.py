...
from x19_cli.subcommands.dashboard import build_dashboard_parser
...
from x19_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`x19 config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="x19")
...
# ── deprecated `x19 login` fails gracefully, not with argparse error ────
#
# `x19 login` is a removed command; its handler (`login_command` in
# `x19_cli/auth.py`) prints a deprecation notice pointing at `x19 auth` /
# `x19 model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `x19 --help`.
