...
`x19 curator pin <name>` on an unmanaged skill (curation-eligible but no
...
becomes meaningful after `x19 curator adopt`) and makes the message say
...
def test_pin_unmanaged_records_flag_and_prints_adopt_hint(monkeypatch, capsys):
    import x19_cli.curator as curator_cli
    ...
    assert "x19 curator adopt legacy-skill" in out
