"""Invariants for ``x19_cli.config_effective.load_user_config_effective`` — the one loader every
defaults-free config reader (gateway runtime, TUI gateway, cron, ``x19 send`` bridge, doctor,
...
def _reset_caches():
    ...
    from x19_cli import config_effective, managed_scope
...
def _reset_caches_keep_last_good():
    import x19_cli.config as cfg
    from x19_cli import config_effective
