...
`x19 update` migrated only the active profile's config.yaml; sibling
...
context-local X19_HOME override.
...
import x19_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        x19_constants, "get_process_x19_home", lambda: active_home
    )
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_x19_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_x19_home_override() == before
