...
plain words and points at `x19 setup terminal`, never at raw TERMINAL_* env vars."""
...
def test_docker_missing_line_names_backend_and_setup_command(monkeypatch, capsys, issues):
    ...
    assert issues and "x19 setup terminal" in issues[0] and "Install Docker" in issues[0]
...
def test_docker_daemon_down_line_says_start_docker_or_switch(monkeypatch, capsys, issues):
    ...
    assert issues and "Start Docker" in issues[0] and "x19 setup terminal" in issues[0]
...
def test_ssh_host_missing_line_points_at_setup_terminal(monkeypatch, capsys, issues):
    ...
    assert issues and "x19 setup terminal" in issues[0] and "SSH host and user" in issues[0]
...
def test_daytona_key_missing_line_points_at_setup_terminal(monkeypatch, capsys, issues):
    ...
    assert issues and "x19 setup terminal" in issues[0]
