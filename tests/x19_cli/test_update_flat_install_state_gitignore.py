"""Flat-install runtime state must be gitignored so ``x19 update``'s untracked
autostash cannot sweep the live state.db (#110648).

On a flat install (checkout root == $X19_HOME) the profile's runtime files
live inside the repo as untracked paths. ``git stash push --include-untracked``
(x19_cli/update_cmd_stash.py) moves the whole untracked set into the stash and
...
.x19-bootstrap-complete / .install_method precedent (#38529 / #66189).
"""
import os
import shutil
import sqlite3
...
# Runtime state that lives at $X19_HOME's root on a flat install, exactly as
# ``x19 update`` would sweep it: one representative per ignored class. The
...
FLAT_INSTALL_RUNTIME_STATE = (
    ...
    # files from x19_state_dbfile / x19_state_repair / x19_state_common /
    # x19_cli/kanban_db_*, the repair-attempts ledger and malformed backups.
...
def flat_install_repo(tmp_path: Path) -> Path:
    ...
    Built in a subdirectory of tmp_path: the suite-wide X19_HOME isolation
    fixture (tests/conftest.py) materialises its own ``x19_test/`` tree in
    ...
    (repo / "app.py").write_text("print('x19')\n")
...
def test_flat_install_runtime_state_is_ignored(flat_install_repo):
    ...
    so `x19 update` never enters its stash step for runtime state alone."""
    status = _run_git(
...
def test_untracked_autostash_cannot_sweep_runtime_state(flat_install_repo):
    ...
    _run_git(
        flat_install_repo,
        "stash", "push", "--include-untracked", "-m", "x19-update-autostash",
    )
    assert (flat_install_repo / "app.py").read_text() == "print('x19')\n"
    missing = [
        rel
        for rel in FLAT_INSTALL_RUNTIME_STATE
...
def test_untracked_autostash_leaves_open_wal_database_readable(flat_install_repo):
    ...
    from x19_cli.update_cmd_stash import _stash_local_changes_if_needed
