"""
Tests for --yes / --force flag separation in `x19 skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("x19_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["x19", "skills", "install", "official/email/agentmail", "--yes"],
    )
