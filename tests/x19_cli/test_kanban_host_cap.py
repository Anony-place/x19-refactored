...
1. The standalone daemon path (``x19 kanban daemon --force`` /
   :func:`x19_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``x19 kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
