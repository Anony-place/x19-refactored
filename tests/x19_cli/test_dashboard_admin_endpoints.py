...
def _client():
    ...
    # Keep the state DB under the isolated X19_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_x19_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        config_text = (x19_home / "config.yaml").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from x19_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.x19/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `x19 auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `x19 auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_x19_home):
        ...
        (get_x19_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_x19_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
        (get_x19_home() / "profiles" / "work" / "config.yaml").write_text("{}\n")  # identity marker
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_x19_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "x19_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("x19-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # x19-index source advertises availability + featured search.
            idx = _Src("x19-index")
        ...
        assert {"official", "github", "x19-index"} <= ids
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "x19_cli.web_routers.skills._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "x19_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/x19/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/x19/update`` runs ``x19 update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import x19_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/x19/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/x19/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("x19_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `x19 tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_x19_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_x19_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _X19_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_x19_action stores
    ...
    _web_server_gateway._spawn_x19_action(["gateway", "restart"], "gateway-restart")

    assert "_X19_GATEWAY" not in captured["env"]
    assert captured["env"]["X19_NONINTERACTIVE"] == "1"
...
def test_named_profile_action_isolates_parent_env_and_loads_target_env(monkeypatch, tmp_path):
    ...
    import x19_cli.env_loader as env_loader
    import x19_cli.web_server as ws

    user_home = tmp_path / "user"
    default_home = user_home / ".x19"
    ...
    for key, value in {
        ...
        "X19_ACP_AUTH_METHOD": "default-acp",
    ...
    monkeypatch.setattr(ws.subprocess, "Popen", _fake_popen)
    _web_server_gateway._spawn_x19_action(["-p", "verifier", "gateway", "restart"], "gateway-restart")

    child_env = captured["env"]
    assert captured["cmd"][-4:] == ["-p", "verifier", "gateway", "restart"]
    assert child_env["X19_HOME"] == str(target_home)
    assert child_env["X19_NONINTERACTIVE"] == "1"
    ...
    for leaked_key in (
        ...
        "A2A_AUTH_MINI", "EXTERNAL_PROFILE_AUTH", "X19_ACP_AUTH_METHOD",
    ):
    ...
    probe = subprocess.run(
        [
            ...
            "load_x19_dotenv(x19_home=os.environ['X19_HOME']); "
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    ...
    startup under X19_DESKTOP=1 so the stale orphan is cleared first.
    """
    import x19_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from x19_cli.gateway, so patch it
    # on that module.
    import x19_cli.gateway as g
