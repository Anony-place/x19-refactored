"""Tests for x19_cli.profile_distribution — git-based profile installs.
...
from x19_cli.profile_distribution import (
    DEFAULT_DIST_OWNED,
    DistributionError,
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/x19_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_x19_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires X19"):
                check_x19_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from x19_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_x19_requires(self, profile_env, monkeypatch):
        # Pin current X19 version to something well below the requirement
        import x19_cli
        monkeypatch.setattr(x19_cli, "__version__", "0.1.0", raising=False)
...
class TestDescribe:
    ...
    def test_describe_missing_profile_raises(self, profile_env):
        with pytest.raises(DistributionError, match="No profile named .*x19 profile list"):
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "x19_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from x19_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from x19_cli.profiles import create_profile, list_profiles, get_profile_dir
