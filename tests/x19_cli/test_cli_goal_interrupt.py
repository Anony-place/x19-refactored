...
minimal ``X19CLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def x19_home(tmp_path, monkeypatch):
    """Isolated X19_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".x19"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("X19_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves X19_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal X19CLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, x19_home):
        ...
        with patch("x19_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, x19_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, x19_home):
        ...
        src = inspect.getsource(X19CLI.chat)
