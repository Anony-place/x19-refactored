...
Bug class: the checkout's ``.py`` files change (``x19 update``, manual
...
def _make_pycache(repo: Path, subdir: str = "x19_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    x19_main._sweep_stale_bytecode_if_checkout_changed()
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.x19/plugins/<name>
# ---------------------------------------------------------------------------
