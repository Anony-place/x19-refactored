"""Regression coverage for #110173: observational `x19 sessions` readers stay read-only."""
...
def test_observational_sessions_actions_open_a_read_only_store(monkeypatch, action):
    factory = MagicMock()
    monkeypatch.setattr("x19_state.SessionDB", factory)
...
def test_sessions_observational_commands_on_missing_store_stay_empty(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(x19_state, "_default_db_path", lambda: db_path)
...
def test_doctor_write_probe_never_touches_a_store_a_live_writer_holds(monkeypatch, tmp_path):
    ...
    probed: list = []
    monkeypatch.setattr(x19_state_repair, "_db_opens_cleanly", lambda path: probed.append(path))

    monkeypatch.setattr(x19_state_holders, "live_writer_holds_db", lambda *_a, **_k: True)
    assert doctor_state._write_health_reason(state_db, should_fix=False) is None
    monkeypatch.setattr(x19_state_holders, "live_writer_holds_db", lambda *_a, **_k: False)
    assert doctor_state._write_health_reason(state_db, should_fix=False) is None
