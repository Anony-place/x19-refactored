...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            x19_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(x19_constants, "get_x19_home", lambda: tmp_path)
    monkeypatch.setattr(x19_state, "SessionDB", FakeSessionDB)
    ...
    assert "x19 sessions optimize-storage" in capsys.readouterr().out
    conn.close()
