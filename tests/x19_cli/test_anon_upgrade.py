"""``x19 auth upgrade``: the free tier signs into a Nous account, keeping its connectors.
...
from x19_cli.auth import _auth_file_path, _load_auth_store
...
class FakePortal:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/oauth/token":
            ...
            self.token_grants += 1
            return httpx.Response(200, json={
                "access_token": _jwt(sub="nas_user:9", client_id="x19-cli", account_tier="free"),
...
def portal(monkeypatch, tmp_path):
    fake = FakePortal()
    monkeypatch.setenv("X19_PORTAL_BASE_URL", PORTAL)
    monkeypatch.setenv("X19_ANON_API_SECRET", "test-secret")
...
def free_account(monkeypatch):
    ...
    from x19_cli import models as m
...
class TestSignInCompletionSettlesTheModel:
    ...
    def test_no_eligible_recommendation_leaves_no_default_rather_than_a_model_the_account_may_not_use(
        ...
        assert "run `x19 model` to pick one" in out
