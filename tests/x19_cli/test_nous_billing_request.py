"""Tests for the x19_cli.nous_billing HTTP client's response handling.
...
from x19_cli import nous_billing as nb
...
def test_billing_token_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    multiplex gateway, where the per-turn X19_HOME override selects the auth.json."""
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: f"tok-{x19_constants.get_x19_home().name}")
    a, b = tmp_path / "a", tmp_path / "b"
    a.mkdir(); b.mkdir()
    seen = []
    for home in (a, b):
        tok = x19_constants.set_x19_home_override(str(home))
        try:
