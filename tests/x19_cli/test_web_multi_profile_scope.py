...
Regression for the cross-profile leak class in ``x19 dashboard`` / ``x19 serve``:
``_config_profile_scope`` bound only X19_HOME, so ``GET /api/config?profile=B`` expanded B's
``${VAR}`` refs to the DEFAULT profile's plaintext credentials (its ``os.environ``), and console
``send`` for B (``send_cmd._load_x19_env``) copied B's ``.env`` into the shared process env with
...
def test_console_send_for_named_profile_does_not_write_process_env(two_homes, monkeypatch):
    ...
    from x19_cli.web_routers.chat_ws import _execute_console_line

    root, b = two_homes
    seen = {}

    def fake_send(args):
        import x19_cli.send_cmd as send_cmd
        from gateway.config import _getenv
        send_cmd._load_x19_env()
        seen["loader_sees"] = _getenv("TELEGRAM_BOT_TOKEN")
        seen["environ_has"] = "TELEGRAM_BOT_TOKEN" in os.environ
        seen["home"] = Path(os.environ.get("X19_HOME", ""))
        return '{"success": true}'
