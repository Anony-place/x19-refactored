"""Tests for x19_cli.stderr_timestamp."""
...
from x19_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.x19.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.x19.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "x19_cli.main", "gateway", "run", "--replace"]
...
def test_main_maps_gateway_ex_config_to_clean_stop(tmp_path):
    ...
    log_path = tmp_path / "gateway.error.log"
    gateway_tail = ["-m", "x19_cli.main", "gateway", "run"]
