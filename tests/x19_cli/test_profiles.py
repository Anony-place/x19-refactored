"""Comprehensive tests for x19_cli.profiles module.
...
from x19_cli import profiles
from x19_cli.profiles import (
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and X19_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.x19/profiles)
    * X19_HOME  -> tmp_path/.x19  (so get_x19_home() agrees)
    * Creates the bare-minimum ~/.x19 directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".x19"
...
class TestGetProfileDir:
    ...
    def test_default_returns_x19_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_sync_imports_carries_manifest_but_never_links_profiles(self, profile_env):
        ...
        from x19_cli.agent_import_sync import SYNC_MANIFEST_NAME, load_sync_manifest
...
class TestNoSkillsOptOut:
    """Tests for `x19 profile create --no-skills` and the opt-out marker."""
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["x19"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `x19 update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".x19" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("x19_cli.profiles._cleanup_gateway_service"), \
             patch("x19_cli.profiles.time.sleep"), \
             patch("x19_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_delete_purges_profile_keyed_identity(self, profile_env):
        ...
        scope = str(tmp_path / ".x19" / "sessions")
        db = SessionDB(tmp_path / ".x19" / "state.db")
        ...
        with patch("x19_cli.profiles._cleanup_gateway_service"), \
             patch("x19_cli.profiles._live_default_multiplexer", return_value=False):
            delete_profile("gone", yes=True)

        check = SessionDB(tmp_path / ".x19" / "state.db")
        try:
    ...
    def test_delete_reports_pending_settlement_for_a_live_multiplexer(self, profile_env, capsys):
        ...
        """
        from x19_state import SessionDB
        from x19_cli.profiles import ProfileIdentitySettlementPending
        ...
        with patch("x19_cli.profiles._cleanup_gateway_service"), \
             patch("x19_cli.profiles._live_default_multiplexer", return_value=True):
        ...
        assert ei.value.retry_command == "x19 profile purge-identity gone"
        assert not ei.value.path.exists()
        assert isinstance(ei.value, RuntimeError)  # the CLI handler catches RuntimeError

        assert "x19 profile purge-identity gone" in capsys.readouterr().err
        check = SessionDB(tmp_path / ".x19" / "state.db")
        try:
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "x19_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "x19_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "x19_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "x19_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_x19_shim(self, profile_env, monkeypatch):
        """A `x19` console-script shim spawned directly (e.g. Electron's
        findOnPath('x19') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "x19" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "x19".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/x19", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/x19", "--profile", "other", "serve"]),
            # Non-x19 script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_x19_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "x19" (e.g.
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/x19-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/x19-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (x19,
        x19-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "x19"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".x19" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom X19_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("X19_HOME", str(custom))
        # With Docker-aware roots, a custom X19_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("x19_cli.profiles.shutil.which", lambda name: "/opt/x19/bin/x19")
        from x19_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/x19/bin/x19 -p mybot" in content

    def test_windows_wrapper_invokes_the_real_launcher(self, profile_env, monkeypatch):
        """The generated .bat must call `x19`.

        It used to hardcode the previous product's launcher name, so every
        Windows profile wrapper invoked an executable that no longer exists.
        """
        exe = "C:/Program Files/x19/x19.exe"
        monkeypatch.setattr(sys, "platform", "win32")
        monkeypatch.setattr("x19_cli.profiles.shutil.which", lambda name: exe)
        from x19_cli.profiles import create_wrapper_script
        ...
        # Quoted, because a real install path routinely contains spaces.
        assert f'"{exe}" -p mybot %*' in content
        assert "hermes" not in content.lower()

    @pytest.mark.parametrize("platform", ["linux", "win32"])
    def test_wrapper_is_recognised_as_ours_on_every_platform(self, profile_env, monkeypatch, platform):
        """`remove_wrapper_script` only deletes files `_is_our_wrapper` accepts.

        The check has to recognise the launcher spelling each platform actually
        writes — a shell-quoted path on POSIX, a quoted `.exe` path on Windows.
        A wrapper it fails to recognise is unremovable, and an alias collision
        against it cannot be reused.
        """
        monkeypatch.setattr(sys, "platform", platform)
        monkeypatch.setattr("x19_cli.profiles.shutil.which", lambda name: "/opt/x19/bin/x19")
        from x19_cli.profiles import _is_our_wrapper, create_wrapper_script, remove_wrapper_script

        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert _is_our_wrapper(wrapper) is True
        assert remove_wrapper_script("mybot") is True
        assert not wrapper.exists()

    def test_is_our_wrapper_stays_strict(self, profile_env, tmp_path):
        """The loosened recognition still gates deletion, so it must not widen
        to arbitrary files."""
        from x19_cli.profiles import _is_our_wrapper

        stranger = tmp_path / "not-ours.sh"
        stranger.write_text('#!/bin/sh\nsome-other-tool -p mybot "$@"\n', encoding="utf-8")
        assert _is_our_wrapper(stranger) is False

        bare = tmp_path / "bare.sh"
        bare.write_text('#!/bin/sh\nexec x19 -p mybot "$@"\n', encoding="utf-8")
        assert _is_our_wrapper(bare) is True

        quoted = tmp_path / "quoted.sh"
        quoted.write_text("#!/bin/sh\nexec '/opt/x19/bin/x19' -p mybot \"$@\"\n", encoding="utf-8")
        assert _is_our_wrapper(quoted) is True

        assert _is_our_wrapper(tmp_path / "absent.sh") is False

    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from x19_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from x19_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from x19_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "x19.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["x19_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["x19_heimdall"]["peerName"] == "user-peer"
    ...
    def test_multiplexed_rename_unroutes_old_then_hot_serves_new(self, profile_env):
        ...
        create_profile("oldname", no_alias=True)
        old_dir = tmp_path / ".x19" / "profiles" / "oldname"
        new_dir = tmp_path / ".x19" / "profiles" / "newname"
        ...
        def _record_notify(name):
            ...
            if name == "oldname" and old_dir.exists():
                ...
                with pytest.raises(FileNotFoundError):
                    mkdir_under_x19_home(old_dir / "logs")

        with patch("x19_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("x19_cli.profiles._live_default_multiplexer", return_value=True), \
             patch("x19_cli.profiles._notify_multiplexer", side_effect=_record_notify):
    ...
    def test_unmultiplexed_rename_does_not_signal_multiplexer(self, profile_env):
        ...
        with patch("x19_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("x19_cli.profiles._live_default_multiplexer", return_value=False), \
             patch("x19_cli.profiles._notify_multiplexer") as notify:
            new_dir = rename_profile("oldname", "newname")

        notify.assert_not_called()
        assert not (tmp_path / ".x19" / "profiles" / ".deleted").exists()
        assert not old_dir.exists() and new_dir.is_dir()
    ...
    def test_rename_migrates_session_identity_without_live_gateway(self, profile_env):
        ...
        pdb.close()
        root_db = SessionDB(tmp_path / ".x19" / "state.db")
        ...
        moved_db.close()
        root_db2 = SessionDB(tmp_path / ".x19" / "state.db")
    ...
    def test_migrate_identity_command_repairs_a_failed_live_migration(self, profile_env, capsys):
        """The failed-live-migration end state must be recoverable: `x19 profile
        ...
        with patch("x19_cli.profiles.check_alias_collision", return_value="skip"), \
             ...
             patch("gateway.control_socket.migrate_gateway_profile_identity", return_value=None):
            rename_profile("oldname", "newname")
        assert "x19 profile migrate-identity oldname newname" in capsys.readouterr().err

        # Gateway restarted/stopped → the retry command repairs both stores.
        with patch("x19_cli.profiles._live_default_multiplexer", return_value=False):
        ...
        moved_db = SessionDB(tmp_path / ".x19" / "profiles" / "newname" / "state.db")
        row = moved_db._read_one(
            "SELECT session_key, profile_name FROM sessions WHERE id = ?", ("sess1",))
...
class TestExportImport:
    ...
    def test_export_default_includes_profile_data(self, profile_env, tmp_path):
        ...
        # Write through X19_HOME, not get_profile_dir("default"): the latter resolves to the
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultX19Home (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_x19_home()."""



    def test_default_x19_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_x19_home() returns X19_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("X19_HOME", str(docker_home))
        home = _get_default_x19_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under X19_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("X19_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        from x19_cli.profiles import _check_gateway_running
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="x19 gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with X19_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("X19_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_x19_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured X19_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("X19_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No X19_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("X19_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_x19_home()
...
class TestCloneAllExcludesRuntimeTrees:
    ...
    runtime trees the local-models flow puts under ``~/.x19``: ``models/``
    ...
    def test_runtime_trio_is_one_constant_shared_with_backup(self):
        ...
        from x19_cli import backup, profiles
        from x19_constants import LOCAL_RUNTIME_ROOT_DIRS
