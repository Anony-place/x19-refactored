"""Regression for #41403 — ``x19 update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.x19.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed X19_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.x19.gateway",
            "ai.x19.gateway-merit-ops",
            "ai.x19.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(x19_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.x19.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.x19.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.x19.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.x19.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.x19.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.x19.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.x19.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        # box with a live ai.x19.gateway* fleet would leak its PIDs into the assertion.
        monkeypatch.setattr(gw.subprocess, "run", lambda *a, **k: _completed(0, ""))
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.x19.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.x19.gateway", "ai.x19.gateway-a", "ai.x19.gateway-b"],
        )
        located = {
            "ai.x19.gateway": (f"gui/{UID}", 100),
            "ai.x19.gateway-a": (f"gui/{UID}", 200),
            "ai.x19.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through x19_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.x19.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.x19.gateway", current, "ai.x19.gateway-user-scoped"],
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.x19.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.x19.gateway",
            f"user/{UID}/ai.x19.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.x19.gateway",
            f"user/{UID}/ai.x19.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.x19.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.x19.gateway-a"],
            located={"ai.x19.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.x19.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.x19.gateway",
            labels=["ai.x19.gateway", "ai.x19.gateway-idle"],
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.x19.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.x19.gateway",
            labels=["ai.x19.gateway", "ai.x19.gateway-dormant"],
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.x19.gateway-dormant"]
        assert restarted == ["ai.x19.gateway", "ai.x19.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.x19.gateway",
            labels=["ai.x19.gateway", "ai.x19.gateway-a"],
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.x19.gateway-a"]
        assert restarted == ["ai.x19.gateway", "ai.x19.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-bad": (f"gui/{UID}", 200),
                "ai.x19.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.x19.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.x19.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.x19.gateway-good"]
        assert restarted == ["ai.x19.gateway", "ai.x19.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.x19.gateway",
                "ai.x19.gateway-wedged",
                "ai.x19.gateway-after",
            ],
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.x19.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.x19.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.x19.gateway-after"]
        assert restarted == ["ai.x19.gateway", "ai.x19.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-wedged": (f"gui/{UID}", 200),
                "ai.x19.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.x19.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.x19.gateway",
            labels=["ai.x19.gateway", "ai.x19.gateway-zombie"],
            located={
                "ai.x19.gateway": (f"gui/{UID}", 100),
                "ai.x19.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.x19.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.x19.gateway"]
        assert failed == ["ai.x19.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.x19.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.x19.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
...
class TestIncompleteWarningOnMacos:
    ...
    def test_launchd_labels_get_bootstrap_hint(self, capsys):
        _warn_incomplete_gateway_fleet_restart(["ai.x19.gateway-merit-ops"])
