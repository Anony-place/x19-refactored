...
next command, and every ``x19 <sub> <cmd>`` it cites is a registered subcommand.
"""
...
from x19_cli import gateway as gateway_cli
from x19_cli import profiles
from x19_cli.update_lock import UpdateHolder, describe_holder
...
def test_systemctl_failure_is_explained_not_tracebacked(monkeypatch, capsys):
    exc = subprocess.CalledProcessError(1, ["systemctl", "--user", "start", "x19-gateway"])
    out = _run_gateway_command_raising(monkeypatch, capsys, exc)
    assert not out.lstrip().startswith("Command '")
    assert "x19 gateway status --deep" in out
    assert "x19 gateway install --force" in out
...
def test_existing_gateway_guard_says_bots_are_online_and_names_status(monkeypatch, capsys):
    ...
    for cmd in ("x19 gateway status", "x19 gateway restart", "x19 gateway stop"):
        assert cmd in out
...
def test_unsupported_platform_copy_says_what_is_unsupported(subcommand):
    ...
    assert "x19 gateway run" in text or subcommand == "uninstall"
...
def test_invalid_profile_name_explains_rule_in_words_with_example():
    ...
    assert "x19 profile create my-work" in msg
...
def test_existing_profile_error_offers_use_and_list(tmp_path, monkeypatch):
    ...
    assert "x19 profile use work" in msg
    assert "x19 profile list" in msg
...
def test_describe_holder_avoids_jargon_and_names_next_steps():
    ...
    assert "x19 logs" in message
    assert "x19 update" in message
