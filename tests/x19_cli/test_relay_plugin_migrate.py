"""``x19 update`` / ``x19 migrate relay``: legacy ``X19_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from x19_cli.relay_plugin_migrate import (
...
X19_NEMO_RELAY_ATOF_ENABLED=1
X19_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
X19_NEMO_RELAY_ATOF_FILENAME=x19-atof.jsonl
X19_NEMO_RELAY_ATOF_MODE=append
X19_NEMO_RELAY_ATIF_ENABLED=1
X19_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
X19_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
X19_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "x19-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file X19 will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: X19_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
