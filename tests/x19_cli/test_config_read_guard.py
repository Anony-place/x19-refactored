...
Canonical owners:
  ...
  * ``x19_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._x19_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/x19/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "x19_cli/managed_scope.py",
