"""``x19 update`` refreshes the installed macOS ``X19.app`` from the rebuilt bundle (#52339).

``x19 desktop --build-only`` only packages into ``apps/desktop/release/``; Finder launches the
...
def _bundle(root: Path, asar: bytes) -> Path:
    ...
    (app / "Contents" / "MacOS" / "X19").write_bytes(b"\xcf\xfa\xed\xfe")
...
def test_stale_bundle_is_replaced_current_and_running_are_left_alone(rebuilt, tmp_path):
    ...
    installed, problems = main_desktop._install_rebuilt_macos_bundles(
        rebuilt, [stale, current, running, tmp_path / "missing" / "X19.app"],
        running={running.resolve()})

    assert installed == [stale]
    assert _asar(stale) == b"rebuilt"
    assert not (stale.parent / "X19.app.x19-update-old").exists()
    assert not (stale.parent / "X19.app.x19-update-new").exists()
    assert current_marker.read_text() == "untouched"
    # A live app is reported, never swapped under.
    assert _asar(running) == b"older"
    assert len(problems) == 1 and str(running) in problems[0] and "quit X19 Desktop" in problems[0]
...
def test_failed_swap_keeps_the_previous_bundle_launchable(rebuilt, tmp_path, monkeypatch):
    ...
    def fail_final_rename(self, target):
        if self.name.endswith(".x19-update-new"):
