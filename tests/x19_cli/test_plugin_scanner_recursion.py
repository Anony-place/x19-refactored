...
def _enable(x19_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<x19_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        x19_home = Path(os.environ["X19_HOME"])  # set by hermetic conftest fixture
        user_plugins = x19_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(x19_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        x19_home = Path(os.environ["X19_HOME"])  # set by hermetic conftest fixture
        _write_plugin(x19_home / "plugins", ["p1"])
        _enable(x19_home, "p1")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(x19_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(x19_home, "bad-img-plugin")
