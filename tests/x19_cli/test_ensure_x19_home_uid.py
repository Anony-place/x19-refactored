"""Regression tests for #34107 — Docker UID/GID handling in ensure_x19_home.

When X19 runs in Docker with ``X19_UID=1000`` / ``X19_GID=911``,
the entrypoint chowns the top-level ``X19_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_x19_home()`` — especially
...
The fix is a ``_chown_to_x19_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_x19_uid_gid
# ---------------------------------------------------------------------------


class TestResolveX19UidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("X19_UID", "1000")
        monkeypatch.setenv("X19_GID", "911")
        from x19_cli.config import _resolve_x19_uid_gid
...
class TestChownToX19Uid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_x19_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("X19_UID", raising=False)
        monkeypatch.delenv("X19_GID", raising=False)
        from x19_cli import config as cfg
