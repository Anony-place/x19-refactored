"""Tests for IRC gateway configuration via `x19 setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh X19 install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new X19 install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#x19")
            monkeypatch.setenv("IRC_NICKNAME", "x19-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `x19 setup gateway` experience with IRC present."""
