...
Regression: the old implementation wrote ``x19_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``x19 sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.x19/sessions/saved/`` and prints
...
def x19_home(tmp_path, monkeypatch):
    ...
    # Clear any cached x19_home computation
    import x19_constants
    if hasattr(x19_constants, "_x19_home_cache"):
        x19_constants._x19_home_cache = None
    return home
...
def test_save_conversation_writes_under_x19_home(x19_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.x19/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the X19_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "x19_constants"]:
    ...
    cli.X19CLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("x19_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.x19/sessions/saved/
    saved_dir = x19_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("x19_conversation_*.json"))
    ...
    assert "x19 --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(x19_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "x19_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(x19_home, capsys):
    ...
    cli.X19CLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(x19_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "x19_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.X19CLI.save_conversation(stub, "/save pdf")
