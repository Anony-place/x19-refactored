...
from x19_cli import kanban_db_dispatch as dispatch
...
def test_goal_mode_worker_takes_the_same_stdout_rich_path_as_a_one_shot_worker(monkeypatch):
    monkeypatch.setattr(dispatch, "_resolve_x19_argv", lambda: ["x19"])
    ...
    # The mode travels in X19_KANBAN_GOAL_MODE; the argv must not opt the worker out of
...
def test_non_quiet_one_shot_runs_the_goal_loop_through_cli_chat(monkeypatch):
    monkeypatch.setenv("X19_KANBAN_TASK", "t_goal1")
    monkeypatch.setenv("X19_KANBAN_GOAL_MODE", "1")
    ...
    monkeypatch.setattr("x19_cli.kanban_db_connect.connect_closing", lambda: _Conn())
    monkeypatch.setattr("x19_cli.kanban_db.get_task", lambda conn, tid: _task(goal_mode=True))
