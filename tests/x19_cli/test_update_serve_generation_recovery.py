"""Regression coverage for stale ``x19 serve`` generations after an abort.

Issue #92145: ``x19 update`` moves the checkout from generation N to N+1
...
``x19 serve`` is not a gateway profile.  It hosts ``tui_gateway.server``,
systemd lists ``x19-gateway.service`` before ``x19-serve.service`` (so
...
from x19_cli import update_abort_recovery as abort_recovery
from x19_cli import update_cmd
from x19_cli import update_restart_recovery as recovery
...
def _identity_module():
    return __import__("x19_cli.process_identity", fromlist=["ledger_entries"])
...
def test_failed_serve_unit_blocks_completion():
    ...
    result["serve_units"] = {"verified": [], "failed": ["x19-serve"]}
    assert (
        update_cmd._abort_recovery_is_complete(
            planned_gateway_profiles={"default"},
            covered_gateway_profiles={"default"},
...
class _Systemctl:
    ...
    def __call__(self, argv, **kwargs):
        if "list-units" in argv:
            ...
            body = "\n".join(
                f"{unit} loaded active running X19" for unit in self.listed
            )
...
def test_serve_unit_verified_when_main_pid_changes(linux_systemctl):
    fake = _Systemctl(
        listed=["x19-serve.service"],
        active={"x19-serve.service": True},
        main_pids={"x19-serve.service": 4242},
    )
    ...
    assert out == {"verified": ["user/x19-serve"], "failed": []}
    assert fake.restarted == ["x19-serve.service"]
...
def test_unchanged_main_pid_is_not_verified(linux_systemctl):
    ...
    assert out == {"verified": [], "failed": ["user/x19-serve"]}
...
def test_unit_that_does_not_come_back_active_is_failed(linux_systemctl):
    ...
    def dies(argv, **kwargs):
        if "restart" in argv:
            fake.restarted.append(argv[-1])
            fake.active["x19-serve.service"] = False
            return _Completed(0)
        return original_call(argv, **kwargs)
...
def test_inactive_serve_unit_is_left_alone(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["x19-serve.service"],
        active={"x19-serve.service": False},
        main_pids={"x19-serve.service": 0},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
...
def test_units_already_restarted_by_the_phase_are_skipped(linux_systemctl):
    fake = _Systemctl(
        listed=["x19-serve.service", "x19-serve-work.service"],
        active={"x19-serve.service": True, "x19-serve-work.service": True},
        main_pids={"x19-serve.service": 1, "x19-serve-work.service": 2},
    )
    out = recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "x19-serve"}],
        run=fake,
        sleep=lambda _: None,
    )
    assert fake.restarted == ["x19-serve-work.service"]
    assert out == {"verified": ["user/x19-serve-work"], "failed": []}


def test_unrelated_x19_server_service_is_never_touched(linux_systemctl):
    """``x19-serve*`` is a systemd glob, not a name gate (review on #83595)."""
    fake = _Systemctl(
        listed=["x19-server.service", "x19-serve.service"],
        active={"x19-server.service": True, "x19-serve.service": True},
        main_pids={"x19-server.service": 7, "x19-serve.service": 8},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == ["x19-serve.service"]
    assert out["verified"] == ["user/x19-serve"]
...
def test_gateway_units_are_not_restarted_by_the_serve_pass(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["x19-gateway.service"],
        active={"x19-gateway.service": True},
        main_pids={"x19-gateway.service": 5},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == []
    assert out == {"verified": [], "failed": []}
...
def test_same_unit_name_in_both_scopes_is_proven_in_both(linux_systemctl):
    ...
    def dual_scope(argv, **kwargs):
        ...
        if "list-units" in argv:
            return _Completed(0, stdout="x19-serve.service loaded active running x")
        if "is-active" in argv:
    ...
    assert out == {
        "verified": ["user/x19-serve"],
        "failed": ["system/x19-serve"],
    }
            ...
            json.dumps(
                {
                    ...
                    "serve_units": {
                        ...
                        "skip": [
                            "x19-serve",
                            "x19-serve-work.service",
                            {"scope": "system", "unit": "x19-serve"},
                            {"scope": "user", "unit": "x19-serve-work.service"},
                            "x19-server",
                            "../../etc/systemd/evil",
                            "x19-serve; rm -rf /",
                            "x19-gateway",
...
def test_serve_only_fleet_still_spawns_the_recovery_child(monkeypatch):
            ...
            stdout=json.dumps(
                {
                    ...
                    "relaunch_attempted": [],
                    "failed": [],
                    "serve_units": {"verified": ["x19-serve"], "failed": []},
                }
            ),
        )
    ...
    assert result["serve_units"] == {"verified": ["x19-serve"], "failed": []}
...
def test_verified_serve_units_do_not_enter_gateway_restart_vocabulary(monkeypatch):
    ...
    assert "x19-serve" not in result["verified"]
...
def test_already_restarted_units_are_forwarded_as_skips(monkeypatch):
    ...
    update_cmd._recover_gateway_restart_after_abort(
        _plan(_runtime("gateway", "default", "systemd", 111)),
        gateway_mode=False,
        skip_units={"user/x19-serve"},
    )
    # The scope travels with the unit: settling `user/x19-serve` must not
    # suppress recovery of a stale system-scope unit of the same name.
    assert captured["payload"]["serve_units"]["skip"] == [
        {"scope": "user", "unit": "x19-serve"}
    ]
...
def test_serve_coverage_reaches_the_persisted_receipt(tmp_path, monkeypatch):
    from x19_cli import update_receipt
    ...
    update_receipt.record_gateway_restart(
        restarted_services=[],
        incomplete=True,
        phase_error="cannot import name 'line_input' from 'x19_cli.cli_output'",
        fresh_recovery={
            "requested": ["default"],
            "verified": ["default"],
            "relaunch_attempted": [],
...
def test_stale_serve_warning_names_the_process_and_the_fix(capsys):
    ...
    assert "4242" in out
    assert "serve" in out
    assert "systemctl --user restart x19-serve.service" in out
...
def test_recovery_module_reports_serve_units_in_a_real_process():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "x19_cli.update_restart_recovery", "--stdin"],
        input=json.dumps(
...
def _stub_dashboard_helpers(monkeypatch, **helpers):
    """Stub the ``x19_cli.main_dashboard`` helpers the dashboard-cleanup path reads at call time."""
...
def test_managed_dashboard_restart_still_scans_for_serve_backends(monkeypatch):
    ...
    The reporter's host runs ``x19-dashboard.service`` AND
    ``x19-serve.service``.  Returning as soon as the dashboard unit was
...
def test_restarted_dashboard_unit_is_not_killed_by_the_continued_scan(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_systemd_service_for_pid=lambda pid: "x19-dashboard.service",
...
def test_serve_backend_survives_selection_when_the_dashboard_unit_restarts(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_pid_cgroup_path=lambda pid: "/user.slice/x19-serve.service",
        _get_systemd_service_for_pid=lambda pid: "x19-serve.service",
        _dashboard_cmdline_for_pid=lambda pid: None,
    ...
    assert restarted == ["x19-serve.service"]
...
def _dual_scope_systemctl(*, user_pid=10, system_pid=20, settle=("user", "system")):
    """Both managers own a ``x19-serve.service``; each is its own process."""
...
def test_settled_user_scope_does_not_suppress_the_stale_system_scope(
    ...
    ``user/x19-serve`` was restarted before the phase aborted;
    ``system/x19-serve`` is a different process still on the pre-update
    generation. A bare ``x19-serve`` skip token suppressed BOTH, leaving
    ...
    assert out == {"verified": ["system/x19-serve"], "failed": []}
...
def test_one_authorized_scope_never_mutates_the_same_name_in_the_other(
    ...
    recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "x19-serve.service"}],
        run=run,
        sleep=lambda _: None,
    )
    user_verbs = [
...
def test_legacy_unqualified_skip_is_honoured_in_both_scopes(linux_systemctl):
    ...
    run, calls = _dual_scope_systemctl()
    out = recovery.restart_serve_units(
        skip_units=["x19-serve"], run=run, sleep=lambda _: None
    )
...
def test_qualified_skip_payload_shape():
    ...
    assert update_cmd._qualified_serve_skips(
        {"user/x19-serve", "system/x19-serve-work"}
    ) == [
        {"scope": "system", "unit": "x19-serve-work"},
        {"scope": "user", "unit": "x19-serve"},
    ]
...
def test_unscoped_bookkeeping_entries_are_forwarded_without_a_scope():
    ...
    assert update_cmd._qualified_serve_skips({"x19-serve"}) == [
        {"unit": "x19-serve"}
    ]
