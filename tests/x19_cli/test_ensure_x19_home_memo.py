"""ensure_x19_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".x19"
    second = tmp_path / "b" / ".x19"

    monkeypatch.setenv("X19_HOME", str(first))
    cfg.ensure_x19_home()

    # Profile switch: X19_HOME moves → the new path is ensured too.
