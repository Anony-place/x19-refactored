"""Tests for x19_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import x19_cli.gui_uninstall as gu


def _make_agent(x19_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = x19_home / "x19"
    (agent_root / "x19_cli").mkdir(parents=True)
    (agent_root / "x19_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(x19_home: Path) -> None:
    """Create the source-built GUI artifacts a `x19 desktop` run produces."""
    desktop = x19_home / "x19" / "apps" / "desktop"
    ...
    (x19_home / "x19" / "node_modules").mkdir(parents=True)
    (x19_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(x19_home: Path) -> None:
    (x19_home / "config.yaml").write_text("x: 1\n")
    (x19_home / ".env").write_text("KEY=secret\n")
    (x19_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(x19_home)
    _make_gui_build(x19_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(x19_home)
    ...
    assert summary["x19_home"] == str(x19_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `x19 desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    x19_home = tmp_path / ".x19"
    _make_agent(x19_home)
    icon = lde.icon_path(x19_home / "x19")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(x19_home)
    ...
    assert lde.icon_path(x19_home / "x19").exists()
    # The agent itself survives a GUI uninstall.
    assert (x19_home / "x19" / "x19_cli").is_dir()
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import x19_cli.uninstall as uninstall
