...
The notice prints the most recent curator run summary on `x19 update`,
...
session start) so users normally never see the rename map. `x19 update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from x19_cli import update_cmd_maint
