...
def _run(monkeypatch, capsys, argv_tail, db):
    ...
    monkeypatch.setattr(x19_state, "SessionDB", lambda *args, **kwargs: db)
    monkeypatch.setattr(sys, "argv", ["x19", "sessions", *argv_tail])
    try:
        main_mod.main()
        code = 0
...
def test_pin_multiple_ids_one_missing(monkeypatch, capsys):
    ...
    assert "No session 'nope'" in out and "x19 sessions list" in out
    assert code == 1
...
def test_pinned_empty_hint(monkeypatch, capsys):
    ...
    assert "x19 sessions pin" in out
