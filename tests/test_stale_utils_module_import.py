...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.x19/x19/utils.py)
...
2. ``x19 update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``x19_cli/main.py`` (the ``x19 update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``x19_constants`` after the
