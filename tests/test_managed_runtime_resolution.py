"""Guard: X19-owned subprocesses must not resolve managed runtimes by bare PATH.

X19 installs runtimes for itself — ``uv`` at ``$X19_HOME/bin/uv``, Node at
``$X19_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in X19's own
...
* when a system copy also exists, the one X19 does not own wins — which is
...
``iter_x19_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes X19 provisions into X19_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not X19-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("x19_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (x19_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        ...
        "  node/npm -> x19_constants.find_node_executable()\n"
        "  PATH env -> x19_constants.iter_x19_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(x19_constants, helper))


def test_managed_uv_helpers_exist():
    from x19_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
