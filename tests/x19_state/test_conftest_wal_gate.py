"""The conftest WAL gate must agree with x19_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``x19_state``. That is deliberate: importing
``x19_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.x19`` before the per-test ``X19_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_x19_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and x19_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_x19_state_at_collection():
    """The gate must stay import-free of x19_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.x19, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``x19_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "x19_state" or name.startswith("x19_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported x19_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.x19 during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("x19_state", None)
