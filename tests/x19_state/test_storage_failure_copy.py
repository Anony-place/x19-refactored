...
@pytest.mark.parametrize(
    ...
    [
        ...
        (sqlite3.OperationalError("attempt to write a readonly database"), "storage_readonly", "x19 doctor --fix"),
        (sqlite3.DatabaseError("database disk image is malformed"), "storage_corrupt", "x19 doctor --fix"),
        (OSError(28, "No space left on device"), "disk_full", "Free some disk space"),
        (None, "storage_unavailable", "x19 doctor --fix"),
    ],
)
...
def test_action_command_is_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    whose state.db is not the process default; a bare ``x19`` follows active_profile)."""
    ...
    for exc in (sqlite3.OperationalError("database is locked"), sqlite3.DatabaseError("malformed"), None):
        ...
        assert f"`x19 {selector}" in action and "`x19 doctor" not in action and "`x19 gateway" not in action
