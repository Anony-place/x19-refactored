...
import x19_state
import x19_state_holders
import x19_state_schema
from x19_state import SessionDB
from x19_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from x19_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m x19_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "x19.service"),
            ("grep", "x19", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m x19_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/x19/.venv/bin/x19 gateway"),
            ("python3", "/opt/x19/tools/check_state.py"),
            ("x19-monitor", "gateway"),
            ("x19ctl", "serve"),
            ("python3", "worker.py", "x19_cli.main"),
            ("python3", "-m", "other.module", "x19_cli.main"),
            ("python3", "-c", "x19_cli.main"),
            ("python3", "-Icprint('x19_cli.main')", "x19_cli/main.py"),
        ),
    )
    def test_uninspectable_non_x19_process_is_not_a_holder(self, argv):
        assert not x19_state_holders._looks_like_x19(argv)
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("/usr/bin/python3", "-Im", "x19_cli.main", "gateway"),
            ("/usr/bin/python3", "-mx19_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "x19_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "x19_cli.main"),
            (
                "/opt/x19/.venv/bin/python",
                "/opt/x19/x19_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "x19_cli/main.py", "gateway"),
            ("python3", "/opt/x19/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_x19_process_remains_a_holder(self, argv):
        assert x19_state_holders._looks_like_x19(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(x19_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(x19_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(x19_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(x19_state_holders.os, "stat", _stat)

        holders = x19_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert x19_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        # PID 222's cmdline is world-readable and looks like X19
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00x19_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(x19_state_holders, "_read_proc_argv", _fake_argv)

        holders = x19_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "x19_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from x19_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(x19_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            x19_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "x19_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(x19_state_schema.time, "time", lambda: clock[0])

        from x19_cli.doctor_state import _render_state_db_stats
        from x19_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(x19_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(x19_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m x19_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + x19_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = x19_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="x19_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
