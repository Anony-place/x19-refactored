...
A raw single-process ``pytest tests/x19_cli/`` used to accumulate every
...
* ``x19_state_guard._register_test_instance`` adds every successfully
  constructed SessionDB to a WeakSet registry when the
  ``X19_TEST_ISOLATION`` marker is set (test-isolation runs only).
...
def test_constructed_sessiondb_is_registered(tmp_path):
    ...
    try:
        assert db in x19_state_guard._test_instance_registry
    finally:
        db.close()
