"""POSIX advisory locks must survive X19' own database inspection.
...
X19 used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `x19 sessions
...
out across X19' inspection calls.
"""
...
from x19_cli.sqlite_safe_read import (
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_x19_fail_close", False):
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import x19_cli.sqlite_safe_read as ssr
