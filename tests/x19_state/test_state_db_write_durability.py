...
The defect: x19_state already knows macOS `fsync()` does not guarantee
...
import x19_state
from x19_state import repair_state_db_schema
from x19_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in x19_state_repair; x19_state only
    # re-imports them.
    import x19_state_repair

    source = Path(x19_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(x19_state_repair.__file__))
