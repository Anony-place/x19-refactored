"""Plain-language contracts for x19_state user-facing errors (CLI UX message campaign, cluster D)."""

import x19_state
from x19_state import SessionResumeTooLargeError, format_session_db_unavailable
...
def test_resume_too_large_names_export_and_config_commands():
    ...
    assert "x19 sessions export" in text
    assert "x19 config set sessions.max_resume_messages 0" in text
...
def test_db_unavailable_points_to_doctor_without_sqlite_internals():
    x19_state._set_last_init_error("OperationalError: database is locked")
    try:
...
def test_db_unavailable_unknown_cause_on_network_drive_points_at_moving_not_doctor_fix():
    ...
    assert "x19 doctor --fix" not in text


def test_db_unavailable_without_cause_still_names_doctor():
    x19_state._set_last_init_error(None)
    text = format_session_db_unavailable(details=True)
    assert "x19 doctor" in text
...
def test_db_unavailable_commands_are_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    for text in (no_cause, network):
        assert f"`x19 {selector}doctor`" in text and "`x19 doctor`" not in text
