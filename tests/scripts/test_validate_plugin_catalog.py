...
VALID_ENTRY = {
    "name": "example-plugin",
    "repo": "https://github.com/Anony-place/x19-refactored-plugins",
...
def test_bad_requires_x19_spec_fails(tmp_path):
    _expect_error(tmp_path, {"requires_x19": "banana"}, "requires_x19")


def test_comma_separated_requires_x19_passes(tmp_path):
    entry = {**VALID_ENTRY, "requires_x19": ">=0.19, <2.0"}
    path = write_entry(tmp_path, entry)
    result = run_validator(str(path))
    assert result.returncode == 0, result.stdout + result.stderr
