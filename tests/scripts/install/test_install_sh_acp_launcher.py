"""`setup_path()` must also install a `x19-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `x19-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report X19 as not installed. `setup_path()` therefore
writes a `x19-acp` launcher next to the `x19` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/x19-acp".*?'
    r'log_success "Installed x19-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the x19-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    x19_bin = tmp_path / "venv" / "bin" / "python"
    x19_bin.parent.mkdir(parents=True)
    x19_bin.write_text("#!/bin/sh\n", encoding="utf-8")
    entrypoint = tmp_path / "x19"
    entrypoint.write_text("#!/bin/sh\n", encoding="utf-8")

    script = (
        "set -e\n"
        f"X19_BIN={x19_bin}\n"
        f"X19_ENTRYPOINT={entrypoint}\n"
    ...
    return command_link_dir / "x19-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `x19-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "x19-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/x19-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/x19-acp must be replaced with a regular file"
    )
...
X19_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/x19-agent".*?'
    r'log_success "Installed x19-agent launcher[^\n]*\n)',
    re.S,
)
