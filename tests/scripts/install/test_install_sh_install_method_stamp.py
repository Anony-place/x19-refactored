...
($INSTALL_DIR), not into the shared $X19_HOME.

Background (shared-$X19_HOME bug)
------------------------------------
$X19_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.x19:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$X19_HOME the two installs would fight over one slot — and the container,
...
``~/.x19/x19``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$X19_HOME/\.install_method"', text), (
        "install.sh must not stamp $X19_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `x19 update`"
    )
