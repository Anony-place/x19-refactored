...
``--install-root`` keeps whatever spelling the app's X19 root had (a symlinked
``~/.x19/x19``, or ``/home`` on Fedora/ostree where it is a link to
...
def test_symlinked_spelling_on_either_side_still_relaunches(tmp_path, root_spelling, target_spelling):
    ...
    target = tmp_path / target_spelling / "apps" / "desktop" / "release" / "linux-unpacked" / "X19"
...
def test_target_outside_the_checkout_is_still_skew(tmp_path):
    ...
    (tmp_path / "link").symlink_to(real)
    foreign = tmp_path / "opt" / "X19"
    foreign.mkdir(parents=True)
    (foreign / "x19").touch()
    assert _gate(tmp_path / "link", foreign / "x19") == "skew"
    # A sibling directory sharing the prefix must not be mistaken for the checkout either.
    assert _gate(tmp_path / "link", Path(str(unpacked) + "-evil") / "X19") == "skew"
