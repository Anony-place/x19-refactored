...
Build the real image and verify: a config.yaml present in $X19_HOME
is migrated by docker_config_migrate.py on boot, running as the x19
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $X19_HOME must be migrated on boot by
    docker_config_migrate.py, running as the x19 user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $X19_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/x19/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by x19 (migration ran as x19)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "x19", (
        f"config.yaml not owned by x19 (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
