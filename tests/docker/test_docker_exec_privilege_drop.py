...
The shim (docker/x19-exec-shim.sh, installed at /opt/x19/bin/x19)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> x19 login` would write /opt/data/auth.json as
...
failed: X19 is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> x19 …`` (defaulting to root) gets dropped to the
   x19 user before the real binary runs.
2. ``docker exec --user x19 <c> x19 …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $X19_HOME from a ``docker exec`` session land
   as x19:x19 — the actual user-visible invariant.
4. The X19_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-x19-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown x19:x19`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to x19:x19,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$X19_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (x19_cli.container_boot), which s6 runs
    *strictly after* ``01-x19-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_x19_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `x19` with a Python-style `-c` shim equivalent — there's no
    pure-x19 "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `x19` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `x19`
    do anything that writes to disk, then check the file's owner.

    Use `x19 config set` which writes config.yaml under X19_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "x19", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by x19, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "x19:x19", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected x19:x19. "
        "The shim did not drop privileges before invoking x19."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/x19/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid x19 x19 <args>` which resolves `x19`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> x19 login`` (root) wrote
    ...
    "X19 is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `x19 config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is x19-readable.

    Specifically: pretend the operator ran `x19 login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the x19 UID. We use `x19 auth list` since that touches the
    ...
    # `x19 auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `x19 config set` first to
    # provoke a write into X19_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "x19", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # X19_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "x19", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the x19 user "
