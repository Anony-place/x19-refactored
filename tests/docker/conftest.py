...
Override the image with ``X19_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("X19_TEST_IMAGE", "x19-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``X19_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("X19_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged x19 user
# ---------------------------------------------------------------------------
#
# Background: every X19 runtime path inside the container drops to UID
# 10000 (the ``x19`` user) via ``s6-setuidgid x19``. ``docker exec``
...
# fails (PermissionError swallowed) for x19, so a test that ran as root
...
# helpers below run every probe under ``-u x19`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: x19).
