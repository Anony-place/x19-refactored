...
Build the real image and verify at runtime:
  ...
  1. /opt/x19 is not writable by the x19 user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and X19_DISABLE_LAZY_INSTALLS are set
  3. /opt/x19/.install_method contains "docker" (code-scoped stamp)
  4. $X19_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $X19_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_x19(
    built_image: str, container_name: str,
) -> None:
    """The x19 user must not be able to modify /opt/x19.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/x19 as the x19 user
        "touch /opt/x19/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"x19 user can write to /opt/x19 (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/x19/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"x19 user can write to /opt/x19/.venv: {r.stdout}"
    )


def test_x19_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    X19_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $X19_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $X19_HOME was not healed on boot: "
        f"{r.stdout}"
    )
