...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$X19_HOME" = "/opt/data" && '
        'test "$X19_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_x19_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/x19/agent/message_sanitization.py")
        try:
