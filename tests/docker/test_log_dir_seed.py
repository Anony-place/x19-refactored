"""Runtime smoke test for Docker $X19_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the x19 user after container boot.
...
registered later runs its log service as the dropped x19 user and
...
def test_logs_gateways_seeded_and_x19_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by x19 after boot."""
    ...
    assert "logs=x19" in r.stdout, (
        f"logs/ not owned by x19: {r.stdout}"
    )
    assert "gateways=x19" in r.stdout, (
        f"logs/gateways/ not owned by x19: {r.stdout}"
    )
