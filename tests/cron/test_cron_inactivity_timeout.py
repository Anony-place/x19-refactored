...
- Unlimited timeout (X19_CRON_TIMEOUT=0)
- Backward compat: X19_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """X19_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """X19_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("X19_CRON_TIMEOUT", "1200")
        raw = os.getenv("X19_CRON_TIMEOUT", "").strip()
...
class TestSysPathOrdering:
    ...
    def test_x19_time_importable(self):
        """x19_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _x19_now
        assert callable(_x19_now)
