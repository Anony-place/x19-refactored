"""
Tests for timezone support (x19_time module + integration points).
...
def _reset_x19_time_cache():
    """Reset the x19_time module cache."""
    x19_time.reset_cache()


# =========================================================================
# x19_time.now() — core helper
# =========================================================================

class TestX19TimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_x19_time_cache()

    def teardown_method(self):
        _reset_x19_time_cache()
        os.environ.pop("X19_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["X19_TIMEZONE"] = "Asia/Kolkata"
        result = x19_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["X19_TIMEZONE"] = "UTC"
        result = x19_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["X19_TIMEZONE"] = "America/New_York"
        result = x19_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["X19_TIMEZONE"] = "Europe/London"
        tz = x19_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching X19_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("X19_TIMEZONE", raising=False)

        monkeypatch.setenv("X19_HOME", str(first_home))
        assert str(x19_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch X19_HOME in one process.
        monkeypatch.setenv("X19_HOME", str(second_home))
        assert str(x19_time.get_timezone()) == "America/New_York"
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway X19_TIMEZONE holds only the DEFAULT profile's value
        ...
        monkeypatch.setenv("X19_TIMEZONE", "America/New_York")

        # Single-profile process: env stays authoritative.
        assert x19_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert x19_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_x19_home_override(str(routed_home))
            try:
                assert x19_time.get_timezone_name() == "Asia/Tokyo"
                assert str(x19_time.get_timezone()) == "Asia/Tokyo"
            finally:
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_x19_home_override(str(homes[key]))
                try:
                    tz = x19_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When X19_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) X19_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("X19_TIMEZONE=" + os.environ.get("X19_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "X19_TIMEZONE=NOT_SET" in result["output"], (
            "X19_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When X19_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("X19_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=x19_tz) which shifted
        absolute time when system-local tz != X19 tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when X19 tz
        ...
        # Use an X19 timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _x19_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["X19_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_x19_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of X19 timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["X19_TIMEZONE"] = "US/Eastern"
        _reset_x19_time_cache()
