...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_x19_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "x19_constants.get_default_x19_root", lambda: root
    )
    token = set_x19_home_override(str(grant_home))
