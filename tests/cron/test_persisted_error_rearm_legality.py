...
def cron_store(tmp_path, monkeypatch):
    ...
    (x19_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("X19_HOME", str(x19_home))
    monkeypatch.setattr(J, "X19_DIR", x19_home)
    monkeypatch.setattr(J, "CRON_DIR", x19_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", x19_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", x19_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_x19_now", lambda: now):
            due = J.get_due_jobs()
