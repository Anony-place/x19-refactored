...
builds must resolve exactly as it would under a standalone ``x19 -p <name> gateway run``.
"""
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        "X19_CRON_TIMEOUT=111\nX19_MODEL=default-model\nTERMINAL_ENV=docker\n"
        "TERMINAL_DOCKER_IMAGE=default-only-image\nX19_LANGUAGE=en\n")
    (alpha / ".env").write_text("X19_CRON_TIMEOUT=222\nX19_LANGUAGE=zh\n")
    ...
    for key, val in (("X19_CRON_TIMEOUT", "111"), ("X19_MODEL", "default-model"),
                     ("TERMINAL_ENV", "docker"), ("TERMINAL_DOCKER_IMAGE", "default-only-image"),
                     ("X19_LANGUAGE", "en")):
        monkeypatch.setenv(key, val)
    set_multiplex_active(True)
    home_token = set_x19_home_override(str(alpha))
    try:
        yield root, alpha
    finally:
...
def test_cron_env_settings_resolve_from_the_served_profile(two_homes):
    """``X19_CRON_TIMEOUT`` / ``X19_MODEL`` are alpha's (or absent) both with the fire-time
    ...
    try:
        ...
        try:
            ...
            assert captured["target_model"] == ""  # alpha has no X19_MODEL; default's must not leak
...
def test_child_env_for_served_profile_drops_launch_profile_settings(two_homes):
    ...
    try:
        ...
        reset_secret_scope(token)
    for key in ("X19_MODEL", "TERMINAL_ENV", "TERMINAL_DOCKER_IMAGE", "X19_LANGUAGE", "X19_CRON_TIMEOUT"):
        assert key not in env, key
    assert env["X19_HOME"] == str(alpha)
    assert "PATH" in env

    # No-op when the scope IS the launch profile (standalone / default profile's own children).
    reset_x19_home_override(set_x19_home_override(None))
    home_token = set_x19_home_override(str(root))
    try:
        env = strip_launch_profile_env(build_subprocess_env(scrub_secrets=True, inherit_profile_home=True))
    finally:
        reset_x19_home_override(home_token)
    assert env["TERMINAL_ENV"] == "docker"
    assert env["X19_MODEL"] == "default-model"
