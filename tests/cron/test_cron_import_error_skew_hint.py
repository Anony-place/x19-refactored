...
Field incident: an interrupted `x19 update` (pull done, restart never ran)
...
had no way to know the fix was one `x19 gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "x19 gateway restart" not in msg
