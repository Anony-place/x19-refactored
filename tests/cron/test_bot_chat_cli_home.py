...
def test_cli_keeps_discovered_home_when_launch_selection_changes(tmp_path, monkeypatch, profile):
    ...
    token = set_x19_home_override(str(root))
    ...
    def run(argv, **kwargs):
        ...
        code = ('import json,sys; sys.argv=["x19"]+json.loads(sys.argv[1]); '
                'import x19_cli.main; from x19_constants import get_x19_home; '
                'print(json.dumps(str(get_x19_home())))')
        # Everything after the launcher (binary or ``python -m x19_cli.main``) is the CLI argv.
        cli_argv = argv[3:] if argv[1:3] == ["-m", "x19_cli.main"] else argv[1:]
    ...
    try:
        ...
        assert seen == [home]
        assert not (tmp_path / ".x19").exists()
    finally:
