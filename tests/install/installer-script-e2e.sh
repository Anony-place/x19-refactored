...
#              install landed on OLD with a working `x19`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `x19`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method x19-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  x19-update|installer-script|installer-script+desktop|x19-desktop-app-update) ;;
  *) echo "error: --update-method must be x19-update, installer-script, installer-script+desktop or x19-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/x19-installer-script-e2e"
LOG_DIR="${X19_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled x19 or a
# developer config, and old installer scripts hardcode $HOME/.x19 (the
# X19_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export X19_HOME="$HOME/.x19"
mkdir -p "$X19_HOME"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/X19" \
    "$release_dir/linux-unpacked/x19" \
    "$release_dir/mac-arm64/X19.app" \
    "$release_dir/mac/X19.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local x19="$INSTALL_DIR/venv/bin/x19"
  [ -x "$x19" ] || fail "no x19 console script at $x19"
  "$x19" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "x19 --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "x19 --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `x19 desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `x19 update` itself makes. Probe the
  # INSTALLED x19 for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `x19 desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local x19="$INSTALL_DIR/venv/bin/x19"
  if ! "$x19" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "x19 desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$x19" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "x19 desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "x19 desktop --build-only ($1) exited $rc; transcript above"
  ok "x19 desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  x19-update)
    ...
    # installed x19; older ones read the prompt from stdin, so close it.
    X19="$INSTALL_DIR/venv/bin/x19"
    if "$X19" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$X19" update --yes)
    else
      update_cmd=("$X19" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "x19 update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "x19 update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  x19-desktop-app-update)
    # The real user surface: `x19 desktop` launches the app, the user
    ...
    # suite's own) is configured into X19_HOME exactly like the dev:mock
    ...
    step "capturing the x19 desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      X19_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$X19" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "x19 desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "x19 desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "x19 desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$X19_HOME/logs" "$ildest/x19-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/x19/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$X19_HOME/.x19-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$X19_HOME" > "$ildest/x19-home-ls.txt" 2>/dev/null || true
