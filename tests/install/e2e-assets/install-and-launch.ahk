...
; Drive the REAL X19-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real X19.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-x19-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "X19-Setup.exe"
...
Log("Launch clicked; waiting for the X19 desktop app window")
...
try {
    ...
    throw Error("X19.exe real-sized window did not appear within 300s of clicking Launch")
}
