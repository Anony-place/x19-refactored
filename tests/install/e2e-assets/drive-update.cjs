// drive-update.cjs — launch the INSTALLED X19.exe (real Electron desktop
...
//
//   node <this file> <path-to-X19.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <X19.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: X19_HOME (isolated install) and
  ...
  // The real backend (`x19 serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting X19... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.x19Desktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating X19 — this window will close" overlay should appear,
  ...
  // the staged binary), which claims X19_HOME/.x19-update-in-progress
  ...
  // proved the desktop had exited and `x19 update` was already running).
  //
  ...
  const x19Home = process.env.X19_HOME
  const markerPath = x19Home ? path.join(x19Home, '.x19-update-in-progress') : null
  const resultPath = x19Home ? path.join(x19Home, '.x19-update-result.json') : null
