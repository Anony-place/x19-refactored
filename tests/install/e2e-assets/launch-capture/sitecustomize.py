"""Driver-side spawn interception for x19 desktop E2E legs.

The installed ``x19`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``x19
...
Opt-in: ``X19_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "x19
...
_SPEC_PATH = os.environ.get("X19_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/X19.exe, linux-unpacked/...,
        # mac*/X19.app/Contents/MacOS/...).
