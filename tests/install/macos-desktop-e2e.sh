...
# (X19-Setup.dmg from the website) can reach HEAD.
#
...
#              x19-desktop-app-update  capture `x19 desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              x19-update              CLI update from the installed venv
...
#     --update-method open-app-update|x19-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://x19-assets.nousresearch.com/X19-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|x19-desktop-app-update|x19-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${X19_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/x19-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$X19_HOME/x19"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$X19_HOME"
  touch "$X19_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/X19.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/X19.app" \
    "/Applications/X19.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published X19-Setup.dmg"

  local dmg="$WORK_ROOT/X19-Setup.dmg"
  ...
  # "Install X19" button with native input.
  local rc=0
  ...
  log_group "X19-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$x19" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "x19 --version failed after install"
  ok "x19 --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed X19.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    x19-update)
      ...
      # update subcommand only in later releases; ask the installed x19.
      local x19="$INSTALL_DIR/venv/bin/x19"
      local update_cmd=("$x19" update)
      ...
      [ "$rc" -eq 0 ] || fail "x19 update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/X19.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/X19.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built X19.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, x19 update) need.
      local app app_bin
...
PYEOF
    ...
    x19-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "x19 desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "x19 desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.x19/logs" "$ildest/x19-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/X19"
  ...
  "$INSTALL_DIR/venv/bin/x19" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "x19 --version failed after update"
  ok "x19 --version works post-update"
