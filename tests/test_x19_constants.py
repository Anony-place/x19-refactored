"""Tests for x19_constants module."""
...
from x19_constants import (
    ...
    heal_x19_managed_node,
    x19_managed_node_tree_present,
    iter_x19_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultX19Root:
    """Tests for get_default_x19_root() — Docker/custom deployment awareness."""

    @pytest.mark.linux_only
    def test_no_x19_home_returns_native(self, tmp_path, monkeypatch):
        """When X19_HOME is not set, returns ~/.x19."""
        monkeypatch.delenv("X19_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_x19_root() == tmp_path / ".x19"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (X19_HOME=<root>/profiles/<name>),
        ...
        assert get_default_x19_root() == docker_root

    @pytest.mark.windows_only
    def test_no_x19_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\x19, not ~/.x19."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("X19_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_x19_root() == local_appdata / "x19"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; X19_HOME / home changes invalidate.

        get_default_x19_root() resolves X19_HOME against the native
        ...
        (native home, X19_HOME) compared for free each call.
        """
        # X19_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_x19_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            x19_constants, "_default_x19_root_memo", None, raising=False
        )

        first = get_default_x19_root()
        ...
        # X19_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("X19_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_x19_root() == docker_root
        assert resolve_calls["n"] > before, (
            "X19_HOME change must force a fresh resolution"
        )
...
class TestGetX19Home:
    """Tests for get_x19_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When X19_HOME is unset on Windows, use %LOCALAPPDATA%\\x19."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("X19_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_x19_home() == local_appdata / "x19"

    @pytest.mark.linux_only
    def test_resolution_order_is_override_then_env_then_default(self, tmp_path, monkeypatch):
        """The documented chain: context override, then X19_HOME, then ~/.x19."""
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setattr(x19_constants, "_profile_fallback_warned", True)

        env_home = tmp_path / "from-env"
        override_home = tmp_path / "from-override"
        monkeypatch.setenv("X19_HOME", str(env_home))

        # X19_HOME beats the platform default.
        assert get_x19_home() == env_home

        # A context-local override beats X19_HOME, and is undone on reset.
        token = set_x19_home_override(str(override_home))
        try:
            assert get_x19_home() == override_home
            # The process-scoped view deliberately ignores the override.
            assert get_process_x19_home() == env_home
        finally:
            reset_x19_home_override(token)
        assert get_x19_home() == env_home

        # With neither, the canonical platform default is used.
        monkeypatch.delenv("X19_HOME", raising=False)
        assert get_x19_home() == tmp_path / ".x19"

    def test_home_resolution_reads_only_the_x19_home_env_var(self):
        """No legacy or alias environment variable participates in resolution.

        Legacy compatibility must not leak into the normal configuration path,
        so the resolver is pinned to exactly one variable. This also catches the
        collapse a brand rename caused here: `A or A` read the same variable
        twice, silently deleting a fallback step the docstring still promised.
        """
        import ast

        src = Path(x19_constants.__file__).read_text(encoding="utf-8")
        wanted = {"get_x19_home", "get_process_x19_home"}
        reads: dict = {}
        ...
        for node in ast.walk(ast.parse(src)):
            ...
            for sub in ast.walk(node):
                ...
                if (
                    ...
                    target, key = sub.func.value, sub.args[0].value
                elif isinstance(sub, ast.Subscript) and isinstance(sub.slice, ast.Constant):
                    target, key = sub.value, sub.slice.value
                else:
                    continue
                if isinstance(target, ast.Attribute) and target.attr == "environ" and isinstance(key, str):
                    keys.append(key)
            reads[node.name] = keys

        assert set(reads) == wanted, f"resolver functions moved or renamed: {sorted(reads)}"
        for name, keys in reads.items():
            assert set(keys) == {"X19_HOME"}, f"{name} reads {sorted(set(keys))}; only X19_HOME is supported"
            # Counted, not de-duplicated: the same variable read twice in one
            # expression is how the fallback step went missing.
            assert len(keys) == 1, f"{name} reads the environment {len(keys)} times: {keys}"



class TestGetProcessX19Home:
    """Tests for get_process_x19_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_x19_home() == home




class TestX19ManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_x19_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("X19_HOME", str(home))
        monkeypatch.setattr(x19_constants, "node_tool_runnable", lambda path: True)

        assert find_x19_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(x19_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(x19_constants, "heal_x19_managed_node", lambda: False)
        monkeypatch.setattr(
            x19_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert x19_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken X19-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(x19_constants, "heal_x19_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = x19_constants._X19_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = x19_constants.find_x19_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert x19_constants.find_x19_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert x19_constants.find_x19_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(x19_constants, "_container_detected", None)
    ...
    def test_cgroup_v2_fallback_inspects_only_the_root_mount(self, tmp_path):
        ...
        from x19_constants import _root_mount_has_marker
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(x19_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from x19_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".x19"
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/x19 to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (x19 user) from traversing the install dir.
        """
        install_root = Path(x19_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/x19/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/x19-data/...).
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import x19_cli._subprocess_compat as subprocess_compat
...
class TestGetX19Dir:
    """Tests for ``get_x19_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_x19_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_x19_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert x19_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert x19_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert x19_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert x19_constants.windows_path_to_wsl("/home/alex") is None
        assert x19_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(x19_constants, "is_wsl", lambda: True)
        assert x19_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert x19_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert x19_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(x19_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(x19_constants.sys, "platform", "darwin")
        monkeypatch.setenv("X19_HOME", str(tmp_path))
        assert x19_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert x19_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("X19_HOME", str(home))
        monkeypatch.setenv(
            "X19_NODE_TARGET_MAJOR",
            str(x19_constants._X19_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(x19_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            x19_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            x19_constants,
        ...
        monkeypatch.setattr(
            x19_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(x19_constants._X19_NODE_TARGET_MAJOR)
        ...
        result = x19_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(x19_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_x19_managed_node() is False
        assert x19_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_x19_managed_node() is False
        assert x19_constants._managed_node_heal_attempted is True
