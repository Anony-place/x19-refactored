...
guard is covered in ``tests/x19_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``x19_cli.model_switch_providers.prewarm_picker_cache_async``
...
import x19_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from x19_cli.model_switch,
