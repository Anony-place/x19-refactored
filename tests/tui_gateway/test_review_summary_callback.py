    ...
    with patch.dict(
        "sys.modules",
        {
            "x19_constants": MagicMock(
                get_x19_home=MagicMock(return_value="/tmp/x19_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'x19-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'x19-release' patched"
    }
