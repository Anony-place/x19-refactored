...
X19 exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_x19_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    X19_HOME (cache/terminal), NOT tmpfs /tmp."""
    import x19_constants  # noqa: F401 — resolves X19_HOME per call
    ...
    assert result == str(tmp_path / ".x19" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "x19_bg_dead1.log").exists()
