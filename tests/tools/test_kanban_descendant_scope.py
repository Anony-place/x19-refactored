...
def _worker_board(tmp_path, monkeypatch):
    ...
    for key, value in {
        "X19_KANBAN_DB": str(db), "X19_KANBAN_BOARD": "default",
        "X19_KANBAN_TASK": own, "X19_KANBAN_RUN_ID": str(task.current_run_id),
        "X19_KANBAN_CLAIM_LOCK": task.claim_lock, "HOME": str(tmp_path),
    }.items():
...
def test_terminal_descendants_cannot_mutate_even_after_task_is_removed(tmp_path, monkeypatch):
    ...
    script.write_text(
        ...
        " 'db': os.getenv('X19_KANBAN_DB'), 'board': os.getenv('X19_KANBAN_BOARD')}\n"
        ...
        "os.environ.pop('X19_KANBAN_TASK', None)\n"
        f"p = subprocess.run([sys.executable, '-m', 'x19_cli.main', 'kanban', 'complete', foreign, '--result', 'must refuse'], cwd={str(ROOT)!r}, capture_output=True, text=True, stdin=subprocess.DEVNULL, timeout=45)\n"
    ...
    child_envs = [
        ...
        _build_safe_env({"X19_HOME": os.environ["X19_HOME"]}),
    ]
...
def test_worker_cli_cannot_use_foreign_task_to_drop_run_scope(tmp_path, monkeypatch):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "x19_cli.main", "kanban", "complete", foreign, "--result", "no"],
    ...
    for arguments in (["attach", foreign, str(attachment)], ["unblock", foreign]):
        proc = subprocess.run([sys.executable, "-m", "x19_cli.main", "kanban", *arguments],
                              cwd=ROOT, env=dict(os.environ), stdin=subprocess.DEVNULL,
                              capture_output=True, text=True, timeout=45)
...
def test_child_shell_can_write_a_kanban_board_outside_its_lineage_root(tmp_path, monkeypatch):
    ...
    DB its shell touches: a repro run against a scratch X19_HOME got a silently read-only board
    ...
    monkeypatch.setenv("X19_HOME", str(lineage_home))
    monkeypatch.delenv("X19_DELEGATED_CHILD_CONTEXT", raising=False)
    for key in ("X19_KANBAN_DB", "X19_KANBAN_BOARD", "X19_KANBAN_TASK", "X19_KANBAN_HOME"):
    ...
    script.write_text(
        ...
        "from x19_cli import kanban_db as kb\n"
        "from x19_cli.kanban_db_connect import connect\n"
        "out = {'marker': os.environ.get('X19_DELEGATED_CHILD_CONTEXT')}\n"
        "try:\n"
        "    conn = connect(kb.kanban_db_path()); kb.create_task(conn, title='lineage'); out['lineage'] = 'WROTE'\n"
        "except PermissionError as exc:\n"
        "    out['lineage'] = 'fenced: ' + str(exc)\n"
        f"os.environ['X19_HOME'] = {str(scratch_home)!r}\n"
        "import x19_constants; x19_constants._default_x19_root_memo = None\n"
