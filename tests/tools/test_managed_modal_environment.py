...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "x19_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "x19_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    x19_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["x19_cli"] = x19_cli
    sys.modules["x19_cli.config"] = types.SimpleNamespace(
        get_x19_home=lambda: Path(tempfile.gettempdir()) / "x19-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.x19/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
