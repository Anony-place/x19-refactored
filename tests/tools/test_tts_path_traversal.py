...
def test_output_path_rejects_x19_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    x19_home = tmp_path / "x19-home"
    x19_home.mkdir()
    monkeypatch.setattr(file_safety, "_x19_home_path", lambda: x19_home)
    monkeypatch.setattr(file_safety, "_x19_root_path", lambda: x19_home)

    target = x19_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
