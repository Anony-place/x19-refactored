...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        x19_home.mkdir()
        (x19_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"X19_HOME": str(x19_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(x19_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.x19/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.x19/skills"


    def test_custom_container_base(self, tmp_path):
        x19_home = tmp_path / ".x19"
        (x19_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"X19_HOME": str(x19_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.x19")

        assert mounts[0]["container_path"] == "/home/user/.x19/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.x19/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.x19/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.x19/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.x19/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape X19_HOME."""
        monkeypatch.setenv("X19_HOME", str(tmp_path / ".x19"))
        (tmp_path / ".x19").mkdir()

        # Create a sensitive file one level above x19_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside x19_home
    ...
    def test_nested_subdir_inside_x19_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of X19_HOME must be allowed."""
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
        subdir = x19_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside X19_HOME pointing outside must be rejected."""
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
        monkeypatch.setenv("X19_HOME", str(x19_home))

        # Create a sensitive file outside x19_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside x19_home pointing outside
        symlink = x19_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes X19_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, x19_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape X19_HOME."""
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
        monkeypatch.setenv("X19_HOME", str(x19_home))
        ...
        sensitive.write_text("{}")
        self._write_config(x19_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(x19_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside X19_HOME via config must still mount."""
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
        monkeypatch.setenv("X19_HOME", str(x19_home))

        (x19_home / "oauth.json").write_text("{}")
        self._write_config(x19_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
        (x19_home / "cache" / "documents").mkdir(parents=True)
        (x19_home / "cache" / "audio").mkdir(parents=True)
        (x19_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("X19_HOME", str(x19_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.x19/cache/documents" in paths
        assert "/root/.x19/cache/audio" in paths
        assert "/root/.x19/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_x19_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
        # Use legacy dir name with content — get_x19_dir prefers
        # populated old over new.
        legacy_doc = x19_home / "document_cache"
        legacy_img = x19_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(x19_home / "document_cache") in host_paths
        assert str(x19_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.x19/cache/documents" in container_paths
        assert "/root/.x19/cache/images" in container_paths
    ...
    def test_empty_x19_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.x19/attachments" in container_paths
        assert "/root/.x19/images" in container_paths
        assert "/root/.x19/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``X19_HOME/images``, not
        ...
        """
        x19_home = tmp_path / ".x19"
        (x19_home / "images").mkdir(parents=True)
        monkeypatch.setenv("X19_HOME", str(x19_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.x19/images" in by_container
        assert by_container["/root/.x19/images"] == str(x19_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        x19_home = tmp_path / ".x19"
        (x19_home / "images").mkdir(parents=True)
        upload = x19_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("X19_HOME", str(x19_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.x19/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.x19/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(x19_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.x19/cache/images/x.png"
        assert (x19_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    X19 cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        x19_home = tmp_path / ".x19"
        (x19_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("X19_HOME", str(x19_home))
        return str(x19_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_x19(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.x19/attachments/drop.zip"

    def test_ssh_maps_to_tilde_x19(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.x19/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        x19_home = tmp_path / ".x19"
        doc_dir = x19_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — X19_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping X19_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"X19_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.x19/google_token.json" in paths
        assert "/root/.x19/.env" not in paths
