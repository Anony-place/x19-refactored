...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects X19_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("X19_INTERACTIVE", "X19_GATEWAY_SESSION", "X19_EXEC_ASK", "X19_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["X19_INTERACTIVE"] = "1"
        session_key = os.getenv("X19_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No X19_INTERACTIVE or X19_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
