...
Older images made /opt/x19/ui-tui and /opt/x19/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV X19_TUI_DIR=/opt/x19/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R x19:x19 /opt/x19/ui-tui" not in text
    assert "chown -R x19:x19 /opt/x19/node_modules" not in text
