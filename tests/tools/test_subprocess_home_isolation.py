...
X19 state stays profile-scoped through X19_HOME. Host subprocesses should
...
See: https://github.com/Anony-place/x19-refactored/issues/25114
See: https://github.com/Anony-place/x19-refactored/issues/36144
See: https://github.com/Anony-place/x19-refactored/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for x19_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(x19_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("X19_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        x19_home = real_home / ".x19" / "profiles" / "coder"
        profile_home = x19_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("X19_HOME", str(x19_home))
        from x19_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("X19_REAL_HOME", str(real_home))

        from x19_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("X19_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("X19_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["X19_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(x19_home / "home")
        assert result["X19_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_x19_home_unset(self, monkeypatch):
