...
TTS provider tools must resolve API keys from ``~/.x19/.env`` (via
``x19_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``x19_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.x19/.env`` carries the
    key, the provider must find it. These per-provider tests model that
    dotenv-backed lookup by mocking ``x19_cli.config.get_env_value`` directly;
    the separate regression-guard tests cover the lower-level
    ``x19_cli.config.load_env`` integration. Before the fix, ``os.getenv``
    ...
    def test_elevenlabs_reads_dotenv_key(self, tmp_path):
        ...
        with patch("x19_cli.config.get_env_value", return_value="el-dotenv-key"), \
    ...
    def test_xai_reads_dotenv_key(self, tmp_path):
        ...
        canonical ``x19_cli.config.get_env_value`` — the dotenv contract from #17140.
        """
        from tools import tts_tool
        ...
        with patch("x19_cli.config.get_env_value", return_value="xai-dotenv-key"), \
    ...
    def test_gemini_reads_dotenv_key(self, tmp_path):
        ...
        with patch("x19_cli.config.get_env_value", side_effect=fake_get_env_value), \
             patch("requests.post", side_effect=fake_post):
...
class TestRegressionGuard:
    ...
    ``x19_cli.config.load_env`` to simulate ``~/.x19/.env`` carrying the
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.x19/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
