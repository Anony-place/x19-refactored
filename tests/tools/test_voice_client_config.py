...
Real config files in a temp X19_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated X19_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".x19"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "X19_LOCAL_STT_LANGUAGE",
    ):
