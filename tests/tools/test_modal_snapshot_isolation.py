...
def _restore_tool_modules():
    original_x19_home = os.environ.get("X19_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_x19_home is None:
            os.environ.pop("X19_HOME", None)
        else:
            os.environ["X19_HOME"] = original_x19_home
        _reset_modules(("tools", "x19_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    x19_cli = types.ModuleType("x19_cli")
    x19_cli.__path__ = [str(REPO_ROOT / "x19_cli")]  # type: ignore[attr-defined]
    sys.modules["x19_cli"] = x19_cli
    x19_home = tmp_path / "x19-home"
    os.environ["X19_HOME"] = str(x19_home)
    sys.modules["x19_cli.config"] = types.SimpleNamespace(
        get_x19_home=lambda: x19_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="x19", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": x19_home / "modal_snapshots.json",
