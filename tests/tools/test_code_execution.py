...
UDS socket lifecycle, x19_tools generation, timeout enforcement,
...
class TestX19ToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_x19_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"x19_rpc\")", src)
        self.assertNotIn('os.environ.get("X19_RPC_DIR", "/tmp/x19_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_x19_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "x19_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "x19_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/x19_exec_", mkdir_cmd)
        self.assertIn("X19_RPC_DIR=/data/data/com.termux/files/usr/tmp/x19_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/x19_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/x19_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """X19_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"X19_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import x19_constants; print(x19_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("x19_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from x19_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from x19_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_x19_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("X19_RPC_SOCKET", child_env)
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching X19_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated x19_tools module reads X19_RPC_TOKEN and sends it."""
        src = generate_x19_tools_module(["terminal"], transport="uds")
        self.assertIn("X19_RPC_TOKEN", src)
