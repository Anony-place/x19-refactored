...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("x19_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="x19-spill-test-")
    ...
    def test_default_directory_uses_x19_home(self):
        """When no directory override, spill under X19_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="x19-home-")
        try:
            with patch.dict(os.environ, {"X19_HOME": test_home}):
                # Also patch get_x19_home to the env var to mirror production.
            ...
            # ~/.x19/hook_outputs depending on get_x19_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.x19/hook_outputs/sess")),
            ]
