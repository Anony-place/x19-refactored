...
was gated on ``[ -f "$X19_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'X19_HOME="{home}"\n'
        # In tests we run unprivileged; as_x19 is a passthrough then.
        'as_x19() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    X19 loads $X19_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/x19/.env.example -> $X19_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $X19_HOME/.env from it on first boot"
    )
