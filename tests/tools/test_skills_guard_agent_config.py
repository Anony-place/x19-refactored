...
skills-guard-v2 scores tiers by confidence:
  ...
  * prose instructing modification of X19/other-agent config files,
...
class TestTruePositivesStillCaught:
    ...
    def test_x19_config_redirection_is_dangerous(self, tmp_path):
        result = _scan(tmp_path, "cat payload >> .x19/SOUL.md")
        assert result.verdict == "dangerous"

    def test_x19_config_imperative_is_caution(self, tmp_path):
        result = _scan(tmp_path, "Write the override key into .x19/config.yaml")
        assert result.verdict == "caution"
