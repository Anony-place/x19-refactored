...
A package-manager install (NixOS, and anything else shipping X19 from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.x19/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("x19_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets X19_MANAGED *and* a writable target.

    Dockerfile sets X19_LAZY_INSTALL_TARGET and the NixOS container module
    passes X19_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("x19_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("x19_cli.config.get_managed_system", _raise)
