...
def worker_home(tmp_path, monkeypatch):
    ...
    for var in ("X19_KANBAN_DB", "X19_KANBAN_WORKSPACES_ROOT", "X19_KANBAN_HOME", "X19_KANBAN_BOARD"):
...
def test_seed_then_inject_new_comment(worker_home, monkeypatch):
    ...
    monkeypatch.setenv("X19_PROFILE", "worker-bot")
    agent = FakeAgent()
...
def test_delegated_child_in_worker_process_neither_receives_nor_consumes_notes(worker_home, monkeypatch):
    """A delegate_task child inherits the worker's ``X19_KANBAN_TASK``; operator notes
