...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``X19_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("x19_cli.profiles.profiles_to_serve", return_value=served), \
            patch("x19_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
            patch("x19_cli.profiles.profile_exists", return_value=True):
