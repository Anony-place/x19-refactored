...
X19_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env X19_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``x19 config get terminal.backend`` still says local.
    """
    x19_home = tmp_path / ".x19"
    x19_home.mkdir()
    (x19_home / "config.yaml").write_text(
    ...
    (x19_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
