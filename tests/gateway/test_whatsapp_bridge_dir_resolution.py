...
(/opt/x19/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable X19_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_x19_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable X19_HOME."""
    ...
    monkeypatch.setattr(
        "x19_constants.get_x19_home", lambda: x19_home
    )
    ...
    expected = x19_home / "scripts" / "whatsapp-bridge"
