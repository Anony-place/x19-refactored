...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/Anony-place/x19-refactored/issues/23994:
    reply_to_mode 'off' was ignored when sending via X19-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/Anony-place/x19-refactored/issues/87051:
    ...
    response in a different chat lane than the X19 topic the session runs
    in. The no-anchor branch must prefer the X19 topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_x19_topic_thread_id(self):
