...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _x19_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._x19_home", tmp_path), \
        ...
        with patch("gateway.run._x19_home", tmp_path):
            ...
            # project_root will be the real x19 dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_resolve_x19_bin_module_argv(self):
        """_resolve_x19_bin uses the running interpreter's module argv when x19_cli is
        importable, even when PATH also offers a ``x19`` binary (#111569: a PATH-first
        lookup would re-exec an attacker-planted executable on /update and /restart)."""
        import sys
        from gateway.run import _resolve_x19_bin

        fake_spec = MagicMock()
        with patch("shutil.which", return_value="/tmp/attacker/x19"), \
             patch("importlib.util.find_spec", return_value=fake_spec):
            result = _resolve_x19_bin()

        assert result == [sys.executable, "-m", "x19_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_x19_bin_falls_back_to_path_then_none(self):
        """Without an importable x19_cli the argv degrades to PATH, then to None — never a
        bare ``x19`` string that a hostile PATH entry could shadow."""
        from gateway.run import _resolve_x19_bin

        with patch("shutil.which", return_value="/usr/local/bin/x19"), \
             patch("importlib.util.find_spec", return_value=None):
            assert _resolve_x19_bin() == ["/usr/local/bin/x19"]

        with patch("shutil.which", return_value=None), \
             patch("importlib.util.find_spec", side_effect=ImportError):
            assert _resolve_x19_bin() is None
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._x19_home", x19_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/x19" if x == "x19" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = x19_home / ".update_pending.json"
        ...
        assert not (x19_home / ".update_exit_code").exists()
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting X19 update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("X19_MANAGED", "")
        ...
        # Later steps may legitimately return success ("Starting X19
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (x19_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = x19_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (x19_home / ".update_output.txt").write_text("done")
        (x19_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (x19_home / ".update_pending.json").write_text(json.dumps(pending))
        (x19_home / ".update_output.txt").write_text(
    ...
    async def test_cleans_up_on_error(self, tmp_path):
        ...
        pending_path = x19_home / ".update_pending.json"
        output_path = x19_home / ".update_output.txt"
        exit_code_path = x19_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (x19_home / ".update_pending.claimed.json").exists()
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        assert "X19 update finished" in sent_text
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
    ...
    async def test_failed_update_notice_says_still_running_and_trims_log(self, tmp_path):
        ...
        x19_home = tmp_path / "x19"
        x19_home.mkdir()
        (x19_home / ".update_pending.json").write_text(
            json.dumps({"platform": "discord", "chat_id": "111", "user_id": "222"}))
        (x19_home / ".update_output.txt").write_text("x" * 3000 + "\nERROR: pip failed\n")
        (x19_home / ".update_exit_code").write_text("1")
        mock_adapter = AsyncMock()
        runner.adapters = {Platform.DISCORD: mock_adapter}
        ...
        assert "x19 update" in sent_text and "/update" in sent_text
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        (x19_home / ".update_pending.json").write_text(json.dumps({
            "platform": "telegram",
            "chat_id": "67890",
            "user_id": "12345",
        }))
        (x19_home / ".update_output.txt").write_bytes(
        ...
        assert "X19 update finished" in sent
        assert not (x19_home / ".update_pending.json").exists()
