...
closure the PR changed, against a real temp ``X19_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "x19_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_x19_home() / config.yaml`` — point it here.
    monkeypatch.setattr("x19_constants.get_x19_home", lambda: x19_home)
    monkeypatch.setattr("x19_cli.config.get_x19_home", lambda: x19_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_x19_home", default_home)
    _stub_picker_dependencies(monkeypatch)
