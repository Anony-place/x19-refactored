"""Tests for x19_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every X19 entry point
(x19, x19-agent, x19-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every X19 entry point imports x19_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported x19_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("x19_bootstrap", None)
    import x19_bootstrap  # noqa: WPS433
    return x19_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the X19 CLI
...
class TestEntryPointsImportBootstrap:
    """Every X19 entry point must import x19_bootstrap as its
    ...
    # Entry points that invoke X19 as a process.  Each one must
    # import x19_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "x19_cli/main.py",   # x19 CLI (console_script)
        "run_agent.py",          # x19 (console_script)
        "acp_adapter/entry.py",  # x19-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import x19_bootstrap' and that
        ...
        half-finished ``x19 update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``x19_bootstrap`` as a top-level module) leaves x19
        ...
        """
        # Resolve relative to the x19 repo root.  Tests live
        # at tests/test_x19_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # x19 start even when x19_bootstrap hasn't been
        ...
        assert first_import_name == "x19_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'x19_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import x19_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing X19's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("X19_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["X19_PYTHON_SRC_ROOT"] = env
            elif "X19_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["X19_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/x19")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("X19_PYTHON_SRC_ROOT", None)
            else:
                os.environ["X19_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/x19", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # X19 root.  The guard must relocate X19 to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/x19"])
        assert result[0] == "/opt/x19"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the X19 root.
        assert result.index("/opt/x19") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["X19_PYTHON_SRC_ROOT"] = "/env/x19"
            hb.harden_import_path()
            assert sys.path[0] == "/env/x19"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("X19_PYTHON_SRC_ROOT", None)
