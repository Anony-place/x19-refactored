...
def test_gateway_health_event_maps_to_span_with_attrs():
    ...
    assert spans[0].name == "x19.gateway_health"
    attrs = dict(spans[0].attributes or {})
    assert attrs["x19.old_state"] == "starting"
    assert attrs["x19.new_state"] == "running"
    assert attrs["x19.active_agents"] == 2
...
def test_trace_resource_includes_stable_hashed_instance():
    ...
    assert attrs["service.name"] == "x19-gateway"
...
def test_streamer_receives_events_and_respects_filter(monkeypatch):
    ...
    spans = mem.get_finished_spans()
    assert [s.name for s in spans] == ["x19.gateway_health"]
