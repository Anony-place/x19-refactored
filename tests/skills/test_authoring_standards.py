...
def test_required_frontmatter_fields(p):
    ...
    x19 = (fm.get("metadata") or {}).get("x19") or {}
    if not (x19.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.x19.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    x19 = (fm.get("metadata") or {}).get("x19") or {}
    dangling = [
        rs for rs in (x19.get("related_skills") or []) if rs not in _all_names()
    ]
