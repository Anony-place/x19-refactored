...
def test_root_session_no_compression(db):
    ...
    assert prov["currentX19SessionId"] == "root1"
    assert prov["rootX19SessionId"] == "root1"
    assert prov["parentX19SessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_x19_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentX19SessionId"] == "old"
    assert prov["rootX19SessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousX19SessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootX19SessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"x19"}
    assert "sessionProvenance" in meta["x19"]
    assert meta["x19"]["sessionProvenance"]["currentX19SessionId"] == "root1"
