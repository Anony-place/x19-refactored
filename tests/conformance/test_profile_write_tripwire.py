...
--------------------
When a non-default profile is active (``X19_HOME`` points at
...
class ProfileTripwire:
    ...
    Attributes:
        root:     the default profile's home (``<tmp>/x19``).
...
def profile_tripwire(tmp_path, monkeypatch):
    ...
    1. A temp x19 root acting as the *default* profile, seeded with the
       ...
       detectable.
    2. ``X19_HOME`` (env var AND the context-local override) pointed at
       the testprof home — the exact activation shape ``--profile`` uses.
    3. ``x19_state.DEFAULT_DB_PATH`` restored to its import-time snapshot.
       ...
       makes resolution flow through ``get_x19_home()`` — the production
