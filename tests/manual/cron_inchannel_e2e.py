...
Uses a throwaway X19_HOME so it never touches ~/.x19. No real names.
"""
...
def _fresh_home():
    """Point X19_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_x19_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["X19_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER X19_HOME is set.
...
# a different get_x19_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"X19_HOME (throwaway): {HOME}")
