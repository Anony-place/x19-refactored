...
@pytest.fixture
def x19_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.x19/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".x19"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(x19_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = x19_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different X19_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".x19"
