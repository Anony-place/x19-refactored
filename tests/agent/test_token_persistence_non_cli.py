...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    x19_state = ModuleType("x19_state")
    x19_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "x19_state", x19_state)
    x19_state_registry = ModuleType("x19_state_registry")
    x19_state_registry.acquire = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "x19_state_registry", x19_state_registry)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "X19", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "X19"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "X19", "detail": "full"}),
        ),
    )
