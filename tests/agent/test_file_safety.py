...
class TestCacheFileReadBlocking:
    """Internal X19 cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        x19_home = tmp_path / ".x19"
        cache = x19_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._x19_home_path", return_value=x19_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal X19 cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        x19_home = tmp_path / ".x19"
        hub = x19_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_x19_home(self, tmp_path):
        """The env basename guard does not depend on X19_HOME resolution."""
        x19_home = tmp_path / ".x19"
        x19_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        x19_home = tmp_path / ".x19"
        cache = x19_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
