...
path via ``x19 chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp X19_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point X19_HOME at a temp dir and turn moa.save_traces on."""
    x19_home = tmp_path / ".x19"
    x19_home.mkdir()
    monkeypatch.setenv("X19_HOME", str(x19_home))

    # save_moa_turn reads config via x19_cli.config.load_config; stub it to
