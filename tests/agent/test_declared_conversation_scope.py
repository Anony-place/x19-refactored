...
conversation-affinity hint X19 sends — ``prompt_cache_key`` on both
...
bucket it warmed. X19 cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-X19-Session-Key`` /
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-X19-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``X19_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("X19_SESSION_SOURCE", "override-src")
