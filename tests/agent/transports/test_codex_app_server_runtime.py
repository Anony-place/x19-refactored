...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. X19 passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("X19_HOME", "/users/alice/.x19/profiles/backend-worker")
        monkeypatch.setenv("X19_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "X19_KANBAN_DB",
            "/users/alice/.x19/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.x19/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through x19_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 X19 secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "X19_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
