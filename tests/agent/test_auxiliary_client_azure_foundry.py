...
that delegates to :func:`x19_cli.runtime_provider._resolve_azure_foundry_runtime`
...
class TestResolveProviderClientAzureFoundry:
    ...
    def test_warns_and_returns_none_on_failure(
        ...
        clear warning pointing at ``x19 doctor``."""
        import logging
        ...
        assert resolved is None
        assert any(
            "azure-foundry" in rec.message and "x19 doctor" in rec.message
