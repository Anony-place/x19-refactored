...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated X19_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="x19_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".x19"))
    prev_home = os.environ.get("X19_HOME")
    os.environ["X19_HOME"] = os.path.join(test_home, ".x19")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("x19_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into x19_cli.*/tools.* and their monkeypatches would otherwise
