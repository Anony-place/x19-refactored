"""Tests for X19_HOME credential-file read blocking in file_safety.

Regression for https://github.com/Anony-place/x19-refactored/issues/17656 —
``read_file`` was previously only sandboxed against ``X19_HOME`` itself,
...
for the credential stores while leaving arbitrary ``X19_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_x19_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "x19_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_x19_home_path", lambda: home)
    return home
...
def test_arbitrary_x19_home_file_not_blocked(fake_home):
    """Non-credential files inside X19_HOME stay readable."""
...
def test_identically_named_x19_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """X19-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside X19_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside X19_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside X19_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, X19_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_x19_home_path", lambda: profile)
