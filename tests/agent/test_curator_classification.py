...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"x19-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "x19-dev", "declared": True},
        },
    )
...
# `x19 curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `x19 curator pin <umbrella>` in the rename block so
