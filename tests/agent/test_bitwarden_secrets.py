...
binary download are exercised manually by `x19 secrets bitwarden
...
def x19_home(tmp_path, monkeypatch):
    """Point X19 at an isolated home directory."""
    home = tmp_path / ".x19"
    home.mkdir()
    monkeypatch.setenv("X19_HOME", str(home))
    # Some modules cache get_x19_home; clear if needed.
    import x19_constants
    if hasattr(x19_constants, "_X19_HOME_CACHE"):
        x19_constants._X19_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(x19_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from x19_cli.env_loader import _apply_external_secret_sources
