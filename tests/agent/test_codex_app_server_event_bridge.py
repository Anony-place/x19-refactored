"""Regression tests for the codex_app_server → X19 UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_x19_tools_mcp_server_emits_bare_tool_name(self):
        """The x19-tools MCP server wraps X19' own tools for codex;
        ...
        mcp.x19-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "x19-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "x19-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "x19 agent docs"
        assert calls[0].args[3] == {"query": "x19 agent docs"}
