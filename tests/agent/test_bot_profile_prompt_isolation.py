...
thread that did not bind the X19_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_x19_home()``,
so an unbound build thread fell back to ``~/.x19`` (default) and leaked
default's full skills index + "Active X19 profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient X19_HOME, on a
    ...
    def build():
        # No set_x19_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_x19_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    x19 ROOT (get_default_x19_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: X19_HOME env points at the profile dir itself.
    monkeypatch.setenv("X19_HOME", str(bot_home))
