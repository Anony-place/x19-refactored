...
def backup_env(monkeypatch, tmp_path):
    """Isolate X19_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".x19"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_x19_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd x19_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under X19_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level X19_DIR picks up the tmp HOME."""
    import x19_constants
    importlib.reload(x19_constants)
