...
# ---------------------------------------------------------------------------
# Helpers — set up a fake X19 root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, x19_home: Path):
    """Point file_safety._x19_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_x19, monkeypatch):
        _set_active_home(monkeypatch, fake_x19["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_x19, monkeypatch):
        """If X19_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_x19_home_path", _boom)
