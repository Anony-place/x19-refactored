...
OPENVIKING_* / HINDSIGHT_BANK_ID / X19_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "X19_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("x19", "", "platform")
    ...
    provider.initialize("s1", x19_home=str(secondary_profile))
    assert provider._client.project == "x19-b"
    ...
    assert (hcfg["banks"]["x19"]["bankId"], hcfg["mode"]) == ("x19", "cloud")
