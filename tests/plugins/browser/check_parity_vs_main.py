...
Run from the PR worktree:
    ...
    cd ~/.x19/x19/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.x19/x19``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.x19/x19
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a x19 checkout"
)
...
# Isolated X19_HOME for the config write.
