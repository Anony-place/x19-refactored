...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate X19_HOME for each test.

    The global hermetic fixture already redirects X19_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    X19_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "x19_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "x19_plugins" not in sys.modules:
        ns = types.ModuleType("x19_plugins")
        ns.__path__ = []
        sys.modules["x19_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "x19_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["x19_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_x19_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_x19_home(self, _isolate_env):
        dg = _load_lib()
...
class TestProfileUserTreesNeverCleaned:
    ...
    def test_session_end_hook_leaves_workspace_files_alone(self, _isolate_env):
        ...
        the X19_HOME root is the control: it is still tracked and removed."""
...
class TestBundledDiscovery:
    def _write_enabled_config(self, x19_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from x19_cli import plugins as pmod
