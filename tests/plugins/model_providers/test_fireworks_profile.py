...
class TestFireworksHeaders:
    def test_attribution_matches_canonical_x19_values(self, fireworks_profile):
        """Fireworks requests carry the same attribution identity X19 sends
        everywhere else.
    ...
    def test_user_agent_identifies_x19(self, fireworks_profile):
        # Prefix, not the full string — the version moves every release.
        assert fireworks_profile.default_headers["User-Agent"].startswith("X19Agent/")
