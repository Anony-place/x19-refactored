...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "x19_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "x19_cli.web_server_chat", stub)
    monkeypatch.setattr(x19_cli, "web_server_chat", stub, raising=False)
...
# tests/x19_cli/test_kanban_db.py. The OUTER fallback here is not, which
