...
X19_HOME override (``x19_constants.set_x19_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``X19_HOME``) and profile B with different config/.env values."""
    root = tmp_path / ".x19"
...
def test_router_efforts_cache_and_base_url_follow_the_active_profile(homes, monkeypatch):
    ...
    profile's .env."""
    import x19_cli.urllib_security as urllib_security
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from x19_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", x19_home=str(home))
