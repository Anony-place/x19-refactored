...
from x19_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(x19_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <x19_home>/plugins/<name> and opt it in."""
    plugin_dir = x19_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("X19_HOME", str(tmp_path / "x19_empty"))
