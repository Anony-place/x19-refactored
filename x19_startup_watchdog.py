...
timeout. Config is env-only (``X19_STARTUP_WATCHDOG=0``,
``X19_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "X19_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "X19_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (x19_cli.main / x19_cli.gateway /
...
def _process_x19_home() -> Path:
    """X19_HOME for diagnostic files — stdlib-only replica of the x19_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<X19_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_x19_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``X19_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``X19_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
