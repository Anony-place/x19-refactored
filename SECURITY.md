...
This document describes X19's trust model, names the one
...
- Environment details (`x19 --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted X19 access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in X19's documentation
...
X19 supports two OS-level isolation postures. They address
...
X19 supports this in two ways:

- **X19's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, X19's in-process heuristics
...
X19 filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in X19
itself. Bugs in X19's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   X19 does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, X19's own documentation, or reasonable
  operator expectations would predict — including cases where
  X19 has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  x19-home security, and similar. Reports against those
...
- X19 includes supply-chain guards for MCP server
