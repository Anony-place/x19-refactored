"""disk-cleanup plugin — auto-cleanup of ephemeral X19 session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/x19-x/test_foo.py`
...
All operations are scoped to X19_HOME and /tmp/x19-*.
...
def _cmd_track(argv: List[str]) -> str:
    ...
    return f"Not tracked (already present, missing, or outside X19_HOME): {path_arg}"
...
def register(ctx) -> None:
    ...
    ctx.register_command("disk-cleanup", handler=_handle_slash,
                         description="Track and clean up ephemeral X19 session files.")
