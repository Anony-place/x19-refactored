...
Auto-tracks and cleans up ephemeral files created during X19
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$X19_HOME` and `/tmp/x19-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `X19_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `X19_HOME` or `/tmp/x19-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$X19_HOME/disk-cleanup/` is itself excluded
- `$X19_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
