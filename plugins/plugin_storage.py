"""Per-plugin persistent storage: ``<x19 home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<x19 home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `x19 plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<x19 home>/plugin-data/<name>/``; resolves ``get_x19_home()`` on
    every call so it follows the active profile — don't cache across profile switches."""
    from x19_constants import get_x19_home
    root = get_x19_home() / "plugin-data" / _validate_name(name)
