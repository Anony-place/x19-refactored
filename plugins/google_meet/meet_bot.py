...
Standalone subprocess spawned by ``process_manager.py``; configured via ``X19_MEET_*`` env,
status + transcript written under ``$X19_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``X19_MEET_URL=... X19_MEET_OUT_DIR=/tmp/x X19_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__x19MeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__x19MeetInstalled) return;
  window.__x19MeetInstalled = true;
  window.__x19MeetQueue = [];
  ...
  window.__x19MeetDrain = () => {
    const out = window.__x19MeetQueue.slice();
    window.__x19MeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__x19MeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'x19_meet_sink'}", str(pcm_path)]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in X19_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``X19_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("X19_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("X19_MEET_URL", "").strip(),
        ...
        realtime_api_key=env("X19_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("X19_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("X19_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("X19_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("X19_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__x19MeetDrain && window.__x19MeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — X19_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: X19_MEET_OUT_DIR is required\n")
        return 2
