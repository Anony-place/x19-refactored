...
Let the x19 agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where x19 runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `x19 meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `x19 meet` CLI |
...
| `cli.py` | `x19 meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$X19_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `x19 meet node {run,list,approve,remove,status,ping}` |
...
x19 meet install                                      # pip + Chromium
x19 meet setup                                        # preflight
x19 meet auth                                         # optional
x19 meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
x19 meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.x19/.env
x19 meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
x19 meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
x19 meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.x19/.env
x19 meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, x19 will **not** switch your system audio input automatically — the
...
x19 plugins enable google_meet
x19 meet node run --display-name my-mac --host 0.0.0.0 --port 18789
