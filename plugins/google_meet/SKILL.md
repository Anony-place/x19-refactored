...
x19 meet install                 # pip deps + Chromium (transcribe only)
x19 meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
x19 meet auth                    # optional; skips guest-lobby wait
x19 meet setup                   # preflight checks
```

`x19 meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or X19_MEET_REALTIME_KEY in ~/.x19/.env
```
...
pip install playwright websockets && python -m playwright install chromium
x19 plugins enable google_meet
x19 meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
x19 meet node approve my-mac ws://<mac-ip>:18789 <token>
x19 meet node ping my-mac                   # confirm reachable
```

Run `x19 meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "An X19 bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `x19 meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `X19_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `x19_meet_src`). |
...
Local:
```
$X19_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$X19_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `x19 meet node approve`.
