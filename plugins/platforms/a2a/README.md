# A2A — Agent-to-Agent protocol for X19
...
peer (another X19, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
x19 gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, X19 serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.x19/a2a_audit.jsonl`.
- Conversations persist to `~/.x19/a2a_conversations/` — they survive context
