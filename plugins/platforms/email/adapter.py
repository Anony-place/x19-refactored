"""Email platform adapter for the X19 gateway: users talk to X19 by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _x19_version = "0"
        imap.xatom("ID", f'("name" "x19" "version" "{_x19_version}" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `x19 gateway setup` (env) or platforms.email in config.yaml."
    ...
    def _new_reply(self, to_addr: str, body: str, reply_to_msg_id: Optional[str] = None, *,
        ...
        msg_id = f"<x19-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
