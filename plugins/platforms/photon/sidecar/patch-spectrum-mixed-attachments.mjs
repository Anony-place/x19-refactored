...
// `message.content.text` before X19 can see it. We rewrite the two inbound
...
const MARKER = "X19 patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // X19 to patch; keep the legacy v8 path below for older pinned installs.
    if (
