...
//
// Inbound  (gRPC -> X19): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (X19 -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-X19-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `x19 photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so X19 can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "x19-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply X19'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local X19 allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of X19, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-x19-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when X19 supplied them so a known-good
