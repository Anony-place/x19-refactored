...
Small Node helper that bridges X19 to Photon's Spectrum SDK
(`spectrum-ts`).  X19 is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-X19-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and X19 can receive inbound messages
...
The X19 plugin's `x19 photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. X19 is Python, so replies go through this sidecar
