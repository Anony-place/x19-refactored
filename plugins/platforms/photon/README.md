...
This plugin connects X19 to iMessage (and other Spectrum
...
TypeScript-only, X19 runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other X19 channel,
...
Run `x19 photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.x19/.env` (the same place every other
...
Management metadata lives in `~/.x19/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `x19 photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, X19
  ...
  `.pluginPayloadAttachment` images immediately after the URL; X19 coalesces
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage read
  after forwarding it to X19, so the sender sees `Read` without waiting for
  a model/tool turn. Inbound receipts for X19-sent messages are consumed as
  presence telemetry and never create an agent turn. Set
  `PHOTON_READ_RECEIPTS=false` to keep messages at `Delivered`.
- **Native polls are supported.** X19 posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `x19 send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<x19-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `x19 photon status`, a DM and a group roundtrip,
