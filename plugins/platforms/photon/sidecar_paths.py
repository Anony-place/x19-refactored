...
``$X19_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `x19 photon install-sidecar` on npm failure so check_requirements() can
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_x19_home() / "photon" / "sidecar"
    try:
