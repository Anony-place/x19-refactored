...
This plugin ships bundled with X19 but is **opt-in** — it only loads when
...
x19 tools  # → Langfuse Observability
...
Set these in `~/.x19/.env` (or via `x19 tools`):
...
x19 chat -q "hello"              # then check Langfuse for a "X19 turn" trace
```

Generation observations include the X19 system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `X19_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
X19_LANGFUSE_ENV=production       # environment tag
X19_LANGFUSE_RELEASE=v1.0.0       # release tag
X19_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
X19_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
X19_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
X19_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`X19_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
x19 plugins disable observability/langfuse
```
