...
- OpenViking server running and reachable from X19

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
X19 can identify older servers that expose the legacy status-only health
...
connection values into X19, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.x19/.env`; for a named profile use
...
OpenViking's server config is separate from X19:
...
X19-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, X19 lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
X19 sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
X19 also sends `User-Agent: openviking-memory-x19/<version>` on
OpenViking requests. This standard harness identifier contains the X19
...
does not ask for a peer ID. Without a configured peer, X19 sends neither
...
order remains environment, linked OpenViking config, then X19 YAML. To use
...
on the old implicit `x19` peer now use user memory for new writes. Without
...
Set `agent: x19` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `x19-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live X19
...
Use the same OpenViking profile and credentials as X19 for manual recovery.
...
X19 built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| X19 action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because X19
...
file URIs, such as
`viking://user/default/peers/x19/memories/preferences/mem_abc123.md` (any
