...
Behavioral settings live in `$X19_HOME/mem0.json` (set them via `x19 memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.x19/.env`.
...
| `user_id` | `x19-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh X19 session and call `mem0_search` — it connects to your server.
...
```bash
x19 memory setup mem0 --mode oss \
...
```bash
x19 memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$X19_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.x19/mem0_qdrant"}}
  }
}
```
...
```bash
x19 memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$X19_HOME/mem0.json`).
