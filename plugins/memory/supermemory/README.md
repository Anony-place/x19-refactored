...
- Hosted: API key from [app.supermemory.ai/integrations?connect=x19](http://app.supermemory.ai/integrations?connect=x19)
...
Before running `x19 memory setup`, add the local endpoint to
...
Then run `x19 memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$X19_HOME/supermemory.json`
...
| `container_tag` | `x19` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `x19-{identity}` → `x19-coder`). |
...
`https://api.supermemory.ai`. X19 resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: x19`, and document writes stamp
`metadata.sm_source: x19`. This is a **functional routing key, not telemetry**:
it groups X19-written memories into a dedicated "X19" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per X19 profile:

```json
{
  "container_tag": "x19-{identity}"
}
```

For a profile named `coder`, this resolves to `x19-coder`. The default profile resolves to `x19-default`. Without `{identity}`, all profiles share the same container.
