...
_DEFAULT_CONTAINER_TAG = "x19"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=x19"
...
def _load_supermemory_config(x19_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $x19_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if x19_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(x19_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, x19_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(x19_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "x19"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes X19 writes into the "X19" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "x19", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, x19_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(x19_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._x19_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `x19 memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, x19_home):
        ...
        _save_supermemory_config(sanitized, x19_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from x19_constants import get_x19_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_x19_home())))}

    def post_setup(self, x19_home: str, config: dict) -> None:
        from x19_cli.config import save_config
        from x19_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, x19_home=x19_home)
        ...
        status = _probe_supermemory_connection(api_key, x19_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from x19_constants import get_x19_home
        self._x19_home = kwargs.get("x19_home") or str(get_x19_home())
        self._session_id, self._turn_count, self._pending_turns = session_id, 0, []
        config = _load_supermemory_config(self._x19_home)
    ...
    def _write_turns(self, mode: str, new_turn: Optional[Dict[str, str]] = None) -> None:
        ...
        with self._capture_lock:
            ...
            for sid in dict.fromkeys(t["session_id"] for t in turns):
                ...
                metadata = {"type": "conversation", "session_id": sid, "timestamp": now.isoformat()}  # no sm_capture_mode: X19 policy
