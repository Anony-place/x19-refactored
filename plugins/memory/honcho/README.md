...
```bash
x19 memory setup honcho   # configure Honcho directly (works on a fresh install)
x19 memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
x19 config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.x19/.env
```

> `x19 honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `x19 memory setup honcho`.
...
Set `"recallSync": true` in `$X19_HOME/honcho.json` (at the root or under
`hosts.x19`), or enable **Current-query recall** in the memory settings or
`x19 honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `x19 model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$X19_HOME/honcho.json` | Profile-local (isolated X19 instances) |
| 2 | `~/.x19/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active X19 profile: `x19` (default) or `x19_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `x19 honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
Automatically generated X19 titles (`derived` or `llm`) are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions.
...
If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-x19`.
...
- **`per-session`** — X19 session ID (timestamp + hex). Every `x19` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple X19 profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`x19`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active X19 profile: `x19` (default) or `x19_<profile>` (e.g. `x19 -p coder` -> host key `x19_coder`). Older `x19.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `X19_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `x19`) |
...
|---------|-------------|
| `x19 memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `x19 honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `x19 memory setup`) |
| `x19 honcho status` | Show resolved config for active profile |
| `x19 honcho enable` / `disable` | Toggle Honcho for active profile |
| `x19 honcho mode <mode>` | Change recall or observation mode |
| `x19 honcho peer --user <name>` | Update user peer name |
| `x19 honcho peer --ai <name>` | Update AI peer name |
| `x19 honcho tokens --context <N>` | Set context token budget |
| `x19 honcho tokens --dialectic <N>` | Set dialectic max chars |
| `x19 honcho map <name>` | Map current directory to a session name |
| `x19 honcho sync` | Create host blocks for all X19 profiles |
