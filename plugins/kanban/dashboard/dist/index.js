/**
 * X19 Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.x19/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__X19_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__X19_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "x19-kanban-dot-triage",
    todo: "x19-kanban-dot-todo",
    ready: "x19-kanban-dot-ready",
    running: "x19-kanban-dot-running",
    blocked: "x19-kanban-dot-blocked",
    review: "x19-kanban-dot-review",
    done: "x19-kanban-dot-done",
    archived: "x19-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-x19-task";
  ...
  const DOCS_URL = "https://anony-place.github.io/x19-refactored/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://anony-place.github.io/x19-refactored/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "x19.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="x19-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])x19-kanban-md-code\1/i.test(attrs)) {
      return ' class="x19-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "x19-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "x19-kanban-md",
  ...
  // standard `drop` event and our `x19-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("x19-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("x19-kanban-column--drop");
          if (target) target.classList.add("x19-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("x19-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("x19-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("x19-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__X19_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "x19-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "x19-kanban-attention",
        "x19-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "x19-kanban-attention-bar" },
        h("span", { className: "x19-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "x19-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "x19-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "x19-kanban-attention-row",
                  "x19-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "x19-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "x19-kanban-attention-row-id" }, task.id),
                h("span", { className: "x19-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "x19-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "x19-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "x19-kanban-diag-action-btn",
      action.suggested ? "x19-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " x19-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".x19-kanban-drawer-comment-row input, .x19-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "x19-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("x19-kanban-diag", sevClass) },
      h("div", { className: "x19-kanban-diag-header" },
        h("span", { className: "x19-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "x19-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "x19-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "x19-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "x19-kanban-diag-data-row" },
                  h("span", { className: "x19-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "x19-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "x19-kanban-diag-data-row" },
                h("span", { className: "x19-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "x19-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "x19-kanban-diag-reassign-row" },
            h("span", { className: "x19-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "x19-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "x19-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "x19-kanban-diag-msg",
              msg.ok ? "x19-kanban-diag-msg--ok" : "x19-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "x19-kanban-section" },
      h("div", { className: "x19-kanban-section-head-row" },
        h("span", { className: "x19-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "x19-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "x19-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "x19-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "x19-kanban-docs-link",
      title: "Open X19 Kanban docs in a new tab",
      "aria-label": "X19 Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `x19 kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "x19-kanban-msg-ok" : "x19-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "x19-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "x19-kanban-boardswitcher" },
      h("div", { className: "x19-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "x19-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "x19-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "x19-kanban-dialog-title" },
        ...
        h("div", { className: "x19-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned X19 profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "x19-kanban-bulk" },
      h("span", { className: "x19-kanban-bulk-count" },
      ...
      h("div", { className: "x19-kanban-bulk-priority",
      ...
      h("div", { className: "x19-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different X19 profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "x19-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("x19-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("x19-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "x19-kanban-trash",
        dragOver ? "x19-kanban-trash--drop" : "",
        props.draggingTaskId ? "x19-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "x19-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "x19-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".x19-kanban-card")) return true;
      if (target.closest && target.closest(".x19-kanban-column-add")) return true;
      if (target.closest && target.closest(".x19-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("x19-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("x19-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".x19-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "x19-kanban-columns",
        isScrollable ? "x19-kanban-columns--scrollable" : "",
        isPanning ? "x19-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("x19-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("x19-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "x19-kanban-column",
        dragOver ? "x19-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "x19-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "x19-kanban-col-check",
        ...
        h("span", { className: cn("x19-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "x19-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "x19-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "x19-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "x19-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "x19-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "x19-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "x19-kanban-lane" },
                  h("div", { className: "x19-kanban-lane-head" },
                    h("span", { className: "x19-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "x19-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "x19-kanban-card--stale-red";
    if (age >= tier.amber) return "x19-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".x19-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "x19-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "x19-kanban-card",
        props.selected ? "x19-kanban-card--selected" : "",
        props.failed ? "x19-kanban-card--failed" : "",
        props.draggingSource ? "x19-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "x19-kanban-card-content" },
          h("div", { className: "x19-kanban-card-row" },
            h("label", {
              className: "x19-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "x19-kanban-card-check",
            ...
            h("span", { className: "x19-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or x19 kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "x19-kanban-warning-badge",
                    "x19-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "x19-kanban-progress",
                    progress.done === progress.total ? "x19-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "x19-kanban-needs-assignee",
          ...
          h("div", { className: "x19-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "x19-kanban-card-row x19-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "x19-kanban-assignee",
                            title: `Assigned to X19 profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "x19-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "x19-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "x19-kanban-ago",
  ...
  // centered modal reusing the x19-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "x19-kanban-dialog x19-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "X19 profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "X19 profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "x19-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "x19-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "x19-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "x19-kanban-drawer-close",
        ...
        data ? h("div", { className: "x19-kanban-drawer-comment-foot" },
          h("div", {
            className: "x19-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "x19-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "x19-kanban-section" },
      h("div", { className: "x19-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "x19-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "x19-kanban-drawer-body" },
      h("div", { className: "x19-kanban-drawer-title" },
        h("span", { className: cn("x19-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "x19-kanban-section" },
            h("div", { className: "x19-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "x19-kanban-section" },
            h("div", { className: "x19-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "x19-kanban-done-no-result x19-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "x19-kanban-section" },
            h("div", { className: "x19-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "x19-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "x19-kanban-section" },
        h("div", { className: "x19-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "x19-kanban-comment" },
            h("div", { className: "x19-kanban-comment-head" },
              h("span", { className: "x19-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "x19-kanban-diag-action-btn",
      ...
      h("div", { className: "x19-kanban-section" },
        h("div", { className: "x19-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "x19-kanban-comment" },
                h("div", { className: "x19-kanban-comment-head" },
                  h("span", { className: "x19-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "x19-kanban-comment-ago" },
      ...
      h("div", { className: "x19-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "x19-kanban-event",
              isDiag ? "x19-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "x19-kanban-event-header" },
                  h("span", { className: "x19-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "x19-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "x19-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "x19-kanban-event-header-plain" },
                  h("span", { className: "x19-kanban-event-kind" }, e.kind),
                  h("span", { className: "x19-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "x19-kanban-event-phantom-row" },
                  h("span", { className: "x19-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "x19-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "x19-kanban-event-payload" },
    ...
    return h("div", { className: "x19-kanban-section" },
      h("div", { className: "x19-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "x19-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `x19-kanban-run--${r.outcome || r.status || "ended"}`
          : "x19-kanban-run--active";
        return h("div", { key: r.id, className: cn("x19-kanban-run", outcomeClass) },
          h("div", { className: "x19-kanban-run-head" },
            h("span", { className: "x19-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "x19-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "x19-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "x19-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "x19-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "x19-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "x19-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "x19-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "x19-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "x19-kanban-pre x19-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "x19-kanban-meta-row" },
      h("span", { className: "x19-kanban-meta-label" }, props.label),
      h("span", { className: "x19-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "x19-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "x19-kanban-meta-row" },
        h("span", { className: "x19-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "x19-kanban-meta-value x19-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "x19-kanban-meta-row" },
        h("span", { className: "x19-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "x19-kanban-meta-value x19-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "x19-kanban-meta-row" },
        h("span", { className: "x19-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "x19-kanban-meta-value x19-kanban-editable",
    ...
    return h("div", { className: "x19-kanban-meta-row" },
      h("span", { className: "x19-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "x19-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "x19-kanban-section" },
      h("div", { className: "x19-kanban-section-head-row" },
        h("span", { className: "x19-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "x19-kanban-section" },
      h("div", { className: "x19-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "x19-kanban-deps-row" },
        h("span", { className: "x19-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "x19-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "x19-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "x19-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "x19-kanban-dep-chip-x",
      ...
      h("div", { className: "x19-kanban-deps-row" },
        h("span", { className: "x19-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "x19-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "x19-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "x19-kanban-section" },
      ...
      h("div", { className: "x19-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "x19-kanban-home-sub x19-kanban-home-sub--on"
              : "x19-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__X19_PLUGINS__ && typeof window.__X19_PLUGINS__.register === "function") {
    window.__X19_PLUGINS__.register("kanban", KanbanPage);
  }
})();
