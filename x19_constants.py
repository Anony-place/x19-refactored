...
Internal helper names retain their historical x19_* API for compatibility. The canonical
...
def set_x19_home_override(path: str | Path | None) -> Token:
...
def _get_platform_default_x19_home() -> Path:
...
def _warn_profile_fallback_once() -> None:
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_x19_home()
...
def get_x19_home() -> Path:
    """X19 home: context override → ``X19_HOME`` → canonical platform default."""
    override = get_x19_home_override()
    if override:
        return Path(override)
    if not os.environ.get("X19_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_x19_home()
...
def x19_home_key(path: str | Path | None = None) -> str:
    ...
    """
    candidate = Path(path) if path is not None else get_x19_home()
    raw = str(candidate)
...
# ``x19 backup`` (excludes them) and ``profile create --clone-all`` (skips them from the
# default profile) so the two lists cannot drift apart.
LOCAL_RUNTIME_ROOT_DIRS: frozenset[str] = frozenset({"models", "runtimes", "node"})

# get_default_x19_root() memo keyed on (native home, X19_HOME) so it stays
# fresh when a test or plugin mutates X19_HOME; saves ~80us/call at 31+ sites.
_default_x19_root_memo: "tuple[str, str, Path] | None" = None


def get_default_x19_root() -> Path:
    """Root X19 dir for profile-level ops: ``<root>`` when ``X19_HOME=<root>/profiles/<name>``."""
    global _default_x19_root_memo
    native_home = _get_platform_default_x19_home()
    env_home = os.environ.get("X19_HOME", "")
    memo = _default_x19_root_memo
    ...
    _default_x19_root_memo = (str(native_home), env_home, result)
    return result
...
def _is_x19_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<x19-home>/profiles``.

    Accepts the classic ``~/.x19`` layout, a root carrying X19-home marker files, a
    ...
    try:
        ...
        pass
    try:
        return root.resolve(strict=False) == get_default_x19_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_x19_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".x19":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def profile_name_for_home(path: str | Path | None) -> str | None:
    ...
    The default home is the X19 root itself, so its basename is an installation detail (``.x19``
    on POSIX and commonly ``x19`` on Windows), not the profile id ``default``.
    """
...
def named_profile_is_live(profile_home: str | Path) -> bool:
    ...
    never be started as a backend (whose ``ensure_x19_home`` would rebuild the full tree)."""
    home = Path(profile_home)
...
def mkdir_under_x19_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_x19_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("X19_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``X19_OPTIONAL_MCPS``)."""
    return _packaged_dir("X19_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``X19_BUNDLED_SKILLS``)."""
    return _packaged_dir("X19_BUNDLED_SKILLS", default, "skills")
...
def iter_x19_node_dirs(home: Path | None = None) -> list[Path]:
    """X19-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with x19ManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_x19_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_x19_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    ...
    result = _run_version_probe([path, "--version"], env=with_x19_node_path())
    return result is not None and result.returncode == 0


_X19_NODE_TARGET_MAJOR = int(os.environ.get("X19_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/x19_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks x19-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def x19_managed_node_tree_present(home: Path | None = None) -> bool:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        ...
        "upgrade until the app is closed (re-run `x19 update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_X19_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_X19_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the x19.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "X19_HOME": str(get_x19_home()), **extra_env},
...
def bootstrap_x19_managed_node() -> str | None:
    ...
    """
    existing = find_x19_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # X19_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, X19_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_x19_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted or not x19_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _X19_NODE_TARGET_MAJOR
    return False


def find_x19_node_executable(command: str) -> str | None:
    ...
    if needs_heal and heal_x19_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_x19_node_executable(command)
    if managed:
        return managed
    if x19_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_x19_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for entry in reversed([str(path) for path in iter_x19_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``x19 update``
...
def display_x19_home(home: Path | None = None) -> str:
    ...
    ``active_profile`` (``get_x19_home()`` would emit the wrong-profile fallback warning there).
    """
    if home is None:
        home = get_x19_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\x19/skills/
...
def profile_cli_selector() -> str:
    """``-p <name> `` (trailing space) pinning copy-pasteable ``x19 ...`` guidance to the
    active NAMED profile, else ``""``: a bare ``x19`` follows the sticky ``active_profile``
    ...
    name = profile_name_for_home(get_x19_home())
...
def secure_parent_dir(path: Path) -> None:
    ...
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks x19-user traversal in Docker (UID 10000).
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "x19 install directory (%s). Credential files are "
            "normally stored under the x19 home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    x19_home = get_x19_home_override() or (env or {}).get("X19_HOME") or os.getenv("X19_HOME")
    if not x19_home:
        return None
    profile_home = os.path.join(x19_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "X19_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the X19 profile HOME.
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply X19' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["X19_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when X19 runs in WSL.
...
def is_container() -> bool:
    ...
    See: Anony-place/x19-refactored#47111
    """
...
def get_config_path() -> Path:
    ...
    return get_x19_home() / "config.yaml"
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_x19_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._x19_ipv4_patched = True  # type: ignore[attr-defined]
...
def openrouter_variant_base(model_id: str) -> str | None:
    ...
    Lives here rather than in ``x19_cli.models`` so the metadata layer
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``x19_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``x19_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with X19 (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("x19_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    x19 update",
        "If that also fails, reinstall: https://anony-place.github.io/x19-refactored",
    ]
