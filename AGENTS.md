# X19 - Development Guide

Instructions for AI coding assistants and developers working on the x19 codebase.
...
Read the balance right: X19 ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`x19 tools`, `x19 setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `X19_HOME` — two of them (A→B→A) when the
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `x19 tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.x19/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `x19 <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`x19 webhook`, `x19 cron`, `x19 tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.x19/plugins/` or a pip
...
├── ui-tui/               # Ink (React) terminal UI — `x19 --tui`
...
**User state:** `~/.x19/config.yaml` (settings), `~/.x19/.env` (secrets only),
`~/.x19/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_x19_home()`. Browse logs with `x19 logs [--follow] [--level] [--session]`.
...
families: `x19_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`x19_cli/kanban.py` (14), `x19_cli/web_server.py` (13 + 24 routers), `x19_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `x19_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `x19_cli.update_cmd._x19_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `x19_cli/AGENTS.md`.
...
- **One process may serve many profiles; code that runs outside a turn binds the owning
  ...
  module constants are a bug class — key slots by `x19_home_key()` or resolve at call time.
  ...
  `X19_HOME`. Advisory lint: `scripts/check_profile_scope_patterns.py`.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `x19 webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `X19_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `X19_HOME` → temp dir, and per-file subprocess
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/x19_cli/`,
  `tests/agent/`, `tests/x19_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `x19_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.x19/`.** The autouse `_isolate_x19_home` fixture in
  `tests/conftest.py` redirects `X19_HOME`; never hardcode `~/.x19/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_x19_home()` stay
  in the temp dir (pattern: `tests/x19_cli/test_profiles.py`):
  ```python
  @pytest.fixture
...
`tests/x19_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cron/`, kanban (`x19_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
