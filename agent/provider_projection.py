"""Fold an agent-as-provider's own activity back into X19' turn state.

Agent providers (ACP CLI shims, the codex app-server) run their own tools, so that
work must never come back as pending ``tool_calls`` (X19 would re-run it) — but
...
``reasoning``. The client hands back ``x19_projected_messages`` (completed
assistant/tool rows) and ``x19_provider_tool_iterations`` on the completion
...
def splice_provider_projection(agent: Any, response: Any, messages: list[dict[str, Any]]) -> int:
    ...
    """
    projected = getattr(response, "x19_projected_messages", None)
    ...
    try:
        iterations = int(getattr(response, "x19_provider_tool_iterations", 0) or 0)
