...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time X19 finds ``~/.openclaw/``: migrate first, cleanup (which breaks OpenClaw) after."""
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to X19, run `x19 claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, run `x19 claw cleanup` "
