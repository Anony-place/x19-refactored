...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; X19 only
    initiates compaction in "x19" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping X19 preflight compression for codex app-server "
            "(mode=%s); X19 will not start thread compaction here.",
