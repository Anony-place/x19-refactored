...
from x19_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
_SENSITIVE_X19_DIRS = (Path("skills") / ".hub",)
...
def preprocess_context_references(
    ...
    # X19_HOME override is lost and the credential-path guard checks the launch profile's .env.
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, x19_home = Path(os.path.expanduser("~")).resolve(), get_x19_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {x19_home / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [x19_home / rel for rel in _SENSITIVE_X19_DIRS]
    if path in blocked_exact:
        raise ValueError("path is a sensitive credential file and cannot be attached")
    if any(_is_under(path, blocked_dir) for blocked_dir in blocked_dirs):
        raise ValueError("path is a sensitive credential or internal X19 path and cannot be attached")
