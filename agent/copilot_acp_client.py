"""OpenAI-compatible shim that forwards X19 requests to `copilot --acp`.
...
_PROMPT_PREAMBLE = (
    "You are being used as the active ACP agent backend for X19.",
...
_DEPRECATED_CLI_ERROR = (
    ...
    "If `copilot` already resolves to the new CLI but you still see this,\npoint X19 at it explicitly:\n"
    "  export X19_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
...
def _resolve_command() -> str:
    return os.getenv("X19_COPILOT_ACP_COMMAND", "").strip() or os.getenv("COPILOT_CLI_PATH", "").strip() or "copilot"


def _resolve_args() -> list[str]:
    return shlex.split(os.getenv("X19_COPILOT_ACP_ARGS", "").strip()) or ["--acp", "--stdio"]
...
def _build_subprocess_env() -> dict[str, str]:
    from x19_constants import apply_subprocess_home_env
...
def _format_messages_as_prompt(
    ...
    # one. Copilot has no tools of its own that collide with X19', so forward the whole toolset.
...
class CopilotACPClient:
    ...
    X19_SKIP_TRANSPORT_WRAP = True
    X19_SKIP_ASYNC_WRAP = True
    ...
    def _spawn(self) -> subprocess.Popen[str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                "X19_COPILOT_ACP_COMMAND / X19_COPILOT_ACP_ARGS to a working pair."
            )
        try:
            from x19_cli._subprocess_compat import windows_hide_flags  # hide the Windows console flash (#56747); pipes intact for the ACP wire
            ...
            raise RuntimeError(f"Could not start Copilot ACP command '{self._acp_command}'. Install GitHub Copilot CLI or set "
                               "X19_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.") from exc
    ...
    def _handle_server_message(
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(message_id, -32601, f"ACP client method '{method}' is not supported by X19 yet.")
