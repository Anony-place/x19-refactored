...
def _state_file() -> Path:
    return get_x19_home() / "skills" / ".curator_state"
...
def _read_config_section(*path: str, label: str, log: logging.Logger = logger) -> Dict[str, Any]:
    """Read a nested section of ~/.x19/config.yaml. Tolerates missing file."""
    try:
...
def get_consolidate() -> bool:
    """LLM consolidation pass — OFF by default (prune only, no aux-model fork); ``x19 curator run --consolidate`` overrides per invocation."""
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``x19 curator run`` bypasses this; the idle check is the caller's."""
    ...
    if last is None:
        try:
            ...
            state["last_run_summary"] = "deferred first run — curator seeded, will run after one interval; use `x19 curator run --dry-run` to preview now"
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as X19' background skill CURATOR. This is an "
    ...
    "into ~/.x19/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: x19-config-*, x19-dashboard-*, gateway-*, codex-*, "
    ...
    "`x19 curator rollback` restores a hollow skill (issue #96962).\n\n"
...
def _reports_root() -> Path:
    """``~/.x19/logs/curator/`` (telemetry next to agent.log, not under skills/). mkdir'd here too so gateway-only / bare-library entry paths work."""
    root = get_x19_home() / "logs" / "curator"
    try:
...
def _build_rename_summary(*, before_names: Set[str], after_report: List[Dict[str, Any]], tool_calls: List[Dict[str, Any]], model_final: str) -> str:
    ...
    lines.append("full report: x19 curator status")
    umbrellas = sorted({e.get("into") for e in diff.consolidated if e.get("into")})
    if umbrellas:
        lines.append(f"keep an umbrella stable: x19 curator pin {umbrellas[0]}")
    return "\n".join(lines)
...
_REPORT_SECTIONS = (
    ("consolidated", "Consolidated into umbrella skills",
     ...
     "The original directory was moved to `~/.x19/skills/.archive/` for safety and can be restored via "
     "`x19 curator restore <name>` if the consolidation was wrong._\n", _consolidated_lines, 50, "see `run.json`"),
    ("pruned", "Pruned — archived for staleness",
     ...
     "Directories live under `~/.x19/skills/.archive/`. Restore any via `x19 curator restore <name>`._\n",
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if final:
        ...
        lines += ["## LLM summary\n", p.get("llm_summary"), ""]
    lines += ["## Recovery\n", "- Restore an archived skill: `x19 curator restore <name>`",
              "- All archives live under `~/.x19/skills/.archive/` and are recoverable by `mv`",
...
def run_curator_review(
    ...
    *dry_run* SKIPS the stale/archive transitions and instructs the fork to report only; REPORT.md is still written and
    ...
    # next real pass out) but still records a summary for `x19 curator status`.
    ...
    def _llm_pass():
        ...
        # Per-run report, best-effort; path recorded for `x19 curator status`.
        try:
