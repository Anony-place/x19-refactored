...
Every X19 request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=x19", "client=x19-client-v<__version__>"]``.
The version is read live from ``x19_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("x19_affinity_scope", default=None)
...
def x19_client_tag() -> str:
    """``client=x19-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if x19_cli is unimportable)."""
    try:
        from x19_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=x19-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=x19", x19_client_tag()]
