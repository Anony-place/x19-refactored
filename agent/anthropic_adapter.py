"""Anthropic Messages API adapter: client construction + the Messages call for X19's
...
# X19 effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain X19 tool schemas/prose as a
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    x19's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.x19/.env) and then sends dual
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to x19's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    X19 tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning X19 shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that X19 surfaces in its CLI. We request "summarized" so the
