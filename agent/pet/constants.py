...
inferred from the atlas shape so X19 renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (X19 names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy X19/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical X19 names -> accepted row-name aliases in descending preference.
