"""Language Server Protocol (LSP) integration for X19.
...
# Routed multiplex profiles (X19_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``x19 chat`` exit leaks pyright processes for a few seconds
