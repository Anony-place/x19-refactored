...
# ``bin`` in ``<X19_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `x19 lsp status` reports its presence.
...
def x19_lsp_bin_dir() -> Path:
    """Return the X19-owned bin staging dir for LSP servers."""
    from x19_constants import get_x19_home

    p = get_x19_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(x19_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = x19_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $X19_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that X19 installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = x19_lsp_bin_dir().parent  # <X19_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = x19_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``x19 lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "x19_lsp_bin_dir"]
