"""Unified removal contract for every credential source X19 reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``x19 auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.x19/.env; hint when the shell exports it."""
    from x19_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like x19_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to X19.",
            f"  The pool entry is now suppressed — X19 will ignore "
            f"{env_var} until you run `x19 auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_x19_pkce(provider: str, removed) -> RemovalResult:
    """~/.x19/.anthropic_oauth.json is ours — delete it outright."""
    from x19_constants import get_x19_home

    result = RemovalResult()
    oauth_file = get_x19_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared X19 Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `x19 model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `x19 auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `x19 auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `x19 auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="x19_pkce",
        remove_fn=_remove_x19_pkce,
        description="~/.x19/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `x19 auth add qwen-oauth` to re-enable if needed.",
        ),
