...
# `export X19_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in x19_cli/main.py, gateway/run.py, and cli.py) or `X19_REDACT_SECRETS=false` in
# ~/.x19/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("X19_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a X19_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in x19_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("X19_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``x19 config`` / ``status`` / ``dump``):
...
# a shell rc/profile, X19' own ``config.yaml`` where ``x19 mcp add --env`` writes
...
_X19_HOME_PREFIXES = ("$X19_HOME/", "${X19_HOME}/")
# ``$HOME/.x19/config.yaml`` keeps the ``.x19`` segment, so stripping the prefix is
...
def _is_under_x19_home(path: str) -> bool:
    """True when an absolute ``config.yaml`` path sits under the active X19 home or root.

    The default home's basename is an installation detail — ``.x19`` on POSIX, ``x19``
    under ``AppData/Local`` on Windows — and a resolved path never spells ``$X19_HOME``,
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.x19`` directory, ``$X19_HOME``, or the resolved X19 home (never arbitrary
    YAML). The resolved-home arm is what covers native Windows, where the home directory
    is ``%LOCALAPPDATA%\\x19`` and carries no ``.x19`` segment."""
    path = arg.strip("\"'").replace("\\", "/")
    x19_home = False
    for prefix in _X19_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    # ``config.yaml`` plus the ``config.yaml.good.<stamp>`` / ``.corrupt.<stamp>`` copies X19
    ...
    if parts[-1] != "config.yaml" and not parts[-1].startswith(("config.yaml.good.", "config.yaml.corrupt.")):
        return False
    return x19_home or ".x19" in parts[:-1] or _is_under_x19_home(path)
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, X19 ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
