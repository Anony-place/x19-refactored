...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("x19_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("x19_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "X19_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "X19_KANBAN_TASK", "X19_KANBAN_RUN_ID", "X19_KANBAN_CLAIM_LOCK",
    "X19_KANBAN_GOAL_MODE", "X19_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``X19_KANBAN_*`` identity gate should use."""
    return not (is_delegated_child_process_context() or _NON_DISPATCHER_OWNED_CONTEXT.get())


def owned_kanban_task() -> str:
    """The board task this execution OWNS: ``X19_KANBAN_TASK`` for the dispatcher-owned
    ...
    if not is_dispatcher_owned_worker_context():
        return ""
    return (os.environ.get("X19_KANBAN_TASK") or "").strip()
...
def _fenced_kanban_root() -> str:
    ...
    try:
        from x19_cli.kanban_db import kanban_home
...
def scrub_kanban_env(env: Mapping[str, str] | MutableMapping[str, str]) -> dict[str, str]:
    ...
    against a temp ``X19_HOME`` got a silently read-only board there. An inherited
    path-valued marker is kept (a grandchild that moved X19_HOME must not re-fence
...
def kanban_path_is_fenced(path: "os.PathLike[str] | str") -> bool:
    ...
    descendant only when *path* is the dispatcher-pinned ``X19_KANBAN_DB`` or lies under the
    ...
    pinned = os.environ.get("X19_KANBAN_DB", "").strip()
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("X19_KANBAN_TASK")
            or (env and (env.get("X19_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
