"""OpenAI-shape bridge shared by X19' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so X19' tool schemas travel INTO the
...
read/edit/execute tools forwards only X19' agent-level tools, since re-offering overlapping ones
makes X19 redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``x19_projected_messages``)
    X19 reads off the ``create`` result; a plain list would drop them on the stream path."""
