...
INSPECTION_STAMP_ATTR = "data-x19-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-x19-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-x19-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-x19-vault-slot]").forEach((n) => n.removeAttribute("data-x19-vault-slot"));
