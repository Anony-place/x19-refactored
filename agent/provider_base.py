...
Every tool-provider ABC shares the same identity + ``x19 tools`` picker surface.
...
class ProviderBase(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``x19 tools``. Defaults to ``name``."""
        return self.name

    def get_setup_schema(self) -> Dict[str, Any]:
        """Provider row for the ``x19 tools`` picker.
...
class CatalogProviderBase(ProviderBase):
    ...
    @property
    def display_name(self) -> str:
        """Human-readable label shown in ``x19 tools``. Defaults to ``name.title()``."""
    ...
    def is_available(self) -> bool:
        ...
        ``x19 setup`` call it on every paint.
        """
        return True
