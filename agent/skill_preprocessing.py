"""Shared SKILL.md preprocessing helpers: ``${X19_*}`` template tokens and
...
from x19_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger(__name__)

# ${X19_SKILL_DIR} / ${X19_SESSION_ID} tokens. Unresolvable ones (e.g. no
# session) are left as-is so the author can spot them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(X19_SKILL_DIR|X19_SESSION_ID)\}")
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${X19_SKILL_DIR} / ${X19_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "X19_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "X19_SESSION_ID": str(session_id) if session_id else None,
    }
