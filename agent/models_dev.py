...
thread refreshes) → disk cache (~/.x19/models_dev_cache.json, any age) →
...
from x19_constants import openrouter_variant_base
...
# X19 provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # X19 provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → X19 ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_x19_ids(mdev_id: str) -> List[str]:
    """Return the X19 provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for x19_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(x19_id)
...
def _x19_path(name: str) -> Path:
    from x19_constants import get_x19_home
    return get_x19_home() / name


def _get_cache_path() -> Path:
    return _x19_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _x19_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_x19_home(etag_path.parent)
...
# models the catalog does not know and never displace catalog data. Provider keys accept the X19
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by X19 OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (X19 → models.dev id) and reverse (caller passed a models.dev id, config keyed by X19 id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_x19_ids(provider_key)]
...
def _builtin_model_metadata(provider: str, model: str) -> Optional[Dict[str, Any]]:
    """Built-in metadata for a provider/model pair, if X19 has a vendor-specific entry."""
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by X19 or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by X19 or models.dev provider ID (exact match, then case-insensitive), or
