...
def run_tool_round(
    ...
    X19; a failed canonical append ends the turn rather than running tools from
    process-only state."""
    ...
    # block if a destructive tool restarts X19.
    try:
    ...
    # the gateway inactivity timeout (X19_AGENT_TIMEOUT).
    ...
    # follow-up API call can exceed the gateway inactivity timeout (X19_AGENT_TIMEOUT, default 1800s) and
