...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into X19 accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and x19-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior X19 deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → X19 UI bridge -------------------------------------
...
# codex-level mcpToolCall IS the display event and the mcp.x19-tools.* prefix is stripped (users see X19 tools).
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic X19 tool name for a codex item (mirrors CodexEventProjector)."""
    item_type = item.get("type") or ""
    if item_type == "mcpToolCall":
        server, tool = item.get("server") or "mcp", item.get("tool") or "unknown"
        return tool if server == X19_TOOLS_MCP_SERVER_NAME else f"mcp.{server}.{tool}"
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: X19' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, X19_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # X19' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
