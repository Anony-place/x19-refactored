...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 X19 secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 X19 secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = x19_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only X19' managed MCP tool
        ...
        owned_task = os.environ.get("X19_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "X19_KANBAN_DB", "X19_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.{X19_TOOLS_MCP_SERVER_NAME}.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.{X19_TOOLS_MCP_SERVER_NAME}.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("X19_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("X19_HOME", os.path.expanduser("~/.x19")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("X19_KANBAN_ROOT", default_root)
            cmd += [
