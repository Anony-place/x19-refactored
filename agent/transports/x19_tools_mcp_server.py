"""X19-tools-as-MCP server for the codex_app_server runtime.

Codex owns the loop and tool list there, so a curated subset of X19 tools is
exposed over stdio MCP; codex registers it via ``~/.codex/config.toml
[mcp_servers.x19-tools]``. Run: ``python -m agent.transports.x19_tools_mcp_server``.
"""
...
X19_TOOLS_MCP_SERVER_NAME = "x19-tools"
...
# Each name MUST match a registered X19 tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read X19_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on X19_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with X19 tools attached (lazy imports: importable without ``mcp``)."""
    try:
        # mcp 2.0 renamed `mcp.server.fastmcp` to `mcp.server.MCPServer` (same surface).
        from mcp.server import MCPServer
    except ImportError as exc:  # pragma: no cover - install hint
        raise ImportError(f"x19-tools MCP server requires the 'mcp' package: {exc}") from exc

    from model_tools import get_tool_definitions, handle_function_call

    mcp = MCPServer(
        X19_TOOLS_MCP_SERVER_NAME,
        instructions=(
            "X19's tool surface, exposed for use inside a Codex "
    ...
    # Authoritative X19 schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this X19 process", name)
            continue
        description = spec.get("description") or f"X19 {name} tool"
    ...
    logger.info("x19-tools MCP server registered %d/%d tools", exposed_count, len(EXPOSED_TOOLS))
    return mcp


def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.x19_tools_mcp_server`."""
    ...
    # Keep X19' own banners off stdout (the MCP wire).
    os.environ.setdefault("X19_QUIET", "1")
    os.environ.setdefault("X19_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"x19-tools MCP server cannot start: {exc}\n")
        return 2
    try:
        ...
        logger.exception("x19-tools MCP server crashed")
        sys.stderr.write(f"x19-tools MCP server error: {exc}\n")
        return 1
    return 0
