...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("x19-verify-", "x19-ad-hoc-")
...
def _db_path() -> Path:
    return get_x19_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "x19 verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``x19 verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="x19 verify", kind="verify",
