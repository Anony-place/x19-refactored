...
``X19_SECRET_KEY`` (never interpolated, so a hostile key name is inert); hard
...
def _run_helper(command: str, secret_key: str, timeout_seconds: float, max_output_bytes: int) -> Optional[str]:
    ...
    The key travels as DATA in ``X19_SECRET_KEY``. stdout/stderr are piped
...
class CommandSource(SecretSource):
    ...
    remediation_hints = {
        ...
        ErrorKind.INTERNAL: "Run the helper manually in a shell to see its real error — "
                            "X19 discards helper stderr so diagnostics can't leak "
                            "secret material.",
    }
