...
env-var-shaped values at process startup, AFTER ``~/.x19/.env`` has loaded
and BEFORE the rest of X19 reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
X19_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("x19_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `x19 secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `x19 secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `x19 secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `x19 secrets {name} setup` to re-authenticate.",
