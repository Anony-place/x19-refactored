...
``skill_manage`` per the X19 authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the X19 skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `X19`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as X19.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.x19.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through X19 tools.
...
X19-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference X19 tools by name in backticks: `terminal`, `read_file`,
