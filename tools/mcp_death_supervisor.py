#!/usr/bin/env python3
"""One parent-death supervisor per X19 process, shared by all stdio MCP servers.

Why this exists
---------------
When X19 dies without running its cleanup path (SIGKILL, OOM killer, a hard
crash), stdio MCP servers it spawned are reparented to init and keep running
forever.  macOS has no ``PR_SET_PDEATHSIG``, so something has to outlive X19
and reap them.

This module is deliberately standard-library-only and must not import anything
from ``tools/``: it runs after X19 may already be dead, and pulling in
...
with a single supervisor per X19 process:

* **Death detection is a blocking read on a pipe.**  X19 holds the only write
  end.  When X19 dies -- by any means, including SIGKILL -- the write end
...
X19 never unregistered *is* the orphan set, so a clean X19 shutdown --
...
an unrelated process owned by the same user.  If X19 then dies ungracefully
...
Two things narrow the window.  X19 prunes registrations whose group has no
...
exposure already exists in X19's own killpg-based orphan cleanup, which this
...
def _is_safe_target(pgid: int, *, own_pgid: int, parent_pgid: int) -> bool:
    ...
    Defensive only -- X19 already filters non-MCP children before it
    ...
    X19 process tree or the session.
    """
    if pgid <= 1:
        return False
...
def _serve(stream, *, own_pgid: int, parent_pgid: int) -> set[int]:
    ...
    stopped. Nothing in X19 can produce that today, but this process is the
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Process group of the spawning X19 process; never signalled.",
    )
