...
``metadata.x19.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.x19.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.x19.blueprint`` is a mapping with a
    ...
    x19 = meta.get("x19") if isinstance(meta, dict) else None
    blueprint = x19.get("blueprint") if isinstance(x19, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.x19.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.x19.blueprint`` block) ready for ``x19 skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"x19": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
