...
Registers when either xAI credential path is available (``XAI_API_KEY`` or ``x19 auth add
...
from tools.xai_http import DEFAULT_XAI_BASE_URL, x19_xai_user_agent, resolve_xai_http_credentials
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `x19 auth add xai-oauth` "
