...
Loads a user-managed website blocklist (``security.website_blocklist`` in ~/.x19/config.yaml plus
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    default_path = get_x19_home() / "config.yaml"
    ...
    for shared_file in shared_files:
        ...
        path = path if path.is_absolute() else (get_x19_home() / path).resolve()
