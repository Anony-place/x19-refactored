...
refresh; this module supplies ``X19TokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is X19' Client ID Metadata
...
async def acquire_refresh_fence(path: "Path", *, timeout: float = _REFRESH_FENCE_TIMEOUT_SECONDS) -> int:
    ...
    try:
        ...
        mkdir_under_x19_home(lock_path.parent)
...
_USER_SKIPPED_SENTINEL = "__x19_user_skipped__"


def _get_token_dir(x19_home: str | Path | None = None) -> Path:
    """``X19_HOME/mcp-tokens/`` — per-profile token directory."""
    from x19_constants import get_x19_home

    return Path(x19_home if x19_home is not None else get_x19_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "X19TokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "X19TokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``x19
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `x19 mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
class X19TokenStorage:
    """Persist OAuth state as ``X19_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, x19_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._x19_home = Path(x19_home) if x19_home is not None else None
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._x19_home) / f"{self._server_name}{suffix}"
    ...
    def _fixup_loaded_tokens(self, data: dict) -> None:
        # ``x19_issuer`` is X19 bookkeeping, not an SDK OAuthToken field: pop before validation.
        self.loaded_issuer = data.pop("x19_issuer", None)
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        if self._bound_issuer:  # which authorization server granted these tokens (never sent on the wire)
            payload["x19_issuer"] = self._bound_issuer
    ...
    def stamp_issuer(self, issuer: str) -> None:
        """Backfill ``x19_issuer`` onto a pre-binding token file: adopt the currently discovered
        issuer once instead of forcing a re-login, so the *next* read is protected."""
        data = _read_json(self._tokens_path())
        if data is None or data.get("x19_issuer"):
            return
        data["x19_issuer"] = str(issuer)
        try:
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._x19_home)
        from x19_constants import mkdir_under_x19_home

        mkdir_under_x19_home(token_dir)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to X19.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `x19 mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: https://anony-place.github.io/x19-refactored/docs/guides/oauth-over-ssh\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected X19' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
X19OAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, x19_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    X19TokenStorage(server_name, x19_home=x19_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; X19 only decides
# eligibility. Published from ``website/static/oauth/client-metadata.json``; the github.io origin is
# deliberate — servers MUST NOT follow redirects when fetching it, and anony-place.github.io/x19-refactored/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = "https://nousresearch.github.io/x19/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "X19TokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after X19 must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "X19TokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "X19TokenStorage | None" = None) -> int:
...
def _invalidate_tokens_on_client_change(
    storage: "X19TokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: x19 mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "X19TokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older X19 is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `x19 mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if X19OAuthClientProvider is None:
        from tools.mcp_oauth_provider import X19ProviderMixin

        X19OAuthClientProvider = type("X19OAuthClientProvider", (X19ProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus X19' token-endpoint fixes (see ``X19ProviderMixin``).",
            "__module__": __name__, "_x19_logger": logger})
    return X19OAuthClientProvider(server_url=server_url, **kwargs)
