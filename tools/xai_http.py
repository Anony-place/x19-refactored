...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_x19_home() / "auth.json"
...
def x19_xai_user_agent() -> str:
    """Return a stable X19-specific User-Agent for xAI HTTP calls."""
    try:
        from x19_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"X19/{__version__}"


def x19_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": x19_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per X19 home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from x19_constants import get_x19_home
        marker_dir = get_x19_home() / "state"
...
def _xai_base_url_override() -> str:
    """``X19_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from x19_cli.config import get_env_value
    return str(get_env_value("X19_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: X19-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.x19/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``X19_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers X19-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``x19_cli.config.get_env_value`` so keys stored in ``~/.x19/.env`` (the standard X19
    ...
    scoping is identical to the fallback branch, and the base URL honors ``X19_XAI_BASE_URL`` /
