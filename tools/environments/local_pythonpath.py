"""X19-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the X19 venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven X19-owned by *path provenance* are removed — never by a
cross-version heuristic. Module state (``_x19_repo_root_aliases``, ``_in_venv``,
``_x19_site_packages``) lives in ``tools.environments.local`` (via :func:`_state`)
...
def _build_x19_repo_root_aliases(
    resolved_root: Path, lexical_root: Path, configured_home: Path,
) -> tuple[Path, ...]:
    """Exact repo-root spellings emitted by X19 launchers. Mirrors
    ``gateway_windows._preserve_x19_home_path`` (physical path under the resolved
    X19_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary X19_HOME descendants as X19-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_x19_root() does).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not env.get("VIRTUAL_ENV") or not any(
            _same_path(candidate, root / "venv") for root in _state()._x19_repo_root_aliases):
        return None
    try:
...
def _get_x19_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the X19 runtime (cached):
    ...
    local = _state()
    if local._x19_site_packages is None:
        result: list[Path] = []
        if local._in_venv:
            try:
                import site
        ...
        local._x19_site_packages = list(result)
    result = list(local._x19_site_packages)
...
def _strip_x19_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip X19-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_x19_owned_pythonpath(env: dict) -> None:
    """Remove X19-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if not pp:
        return
    owned_paths = [*_get_x19_site_packages(env), *_state()._x19_repo_root_aliases]
    ...
    if stripped:
        logger.debug("Stripped X19-owned entries from PYTHONPATH: %s", stripped)
