...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(x19_home).expanduser() if x19_home else Path.home() / ".x19") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite X19's live source checkout "
        ...
        "is pushed. To change this checkout, stop X19, run the command externally, then restart "
        "X19.")
