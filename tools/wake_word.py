"""Wake-word ("Hey X19") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey x19" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey x19", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey x19" model (tools/wakewords/) — the default; alias names resolve
# to it, not to an openWakeWord built-in.
_BUNDLED_MODEL_NAME = "hey_x19"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_x19", "hey x19", "x19"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_x19 model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey x19"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from x19_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the X19 backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `x19 tools` "
...
# ── Process-wide singleton (mirrors x19_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_x19_root() / "runtime" / "wake-word.lock"
