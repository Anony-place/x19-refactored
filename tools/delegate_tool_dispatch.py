...
def _capture_origin() -> tuple[str, str, Any, Any, bool]:
    ...
    clobbers the X19_SESSION_ID ContextVar/os.environ with the subagent's id.  The wake-
    ...
    with _quiet(None):
        ...
        _origin_ui_session_id = get_session_env("X19_UI_SESSION_ID", "")
...
_SYNC_FALLBACK_NOTES = {
    "no_async": (
        ...
        "finite chat using -Q, --oneshot, or non-TTY stdio, `x19 -z`, a cron job, a Kanban "
...
def _resolve_async_wake_sid(origin_wake_sid: str, origin_session_history_delivery: bool = False) -> Optional[str]:
    ...
    if get_session_env("X19_SINGLE_QUERY_SESSION") == "1":
        return None
...
def _resolve_async_session_key(parent_agent: Any, origin_ui_session_id: str) -> tuple[str, str]:
    ...
    platform conversation key (agent:main:...). The CLI has no bound approval contextvar and no X19_SESSION_KEY, so
    ...
    with _quiet(None):
        from gateway.session_context import get_session_env
        source = get_session_env("X19_SESSION_SOURCE", "")
        # Refresh from the task-local source when available, else retain the
        # immutable value captured before child construction.
        origin_ui_session_id = get_session_env("X19_UI_SESSION_ID", "") or origin_ui_session_id
