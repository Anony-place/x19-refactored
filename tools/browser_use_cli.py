...
_PRIVATE_BROWSER_SENTINEL = "_X19_BU_PRIVATE_BROWSER"
...
# BU_NAME + daemon pid).
_OWN_TAB_PREAMBLE = """\
# x19: pin this named session to its own tab (once per daemon process)
def _x19_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "x19-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
...
def _base_subprocess_env() -> dict:
    ...
    # (X19's venv) wins over its site-packages → wrong-ABI C-extensions and a crash.
    # PYTHONPATH/PYTHONHOME inherited from the agent process point at X19's venv site-packages, and a
    ...
    # #84841, #86006, #86104). Strip both — the CLI manages its own environment and never needs X19's
    # import path.
...
def _use_gateway(browser_cfg: dict) -> bool:
    """True when the browser section selects the Nous Tool Gateway — by the current ``x19 tools``
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_x19_home()) / "cache" / ".browser_use_default_notice"
        ...
        with contextlib.suppress(OSError):
            ...
            stamp.touch()
        return ("Browser Use CLI not found — using the built-in browser tools. Run `x19 tools` "
...
def _managed_bin_dir() -> str:
    """$X19_HOME/bin — where install.sh puts uv/uvx and install_cli() links browser-use."""
    return str(Path(get_x19_home()) / "bin")


def _find_cli() -> Optional[List[str]]:
    """Locate the browser-use CLI, or None when it can't be run. MANAGED-FIRST: X19' own ``$X19_HOME/bin``
    copy always wins so every session drives one X19-controlled binary; PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    the binary into ``$X19_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()`` resolves it for every profile.
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_x19_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_lightpanda_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Lightpanda could not be started: {e} Set browser.engine to auto "
                   "to use local Chrome, or switch backends via `x19 tools` → Browser Automation."),
...
def _resolve_managed_chromium_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at X19' packaged Chromium, launched through agent-browser for this cache key —
    ...
    if not cdp:
        return (f"The local browser could not be started: {(res or {}).get('error') or 'agent-browser returned no CDP endpoint'} "
                "Run `x19 tools` → Browser Automation to (re)install Chromium, or switch backends.")
    _set_cdp_env(env, cdp)
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    ...
    (4) the local engine — ``browser.engine: lightpanda`` or X19' packaged Chromium via agent-browser
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Cloud browser provider {provider_name} failed to provide a session: {e}. "
                   "Fix the provider configuration or switch backends via `x19 tools` → Browser Automation."),
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    profile, x19_cli.browser_connect) when consented. Two ways in: the effective backend is already local
