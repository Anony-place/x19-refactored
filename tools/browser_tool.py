...
from x19_constants import get_x19_home, x19_home_key
from utils import env_int
from x19_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every X19 secret from process.env.
...
# than a concrete path (also compared in x19_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``x19_home_key()``): the multiplexed
...
_REAL_PROFILE_SESSION = "x19-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-x19_…``
...
# Owner X19 home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. X19' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = x19_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_x19_dir("cache/screenshots", "browser_screenshots")
