"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to X19.
...
delegates every real request to X19 as its *client-delegation* backend. X19 stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — X19' answer is what the voice speaks.
...
# backend (X19) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are X19, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- X19: a full AI agent with tools — it can run commands, read and edit files, "
