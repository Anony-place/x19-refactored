"""Central registry for all x19 tools: each tool file calls ``registry.register()``
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_x19_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales, so results are cached ~30 s: env-var flips via ``x19 tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "X19_SESSION_ID",
    "X19_BROWSER_CONTROL_PRINCIPAL",
    "X19_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def _core_tools_gated_by(fn: Callable) -> Set[str]:
    """Names of ``_X19_CORE_TOOLS`` members whose registered ``check_fn`` is *fn*."""
    try:
        from toolsets import _X19_CORE_TOOLS
        core = frozenset(_X19_CORE_TOOLS)
    except Exception:
        return set()
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``x19 tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved X19_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("x19_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``x19_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``x19_plugins.pkg.handlers`` is still owned by the
                # ``x19_plugins.pkg`` package — exact string equality would wrongly block root-module
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``x19 tools enable`` lands quickly."""
        result = []
