...
# Sensitive write targets, matched via ~ / $HOME / $X19_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once X19_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_X19_ENV_PATH = (
    r'(?:~\/\.x19/|(?:\$home|\$\{home\})/\.x19/|(?:\$x19_home|\$\{x19_home\})/)' r'\.env\b'
)
# ~/.x19/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_X19_CONFIG_PATH = (
    r'(?:~\/\.x19/|(?:\$home|\$\{home\})/\.x19/|(?:\$x19_home|\$\{x19_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_X19_ENV_PATH}|{_X19_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]x19[^\n]*\.env\b', "access to X19 secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # between `x19` and `gateway` (`x19 -p ade gateway restart`) are allowed so a profile flag can't slip past.
    (r'\bx19\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart x19 gateway (kills running agents)"),
    (r'\bx19\s+update\b', "x19 update (restarts gateway, kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart x19-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart x19-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(x19|gateway|cli\.py)\b', "kill x19/gateway process (self-termination)"),
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.x19.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.x19...'; do
    # launchctl bootout "$label"`) never has "x19" after the verb, and that slipped past and restarted 4 gateways
    ...
    (r'\A(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:x19|ai\.x19)\b)', "stop/restart x19 launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/X19 file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.x19/config.yaml/.env), but cp/mv/install was
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_X19_CONFIG_PATH}|{_X19_ENV_PATH})', "in-place edit of X19 config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/X19 home prefixes to ~/ and ~/.x19/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/X19_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). X19 home
    # first: on Windows it nests under the user home, and folding the user home first would eat the prefix it needs.
    command = _rewrite_resolved_x19_home(command)
...
def _rewrite_resolved_x19_home(command: str) -> str:
    """Resolved X19_HOME (and its realpath) -> ``~/.x19/`` so the _X19_CONFIG_PATH /
    _X19_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.x19")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart x19 gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one X19 ad-hoc temp script."""
    try:
    ...
    return (
        ...
        and re.fullmatch(r"x19-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.x19.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a x19-gateway
