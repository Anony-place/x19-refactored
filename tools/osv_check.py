...
# ladders, parked-server self-probes and repeated `x19 mcp test` runs re-run the preflight
...
# The cache is also persisted under the X19 home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``x19_constants.get_x19_home()`` so the cache follows the
    active profile and is isolated across X19 homes. The cache directory
    is created on demand. Returns ``None`` when X19 home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "x19-osv-check/1.0"},
        method="POST")
