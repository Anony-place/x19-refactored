...
NOT_REGULAR_SENTINEL = "__x19_not_regular__"
...
MISSING_SENTINEL = "__x19_missing__"

_READ_SENTINEL_PREFIX = "__X19_RF_"
_WRITE_SENTINEL_PREFIX = "__X19_WF_"
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        understand the MSYS ``/c/...`` form and X19 disables MSYS argument
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        tmpl = self._escape_shell_arg(".x19-tmp.XXXXXX")
        script = (
            "set -e; "
            ...
            '|| mktemp "$d/.x19-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.x19-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        marker = stdout.find("X19_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("X19_UTF16:OK"):].strip()
        try:
    ...
    def _native_read_enabled(self) -> bool:
        ...
        file_operations holds Git-Bash-style paths there). ``X19_NATIVE_FILE_READ=0``
        turns the fast path off."""
        flag = os.environ.get("X19_NATIVE_FILE_READ", "1").strip().lower()
