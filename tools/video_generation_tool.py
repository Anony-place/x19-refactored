...
Ships **no in-tree provider**: enable a plugin and select it in ``x19 tools`` → Video
    ...
    if configured:
        return json.dumps(error_response(
            error=(f"video_gen.provider='{configured}' is set but no plugin registered that name. "
                   f"Run `x19 plugins list` to see installed video gen backends, or "
                   f"`x19 tools` → Video Generation to pick one."),
            error_type="provider_not_registered", provider=configured))
    return json.dumps(error_response(
        error=("No video generation backend is configured. Run `x19 tools` → "
...
_GENERIC_DESCRIPTION = (
    ...
    "`x19 tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `x19 tools` → Video Generation.")
