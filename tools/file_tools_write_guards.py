...
# resolve per call in production (below) because ``get_x19_home()`` /
...
_x19_config_resolved: str | None = None
_x19_config_resolved_loaded = False
_real_x19_home_cached: str | None = None
_real_x19_home_loaded = False
...
def _x19_home_real() -> str:
    from x19_constants import get_x19_home
    return os.path.realpath(str(get_x19_home()))


def _get_x19_config_resolved() -> str | None:
    """Resolved absolute path of the X19 config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``X19_HOME`` scope (#107327);
    a test may pin it via ``_x19_config_resolved`` + ``_x19_config_resolved_loaded``."""
    if _x19_config_resolved_loaded:
        return _x19_config_resolved
    try:
        ...
        # home rather than the active multiplex ``X19_HOME`` — comparing
        ...
        # ``get_x19_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from x19_constants import get_x19_home
            return str((Path(str(get_x19_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_x19_home() -> str | None:
    """Realpath of the authoritative X19 home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``X19_HOME`` scope (#107327);
    a test may pin it via ``_real_x19_home_cached`` + ``_real_x19_home_loaded``.
    Consumers exempting a whole TREE want ``_x19_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_x19_home_loaded:
        return _real_x19_home_cached
    try:
        return _x19_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_x19_config_resolved``): re-derive from ``get_x19_home()``
        # rather than ``_expand_tilde("~/.x19")`` so the protected-instruction
        ...
        # the consumer: the ``~/.x19`` exemption is skipped, so the gate runs.
        try:
            from x19_constants import get_x19_home
            return os.path.realpath(str(get_x19_home()))
        except Exception:
...
def _x19_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the X19 home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the X19 ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.x19`` component rule, which
    gated them like a project-local ``<repo>/.x19/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.x19`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``X19_HOME`` alone."""
    home = _get_real_x19_home()
    if not home:
        return ()
    try:
        from x19_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    x19_config = _get_x19_config_resolved()
    if x19_config and x19_config in candidates:
        return (
            f"Refusing to write to X19 config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.x19/config.yaml directly or use 'x19 config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .x19 tree).
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.x19 itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.x19`` component rule, which would match the home.
    # ``_x19_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.x19/<file> cannot read as project-local ``.x19`` config.
    for real_home in _x19_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .x19 config dirs (<repo>/.x19/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.x19.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".x19":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side X19 mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.x19" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    X19 state (a write the host never reads). Not profile isolation — that guard was
