...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so X19 tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``x19 gateway restart|stop|uninstall``
    targeting x19-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_X19_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (x19
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``x19 gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of X19. Perform authorized "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `x19 gateway restart` from a "
