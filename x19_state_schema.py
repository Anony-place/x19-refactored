...
Plain mixin for ``x19_state.SessionDB`` (no ``__init__``/state of its own).
Must never import x19_state (cycle); shared constants live in x19_state_common.
"""
...
from x19_state_fts import _drop_orphan_fts_shadow_tables
from x19_state_holders import _read_proc_argv
...
# peer (a supervised service on the same X19_HOME), not a transient one worth waiting out (#106393).
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._x19_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._x19_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _defer_stale_fts_for_holders(self, cursor: sqlite3.Cursor, foreign_holders) -> bool:
        ...
        if futile:
            logger.error(
                ...
                "own retry admits the rebuild within %.0fs of the holder leaving. `x19 doctor` shows this.",
            ...
            logger.error(
                ...
                "processes (this process's own retry then rebuilds), or run `x19 sessions optimize-storage` "
                "with every holder stopped. `x19 doctor` reports this degraded state.", attempts, foreign_holders,
            )
        logger.warning(
    ...
    def _trigram_tokenizer_available(self, cursor: sqlite3.Cursor) -> bool:
        ...
        probe = "temp.x19_fts5_trigram_probe"
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        with contextlib.suppress(Exception):  # missing/corrupt cache → recompute below
            # Late import: resolves a test-patched x19_constants.get_x19_home.
            from x19_constants import get_x19_home as _home
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        # install only gets a flag; `x19 sessions optimize-storage` performs it. The FTS
        ...
        # as before, and we only record a flag advertising that the optimization is available. `x19
    ...
    def _backfill_gateway_metadata_from_sessions_json(self, cursor: sqlite3.Cursor) -> None:
        ...
        sessions_file = get_x19_home() / "sessions" / "sessions.json"
