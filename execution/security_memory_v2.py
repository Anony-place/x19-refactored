...
class SecurityMemorySnapshot:
    ...
    def to_prompt(self, max_chars: int = 12000) -> str:
        """Build complete JSON records within budget; never truncate JSON mid-object."""
        ...
        if len(encoded) <= max_chars:
            return encoded
        return json.dumps({"target": self.target, "facts": [], "hypotheses": [],
                           "evidence": [], "dead_ends": [], "lessons": [],
                           "priorities": [], "relations": [], "recent_attempts": []},
                          ensure_ascii=False, separators=(",", ":"))


class SecurityMemory:
    """Canonical mission memory inspired by the strongest offensive-agent patterns.

    SQLite is authoritative. The kernel tracks structured world state, causal
    evidence/hypothesis links, episodic attempts, failure lessons, resumable
    checkpoints and graph relationships. Semantic/vector stores remain optional
    ...
    def upsert(self, target: str, kind: str, key: str, value: Dict[str, Any], *,
        ...
        with self._lock:
            row = self._conn.execute(
                "SELECT item_id, created_at FROM memory_items WHERE target=? AND kind=? AND item_key=?",
            ...
            self._conn.execute("""INSERT INTO memory_items
                ...
                (item_id, target, kind, key, json.dumps(value, ensure_ascii=False),
                 max(0.0, min(1.0, confidence)), status, source, created_at, now))
    ...
    def get(self, target: str, kind: str, key: str) -> Optional[Dict[str, Any]]:
        with self._lock:
            row = self._conn.execute("SELECT * FROM memory_items WHERE target=? AND kind=? AND item_key=?",
                                     (target, kind, key)).fetchone()
        return self._row(row) if row else None

    def list(self, target: str, kind: str, *, status: str = "active", limit: int = 100) -> List[Dict[str, Any]]:
        with self._lock:
            rows = self._conn.execute("SELECT * FROM memory_items WHERE target=? AND kind=? AND status=? "
                                      "ORDER BY updated_at DESC LIMIT ?",
                                      (target, kind, status, limit)).fetchall()
    ...
    def relate(self, target: str, from_id: str, relation: str, to_id: str, *, confidence: float = 1.0) -> str:
        ...
        with self._lock:
            ...
            self._conn.commit()
            row = self._conn.execute("SELECT relation_id FROM memory_relations WHERE target=? AND from_id=? AND relation=? AND to_id=?",
    ...
    def record_failure_lesson(self, target: str, action_key: str, *, level: str,
        ...
        aid = self.record_attempt(target, action_key, outcome=reason, status="failed",
                                  failure_level=level)
        self.upsert(target, "lesson", f"failure:{action_key}:{level}",
                    {"lesson": lesson, "category": f"failure:{level}"}, confidence=0.8)
        return aid
    ...
    def snapshot(self, target: str, *, limits: Optional[Dict[str, int]] = None) -> SecurityMemorySnapshot:
        limits = limits or {}
        with self._lock:
            attempts = self._conn.execute("SELECT * FROM memory_attempts WHERE target=? ORDER BY created_at DESC LIMIT ?",
                                          (target, limits.get("attempts", 20))).fetchall()
        return SecurityMemorySnapshot(
            target=target,
            facts=self.list(target, "fact", limit=limits.get("facts", 40)),
            hypotheses=self.list(target, "hypothesis", limit=limits.get("hypotheses", 20)),
            evidence=self.list(target, "evidence", limit=limits.get("evidence", 30)),
            dead_ends=self.list(target, "dead_end", status="closed", limit=limits.get("dead_ends", 20)),
            lessons=self.list(target, "lesson", limit=limits.get("lessons", 20)),
            priorities=self.list(target, "priority", limit=limits.get("priorities", 20)),
