 ...
 *
 * The root ``package.json`` is installed by ``x19 update`` on every user,
 ...
 *   workspace graph entirely. ``x19 update`` and ``x19 doctor --fix``
 ...
 *   ``x19 tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``x19 update`` for
 ...
 * fails — read the lazy-install guidance in the ``x19-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in x19_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`x19 tools` → Browser Automation → Camofox.'
  )
})
