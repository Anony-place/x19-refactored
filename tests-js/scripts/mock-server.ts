 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `x19 serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. X19 should add
...
export const APPROVAL_COMMAND = 'rm -rf /tmp/x19-e2e-approval-probe'
 ...
 * `E2E_SAY(code-farmer)[{at}x19 Reply with B.] E2E_SAY(x19)[B]`.
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `x19-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated X19_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `x19-dev-mock-${Date.now()}`))
  const x19Home = nodePath.join(root, 'x19-home')
...
function writeMockConfig(x19Home: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(x19Home, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(x19Home, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.x19Home, mock.url)
  console.log(`  X19_HOME: ${sandbox.x19Home}`)
  ...
  const env: Record<string, string> = {
    ...
    X19_DESKTOP_X19_ROOT: repoRoot,
    X19_DESKTOP_APP_NAME: `X19DevMock-${Date.now()}`,
  }
