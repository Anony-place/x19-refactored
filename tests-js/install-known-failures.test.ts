...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/x19.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/x19.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'x19-update',
  error: 'E2E ASSERTION FAILED: x19 update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('x19.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'x19-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured x19 desktop spec; update completed',
      logs: { desktop: '[x19] [updates] no staged updater; surfacing manual `x19 update` for CLI install at C:/install\n[x19] [updates] manual: x19 update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: x19 update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> x19-update (v2026.3.12 -> HEAD)'
