...
from x19_constants import get_x19_home, mkdir_under_x19_home
...
from x19_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from x19_state_sessions import SessionSessionsMixin
from x19_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from x19_state_portability import SessionPortabilityMixin
from x19_state_telegram import SessionTelegramTopicsMixin
from x19_state_schema import SessionSchemaMixin
import x19_state_holders as _state_holders
import x19_state_lockguard as _lockguard
from x19_state_dbfile import (
...
from x19_state_messages import SessionMessagesMixin
from x19_state_rewind import SessionRewindMixin
from x19_state_wal import (
    _WAL_INCOMPAT_MARKERS, _on_disk_journal_mode, apply_database_pragmas, apply_wal_with_fallback,
)
from x19_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from x19_state_titles import SessionTitlesMixin
from x19_state_usage import SessionUsageMixin
from x19_state_maintenance import SessionMaintenanceMixin
from x19_state_gateway import SessionGatewayMixin
from x19_state_compression import SessionCompressionMixin
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_x19_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_x19_home()`` is resolved fresh (a runtime X19_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_x19_home() / "state.db"


# Live-DB guard knobs live HERE (not in x19_state_guard): the hermetic conftest monkeypatches
...
def format_session_db_unavailable(
    ...
    *,
    ...
    if details:
        from x19_state_user_copy import storage_failure_details
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    ...
    if not sid or not messages:
        return None
    sessions_dir = get_x19_home() / "sessions"
...
# connection per resolved path via x19_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in x19_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via x19_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        self._wal_lock_guard: dict = {}  # x19_state_lockguard.hold() record, see _open_writer
        ...
        # Opened via x19_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via x19_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        mkdir_under_x19_home(self.db_path.parent)
    ...
    def _settle_lost_generation_locked(self) -> bool:
        ...
        through x19_state_registry.release_or_close, which swallows the error, so an interpreter
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see x19_state_holders)."""
    ...
    def close(self):
        ...
        if self._shared_registry_owned:
            from x19_state_registry import release
            release(self)
            return
