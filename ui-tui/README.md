...
React + Ink terminal UI for X19. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `X19_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `x19 setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.x19/.x19_history` or under `X19_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `x19 setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/x19-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external x19 setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.x19/tui-parent.log
    ...
    types/
      x19-ink.d.ts            type declarations for @x19/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent X19CLI subprocess for slash commands
```
