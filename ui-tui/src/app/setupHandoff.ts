import type { RunExternalProcess } from '@x19/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`x19 ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching x19: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`x19 ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
