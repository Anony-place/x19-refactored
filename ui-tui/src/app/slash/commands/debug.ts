...
import { terminalBackgroundHex } from '@x19/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $X19_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see X19_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['X19_TUI_BACKGROUND', process.env.X19_TUI_BACKGROUND ?? '(unset)'],
            ['X19_TUI_THEME', process.env.X19_TUI_THEME ?? '(unset)'],
