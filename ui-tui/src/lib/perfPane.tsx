...
//
// Both gate on X19_DEV_PERF=1 and dump JSON-lines (default ~/.x19/perf.log,
// override X19_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// X19_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@x19/ink'
import { scrollFastPathStats } from '@x19/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.X19_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.X19_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.X19_DEV_PERF_LOG?.trim() || join(homedir(), '.x19', 'perf.log')
