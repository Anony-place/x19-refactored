 ...
 * `x19-boot-background` / `x19-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports X19_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.x19 matches get_x19_home()'s default.
const bootFilePath = () => join(process.env.X19_HOME ?? join(homedir(), '.x19'), 'tui-theme-boot.json')

// Never touch the user's real ~/.x19 from test runs (the TS suite has no
// X19_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into X19_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.X19_TUI_THEME || env.X19_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.X19_TUI_BACKGROUND
  ) {
    env.X19_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.X19_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.X19_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into X19_TUI_THEME.
