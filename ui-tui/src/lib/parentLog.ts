...
const logDir = join(process.env.X19_HOME?.trim() || join(homedir(), '.x19'), 'logs')
...
// a real ~/.x19 (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('x19-tui: parent lifecycle log unavailable\n')
    }
  }
}
