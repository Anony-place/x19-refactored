import { Box, Text, useStdout } from '@x19/ink'
...
const TAG_FULL = 'Multi-Agent Orchestration'
const TAG_MID = 'Orchestration'
const TAG_TINY = 'X19'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  // x19_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · X19</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'x19 update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
