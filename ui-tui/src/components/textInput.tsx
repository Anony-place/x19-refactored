import type { InputEvent, Key } from '@x19/ink'
import * as Ink from '@x19/ink'
 ...
 * but that accepts `key.meta` on macOS — and x19-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.X19_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which x19-ink decodes as meta+'d'; without this branch it
