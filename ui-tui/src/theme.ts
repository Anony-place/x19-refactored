import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@x19/shared/color'
import type { SkinBranding, SkinColors } from '@x19/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @x19/shared/color (the primitives,
...
// Mirrors @x19/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @x19/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic X19 navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode X19 look was never hand-authored: for years the
 ...
 *  answer when known (cached in X19_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.X19_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit X19_TUI_THEME / COLORFGBG signals above still win,
...
// `X19_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `X19_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `X19_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `X19_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default X19 palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.X19_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.X19_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.X19_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into X19_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into X19_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (X19 navy
  ...
  // the derived value already carries the identity seeds (e.g. X19 navy).
  const activeRow =
