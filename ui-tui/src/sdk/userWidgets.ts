...
/**
 * User widget apps — X19 authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$X19_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.x19/plugins/`: files under X19_HOME execute
...
const widgetsDir = () => join(process.env.X19_HOME?.trim() || join(homedir(), '.x19'), 'tui-widgets')
 ...
 *  every change, so a widget X19 writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or X19) ever writes must hot-load too — a 10s poll here read
