...
import { parseSlashCommand } from '@x19/shared/slash'
  ...
  try {
    const names = JSON.parse(
      execFileSync(
        ...
        [
          '-c',
          'import json; from x19_cli.commands import COMMAND_REGISTRY; print(json.dumps([c.name for c in COMMAND_REGISTRY]))'
        ],
