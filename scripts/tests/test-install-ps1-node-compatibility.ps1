...
# changing PATH, installing software, or touching the user's X19 home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("x19-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$X19Home = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -X19Home $X19Home -InstallDir $InstallDir
