...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[x19] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[x19`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[x19`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        X19Home = $(if ($paths) { $paths.x19_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\x19\x19'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-x19'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-x19') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
