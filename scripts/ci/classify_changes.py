...
# tree generates llms.txt — the index every LLM (X19 included, via the
# x19 skill) reads to learn what X19 can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/x19_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"x19_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (X19-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``x19 serve`` backend, the Docker image)
