# Behavioral test for install.ps1's x19 launcher staging (PR #92092,
...
#
# The test lifts the real Install-X19CommandLaunchers function from the
...
# the caller (Set-PathVariable passes $X19Home\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-X19CommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-X19CommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'x19-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'x19'
    ...
    $missingThrew = $false
    try {
        Install-X19CommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing x19.exe fails the launcher stage'
    ...
    $x19V2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'x19.exe'), $x19V1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-X19CommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $x19V1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'x19.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'x19-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'x19.exe'), $x19V2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'x19-acp.exe'), $acp)
    Install-X19CommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $x19V2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'x19.exe'))) `
        'installer refreshes an existing X19 launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'x19-acp.exe'))) `
    ...
    Install-X19CommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'x19.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'x19.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'x19.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'x19.exe')) -and $cmdBody.Contains('%*')) `
