...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$X19Home = $(if ($env:X19_HOME) { $env:X19_HOME } else { "$env:LOCALAPPDATA\x19" }),
    [string]$InstallDir = $(if ($env:X19_HOME) { "$env:X19_HOME\x19" } else { "$env:LOCALAPPDATA\x19\x19" }),
    ...
    # builds apps/desktop into a launchable X19.exe.
    #
    # Why opt-in:
    #   * X19-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched X19.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live X19.exe
    ...
    #     `x19 desktop` already builds on demand.
...
# X19_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[x19] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'X19Install.LongPath').Type) {
            Add-Type -Namespace 'X19Install' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [X19Install.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -X19Home / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('X19Home')) {
    $X19Home = ConvertTo-LongPath $X19Home
} else {
    $X19Home = ConvertTo-LongPath $(
        if ($env:X19_HOME) { $env:X19_HOME } else { "$env:LOCALAPPDATA\x19" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:X19_HOME) { "$env:X19_HOME\x19" } else { "$env:LOCALAPPDATA\x19\x19" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: X19Home=$X19Home InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:Anony-place/x19-refactored.git"
$RepoUrlHttps = "https://github.com/Anony-place/x19-refactored.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * X19 Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $X19Home)) {
        New-Item -ItemType Directory -Force -Path $X19Home | Out-Null
    }
    $envFile = Join-Path $X19Home ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `x19 update`'s npx
    ...
    $prefixDir = Join-Path $X19Home "node"
...
function Install-Uv {
    # X19 owns its own uv at $X19Home\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (x19_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `x19 update` stay in sync.
    $managedUv = Join-Path $X19Home "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $X19Home\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $X19Home "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $X19Home\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $X19Home "bin"
        ...
        # (x19_cli/managed_uv.py looks only at $X19Home\bin\uv.exe).
...
# Put the X19-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone x19-setup.exe run, a user typing `npm`) silently
...
# Upgrade the X19-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "X19-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("x19-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and X19_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by X19 belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # x19_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".x19-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first X19-managed interpreter
    ...
    #
    # Under X19-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve X19-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun X19 setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which X19 uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\x19\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    X19 needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the X19 copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\x19\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``X19_GIT_BASH_PATH`` (User scope) so X19 can find it in a fresh
    ...
    Write-Info "Git not found -- downloading PortableGit to $X19Home\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent X19 features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  X19 needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``X19_GIT_BASH_PATH`` (User env scope) so X19 can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $X19Home\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $X19Home\git\usr\bin\bash.exe
    $candidates += "$X19Home\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$X19Home\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("X19_GIT_BASH_PATH", $candidate, "User")
            $env:X19_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set X19_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- X19 may not find Git Bash."
    Write-Info "If needed, set X19_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the X19-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (X19 requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy X19 requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$X19Home\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$X19Home\node"
        Write-Success "Node.js $version found (X19-managed)"
        ...
        Update-ManagedNpm "$X19Home\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing X19-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $X19Home\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $X19Home\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\x19-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$X19Home" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$X19Home" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$X19Home\node") {
                    try {
                        Rename-Item "$X19Home\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "X19-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$X19Home\node\node.exe" --version
                Write-Success "Node.js $version installed to $X19Home\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\x19-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `x19
                    # update` path (#4735).
                    ...
                    $stashName = "x19-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. x19-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``x19 update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if X19 behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running x19 processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Anony-place/x19-refactored/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Anony-place/x19-refactored/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Anony-place/x19-refactored/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\x19-$zipLabel.zip"
                $extractPath = "$env:TEMP\x19-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `x19 update` (see the notes at the shared clone-path
    ...
    # next `x19 update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under X19-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "X19-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running x19 process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running x19 processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*X19_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (x19.exe) plus its child tree.
            & taskkill /F /T /IM x19.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM x19.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m x19_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not x19.exe.
            ...
            # X19 children re-exec through .x19-runtime, so killing only
            ...
            # user (see x19_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-X19 " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `x19 update` / the
...
function Restore-VenvBackup {
    ...
    # venv so X19 (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `x19.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install x19 package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and x19.exe broken with
    ...
    # here, not 30 seconds later when the user runs `x19`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close X19 processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close X19 processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves X19 and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m x19_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `x19 dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\x19_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `x19 dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `x19 dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: x19_cli/web_server.py"
        }
    }
...
function Install-X19CommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the x19 launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring x19.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "x19.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the x19 command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # x19_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("x19", "x19-acp")) {
    ...
    $requiredExe = Join-Path $Destination "x19.exe"
    $requiredCmd = Join-Path $Destination "x19.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the x19 command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up x19 command..."
    
    if ($NoVenv) {
        $x19Bin = "$InstallDir"
    } else {
        # $X19Home\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `x19 update`'s autostash
        ...
        # installer staged under x19\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-X19CommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $x19Bin = "$X19Home\bin"
        Install-X19CommandLaunchers -Root $InstallDir -Destination $x19Bin | Out-Null
    }
    ...
    #   x19\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The x19\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept x19 via $x19Bin)"
        }
    }
    
    if ($currentPath -notlike "*$x19Bin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$x19Bin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $x19Bin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set X19_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\x19 instead
    # of the Unix default ~/.x19
    $currentX19Home = [Environment]::GetEnvironmentVariable("X19_HOME", "User")
    if (-not $currentX19Home -or $currentX19Home -ne $X19Home) {
        [Environment]::SetEnvironmentVariable("X19_HOME", $X19Home, "User")
        Write-Success "Set X19_HOME=$X19Home"
    }
    $env:X19_HOME = $X19Home
    
    # Update current session
    $env:Path = "$x19Bin;$env:Path"
    
    Write-Success "x19 command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.x19-bootstrap-complete which tells the X19
    ...
    $markerPath = Join-Path $InstallDir ".x19-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the X19_HOME directory structure ($X19Home, default %LOCALAPPDATA%\x19)
    New-Item -ItemType Directory -Force -Path "$X19Home\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$X19Home\skills" | Out-Null
    ...
    # Create config.yaml
    $configPath = "$X19Home\config.yaml"
    ...
    # (the default PS5 behaviour), and X19's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$X19Home\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in x19_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $X19Home"
    
    # Seed bundled skills into $X19Home\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $X19Home\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $X19Home\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$X19Home\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $X19Home\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (X19-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'x19 setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\x19-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\x19-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\x19-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `x19 tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $X19Home "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only X19' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside X19' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'x19 tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `x19
...
# paths still lazy-install via install_cua_driver() (x19_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `x19 computer-use install` runs. Bounded
        ...
        # X19' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'x19 tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: x19 computer-use install"
            }
        } else {
...
# the final `electron` -> `X19` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable X19.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_x19_desktop command
    # resolves apps/desktop/release/win-unpacked/X19.exe directly,
    ...
    # enforces the supported Node lines and prepends the X19-managed Node to
    ...
    # The --dir mode produces an unpacked X19.exe in
    ...
    # are NOT trying to work around). The X19 icon + product name are
    # stamped onto X19.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\x19-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\X19.exe",
        "$desktopDir\release\win-arm64-unpacked\X19.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no X19.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The X19 icon + identity are stamped onto X19.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, x19#38216).
    ...
    #    X19.exe. We deliberately do NOT point them at `x19 desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'X19.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'X19.lnk')
        )
        ...
        # with the X19 icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.x19/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'x19 setup'."
        return
    }
    ...
    # Run x19 setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m x19_cli.main setup
    } else {
        python -m x19_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $x19Cmd = "$InstallDir\venv\Scripts\x19.exe"
    if (-not (Test-Path $x19Cmd)) {
        $x19Cmd = "x19"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$X19Home\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'x19 whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: x19 gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$X19Home\logs\gateway.log"
            Start-Process -FilePath $x19Cmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$X19Home\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: x19 gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: x19 gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$X19Home\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$X19Home\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$X19Home\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$X19Home\x19\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   x19              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   x19 setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   x19 config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   x19 config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   x19 gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   x19 update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning X19 repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.x19-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (X19-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding X19 to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
