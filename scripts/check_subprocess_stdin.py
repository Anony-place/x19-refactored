...
When X19 runs in TUI mode, the gateway child process communicates with
...
# ``get_x19_home() / "plugins"`` (user) and ``./.x19/plugins/`` (project,
# gated behind ``X19_ENABLE_PROJECT_PLUGINS``) — see
# ``x19_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``x19_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import x19_constants (this script
    ...
    # plugins in get_x19_home()/plugins/ and project plugins in
    # ./.x19/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_x19_home() / "plugins"]
    if os.environ.get("X19_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".x19" / "plugins")
