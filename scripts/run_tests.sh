#!/usr/bin/env bash
# Canonical test runner for x19. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.x19/...)
#
# Usage:
...
# (X19_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.x19/x19/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.x19/x19/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${X19_PYTHON:-}" ] && [ -x "$X19_PYTHON" ] \
    && "$X19_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: X19_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `x19` binary rather
  # than the devShell hook.
  PYTHON="$X19_PYTHON"
  echo "▶ no local venv — using Nix dev venv via X19_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and X19_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.x19/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.x19"
...
#
#   * X19_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * X19_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     X19_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# X19_RUN_SLOW_PET_TESTS / X19_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no X19_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in X19_TEST_IMAGE X19_TEST_WORKERS X19_TEST_PATHS \
  X19_TEST_FILE_TIMEOUT X19_TEST_FILE_RETRIES X19_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${X19_RUN_SLOW_PET_TESTS:+X19_RUN_SLOW_PET_TESTS="$X19_RUN_SLOW_PET_TESTS"} \
  ${X19_E2E_BROWSER:+X19_E2E_BROWSER="$X19_E2E_BROWSER"} \
