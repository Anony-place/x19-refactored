...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^X19\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (x19-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "x19@nousresearch.com",
    "x19-audit@example.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "Anony-place/x19-refactored",
                "--state", "merged",
