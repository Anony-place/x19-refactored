...
# (%X19_HOME%\x19-setup.exe). That binary has no self-update path --
# copy_self_to_x19_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `x19 update` refreshes
...
#     -InstallRoot <path>   repo checkout (X19_HOME\x19)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] X19.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .x19-update-in-progress in place (tests)
#
...
# .x19-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim X19_HOME\.x19-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining X19_UPDATE_STARTED_AT from the Desktop hand-off.
# x19_cli/update_lock.py's ancestry rule lets our
# `x19 update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new X19.exe pid.
try {
    Add-Type -Namespace X19Handoff -Name Win32 -MemberDefinition @'
...
$X19Home = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $X19Home ".x19-update-in-progress"
$LogDir = Join-Path $X19Home "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $X19Home ".x19-update-result.json"
$script:Ui = $null
$script:UiStage = "X19 will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale x19-update-ui-* leftovers from interrupted
    ...
    # update marker (.x19-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "x19-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-X19Step and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("x19-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [X19Handoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `x19 debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"x19 debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [X19Handoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old X19.exe before writing the replacement,
    ...
    # DO NOT spawn X19.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [X19Handoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [X19Handoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [X19Handoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh X19 process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [X19Handoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `x19 update` deliberately runs its build steps with stdout
# inherited (x19_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:X19_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:X19_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:X19_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:X19_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `x19 update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (x19_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:X19_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:X19_UPDATE_PROGRESS_LOG
}
...
public static class X19UpdateJob {
...
function Invoke-X19Step([string]$Exe, [string[]]$X19Args, [string]$Tag) {
    ...
    # afterwards, where `x19 debug share` picks it up.
    #
    ...
    # .x19-update-result.json, clearing .x19-update-in-progress,
    ...
    # X19" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($X19Args | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [X19UpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `x19 update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [X19UpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
function Resolve-X19VenvDir([string]$Root) {
    # Match x19_constants.project_venv_dir(): installer-created venv wins,
...
# the page can be QA'd in any browser; X19_SELFTEST_FAIL=1 exercises the
# error state, X19_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:X19_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:X19_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:X19_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-X19Step survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no X19 install to
...
#            update log, the shape of every real `x19 update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:X19_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:X19_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:X19_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:X19_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "x19-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "x19-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "x19-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "x19-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "x19-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "x19-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "x19-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "x19-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `x19 update` is this shape -- the Electron/vite build
...
'@
    ...
    $flood = Invoke-X19Step $powershell @(
    ...
    $stall = Invoke-X19Step $powershell @(
    ...
    try {
        $logstall = Invoke-X19Step $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-X19Step returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-X19Step returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:X19_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from X19_HOME; pin the process directory to the
    ...
    $VenvDir = Resolve-X19VenvDir $InstallRoot
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-X19Step $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for X19 to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the X19 window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close X19 fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing X19 files"
    $shim = Join-Path $VenvDir "Scripts\x19.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the X19 install open ($shim locked after 20s). Nothing was changed. Close other X19 windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the x19.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\x19.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_x19_exe must first rename the running x19.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that x19.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second X19 window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at an X19.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m x19_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the X19 installer or `x19 doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "x19_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all X19 windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m x19_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed x19 predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-X19Step $pythonExe $updateArgs "update"
    Write-HandoffLog "x19 update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-X19UpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `x19 update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "x19 update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-X19Step $pythonExe @("-m", "x19_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import x19_cli.main; from x19_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-X19Step $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated X19 runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `x19 desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `x19 debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "X19 is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. X19 was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen X19."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening X19"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen X19 to finish (it could not restart itself)."
