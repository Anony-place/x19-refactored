...
run_py() {
  ...
  if [ "$with_var" = yes ]; then
    PYTHONPATH="$CAP_DIR${PYTHONPATH:+:$PYTHONPATH}" \
      X19_E2E_CAPTURE_LAUNCH="$WORK/spec.json" python3 "$@"
  else
...
run_py yes -c '
import subprocess
r = subprocess.run(["npm", "exec", "--", "electron", "."], cwd="/tmp", env={"X19_DESKTOP_CWD": "/tmp", "PATH": "/usr/bin"})
...
assert spec["env"]["X19_DESKTOP_CWD"] == "/tmp", "env= kwarg not captured"
...
run_py yes -c '
import subprocess
exe = "/x/apps/desktop/release/linux-unpacked/X19"
...
run_py yes -c '
import subprocess
r = subprocess.run(["C:\\x\\apps\\desktop\\release\\win-unpacked\\X19.exe"], env={})
