"""Run a real X19 CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_x19_executable(x19_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "x19",
        Path(".venv") / "Scripts" / "x19.exe",
    ):
        candidate = x19_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("x19")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "X19 executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="X19 source checkout containing .venv/bin/x19",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated X19_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["x19.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['x19.client.active']}"
        )
    [model] = by_name["x19.model_route.count"]
    expected_model = {
        "name": "x19.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['x19.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["x19.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['x19.task_run.started']}"
        )
    [terminal] = by_name["x19.task_run.finished"]
    ...
    [tool] = by_name["x19.tool_call.count"]
    ...
    lifecycle = by_name["x19.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["x19.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The X19 development environment requires jsonschema"
        ) from exc
    ...
    if metrics["x19.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['x19.client.active']}"
        )
    [model] = metrics["x19.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['x19.model_route.count']}"
        )
    [terminal] = metrics["x19.task_run.finished"]
    ...
    [tool] = metrics["x19.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["x19.skill.lifecycle.count"]
    ...
    loads = metrics["x19.skill.load.count"]
    if {
        (
...
def main() -> int:
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="x19-relay-shared-metrics-"))
    home = root / "x19-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["X19_HOME"] = str(home)
        python_paths = [str(x19_repo)]
    ...
    (root / "x19.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "x19.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"X19 exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("X19 model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("X19 did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("X19 did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "x19.shared_metrics.v2.schema.json",
    )

    print("X19 -> NeMo Relay shared-metrics smoke test passed")
