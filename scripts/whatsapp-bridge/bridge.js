 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.x19/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.x19', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (X19_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.X19_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.x19', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.X19_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.x19', 'document_cache');
const AUDIO_CACHE_DIR = process.env.X19_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.x19', 'audio_cache');
...
// `x19 update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '☤ *X19*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls X19 itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into X19 and trigger
      ...
      // Ignore X19' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `x19_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `x19_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
