"""Centralized logging setup for X19.
...
# in X19 (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from x19_constants import get_config_path, get_x19_home, mkdir_under_x19_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_x19_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.x19_home = str(get_x19_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.x19_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._x19_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``x19 logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "x19_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("x19_cli", "cli"),
    "cron": ("cron",),
    "gui": ("x19_cli.web_server", "x19_cli.pty_bridge", "x19_cli.desktop", "tui_gateway", "uvicorn"),
}
...
def setup_logging(
    *,
    ...
    """Configure the X19 logging subsystem; returns the ``logs/`` directory.
    ...
    home = x19_home or get_x19_home()
    log_dir = mkdir_under_x19_home(home / "logs")
    # A second X19 home in a process that already logs for another one — a dashboard or
    # ``x19 serve`` backend building agents for several profiles, a multiplexed gateway —
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_x19_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._x19_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from x19_cli.config import is_managed
        ...
        if _is_unavailable_log_stream(exc):
            ...
            if not self._unavailable_reported:
                ...
                _quietly(lambda: print(
                    f"x19_logging: {self.baseFilename} unavailable ({exc}); "
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their X19 home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._x19_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "x19_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._x19_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="x19-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_x19_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses x19_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare x19_logging consumers.
        try:
