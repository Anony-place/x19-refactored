...
"the code now lives under ``X19_HOME/plugins/<name>``". Two hooks call :func:`migrate_home`:

* ``x19 update`` — for every profile home that shares the venv (primary; runs where the venv was
  just rebuilt anyway).
* agent init — when the configured provider cannot be found at all, once per process (Desktop
  users update through the app and never run ``x19 update`` by hand).
...
def configured_provider(home: Path) -> str:
    ...
    from x19_cli.plugin_python_deps import _read_home_config
    memory = _read_home_config(home).get("memory") or {}
    return str(memory.get("provider") or "").strip()


def provider_present(name: str) -> bool:
    """True when the provider resolves anywhere X19 looks (bundled, user dir, entry point)."""
...
def catalog_source(name: str) -> Optional[str]:
    ...
    from x19_cli.plugin_catalog import get_live_catalog_entry
...
def migrate_home(home: Path, *, install: Callable[[str], dict], say: Callable[[str], None] = print) -> Optional[str]:
    ...
    if catalog_source(name) is None:
        say(f"  ⚠ Memory provider '{name}' is configured but not installed and not in the plugin catalog. "
            f"Install it with `x19 plugins install <source>` or change memory.provider.")
        return None
    try:
        result = install(name)
    ...
    say(f"  ⚠ Memory provider '{name}' moved out of core and could not be installed automatically: "
        f"{result.get('error') or 'unknown error'}. Run `x19 plugins install {name}`.")
    return None
...
def migrate_all_homes(*, say: Callable[[str], None] = print) -> list[str]:
    """``x19 update`` hook: every profile home sharing this venv. Returns installed plugin names."""
    from x19_cli.plugin_python_deps import dependency_homes
    installed: list[str] = []
...
def recover_at_startup(name: str) -> bool:
    ...
    if not _allow_lazy_installs():
        logger.warning("Memory provider '%s' is not installed; security.allow_lazy_installs is off — "
                       "run `x19 plugins install %s`.", name, name)
        return False
