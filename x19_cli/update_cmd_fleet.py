"""Gateway fleet restart + post-update verification for ``x19 update``.

Split out of ``x19_cli/update_cmd.py``; every name is re-imported there so
``x19_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from x19_cli.update_cmd_common import _best_effort
from x19_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("x19_cli.update_cmd")

# Under X19_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "x19-gateway*", "x19-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from x19_cli.update_cmd import get_x19_home
    path = get_x19_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """X19_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from x19_cli.update_cmd import get_x19_home
    return get_x19_home() / _FLEET_RESTART_PENDING_NAME


def _write_fleet_restart_pending_marker(*, expected_sha: str = "") -> None:
    """Drop the pull→restart obligation breadcrumb. Never raises."""
    from x19_cli.update_cmd import _m
...
def _current_checkout_sha() -> str | None:
    ...
    from x19_cli.update_cmd import _capture_head_sha, _m
    try:
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``x19 update`` retriggers
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from x19_cli.update_cmd import _current_checkout_sha
    try:
        from x19_cli.update_receipt import read_latest_receipt
...
def _live_fleet_covers_receipt(expected_sha: str | None, *, accept_states: tuple = ("current",)) -> bool:
    ...
    if not expected_sha:
        return False
    from x19_cli.update_receipt import collect_fleet_versions
...
def _pending_fleet_restart_needed() -> bool:
    ...
    Catch-up semantics (``x19 update`` on a current checkout): the fleet must reach the
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `x19 update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `x19 update` or `x19 gateway restart`.", file=stream)
...
def _restart_systemd_gateway_units_best_effort(failed: list, listings) -> None:
    """Best-effort ``systemctl restart`` of every x19-gateway/serve unit."""
    answered = set()
...
def _run_pending_fleet_restart() -> bool:
    ...
    # Warn if legacy X19 gateway unit files are still installed. When both x19.service (from a
    # pre-rename install) and the current x19-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `x19 update` surfaces the issue until the
    # user migrates.
    try:
        from x19_cli.gateway import (
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all x19-gateway* units (default + profiles) plus
        # x19-serve* units (the Desktop app's backend, #83438).
        if systemd_listings is not None:
            _restart_systemd_gateway_units_best_effort(failed, systemd_listings)
        # --- Launchd services (macOS) --- Restart EVERY ai.x19.gateway* LaunchAgent, not only the
...
def _defer_fleet_restart_after_update(*, update_complete: bool, resume_incomplete: bool = False) -> None:
    ...
    ``x19 update --no-gateway-restart`` (cron running inside the gateway's
    ...
    next normal update (or ``x19 gateway restart``) catches up.
    ...
    print("  Restart them separately: `x19 gateway restart` or a daily-restart cron.")
    print("  (fleet restart deferred — marker kept for catch-up)")
    with suppress(Exception):
        from x19_cli.update_receipt import record_skip
        record_skip("gateway_restart", "--no-gateway-restart: deferred, marker kept")
    partial = (not update_complete) or resume_incomplete
    with suppress(Exception):
        from x19_cli.update_receipt import finalize_update_receipt
...
def _apply_pending_fleet_restart_catchup(*, defer: bool = False) -> None:
    """On an already-up-to-date ``x19 update``, finish a skipped restart.
    ...
    """
    from x19_cli.update_cmd import _run_pending_fleet_restart
    ...
    if defer:
        ...
        print("  Restart separately: `x19 gateway restart` or next non-cron update.")
        return
    print()
    _warn_pending_fleet_restart()
    ...
    if _run_pending_fleet_restart():
        _clear_fleet_restart_pending_marker()
        return
    print("  ⚠ Fleet restart incomplete. Recover with: x19 gateway restart")
    sys.exit(1)
...
def _is_x19_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("x19-serve")``
    would accept ``x19-server.service``."""
    return (
        ...
        unit == "x19-gateway.service"
        or unit.startswith("x19-gateway-")
        or unit == "x19-serve.service"
        or unit.startswith("x19-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each x19-gateway*/x19-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_x19_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``x19-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``x19-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``x19-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "x19-gateway" or svc_name.startswith("x19-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from x19_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    print("    x19 gateway status")
    if any(not name.startswith("ai.x19.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.x19.") for name in ordered):
...
def _restart_launchd_gateway_after_update(*, supervision_verify: bool = True) -> tuple[list, list]:
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: x19 gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: x19 gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.x19.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``x19_cli.gateway`` no longer importing
    ...
    """
    try:
        from x19_cli.gateway import find_gateway_pids
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither X19_SPAWN
# nor X19_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a x19-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``x19_cli.gateway`` in the
    ...
    child's ``x19-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``x19_cli.gateway`` from the freshly
...
def _gateway_home_for_pid(pid: int):
    """X19_HOME of the gateway ``pid`` per the fleet inventory, else None (own profile's file)."""
    with suppress(Exception):
        from x19_cli.update_receipt import _profile_homes
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to x19-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. x19-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    if _manage_cmd is None:
        ...
        print(
            ...
            f"    To let `x19 update` restart it automatically, allow\n"
    ...
    # budget, restart-policy mismatch). Mirrors `x19 gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active x19-gateway*/x19-serve* systemd unit (user + system).
    ...
    """
    from x19_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
    ...
    def _on_list_timeout(scope: str, exc: subprocess.TimeoutExpired) -> None:
        ...
        print(
            ...
            f"Check the gateway with: x19 gateway status"
        )
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from x19_cli.update_receipt import record_gateway_restart
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            print("    Restart manually: x19 gateway run")
            if unmapped_count > 1:
                print("    (or: x19 -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from x19_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from x19_cli.update_abort_recovery import _owed_stale_serve_rows
    from x19_cli.update_cmd import (
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from x19_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/x19-serve`` vs ``system/x19-serve`` are different
    ...
    try:
        ...
        from x19_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from x19_cli.gateway import _get_restart_exit_wait_budget
        ...
        # macOS: EVERY ai.x19.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy x19.service fights x19-gateway.service over the bot token; warn on
    every update until migrated."""
    from x19_cli.gateway import (has_legacy_x19_units, _find_legacy_x19_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_x19_units()):
        return
    print()
    print("⚠ Legacy X19 gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_x19_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (x19.service) fight the current")
    print("  x19-gateway.service for the bot token and cause SIGTERM")
    print("  flap loops. Remove them with:")
    print()
    print("    x19 gateway migrate-legacy")
    print()
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `x19 serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `x19 serve` (no unit) is left running its
    ...
    print("  x19 model              # Select provider and model")
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from x19_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from x19_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import x19_cli.update_receipt as _ur
    ...
    with _best_effort('Multiplex auto-migration after update failed: %s'):
        from x19_cli.gateway_migrate import maybe_auto_migrate_after_update
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``x19_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
