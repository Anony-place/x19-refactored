"""Interactive setup wizard for X19 (config lives in ~/.x19/).
...
``x19_cli.setup.<name>`` keep working.
"""
...
from x19_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_x19_home is
# imported lazily at call sites (stale-module safety during x19 update).
from x19_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_x19_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_x19_home,
)
from x19_cli.colors import Colors, color
from x19_cli.cli_output import print_error, print_info, print_success, print_warning
...
_DOCS_BASE = "https://anony-place.github.io/x19-refactored/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("☤ X19 Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    _info("The interactive wizard cannot be used here.", None,
          "Configure X19 using environment variables or config commands:",
          "  x19 config set model.provider custom",
          "  x19 config set model.base_url http://localhost:8080/v1",
          "  x19 config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'x19 setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("x19_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from x19_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``x19 model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from x19_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``X19_NONINTERACTIVE=1`` (``x19_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("X19_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``X19_NONINTERACTIVE=1`` or a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from x19_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'x19 setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``x19 model`` flow (one code
    ...
    from x19_cli.main import select_provider_and_model
    try:
        ...
        from x19_cli.auth_error_copy import provider_setup_failure_lines
        lead, *rest = provider_setup_failure_lines(exc, retry_command="x19 model")
        print_warning(lead)
        for line in rest:
            print_info(line)
    ...
    # keep safe defaults (configure via `x19 auth add` / `x19 setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into X19_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("X19_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          "  Run `x19 setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.x19/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives X19_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`x19 setup tools` == `x19 tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from x19_cli.tools_config import tools_command
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from x19_cli.observability.shared_metrics import SharedMetricsStore
        from x19_cli.observability.shared_metrics_sender import reconcile_send_consent
        from x19_cli.sqlite_util import write_txn
...
# x19_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from x19_cli.setup_tts import setup_tts  # noqa: E402
from x19_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from x19_cli.setup_platforms import setup_gateway  # noqa: E402
from x19_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from x19_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from x19_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``x19 setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ☤ X19 Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, x19_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {x19_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'x19 config edit'")
    if migration_ran:
    ...
    def _gateway_step() -> None:
        ...
        from x19_cli.gateway import ensure_gateway_service
...
def _run_setup_wizard_impl(args):
    ...
    ``x19 setup <section>`` from SETUP_SECTIONS."""
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, x19_home))])
            return
        print_header("Reconfigure", gap=True)
        print_success("You already have X19 configured.")
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'x19 setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(x19_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up X19?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from x19_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, x19_home, is_existing))])
            return
    _run_full_setup(config, x19_home, is_existing=is_existing, migration_ran=migration_ran)
    ...
    _print_setup_summary(config, x19_home)
