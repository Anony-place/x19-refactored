...
One read-only pass answering, BEFORE any mutation: which X19 runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``x19 update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) X19 runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "x19 update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from x19_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"x19 -p {profile} gateway restart" if profile != "default" else "x19 gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from x19_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from x19_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from x19_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from x19_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from x19_cli.update_receipt import _socket_identity
        ...
        for profile, home in profile_homes:
            ...
            if sock is not None:
                ...
                # touch, so `x19 update` exited partial (#109680).
    ...
    with _probe("PID-file gateway inventory"):
        from x19_cli.gateway import find_profile_gateway_processes
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `x19 serve --host <ip>` for a remote Desktop, a long-lived `x19 dashboard`).
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running X19 services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``x19-serve*``/``x19-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``x19-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``x19-serve-workbench`` — and a scope prefix
    (``user/x19-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"x19-serve{suffix}", f"x19-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``x19-gateway-foobar.service``.
    Launchd labels are ``ai.x19.gateway`` / ``ai.x19.gateway-<profile>`` — they do
    not contain the substring ``x19-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/x19-gateway``,
    ``gui/501/ai.x19.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"x19-gateway", "ai.x19.gateway", "gateway", "gateway-default"}
    return name in {f"x19-gateway-{profile}", f"ai.x19.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``x19-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      x19 gateway restart                # active profile")
        print("      x19 -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        if sys.platform == "linux":
            print("      systemctl --user restart x19-serve.service   # unit-managed serve")
        print("      relaunch `x19 serve` / `x19 dashboard`")
    return True
