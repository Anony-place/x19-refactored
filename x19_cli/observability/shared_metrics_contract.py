"""Bounded product contract for the first X19 shared-metrics slice."""
...
SCHEMA_KEY = "x19.metrics.schema_version"
SCHEMA_VERSION = "x19.metrics.event.v2"
MODEL_CALL_SCOPE = "x19.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "x19.task_run"
TOOL_CALL_SCOPE = "x19.tool_call"
CLIENT_ACTIVE_MARK = "x19.client.active"
TOOL_APPROVAL_MARK = "x19.tool_approval"
SKILL_LIFECYCLE_MARK = "x19.skill.lifecycle"
SKILL_LOAD_MARK = "x19.skill.load"
SUBSCRIBER_NAME = "x19.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "x19.client.active"
LEGACY_MODEL_CALL_METRIC = "x19.model_call.count"
MODEL_ROUTE_METRIC = "x19.model_route.count"
TASK_STARTED_METRIC = "x19.task_run.started"
TASK_FINISHED_METRIC = "x19.task_run.finished"
TOOL_CALL_METRIC = "x19.tool_call.count"
TOOL_APPROVAL_METRIC = "x19.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "x19.skill.lifecycle.count"
SKILL_LOAD_METRIC = "x19.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "x19_version", "install_method", "os_family"})
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("x19_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its X19 logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "x19.call_role"
    )
    call_role = (metadata or {}).get("x19.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from x19_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map X19 terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "x19-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map X19 registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal X19 tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to X19."""
