"""Python dependencies declared by user plugins: read, resolve against X19' own ranges, install,
and re-apply after ``x19 update`` rebuilds the venv.
...
def applicable_requirement(spec: str) -> Optional[str]:
    """``spec`` reduced to ``name[extras]specifier`` when X19 should install it here; ``None`` when
    its environment marker excludes this platform, when it names X19 itself (always satisfied by the
    running checkout — installing ``x19`` from an index would clobber it), or when it points
...
def unsupported_specs(specs: Iterable[str]) -> list[str]:
    """Direct URL/path requirements: never installed by X19 (arbitrary git/tarball sources are a
...
def core_constraints(project_root: Path) -> list[str]:
    """X19' own declared ranges (``[project].dependencies`` + every extra), as constraint lines.
...
class DepsOutcome:
    ...
    specs: tuple[str, ...] = ()
    detail: str = ""
    skipped: tuple[str, ...] = ()  # direct-URL requirements X19 never installs
...
def refuse_conflicting_candidate(plugin_dir: Path, *, home: Path) -> None:
    ...
    try:
        ...
        raise DependencyConflict(
            f"its Python dependencies conflict with X19 or an enabled plugin:\n{exc}") from exc
...
def install_for_plugin_dir(plugin_dir: Path) -> DepsOutcome:
    ...
    try:
        specs = install_declaration(decl, home=get_x19_home(), project_root=project_root())
...
def reapply_all(*, project_root: Path, disable: Callable[[Path, str], None]) -> ReapplyReport:
    ...
    for victim in dropped:
        ...
        reason = "its Python dependencies no longer resolve against this X19"
