"""Baked-in build metadata for X19.

Source installs report their git revision live via ``git rev-parse`` (``x19_cli/dump.py``,
``x19_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.x19_build_sha`` file written by the Dockerfile.
"""
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".x19_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.x19_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``X19_GIT_SHA`` build-arg, full 40-char hash on one
