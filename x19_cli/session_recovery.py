"""Offline, non-destructive recovery for a damaged X19 session database.
...
from x19_state_common import FTS_STORAGE_VERSION, SCHEMA_VERSION
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    guard keeps this path consistent with ``x19_state_repair._backup_db_file``.
    ...
    ``x19_cli.sqlite_safe_read`` exists to prevent (see #71724). Holding the lock means no connection can
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="x19-session-recovery-", dir=str(work_root))
    try:
...
def _salvage_rowid_bounds(source: sqlite3.Connection, table: str) -> dict[str, Any]:
    ...
    # aggregate lets the planner answer from any covering index (every X19 table has at
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from x19_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
