...
from x19_cli._subprocess_compat import harden_git_argv, noninteractive_git_env
...
def _ensure_repo(cwd: str) -> None:
    ...
    if needs_root:
        _git_ok(cwd, ["-c", "user.email=x19@localhost", "-c", "user.name=X19",
...
def worktree_add(cwd: str, options: dict) -> dict:
    ...
    branch = _sanitize_branch(options.get("branch") or "") or f"x19/{slug}"
