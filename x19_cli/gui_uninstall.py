"""X19 Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "X19" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "X19"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "X19"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "X19"


def source_built_gui_artifacts(x19_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``x19 desktop`` inside the checkout (same ``x19/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "X19" product."""
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/X19.app"), home / "Applications" / "X19.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "X19", local_base / "x19-desktop"] + (
            [Path(program_files) / "X19"] if program_files else [])
    ...
    from x19_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "X19.desktop"] + [
        icons / size / "apps" / "x19.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(x19_home: Path) -> bool:
    """True when a usable Python agent install exists under X19_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((x19_home / "x19" / sub).is_dir() for sub in ("x19_cli", "venv", ".venv"))


def gui_is_installed(x19_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(x19_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(x19_home: "Path | None" = None) -> dict:
    ...
    return {"x19_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(x19_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so X19
        # disappears from the launcher.
        try:
            from x19_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove x19' or 'sudo dnf remove x19'). AppImage builds are a single "
