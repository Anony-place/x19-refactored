...
Mixin on ``X19CLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_version(self, cmd_original: str):
        from x19_cli.main import _print_version_info
        _print_version_info(check_updates=True)

    def _cmd_reload(self, cmd_original: str):
        from x19_cli.config import reload_env
        count = reload_env()
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `x19 plugins list`, so
            ...
            # on one line (full catalog behind `x19 plugins list`).
            user_entries = [e for e in entries if e[3] != "bundled"]
            bundled_count = len(entries) - len(user_entries)
            if not user_entries:
                print("No user plugins installed.")
                print("  Install one: x19 plugins install owner/repo")
                print(f"  Or drop a plugin directory into {display_x19_home()}/plugins/")
                if bundled_count:
                    print(f"  ({bundled_count} bundled plugins available — see: x19 plugins list)")
                return
            ...
            if bundled_count:
                print(f"  (+{bundled_count} bundled — see: x19 plugins list)")
            print("  Enable/disable: x19 plugins enable/disable <name>")
    ...
    def _get_heartbeat_manager(self):
        ...
        def load():
            from x19_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_watchdog(self):
        ...
        from x19_cli.heartbeat import POLL_SECONDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        _active_deleg = 0
        try:
            from x19_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
