"""Backup and import commands for x19 CLI."""
...
from x19_constants import (
    LOCAL_RUNTIME_ROOT_DIRS, _get_platform_default_x19_home, get_default_x19_root, get_x19_home,
    display_x19_home,
)
from x19_state_dbfile import RETIRED_GENERATION_DIR_SUFFIX
from utils import (
...
# Where ``x19 backup --quick`` / ``/snapshot`` / the pre-update safety net write state
...
def _snapshot_recovery_hint() -> str:
    """How to restore a state snapshot. There is no `x19 snapshot` subcommand — only the /snapshot
    slash command inside a `x19` session (x19_cli/commands.py)."""
    return ("To restore a newer snapshot, start `x19` in a terminal and run `/snapshot list`, then "
            "`/snapshot restore <id>` (CLI only).")

# Directory names to skip (matched against each path component). ``x19`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../x19/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under X19_HOME walked
...
_EXCLUDED_DIRS = {
    "x19",     # the codebase repo — re-clone instead
...
# X19-managed runtime downloads (see ``LOCAL_RUNTIME_ROOT_DIRS``). Matched ONLY at the root of
# X19_HOME and at ``profiles/<name>/`` — a deeper dir of the same name (a skill's ``models/``)
# is user data.
...
# Files ``x19 import`` must never overwrite, matched by basename so root and named profiles are
...
# Reserved archive subtree for memory-provider state OUTSIDE X19_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the X19 backup slot."""
...
@contextmanager
def _backup_operation_lock(x19_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = x19_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another X19 backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to x19 root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``x19`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "x19" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(x19_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    excluded tree, the root-only ``x19`` carve-out, root runtime trees, per-file rules),
    shared by ``x19 backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(x19_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(x19_root)
        is_root = rel_dir == Path(".")
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "x19" and not is_root))
        ...
        for fname in filenames:
            ...
            # copy data from outside X19_HOME; never archive the output zip into itself.
...
# 30 GB state.db, reading as a hung ``x19 update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart X19) and retry.", dst, exc2)
        return False
...
def run_backup(args) -> None:
    """Create a zip backup of the X19 home directory."""
    x19_root = get_default_x19_root()

    if not x19_root.is_dir():
        print(f"Error: X19 home directory not found at {x19_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(x19_root):
            _run_backup_locked(args, x19_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, x19_root: Path) -> None:
    ...
    print(f"Scanning {display_x19_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(x19_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_x19_home()}.")
    if skipped_external:
    ...
    if errors:
        ...
        print(f"\nRestore with: x19 import {out_path.name}")
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".x19", "x19"}:
...
def _confirm_import_overwrite(x19_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((x19_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has X19 configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, x19_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under X19_HOME; provider configs commonly hold credentials, so tighten to 0600.
...
def run_import(args) -> None:
    ...
    # ``get_default_x19_root()`` would silently retarget a profile restore at the live root.
    x19_root = get_x19_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_x19_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(x19_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, x19_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_x19_home()}.")
        if errors:
            _print_capped(f"\n  Warnings ({len(errors)} files skipped):", errors, "  ")
        if db_shrunk:
        ...
        restored_profiles = _restore_profile_wrappers(x19_root)
        print()
        if not (x19_root / "x19").is_dir():
            print("Note: The x19 codebase was not included in the backup.\n"
                  "  If this is a fresh install, run: x19 update")
        if restored_profiles:
            print("\nTo re-enable gateway services for profiles:")
            for pname in restored_profiles:
                print(f"  x19 -p {pname} gateway install")
        _revive_gateway_after_import(x19_root)
        print("Done. Your X19 configuration has been restored.")


def _restore_profile_wrappers(x19_root: Path) -> List[str]:
    """Re-create shell wrapper scripts for restored named profiles; return the profile names seen."""
    profiles_dir = x19_root / "profiles"
    ...
    except ImportError:  # x19_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: x19 profile list  (after installing x19)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(x19_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_x19_home()
    if x19_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  x19 gateway install")
        return
    try:
        from x19_cli.gateway import ensure_gateway_service, _is_service_running
...
# --- Quick state snapshots (used by /snapshot slash command and x19 backup --quick) ---

# Critical state files (relative to X19_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``x19 update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside X19_HOME.
...
def _quick_snapshot_root(x19_home: Optional[Path] = None) -> Path:
    home = x19_home or get_x19_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, x19_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``x19 update`` while the small files are always captured.
    """
...
def list_quick_snapshots(limit: int = 20, x19_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(x19_home)):
...
def restore_quick_snapshot(snapshot_id: str, x19_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, x19_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent cron-job loss across ``x19 update``.
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_x19_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", x19_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, x19_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent config.yaml model/MoA loss across ``x19 update``.
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, x19_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, x19_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, x19_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(x19_home)), keep, shutil.rmtree, "snapshot")


def run_quick_backup(args) -> None:
    """CLI entry point for x19 backup --quick."""
    snap_id = create_quick_snapshot(label=getattr(args, "label", None))
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_x19_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, x19_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``x19_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(x19_root):
            return _write_full_zip_backup_locked(out_path, x19_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, x19_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(x19_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    x19_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<X19_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    x19_root = x19_home or get_default_x19_root()
    if not x19_root.is_dir():
        return None
    backup_dir = x19_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, x19_root) is None:
        return None
...
def create_pre_update_backup(
    x19_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``x19 update`` continues anyway."""
    return _create_prefixed_full_backup(x19_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    x19_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``x19 claw migrate``
    (same dir as update backups so listings/``x19 import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        x19_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
