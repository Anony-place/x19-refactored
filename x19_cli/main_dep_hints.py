...
``x19 dashboard`` (fastapi + uvicorn) and ``x19 acp`` (agent-client-protocol) are installed by
the ``[all]`` / ``[acp]`` extras. A partial install, a pip-less uv venv or an interrupted update can
leave them out; the built-in repair is ``x19 update``, which reinstalls the extras into the same
...
def missing_optional_deps_message(surface: str, what: str, extra: str) -> str:
    ...
    return (
        ...
        "Run `x19 update` to reinstall dependencies. If that fails, run manually:\n"
