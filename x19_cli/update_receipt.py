...
# ``x19 update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``x19 update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``x19 serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``x19_constants`` (stdlib-only), never ``x19_cli.config``: the receipt must be
    ...
    return get_x19_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``x19 update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from x19_cli.profiles import _get_default_x19_home, _get_profiles_root, _PROFILE_ID_RE
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `x19 gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if stale_or_down:
        ...
        print("  Run `x19 gateway restart` (or `x19 -p <profile> gateway restart` for a named")
        print("  profile), then `x19 gateway status` to confirm.")
