"""Profile distributions — shareable, packaged X19 profiles via git.
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            x19_requires=_str(data, "x19_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("x19_requires", self.x19_requires),
...
def check_x19_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires X19 {op}{target}, but you have {current_version}.")
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from x19_cli.profiles import _canon_valid, get_profile_dir
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.x19).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from x19_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from x19_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="x19_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `x19 profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from x19_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`x19 profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="x19_dist_update_") as tmp:
