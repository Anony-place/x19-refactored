...
Pure leaf: imports nothing from ``x19_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because x19_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("x19_cli.auth.httpx.Client")
...
DEFAULT_NOUS_CLIENT_ID = "x19-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from x19_cli import __version__ as _X19_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _X19_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"x19-cli/{_X19_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://anony-place.github.io/x19-refactored/docs/user-guide/features/spotify"
...
OAUTH_OVER_SSH_DOCS_URL = "https://anony-place.github.io/x19-refactored/docs/guides/oauth-over-ssh"
