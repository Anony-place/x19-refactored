...
in :func:`run_migrations`. Every write goes through ``x19_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_x19_home()}/config.yaml and run `x19 setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``x19_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
    from x19_cli import config
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$X19_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_x19_home() / "plugins"
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    message = (
        ...
        "Use `x19 plugins enable <name>` to activate.")
    _commit(
        config, results, quiet,
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # section; deep-merge made it work but users could not see/edit it and `x19 curator status`
    # had no stable logs dir. Only keys the user hasn't set are written.
    _c = _cfg()
    DEFAULT_CONFIG = _c.DEFAULT_CONFIG

    try:
        curator_dir = _c.get_x19_home() / "logs" / "curator"
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `x19 config set`")
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from x19_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if changed:
        _commit(
            ...
            "lists — video generation now lives under `x19 tools` → "
...
def _migrate_to_41(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from tools.bot_mode_probe import _PROTOCOL_HEADING, _x19_root, _roster, strip_legacy_protocol

    cleaned: List[str] = []
    for name, profile_dir in _roster(_x19_root(get_x19_home())):
...
def _migrate_to_45(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from x19_cli.tools_config import _configurable_keys, _get_plugin_toolset_keys
    ...
    for platform, toolsets in saved.items():
        ...
        if not toolset_allowed_for_platform("connections", platform):
            continue
        # A composite like [x19-cli] already inherits every core tool at read time.
    ...
    _commit(
        ...
        "Uncheck Connections in `x19 tools` to turn it off.")
...
MIGRATIONS: Tuple[Tuple[int, Callable[[Dict[str, Any], bool], None]], ...] = (
    ...
    (44, _rewrite_stale_default(
        ...
        message=(
            ...
            "skills/.archive/ (recoverable with `x19 curator restore`). Set it back to 90 to keep the old window."))),
