...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from x19_cli.main import _resolve_last_session
    target = (
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from x19_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @x19/ink is missing or node_modules is behind package-lock.json.
    ...
    if not (ws_root / "node_modules" / "@x19" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/x19-ink/src")
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/x19-ink/package.json",
    "packages/x19-ink/index.js",
    "packages/x19-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``X19_TUI_FORCE_BUILD=1`` forces a rebuild."""
    force = (os.environ.get("X19_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``X19_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from x19_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("X19_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "X19_HOME": x19_home},
    ...
    extras += [Path(x19_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _find_bundled_tui(x19_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if x19_cli_dir is None:
        x19_cli_dir = Path(__file__).parent
    bundled = x19_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``x19 update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``x19 update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("X19_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this X19 checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `x19 update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the X19 checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `x19 --tui`\n"
        "If the checkout is still inconsistent, run `x19 update --force`.",
        file=sys.stderr)
    sys.exit(1)
...
def _npm_lifecycle_env(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    # the rebuild fails). ``$X19_HOME`` lives outside the git tree and the
    # update hand-off already carries ``X19_HOME`` down to every npm child.
    # An explicit ``NPM_CONFIG_USERCONFIG`` wins (#106373).
    from x19_constants import get_x19_home
    npmrc = get_x19_home() / "npmrc"
    if npmrc.is_file():
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``X19_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$X19_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("X19_NODE")
        if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
            return env_node
    from x19_constants import find_node_executable
    ...
    if not path:
        print(
            ...
            "https://nodejs.org (run `x19 doctor` for the install hint for your OS), then "
            "retry `x19 --tui`. To keep working now, run `x19 --cli`."
        )
        sys.exit(1)
    return path
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_x19_node_path()))
    ...
    if result.returncode != 0:
        ...
        from x19_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (X19_TUI_DIR prebuilt or esbuild)."""
    from x19_cli.main import _is_termux_startup_environment
    _ensure_tui_node()

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("X19_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with X19_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset X19_TUI_DIR (e.g. `unset X19_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr)
        sys.exit(1)
    ...
    # x19_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   x19_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        # --dev runs src/entry.tsx directly, but @x19/ink resolves through
        # packages/x19-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "x19-ink", "TUI dev prebuild failed.")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from x19_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from x19_cli.main import PROJECT_ROOT
    src_root = str(env.get("X19_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["X19_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("X19_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["X19_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("X19_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["X19_CWD"]) / python_path
...
def _launch_tui(
    ...
    import tempfile
    # TUI child is a x19 process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    from x19_cli.shared_session_attach import configure_tui_attachment
    try:
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="x19-tui-active-session-", suffix=".json")
    os.close(active_session_fd)
    env["X19_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["X19_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("X19_MODEL", model), ("X19_INFERENCE_MODEL", model),
        ("X19_TUI_PROVIDER", provider), ("X19_INFERENCE_PROVIDER", provider),
        ("X19_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("X19_TUI_SKILLS", skills_value),
        ("X19_TUI_QUERY", query), ("X19_TUI_IMAGE", image),
        ("X19_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("X19_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("X19_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("X19_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("X19_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # X19_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `x19 --tui` try to resume a non-existent session; only forward the id
    # argparse resolved for this invocation.
    env.pop("X19_TUI_RESUME", None)
    if resume_session_id:
        env["X19_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `x19 update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``X19_KANBAN_BOARD`` so in-process tools and shelled-out
    ``x19 kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``x19 kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``x19 kanban boards switch`` from another session can flip the file mid-turn, so the same
    ...
    """
    if os.environ.get("X19_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from x19_cli.kanban_db import get_current_board
        os.environ["X19_KANBAN_BOARD"] = get_current_board()


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.x19/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``X19_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``x19 chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
