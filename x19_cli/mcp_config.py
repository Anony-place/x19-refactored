"""MCP Server Management CLI — ``x19 mcp`` subcommand."""
...
from x19_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_x19_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.x19/.env`` into ``os.environ`` and
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return X19TokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see x19_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  x19 mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  x19 mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  x19 mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: x19 mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_x19_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `x19 mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  x19 mcp add <name> --url <endpoint>')
        _info('  x19 mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _probe_failure_next_step(name: str, exc: BaseException) -> str:
    ...
    if _is_auth_error(root) or getattr(getattr(root, "response", None), "status_code", None) in (401, 403):
        return f"The server rejected the sign-in. Run: x19 mcp login {name}"
    if "missing executable" in _format_connect_error(exc):
        return (f"Install that command, or set mcp_servers.{name}.command in {display_x19_home()}/config.yaml "
                "to its full path.")
    return f"Check the server is running and the URL/command in its config, then run: x19 mcp test {name}"
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `x19 mcp remove` + `x19 mcp add` to reconfigure auth.")
        return False
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `x19 mcp login` is explicitly
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `x19 mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: x19 mcp reauth <name>   |   x19 mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'x19 mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "x19 mcp                                    Open the catalog picker (default)",
    "x19 mcp catalog                            List Nous-approved MCPs",
    "x19 mcp install <name>                     Install a catalog MCP",
    "x19 mcp serve                              Run as MCP server",
    "x19 mcp add <name> --url <endpoint>        Add a custom MCP server",
    "x19 mcp add <name> --command <cmd>         Add a stdio server",
    "x19 mcp add <name> --preset <preset>       Add from a known preset",
    "x19 mcp remove <name>                      Remove a server",
    "x19 mcp list                               List configured servers",
    "x19 mcp test <name>                        Test connection",
    "x19 mcp configure <name>                   Toggle tools",
    "x19 mcp login <name>                       Re-authenticate OAuth",
    "x19 mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``x19 mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from x19_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `x19 plugin`).
    from x19_cli.mcp_picker import run_picker
    run_picker()
