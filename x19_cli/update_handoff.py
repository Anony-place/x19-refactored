"""Post-swap hand-off: finish ``x19 update`` in an interpreter born on the pulled code.

``x19 update`` starts in an interpreter that imported the PRE-pull tree. Once ``git merge``
...
into a hand-off file and re-executes ``x19 update --post-swap <file>`` under the venv
...
POST_SWAP_ENV = "X19_UPDATE_POST_SWAP"
...
def write_handoff(payload: dict[str, Any]) -> Path:
    """Persist the post-swap payload under X19_HOME; returns its path."""
    from x19_constants import get_x19_home

    directory = get_x19_home() / "logs" / "update_receipts"
...
def _running_from_windows_shim() -> bool:
    from x19_cli.main_install_repair import _windows_shim_in_process_chain
...
def post_swap_command(handoff_path: Path, argv_tail: list[str]) -> list[str]:
    """``python -m x19_cli.main update <original flags> --post-swap <file>``."""
    return [str(post_swap_python()), "-m", "x19_cli.main", "update", *argv_tail, "--post-swap", str(handoff_path)]


def post_swap_child_env() -> dict[str, str]:
    """Environment for the child. ``X19_UPDATE_REEXEC`` marks it as already off the Windows
    ...
    from x19_cli.main_install_repair import _UPDATE_REEXEC_ENV
    from x19_cli.update_lock import HANDOFF_PID_ENV
...
def continue_update_in_fresh_interpreter(payload: dict[str, Any], *, argv_tail: list[str]) -> int | None:
    ...
    ``x19.exe``, the child cannot be awaited: the shim is one of the files the dependency
    ...
    if _running_from_windows_shim():
        try:
            ...
            return None
        print("→ Windows: x19.exe cannot replace itself while it runs; the update")
