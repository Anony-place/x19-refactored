...
Mixed into :class:`x19_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
PLUGIN_SECTIONS_START = "<!-- x19-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- x19-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- x19-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``x19:<event>``.
X19_EVENT_NAMESPACE = "x19"
...
class PluginDispatchMixin:
    ...
    def _invoke_hook_callback(callback: Callable, payload: Dict[str, Any]) -> Any:
        ...
        """
        from x19_cli.plugins import resolve_plugin_command_result
        try:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from x19_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"x19-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="x19-plugin-events",
            daemon=True,
        )
