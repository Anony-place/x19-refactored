"""X19 — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m x19_cli.main web [--port 8080]``.
"""
...
from x19_cli.install_identity import get_install_id as _shared_get_install_id
from x19_cli.pty_session import run_reaper
...
try:
    ...
    # running `x19 dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["X19_WEB_DIST"]) if "X19_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from x19_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``x19 dashboard`` backend, not a gateway, so without
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `x19 update` otherwise 500s
    ...
    # Import x19_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `x19 update` replaced the code
    ...
    # Desktop-spawned backends (X19_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `x19
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("X19_DESKTOP") == "1":
        ...
        try:
            from x19_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from x19_cli.config import load_config
            from x19_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    # when X19_GUEST_ONBOARDING=1, records the answer for setup.status / free_tier.status and
    ...
    from x19_cli.free_tier_bootstrap import start_background_bootstrap
    ...
    try:
        ...
        try:
            from x19_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="X19 Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from x19_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# X19_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("X19_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-X19-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".x19-ssh-runtime-{nonce}")
...
from x19_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from x19_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 x19-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``X19_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("X19_DESKTOP") == "1"
        and bool(os.environ.get("X19_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from x19_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from x19_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from x19_cli.dashboard_auth.token_auth import token_auth_middleware
...
from x19_cli import web_server_gateway as _gateway_mod  # noqa: E402
from x19_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from x19_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from x19_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from x19_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT X19 home (not the profile X19_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``x19 gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_x19_action(subcommand, "gateway-restart")
...
from x19_cli.web_routers import (  # noqa: E402
...
from x19_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset X19_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OAuth: run `x19 dashboard register` (Nous Portal) or "
    ...
    try:
        ...
        from x19_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`x19 plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 x19-0day
    ...
    if app.state.auth_required:
        ...
        from x19_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from x19_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from x19_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from x19_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `x19 serve` (no X19_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from x19_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `x19 update` relaunch a manually-started serve
    ...
    ready_token = "X19_BACKEND_READY" if headless else "X19_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  X19 backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  X19 Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from x19_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``X19_SERVE_HEADLESS``).
