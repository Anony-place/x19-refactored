"""PTY bridge for `x19 dashboard` chat tab.
...
``x19_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/x19-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "X19_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "X19 supports Windows only via WSL.")
