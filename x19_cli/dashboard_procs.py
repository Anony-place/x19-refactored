"""Dashboard process-hygiene helpers — extracted from ``x19_cli/main.py``.

Helpers defined in ``x19_cli.main_dashboard`` / ``x19_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("x19", "x19_cli.main", "x19_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "x19.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``x19 update`` (every API call 401s). *exclude_pids* (Desktop's X19_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the X19
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``x19 serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``X19_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`x19 --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from x19_cli.process_identity import ledger_entries
...
def _x19_home_for_pid(pid: int) -> str | None:
    """Best-effort ``X19_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("X19_HOME"):
            return home
    try:
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``x19 update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _profile_key_for_respawn(argv: list[str], x19_home: str | None = None) -> str:
    """Stable owner key: ``X19_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if x19_home:
        parts = _resolved_home(x19_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(x19_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, x19_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``X19_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``X19_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``x19 update`` respawn detaches
    ...
    for _pid, argv, x19_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and x19_home and _normalized_home_for_compare(x19_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, x19_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``X19_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("X19_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_x19(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not x19-owned or process identity changed"))
            else:
...
# x19_cli/web_server.py::_lifespan: stop_hosted_room_service(timeout=5.0) + the startup-thread
...
# and keep the deleted state.db-wal inode open — the next x19 start refuses with a FATAL
...
def _is_detached_session_leader(pid: int, tty: str) -> bool:
    ...
    ``x19 --tui`` child is a session leader too (``pty.fork``) but owns the pts whose master the
...
def _posix_descendants(roots: list[int]) -> dict[int, tuple[int, int | None]]:
    ...
    subtrees. So is the calling process with its subtree and its ancestor chain: ``x19 dashboard
    --stop`` / ``x19 update`` run from a shell escape inside the hosted Chat TUI are same-session
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``x19 dashboard`` / ``x19 serve`` processes (update end, ``--stop``).
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``x19 update``'s systemd fleet-restart loop, which restarts ``x19-serve*`` units before this
    ...
    """
    from x19_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. x19-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "x19-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if launchd_jobs and (job := _launchd_owner(pid, cmdline)):
                ...
                # Manual process: exact argv + X19_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot X19_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _x19_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (x19 serve) under Restart=on-failure never comes
...
def _detect_concurrent_x19_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``x19.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from x19_cli.main_install_repair import _x19_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _x19_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``x19 serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    from x19_cli.update_cmd_windows import _x19_holder_subcommand
    # Canonical token matcher, never argv substrings: ``kanban --preserve-cache`` contains "serve" and
    # ``vim notes about x19 serve`` contains both markers — this predicate decides a kill.
    if _x19_holder_subcommand(command) != "serve":
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``x19 serve`` it spawns. Such a backend
...
def _x19_home_dir() -> Path:
    """The process's X19 home: remote-backend locks are a process-level asset, so a request scoped
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "x19Path", "x19Home", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated X19_HOME can't reject a legitimate backend.
...
def _remote_lock_roots(base_dir: Path | None) -> list[Path]:
    ...
    The Desktop writes SSH locks beneath the ROOT home (``~/.x19/desktop-ssh``), but a profile
    backend (``x19 --profile X serve``) runs with ``X19_HOME=<root>/profiles/X`` — scanning only
    ...
    roots: list[Path] = []
    for home in (_x19_home_dir(), get_default_x19_root()):
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{x19_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local x19 serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``x19 serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / X19_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after X19_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
