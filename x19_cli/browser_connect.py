"""Shared helpers for attaching X19 to a local Chromium-family CDP port.
...
# the real profile into ``~/.x19/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the x19-owned snapshot dir for ``browser``'s real profile."""
    return str(get_x19_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, X19_UID/GID) as every X19 secret dir — via ``_secure_dir``/
    ...
    ``X19_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from x19_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
def _unavailable_auth_dbs_error(browser: str, failed: dict[str, str]) -> str:
    ...
    if all(reason == _AUTH_DB_LOCKED for reason in failed.values()):
        return (f"{browser} is running and holds the profile's {names} with a write lock, so their "
                "SQLite backup made no progress within five seconds. X19 does not fall back to a "
...
_SNAPSHOT_DONE_MARKER = ".x19-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. X19 can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let X19 "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the x19 copy dir; returns ``(dst, err)``.
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_x19_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_x19_home() / "chrome-debug")
