"""Security advisory checker for X19.
...
class Advisory:
    ...
    ``{x19_home}``; ``full_remediation_text`` fills it with ``display_x19_home()`` at render
    time so a profile / X19_HOME user is sent to their own .env.
    """
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including x19 when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in {x19_home}/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: x19 doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'x19 doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def full_remediation_text(hit: AdvisoryHit) -> list[str]:
    ...
    return [
        ...
        "",
        "Remediation:",
        *(f"  {i}. {step.format(x19_home=display_x19_home())}" for i, step in enumerate(a.remediation, 1)),
    ]


# ─── Startup-banner gating ────────────────────────────────────────────────────
# Once the banner is seen we cache that in ``~/.x19/cache/advisory_banner_seen`` (one
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_x19_home()) / "cache"
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active (IDs: {', '.join(h.advisory.id for h in fresh)}). "
            "Run `x19 doctor` on the gateway host for details.")
