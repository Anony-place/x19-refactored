"""``x19 slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible X19 defaults, tweakable in the Slack
    UI after pasting.
    """
    from x19_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with X19 in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with X19 in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your X19 on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``x19_cli/main.py``)."""
    name = getattr(args, "name", None) or "X19"
    description = getattr(args, "description", None) or "Your X19 on Slack"
    ...
    def fail(msg: str) -> int:
        print(f"x19 slack manifest: {msg}", file=sys.stderr)
        return 2
    ...
    if isinstance(write_target, bool) and write_target:  # bare --write → default location
        ...
        target = Path(get_x19_home()) / "slack-manifest.json"
    else:
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your X19 app\n"
