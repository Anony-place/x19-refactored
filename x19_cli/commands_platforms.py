...
from x19_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "x19_cli.commands").
logger = logging.getLogger("x19_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``x19 skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/x19 <command>`` on Slack only: the registry
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_X19_ONLY are skipped. ``/x19`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://x19.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/x19`` handler, plugin commands included."""
