"""User-facing copy for ``x19 gateway start/stop/restart`` failures on systemd hosts.

``x19_cli/gateway.py`` is a facade; this sibling owns the small exception -> guidance table so
...
_JOURNAL_HINT = 'journalctl --user -u x19-gateway --since "5 min ago"'

_SYSTEMCTL_FAILED_LINES = (
    "Could not {verb} the gateway service; systemd reported an error.",
    "See why with `x19 gateway status --deep` or `{journal}`.",
    "To reinstall the service run `x19 gateway install --force`.",
)

_NO_SYSTEMCTL_LINES = (
    "This system has no systemd, so X19 cannot install a background service here.",
    "Run the gateway directly with `x19 gateway run` (keep it alive with tmux or screen).",
)
