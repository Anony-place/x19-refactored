"""x19 fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `x19 model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  x19 fallback add\n")
        return
    print()
    ...
    print("  Docs: https://anony-place.github.io/x19-refactored/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `x19 model`, then append the selection to the chain."""
    from x19_cli.main import _require_tty, select_provider_and_model
    from x19_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `x19 model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `x19 fallback list` to view, or `x19 fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``x19 fallback [subcommand]``."""
