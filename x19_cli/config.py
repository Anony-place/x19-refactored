...
from x19_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from x19_cli.secret_prompt import masked_secret_prompt
# Re-export from x19_constants — canonical definition lives there.
from x19_constants import get_x19_home, get_process_x19_home  # noqa: F401
...
def _warn_config_parse_failure(
    ...
    from x19_cli.config_backups import backup_config
...
#
# ``X19_*`` overall is NOT blocked — many integration credentials use that prefix
# (X19_LANGFUSE_PUBLIC_KEY, X19_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "X19_COPILOT_ACP_COMMAND", "X19_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "X19_YOLO_MODE", "X19_ACCEPT_HOOKS", "X19_REDACT_SECRETS",
    "X19_INTERACTIVE", "X19_EXEC_ASK", "X19_GATEWAY_SESSION",
    "X19_CRON_SESSION", "X19_SINGLE_QUERY_SESSION",
    "X19_SESSION_KEY", "X19_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or X19 runtime location and security policy (X19_HOME, X19_YOLO_MODE, ...) "
...
# acquires it again (x19_cli/plugins.py: `with ..., config_mod._CONFIG_LOCK:` then
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean X19_TOOL_PROGRESS variant is unsupported (its only
    ...
    "X19_LANGFUSE_ENV", "X19_LANGFUSE_RELEASE", "X19_LANGFUSE_SAMPLE_RATE",
    "X19_LANGFUSE_MAX_CHARS", "X19_LANGFUSE_CAPTURE", "X19_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "X19_ACP_AUTH_METHOD", "X19_ACP_AUTO_APPROVE", "X19_COPILOT_ACP_COMMAND",
    "X19_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# X19_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``X19_MANAGED=false``): without this a bool-shaped value became a package
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: X19_MANAGED env var (systemd service) or a ``.managed`` marker file in
    X19_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("X19_MANAGED", "").strip().lower() or None
    managed_marker = get_x19_home() / ".managed"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    """
    # The stamp is a property of the running code tree (parent of x19_cli/), NOT of $X19_HOME,
    ...
    method = _install_method_stamp(get_x19_home() / ".install_method")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (x19_cli/main.py) so the wording never
...
def get_container_exec_info() -> Optional[dict]:
    ...
    already inside the container, or when X19_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("X19_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_x19_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "x19_bin": info.get("x19_bin", "/data/current-package/bin/x19")}
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / X19_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("X19_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_x19_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read X19_UID / X19_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    ...
    return _env_int("X19_UID"), _env_int("X19_GID")


def _chown_to_x19_uid(path) -> None:
    """Chown ``path`` to ``X19_UID:X19_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_x19_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_x19_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply X19_UID/GID ownership. No-op when managed;
    ...
    Also applies ``X19_UID``/``X19_GID``-based ownership when those env vars are set (#34107 — Docker
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or X19_CONTAINER / X19_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("X19_CONTAINER") or os.environ.get("X19_SKIP_CHMOD")
...
def ensure_x19_home():
    ...
    from x19_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _X19_HOME_ENSURED and home.is_dir():
        return
    from x19_cli.config_home import initialize_home
    initialize_home(home, _X19_HOME_SUBDIRS, _X19_HOME_ENSURED)


# ---- Config loading/saving ----

from x19_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from x19_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use x19_cli.config.<name>)
...
from x19_cli.personality import (  # noqa: E402,F401
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (x19_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # x19_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. X19_TIMEZONE overrides "
            "this key when set.")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from x19_cli.config_home import config_load_issue
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from x19_cli.config_migrations import (
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from x19_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from x19_cli.toolset_validation import validate_platform_toolsets
        from x19_cli.toolset_scope import toolset_allowed_for_platform
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_x19_dotenv()``, in-process
...
# the X19_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _backups_dir_display() -> str:
    from x19_constants import display_x19_home
    return f"{display_x19_home()}/backups/config/"
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        ...
        from x19_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:8] == cache_sig:
            ...
            # load_x19_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_x19_dotenv() pins unexpanded literals
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``X19_CUSTOM_`` prefix keeps the
    ...
    return f"X19_CUSTOM_{slug}_API_KEY" if slug else "X19_CUSTOM_API_KEY"
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from x19_cli.credential_lifecycle import save_provider_env_credential
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale X19_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("X19_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale X19_MAX_ITERATIONS={env_ghost} "
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from x19_cli.personality import active_personality_name
...
def show_config():
    ...
    from x19_cli.auth import get_anthropic_key
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``X19_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("X19_MODEL", ""))
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_x19_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from x19_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (X19_YOLO_MODE, PATH, ...)
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from x19_cli.config_env_routing import is_env_setting_key, read_env_setting
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from x19_cli.credential_lifecycle import remove_provider_env_credential

        if not remove_provider_env_credential(key.upper()).get("found"):
            _exit_invalid(f"Config key not set: {key}")
        print(f"✓ Unset {key} from {get_env_path()}")
        return
    from x19_cli.config_env_routing import is_env_setting_key, remove_env_setting
