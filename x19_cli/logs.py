"""``x19 logs`` — view and filter X19 log files.

``x19 logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``x19 logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_x19_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when X19 runs — try 'x19 chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_x19_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_x19_home()}/logs/")
        return

    print(f"Log files in {display_x19_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'x19 chat' to generate logs)")
