"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (x19 gui/desktop).

Split out of ``x19_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
from x19_cli.desktop_console import desktop_console_output, desktop_launch_notice
from x19_cli.main_tui_launch import _npm_lifecycle_env
from x19_cli.main_web_build import (
    _hash_source_tree, _nixos_build_env, _stamp_is_current, _write_build_stamp)

# Log-record parity with the origin module.
logger = logging.getLogger("x19_cli.main")
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $X19_HOME."""
    from x19_constants import get_x19_home
    return get_x19_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/X19.app/Contents/MacOS/X19 → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/X19.app/Contents/MacOS/X19"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "X19.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("x19", "X19")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture X19.exe to the launcher. Prefer
        ...
        # wrong-architecture X19.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``X19.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``x19 update`` used to report
...
# (Desktop → x19-setup --update → `x19 update` → `x19 desktop --build-only` → relaunch) rebuilds
# X19.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# X19.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``X19.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 x19-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built X19.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working X19.exe from backup.")
        print("    Your existing version was kept and still works. Run `x19 desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `x19 desktop --force-build` to rebuild, or re-run the X19")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from x19_constants import find_node_executable, with_x19_node_path
...
def _desktop_macos_relaunchable_fixup(
    ...
    "X19 is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../X19.app/Contents/MacOS/X19  ->  app bundle = .../X19.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="x19-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:x19local",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "X19 Local Signing") -> bool:
    ...
    from x19_cli.main import PROJECT_ROOT
    if sys.platform != "darwin":
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.x19"
    )
    return True
...
def _running_macos_app_bundles() -> set[Path]:
    """``.app`` bundles of every live X19 Desktop process. A running bundle is never swapped
    ...
    for proc in psutil.process_iter(["exe"]):
        ...
        if exe.endswith("/Contents/MacOS/X19"):
...
def _install_rebuilt_desktop_app(desktop_dir: Path) -> tuple[list[Path], list[str]]:
    """Copy the rebuilt macOS bundle over every stale installed ``X19.app`` (#52339).

    ``x19 desktop --build-only`` (what ``x19 update`` runs) packages into
    ...
    if rebuilt_exe is None:
        return [], []
    from x19_cli.gui_uninstall import packaged_gui_app_paths  # noqa: PLC0415
    # .../X19.app/Contents/MacOS/X19 -> .../X19.app
...
def _install_rebuilt_macos_bundles(
    ...
    for app in candidates:
        ...
        if app.resolve() in running:
            problems.append(
                f"{app} is running and was not refreshed; quit X19 Desktop and run "
                "`x19 update` again (or update from inside the app)")
            continue
        tmp = app.parent / f"{app.name}.x19-update-new"
        old = app.parent / f"{app.name}.x19-update-old"
        shutil.rmtree(tmp, ignore_errors=True)
        shutil.rmtree(old, ignore_errors=True)
        try:
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ X19 Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ X19 Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry(defer: bool = False):
    """Install the XDG desktop entry for X19 Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``x19 uninstall --gui`` removes it.
    ...
    from x19_cli.main import PROJECT_ROOT
    try:
        from x19_cli.linux_desktop_entry import DeferredDesktopEntryInstall, install_desktop_entry, is_supported
        if not is_supported():
            return None
        if defer:
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from x19_cli.main import PROJECT_ROOT
    from x19_cli.main_web_build import _run_npm_install_deterministic
    from x19_constants import with_x19_node_path
    ...
    nixos_env = with_x19_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose X19.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # X19.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds X19.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on X19.exe, close any")
            print("  running X19 desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> x19 desktop --force-build")
...
def _prefer_wsl_d3d12(env: dict) -> None:
    ...
    from x19_constants import is_wsl
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from x19_constants import with_x19_node_path
    # with_x19_node_path() copies os.environ when called with no arg.
    env = with_x19_node_path()
    _prefer_wsl_d3d12(env)
    for attr, key in (
        ("fake_boot", "X19_DESKTOP_BOOT_FAKE"), ("ignore_existing", "X19_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "x19_root", None):
        env["X19_DESKTOP_X19_ROOT"] = str(Path(args.x19_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["X19_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "X19_DESKTOP_DISABLE_GPU" not in os.environ:
        env["X19_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "X19_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["X19_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from x19_cli.main import PROJECT_ROOT
    from x19_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "X19 Local Signing"
    ...
    if source_mode or needs_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  x19 gui")
            sys.exit(1)
    ...
    from x19_cli.linux_desktop_entry import launched_from_shell
    ...
    if source_mode:
        print("→ Launching X19 Desktop from source build...")
    ...
    if not source_mode:
        desktop_launch_notice(f"→ Launching packaged X19 Desktop: {' '.join(launch_command)}")
    pass_fds: tuple[int, ...] = ()
