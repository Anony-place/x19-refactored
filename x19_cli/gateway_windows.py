...
_TASK_NAME_DEFAULT = "X19_Gateway"
_TASK_DESCRIPTION = "X19 Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("X19_GATEWAY_DETACHED", "1"), ("X19_SUPERVISED_CHILD", "1"))
...
def _x19_home() -> Path:
    ...
    return Path(get_x19_home())


def _preserve_x19_home_path(path: str | Path) -> str:
    r"""Render X19-owned paths under the configured X19_HOME spelling.

    ``%LOCALAPPDATA%\x19`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under X19_HOME.
    """
    candidate = Path(path)
    try:
        home = _x19_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current X19 profile."""
    from x19_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "x19_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"X19_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "X19_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "X19_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from x19_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<X19_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _x19_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at X19_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m x19_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "x19_cli.main"]
    if profile_arg:
...
def _launcher_settings(home: Path | None = None) -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, x19_home, profile_arg) for generated launchers.
    ``home`` targets another profile's X19_HOME (per-profile cold-start, #110959)."""
    from x19_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    x19_home = str(home if home is not None else _x19_home())
    return (
        _preserve_x19_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        x19_home,
        _profile_arg(x19_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_x19_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_x19_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, x19_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "X19_HOME={x19_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_x19_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, x19_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('X19_HOME')}) = {q(x19_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_x19_home_path(venv_dir))}",
...
def _build_gateway_argv(home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from x19_cli.gateway import PROJECT_ROOT

    python_path, working_dir, x19_home, profile_arg = _launcher_settings(home)
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"X19_HOME": x19_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_x19_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_x19_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (X19_HOME,
    ...
    try:
        x19_home = str(_x19_home().resolve())
    except Exception:
        x19_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "X19_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if x19_home:
        env_overlay["X19_HOME"] = x19_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("X19_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("X19_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from x19_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"x19 {profile_arg} gateway start" if profile_arg else "x19 gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated X19 gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: x19 gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: x19 gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: x19 gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path(home: Path | None = None) -> Path:
    ...
    return (home if home is not None else _x19_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    newer one written by a concurrent ``x19 gateway start`` (#110020 review)."""
    try:
        path = _start_attestation_path(home)
        path.parent.mkdir(parents=True, exist_ok=True)
        from x19_cli.process_identity import _process_create_time
...
# meant to bridge the seconds between a ✓ and the next ``x19 gateway status``/``update``; a
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None, home: Path | None = None) -> bool:
    ...
    try:
        ...
        sentinel = get_lifecycle_sentinel_path(home if home is not None else _x19_home())
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    one-shot marker — ``x19 update`` consults it to decide whether a Desktop-owned install
...
def _format_attestation_warning(attested: list[int], data: dict) -> str:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `x19 gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_x19_home()}\\logs\\gateway.log\n    type {_x19_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_x19_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated X19 gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: x19 gateway status")
                    return
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: x19 gateway install")
            return
        install(force=False)
        if not is_task_registered() and not is_startup_entry_installed():
            print("⚠ Gateway install did not complete in this process.")
            print("  If a UAC prompt opened, approve it, then run: x19 gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `x19 gateway status`."
        )
