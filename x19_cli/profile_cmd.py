"""``x19 profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``x19_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from x19_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.x19_requires:
        print(f"  Requires: X19 {mf.x19_requires}")
...
def _profile_status(args):
    """Bare ``x19 profile`` — show current profile status."""
    from x19_constants import display_x19_home
    from x19_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    profile_name = get_active_profile_name()
    dhh = display_x19_home()
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → x19 -p {p.name}")
    print()
...
def _shared_credential_warnings(profiles) -> list:
    ...
    from x19_cli.profile_channels import shared_channel_credentials, shared_credential_warning
...
def _profile_use(args):
    from x19_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.x19)" if name == "default" else f"Switched to: {name}")
...
def _print_channel_clone_notice(name: str, source_label: str, clone_channels: bool, clone_flag: str) -> None:
    from x19_cli.profile_channels import (
...
def _profile_create(args):
    ...
    if cloned:
        ...
        if sync_imports:
            print(f"Import sources carried over — `x19 -p {name} import-agent --sync` "
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  x19 profile alias {name} --name <custom>")
            print(f"  Or access via flag:     x19 -p {name} chat")
        else:
    ...
    if served is not None and normalize_profile_name(name) in {normalize_profile_name(p) for p in served}:
        ...
        print("  x19 gateway restart    Serve this profile from the running multiplexed gateway")
    else:
...
def _profile_delete(args):
    from x19_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_x19_home),
    everything else to its named directory."""
    from x19_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from x19_constants import get_x19_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from x19_cli import profile_describer as _pd
    if all_flag:
...
def _profile_show(args):
    ...
    if dist_name:
        ...
        print(f"  (run `x19 profile info {name}` for full manifest)")
    if alias_name:
        print(f"Alias:   {alias_name} → x19 -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from x19_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_migrate_identity(args):
    ...
    from x19_cli.profile_identity import migrate_profile_identity
    try:
    ...
    if not migrated:
        _die(f"Error: session identity was not migrated. Restart or stop the gateway, then run:\n"
             f"    x19 profile migrate-identity {args.old_name} {args.new_name}", err=True)
...
def _profile_purge_identity(args):
    ...
    if not purged:
        _die(f"Error: session identity was not purged. Restart or stop the gateway, then run:\n"
             f"    x19 profile purge-identity {args.profile_name}", err=True)
    print(f"✓ Session/routing identity purged: {args.profile_name}")


def _profile_export(args):
    from x19_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from x19_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="x19_dist_preview_") as tmp:
        ...
        if plan.has_cron:
            print(
                ...
                f"  Review them with:  x19 -p {plan.manifest.name} cron list"
            )
        print(f"\n  Use with:      x19 -p {plan.manifest.name} chat")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")


def _profile_update(args):
    from x19_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from x19_cli.profiles import get_profile_dir, normalize_profile_name
    try:
        ...
        if current is None:
            _die(
                ...
                "Only profiles installed via `x19 profile install` can be updated."
            )
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  x19 -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
def _profile_info(args):
    from x19_cli.profile_distribution import describe_distribution, DistributionError
    try:
...
# Order mirrors the original if/elif chain; None = bare ``x19 profile``.
PROFILE_ACTIONS = {
