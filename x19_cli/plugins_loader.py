...
dependency/config-schema warnings. Mixed into :class:`x19_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``x19_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from x19_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from x19_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from x19_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from x19_cli.plugins import LoadedPlugin

logger = logging.getLogger("x19_cli.plugins")

_NS_PARENT = "x19_plugins"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable X19 home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``x19 tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``x19 tools`` checklist, and even an explicit
        ...
        """
        from x19_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `x19 plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        ``x19 plugins install``/``enable`` and after ``x19 update`` (``x19_cli.plugin_python_deps``)
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Run `x19 plugins enable %s` to install them, "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from x19_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_x19 gate: skip cleanly (no import, no traceback) on a version mismatch.
        from x19_cli.plugins_manifest import requires_x19_error
        reason = requires_x19_error(manifest)
        if reason:
            loaded.error = reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from x19_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from x19_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `x19 plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from x19_cli.agent_plugins import load_agent_plugin
            package = load_agent_plugin(
                Path(manifest.path), get_x19_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``x19_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``x19_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another X19 home, or an earlier force
