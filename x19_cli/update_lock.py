"""Cross-process mutual exclusion for in-flight X19 updates.
...
MARKER_NAME = ".x19-update-in-progress"

# Set by an orchestrating updater (Tauri `x19-setup --update`) to its own pid before
# spawning `x19 update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "X19_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "X19 is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* X19 home (never the context-local profile override): the Rust
    updater resolves ``$X19_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_x19_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``x19 update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another X19 update is already running (started {elapsed} ago, "
        f"process {holder.pid}).\n"
        "\n"
        "  Running two at once would corrupt the install. Wait for it to finish\n"
        "  (watch `x19 logs`), or close the Desktop/dashboard window that\n"
        "  started it, then run `x19 update` again."
    )
