...
Imported by ``x19_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
x19_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(x19_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect X19_HOME?"""
    active = (_read_text(os.path.join(x19_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".x19")


def _resolved_home() -> str:
    return os.environ.get("X19_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("X19_DEV") == "1" or is_container_startup_environment():
        return False
    x19_home = os.environ.get("X19_HOME", "").strip()
    if x19_home:
        if os.path.exists(os.path.join(x19_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(x19_home)))
        return parent_name != "profiles" and active_profile_may_override_home(x19_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``x19 --version`` output (also used by /version).
    ...
    try:
        ...
        from x19_cli import __release_date__, __version__

        print(f"X19 v{__version__} ({__release_date__})")
    ...
    try:
        from x19_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``x19 --version`` before the heavy import wall.
    ...
    X19_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("X19_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
