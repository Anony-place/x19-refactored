#!/usr/bin/env python3
"""X19 CLI - Main entry point.

Usage:
    x19                     # Interactive chat (default)
    x19 chat / gateway / setup / status / cron / doctor / update / ...
    x19 --version           # Show version and update status
    x19 <cmd> --help        # Per-command help
"""

# x19_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``x19 update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``x19 update``.
try:
...
from x19_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python x19_cli/main.py`` (script mode: sys.path[0]
# is x19_cli/, not the repo root) can import x19_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from x19_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``x19 update`` may have left a recovery marker with a core package wiped;
# the x19_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``x19_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from x19_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from x19_cli.oneshot import run_oneshot
...
def _warn_if_unsupervised_pid1(pid: "int | None" = None) -> None:
    ...
    children). A Compose service that overrides ``entrypoint:`` to invoke x19 directly makes
    x19 itself PID 1 — nothing then ``wait()``s on those orphans and they accumulate as
    ...
    if (pid if pid is not None else os.getpid()) != 1:
        return
    print(
        "[x19] WARNING: this process is PID 1 with no init above it "
...
# x19_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".x19", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``X19_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("X19_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``X19_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("X19_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``x19 --version`` is answered before config/logging imports.
...
from x19_cli.subcommands.cron import build_cron_parser
from x19_cli.subcommands.sync import build_sync_parser
from x19_cli.subcommands.gateway import build_gateway_parser
from x19_cli.subcommands.profile import build_profile_parser
from x19_cli.subcommands.model import build_model_parser
from x19_cli.subcommands.setup import build_setup_parser

from x19_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from x19_cli.subcommands.slack import build_slack_parser
from x19_cli.subcommands.login import build_login_parser
from x19_cli.subcommands.logout import build_logout_parser
from x19_cli.subcommands.auth import build_auth_parser
from x19_cli.subcommands.status import build_status_parser
from x19_cli.subcommands.pause import build_pause_parser
from x19_cli.subcommands.webhook import build_webhook_parser
from x19_cli.subcommands.hooks import build_hooks_parser
from x19_cli.subcommands.doctor import build_doctor_parser
from x19_cli.subcommands.verify import build_verify_parser
from x19_cli.subcommands.security import build_security_parser
from x19_cli.subcommands.approvals import build_approvals_parser
from x19_cli.subcommands.dump import build_dump_parser
from x19_cli.subcommands.debug import build_debug_parser
from x19_cli.subcommands.backup import build_backup_parser
from x19_cli.subcommands.import_cmd import build_import_cmd_parser
from x19_cli.subcommands.import_agent import build_import_agent_parser
from x19_cli.subcommands.config import build_config_parser
from x19_cli.subcommands.skin import build_skin_parser
from x19_cli.subcommands.console import build_console_parser
from x19_cli.subcommands.update import build_update_parser
from x19_cli.subcommands.uninstall import build_uninstall_parser
from x19_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from x19_cli.subcommands.gui import build_gui_parser
from x19_cli.subcommands.logs import build_logs_parser
from x19_cli.subcommands.prompt_size import build_prompt_size_parser
from x19_cli.subcommands.memory import build_memory_parser
from x19_cli.subcommands.acp import build_acp_parser
from x19_cli.subcommands.tools import build_tools_parser
from x19_cli.subcommands.insights import build_insights_parser
from x19_cli.subcommands.monitoring import build_monitoring_parser
from x19_cli.subcommands.skills import build_skills_parser
from x19_cli.subcommands.pairing import build_pairing_parser
from x19_cli.subcommands.plugins import build_plugins_parser
from x19_cli.subcommands.mcp import build_mcp_parser
from x19_cli.subcommands.claw import build_claw_parser
from x19_cli.subcommands.vault import build_vault_parser
from x19_cli.subcommands.moa import build_moa_parser
from x19_cli.subcommands.fallback import build_fallback_parser
from x19_cli.subcommands.worktree import build_worktree_parser
from x19_cli.subcommands.browser import build_browser_parser
from x19_cli.subcommands.secrets import build_secrets_parser
from x19_cli.subcommands.egress import build_egress_parser
from x19_cli.subcommands.migrate import build_migrate_parser
from x19_cli.subcommands.checkpoints import build_checkpoints_parser
from x19_cli.subcommands.bundles import build_bundles_parser
from x19_cli.subcommands.curator import build_curator_parser
from x19_cli.subcommands.pets import build_pets_parser
from x19_cli.subcommands.journey import build_journey_parser
from x19_cli.subcommands.computer_use import build_computer_use_parser
from x19_cli.subcommands.sessions import build_sessions_parser
from x19_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'x19 {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any x19 module import: many modules
# cache X19_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# X19_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.x19/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors x19_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `x19 mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to X19' own profile selector.
    """
    try:
...
def _looks_like_x19_invocation() -> bool:
    """False when ``sys.argv`` belongs to a test runner rather than a ``x19`` run.

    pytest's own ``-p no:xdist`` reaches ``_scan_profile_flag`` through ``sys.argv`` at import
    time; it must stay a silent skip, while a real ``x19 -p 'Work Bot'`` must fail loudly.
    """
    return "pytest" not in (sys.argv[0] or "")


def _exit_invalid_profile_name(value: str) -> None:
    from x19_cli.profiles import _invalid_profile_name_error

    print(f"Error: {_invalid_profile_name_error(value)}", file=sys.stderr)
    print("Run `x19 profile list` to see your profiles.", file=sys.stderr)
    sys.exit(2)
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`x19 chat -p
    ...
    ``x19`` run — after a subcommand, ``-p`` may belong to that subcommand or a
    plugin (`x19 kanban ... -p 8080`), and option-looking values (``no:xdist``,
    ``--flag``) are always a silent skip.
    """
    from x19_cli._parser import top_level_value_flag_sets
    ...
    while i < len(argv):
        ...
        if arg in {"--profile", "-p"} and i + 1 < len(argv):
            ...
            if re.match(_PROFILE_NAME_RE, value):
                return value, 2, i
            if not saw_subcommand and not _looks_like_option_value(raw) and _looks_like_x19_invocation():
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo x19 -p <name>` against the invoking user's home.
    ...
    if name == "default":
        return None
    from x19_constants import named_profile_is_live, sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin X19_HOME to the profile dir; a bare invocation
    means "the root X19_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``x19 profile use``) would silently redirect the default gateway into
    ...
    X19_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    X19_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), X19_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("X19_SUPERVISED_CHILD") or os.environ.get("X19_S6_SUPERVISED_CHILD"):
        return True
    ...
    return os.environ.get(
        "X19_GATEWAY_EXTERNAL_SUPERVISOR", ""
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set X19_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # X19_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the x19 root (systemd hardcodes X19_HOME=/root/.x19)
    # we must still read active_profile — the user may have run
    # `x19 profile use` and the gateway should honour it (#22502).
    x19_home_env = os.environ.get("X19_HOME", "")
    if profile_name is None and x19_home_env and Path(x19_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and x19_home_env and os.environ.get("X19_UPDATE_POST_SWAP") == "1":
        return
    ...
    if profile_name is None and not _under_gateway_supervisor(argv) and not _desktop_ssh_backend(argv):
        try:
            ...
            active_path = get_default_x19_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from x19_cli.profiles import resolve_profile_env

        x19_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        x19_home = _resolve_sudo_user_profile_env(profile_name)
        if not x19_home:
        ...
        # A bug in profiles.py must NEVER prevent x19 from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["X19_HOME"] = x19_home
...
# Windows launcher self-heal — the ``x19`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``x19 update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m x19_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no x19 module may import before
...
# ``<checkout>\bin``, where ``x19 update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``x19`` stopped
...
if sys.platform == "win32":
    try:
        from x19_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.x19/.env first, then project root as dev fallback.
...
load_x19_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets → X19_REDACT_SECRETS BEFORE x19_logging
...
try:
    ...
    # serves x19_logging, x19_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from x19_cli.config_effective import load_user_config_effective as _load_effective_early

    _cfg_path = get_x19_home() / "config.yaml"
    if _cfg_path.exists():
        _early_cfg_raw = _load_effective_early(_cfg_path)
        if "X19_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["X19_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from x19_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from x19_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if x19_constants not importable yet

import logging
import threading
from datetime import datetime

from x19_cli import __version__, __release_date__

from x19_cli.model_setup_flows import (
...
from x19_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from x19_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from x19_cli.main_dashboard import (
...
from x19_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from x19_cli.main_provider_setup import (
...
from x19_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from x19_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from x19_cli.main_desktop import (
    cmd_gui,
)
from x19_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
...
from x19_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from x19_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from x19_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_x19_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("X19_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("X19_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.x19/.env assigns a non-empty value to any provider key."""
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    counts only once its identity exists, and the boot bootstrap (``x19_cli.free_tier_bootstrap``)
    ...
    """
    from x19_cli.config import DEFAULT_CONFIG, get_env_path, get_x19_home, load_config
    from x19_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from x19_cli.config import split_model_config_default
    ...
    _has_x19_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_x19_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once X19 is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_x19_config and not strict_profile_scope:
        try:
    ...
    try:
        from x19_cli.anon_auth import guest_enabled, has_guest
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from x19_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``x19_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``x19 -c`` from repo A continues repo A's last session rather than the
    ...
    """
    # A finite `x19 -z`/`chat -q` run is CLI history too: `x19 -z … --resume latest` chains on it.
    if source == "cli":
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, x19_bin
        cli_args: the original CLI arguments (everything after 'x19')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    x19_bin = container_info["x19_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo x19 {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo x19 {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    db = None
    try:
        from x19_state_ids import new_session_id as mint_session_id
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'x19 sessions list' to see available sessions, or "
            ...
            try:
                from x19_cli.terminal_breadcrumbs import resolve_breadcrumb_session
            ...
            if _crumb_id:
                ...
                if last_id:
                    ...
                    print(
                        ...
                        "`x19`, or list sessions with `x19 sessions list`.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from x19_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: x19 --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'x19 sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from x19_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from x19_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'x19 doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.x19/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from x19_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from x19_cli.config import get_x19_home
            skills_dir = Path(get_x19_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import x19_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `x19 setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like X19 isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  x19 setup")
    print()
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'x19 setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    from x19_cli.stream_json import stream_json_requested
    # Structured stdout is a non-interactive protocol: it overrides X19_TUI/display.interface too.
    ...
    from x19_cli.free_tier_bootstrap import run_bootstrap
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["X19_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["X19_SESSION_SOURCE"] = args.source
        ...
        os.environ["X19_SESSION_SOURCE_EXPLICIT"] = "1"
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older x19_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: X19CLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `x19` / `x19 chat`.
        from x19_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from x19_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from x19_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``x19 <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "x19_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "x19_cli.auth", "login_command", doc='Authenticate X19 CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "x19_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "x19_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "x19_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "x19_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "x19_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "x19_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "x19_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "x19_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "x19_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
...
cmd_pairing = _forward_command("cmd_pairing", "x19_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "x19_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "x19_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "x19_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "x19_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from x19_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from x19_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from x19_cli.auth import AuthError, format_auth_error, resolve_provider
    from x19_cli.config import get_compatible_custom_providers, get_env_value
    from x19_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'x19 model' for "
                "available providers, or run 'x19 doctor' to diagnose config "
    ...
    if not active:
        try:
            ...
            if exc.code == "no_provider_configured":
                ...
                # "run `x19 model`" from inside `x19 model` is circular.
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``x19 model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("X19_INFERENCE_PROVIDER") or "auto"
    ...
    from x19_cli.models import _PROVIDER_LABELS
    ...
    # x19_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.x19/.env that poisons
...
# ``x19_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on x19_cli.main; importing update_cmd eagerly would cost every ``x19``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    ...
    "x19_cli.dashboard_procs": (
        "_detect_concurrent_x19_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from x19_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `x19 security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from x19_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `x19 approvals <subcmd>`."""
    from x19_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from x19_cli.config import config_command
...
def cmd_backup(args):
    """Back up X19 home directory to a zip file."""
    from x19_cli import backup

    (backup.run_quick_backup if getattr(args, "quick", False) else backup.run_backup)(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `x19 --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall X19 (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from x19_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from x19_cli.uninstall import run_gui_uninstall
    ...
    from x19_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from x19_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from x19_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update X19")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running X19 runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from x19_cli.update_inventory import (
    ...
    from x19_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from x19_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update X19: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from x19_cli.update_lock import (
    ...
    from x19_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from x19_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``x19 -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from x19_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        if not _find_stale_dashboard_pids():
            print("No x19 dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `x19 update`;
        ...
        from x19_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `x19 serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with x19 serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with X19_DESKTOP=1 plus
    X19_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    X19_SERVE_HEADLESS=1). A shell inheriting those then running
    `x19 dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (X19_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    X19_SERVE_HEADLESS itself.
    """
    if os.environ.get("X19_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("X19_WEB_DIST", "")):
            os.environ.pop("X19_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("X19_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other X19 surface.
    try:
        from x19_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        from x19_cli.main_dep_hints import missing_optional_deps_message
    ...
    try:
        ...
        # #61115, #65696.
        from x19_cli.config import apply_terminal_config_to_env
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("X19_DESKTOP") == "1"
...
def cmd_dashboard(args):
    ...
    from x19_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from x19_cli import completion
...
def cmd_logs(args):
    """View and filter X19 log files."""
    from x19_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe X19 command console."""
    from x19_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``x19`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``x19 photon`` failing
    ...
    not import it, so the CLI registration never happens and ``x19 photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("X19_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, X19_TUI env, display.interface config). Bare
    # `x19`/`x19 chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: X19_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from x19_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from x19_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from x19_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["X19_SAFE_MODE"] = "1"
    os.environ["X19_IGNORE_USER_CONFIG"] = "1"
    os.environ["X19_IGNORE_RULES"] = "1"
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("X19_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from x19_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("X19_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("X19_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["X19_DEFER_AGENT_STARTUP"] = "1"
            os.environ["X19_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["X19_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `x19 --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``x19``) — matching is exact. ``X19_AGENT`` is the
    X19-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. X19 running inside another agent's terminal).
    """
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from x19_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``x19 sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from x19_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``x19`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``x19 --help`` order; keep it stable. Groups
    live in ``x19_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``x19 proxy`` (gateway group) is the INBOUND one.
    ...
    from x19_cli.send_cmd import register_send_subparser
    ...
    from x19_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from x19_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    from x19_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from x19_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (X19-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``x19 -c model`` — 'model' is the session name)
...
def main():
    ...
    # Sweep stale ``x19.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_x19_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``x19 update``, and
    ...
    # substring match is deliberately loose: over-matching (``x19 skills
    # install update``) only defers recovery one launch; under-matching
    # (``x19 -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from x19_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
