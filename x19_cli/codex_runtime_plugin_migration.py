"""Migrate X19 MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
from agent.transports.x19_tools_mcp_server import X19_TOOLS_MCP_SERVER_NAME
...
MIGRATION_END_MARKER = "# end x19 managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in X19 config.")
...
# X19 MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown X19 keys.
_KNOWN_X19_KEYS = {
...
# (x19 key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, x19_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one X19 MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. X19' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(x19_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if x19_cfg.get("command"):
        if x19_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(x19_cfg["command"])
        if x19_cfg.get("args"):
            out["args"] = [str(a) for a in x19_cfg["args"]]
        if x19_cfg.get("env"):
            out["env"] = _str_map(x19_cfg["env"])
        if x19_cfg.get("cwd"):
            out["cwd"] = str(x19_cfg["cwd"])
    elif x19_cfg.get("url"):
        out["url"] = str(x19_cfg["url"])
        if x19_cfg.get("headers"):
            out["http_headers"] = _str_map(x19_cfg["headers"])
        if x19_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for x19_key, codex_key, note in _TIMEOUT_KEYS:
        if x19_key in x19_cfg:
            try:
                out[codex_key] = float(x19_cfg[x19_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if x19_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in x19_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_X19_KEYS:
            skipped.append(f"{key} (unknown X19 key)")
    return out, skipped
...
# env-var passthrough (X19_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by X19)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="x19-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a X19_HOME pointing there burned into
...
def _build_x19_tools_mcp_entry() -> dict:
    """Codex stdio entry launching X19' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    X19_HOME passes through only IF SET, read from os.environ (not get_x19_home()): when
    unset the codex subprocess must inherit its launcher's runtime X19_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched X19_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched x19 finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # X19_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_x19_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever X19_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's X19_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("X19_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    x19_home = os.environ.get("X19_HOME") or ""
    if x19_home and not _looks_like_test_tempdir(x19_home):
        env["X19_HOME"] = x19_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["X19_QUIET"] = "1"
    env["X19_REDACT_SECRETS"] = env.get("X19_REDACT_SECRETS", "true")
    return {
...
def migrate(
    x19_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_x19_tools: bool = True) -> MigrationReport:
    """Translate X19 mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_x19_tools`` registers X19' own tool surface
    (agent/transports/x19_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    x19_servers = (x19_config or {}).get("mcp_servers") or {}
    if not isinstance(x19_servers, dict):
        report.errors.append("mcp_servers in X19 config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in x19_servers.items():
        name = str(raw_name)
    ...
    if expose_x19_tools:
        translated[X19_TOOLS_MCP_SERVER_NAME] = _build_x19_tools_mcp_entry()
        if X19_TOOLS_MCP_SERVER_NAME not in report.migrated:
            report.migrated.append(X19_TOOLS_MCP_SERVER_NAME)
