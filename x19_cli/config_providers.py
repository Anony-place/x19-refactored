...
Split out of ``x19_cli/config.py``; every name is re-imported there, so
``x19_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``x19_cli.config``
...
logger = logging.getLogger("x19_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but X19'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by X19, not hand-curated.
...
def get_custom_provider_model_capability(
    ...
    from x19_cli.config import get_compatible_custom_providers, load_config_readonly
