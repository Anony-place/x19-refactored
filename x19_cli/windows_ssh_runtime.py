...
def _root() -> Path:
    ...
    # X19_HOME) cannot move the reader away from the helper's token.
    return get_default_x19_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, x19_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(x19_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is the x19 exe or (normally) the base Python, whose path varies by venv/uv
    ...
    launches_module = (
        argv[1:3] == ["-m", "x19_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "x19_cli.main" in argv[2]))
...
def terminate_owned(pid: int, creation_time_ns: int, x19_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, x19_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    process that both owns the port and is the one we lock. x19_cli's parent is prepended
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the x19_cli package")
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('x19_cli.main',run_name='__main__',alter_sys=True)")
...
def inspect_x19(x19_path: str) -> dict[str, Any]:
    path = os.path.abspath(x19_path)
    if not os.path.isabs(x19_path) or not os.path.isfile(path):
        raise ValueError("X19 path is not an executable file")
...
def _probe(*_: str) -> dict[str, Any]:
    import platform
    return {"os": "Windows", "arch": platform.machine(), "x19Home": str(get_default_x19_root()), "python": sys.executable}
