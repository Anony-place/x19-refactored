"""Persistent session goals — the Ralph loop for X19.
...
# (schema DDL, FTS tables, first x19_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per X19_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``x19 profile delete`` force-closes every handle under the profile home
        # (``x19_state_registry.close_all_under``) before rmtree; a same-name recreate in this
