...
from x19_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from x19_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from x19_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.x19/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_x19_home() / "logs"
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from x19_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``x19 <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``x19 -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_x19_dotenv``
    ...
    Named-profile actions therefore start from X19' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``X19_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from x19_cli.env_loader import (
            _PROFILE_MANAGED_ENV_KEYS, _env_keys_defined_in_dotenv, get_secret_source_values,
        )
        from x19_cli.web_server_profiles import _resolve_profile_dir
        from x19_constants import apply_subprocess_home_env, get_default_x19_root
        ...
        try:
            source_homes = {str(get_default_x19_root()), str(get_x19_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["X19_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["X19_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _X19_GATEWAY=1;
    ...
    action_env.pop("_X19_GATEWAY", None)
    if env_overrides:
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    ``x19 update`` fleet restart (``update_cmd_fleet._needs_sudo`` / ``_sudo_noninteractive_ok``).
    """
    from x19_cli.update_cmd_fleet import _needs_sudo
    ...
    from x19_cli.gateway import _select_systemd_scope
    from x19_constants import reset_x19_home_override, set_x19_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from X19_HOME, so a selector-bearing action must be resolved
    ...
    from x19_cli.web_server_profiles import _resolve_profile_dir
    token = set_x19_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_x19_action(
    ...
    if _action_targets_system_gateway(subcommand):
        ...
        # X19_HOME past sudo's env_reset and reads SUDO_USER for the service identity.
        ...
        # two-step gate as the ``x19 update`` fleet restart: a refused blanket probe falls
        # back to ``sudo -l`` on the exact argv, so a command-scoped NOPASSWD entry qualifies.
        from x19_cli.update_cmd_fleet import _sudo_noninteractive_ok
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo x19 {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _X19_GATEWAY).
    ...
    action_id = (env_overrides or {}).get("X19_ACTION_ID")
    if action_id:
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``x19 --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_x19_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``x19 [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``X19_HOME`` and hit the same exit-78 refusal."""
    from x19_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from x19_constants import get_process_x19_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_x19_home()) == "default" else ["-p", "default"]
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    The spawned ``x19 -p X gateway <verb>`` would only print exit-78 / "no gateway running for this
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from x19_cli.web_server import _spawn_gateway_restart
    try:
