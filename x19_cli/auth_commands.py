...
# ``x19 auth add openrouter --api-key sk-or-...`` must keep working with no ``--type``.
...
def _unknown_provider_exit(provider: str) -> SystemExit:
    ...
    return SystemExit(
        f"Unknown provider '{provider}'.{hint} Run `x19 auth` to see the provider list, or "
        "`x19 model` to pick one interactively.")
...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_x19_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from x19_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``x19 auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:x19_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> PooledCredential:
    """``x19 auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared Nous credential at <x19-root>/shared/nous_auth.json
    # (written by any previous login) makes `x19 --profile <name> auth add nous --type oauth`
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if spec is None:
        raise SystemExit(f"`x19 auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `x19 auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`x19 auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def _free_tier_lines() -> tuple[str, str]:
    ...
    from x19_cli.anon_auth import FREE_TIER_LABEL, GUEST_MODEL, UPGRADE_HINT
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source X19 reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`x19 auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `x19 auth list {provider}`).")
    ...
    if provider == "nous" and matched.source != "device_code":
        raise SystemExit(
            ...
            "Reauthenticate with `x19 auth add nous --type oauth`.")
    ...
    if refreshed is None:
        ...
        raise SystemExit(
            ...
            f"Sign in again with `x19 auth add {provider} --type oauth`.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `x19 auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `x19 auth` is called bare."""
...
def auth_upgrade_command(args) -> None:
    """``x19 auth upgrade``: sign the free tier into a Nous account, keeping its connectors."""
    from x19_cli.anon_auth import upgrade_guest
