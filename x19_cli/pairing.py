...
def pairing_command(args):
    """Handle x19 pairing subcommands."""
    ...
    if handler is None:
        print("Usage: x19 pairing {list|approve|revoke|clear-pending}")
        print("Run 'x19 pairing --help' for details.")
    else:
        handler()
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: x19 pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  To reset sooner, delete the '_lockout:{platform}' entry from ~/.x19/platforms/pairing/_rate_limits.json\n")
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'x19 pairing list' to see pending requests.\n")
