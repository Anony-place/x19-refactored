...
from x19_cli.config import get_env_path, get_x19_home, get_project_root
from x19_cli.env_loader import load_x19_dotenv
from x19_constants import display_x19_home

PROJECT_ROOT = get_project_root()
X19_HOME = get_x19_home()
_DHH = display_x19_home()  # user-facing display path (e.g. ~/.x19 or ~/.x19/profiles/coder)

# Load environment variables from ~/.x19/.env so API key checks work
_env_path = get_env_path()
load_x19_dotenv(x19_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from x19_cli.colors import Colors, color
from x19_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from x19_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from x19_cli.doctor_tools import _safe_which

from x19_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from x19_cli.doctor_platform import (
    _check_certificates,
...
from x19_cli.doctor_tools import (
    _check_git_and_rg,
...
from x19_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from x19_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Nous Portal auth", get_nous_auth_status_local())
        # Native OAuth is X19' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
...
def _ack_advisory(ack_target: str) -> None:
    """`x19 doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from x19_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Could not save the acknowledgement for {ack_target}. Make sure {_DHH}/config.yaml is "
                    f"writable (`x19 config path` prints the exact file), then re-run "
                    f"`x19 doctor --ack {ack_target}`.", Colors.RED))
        sys.exit(1)
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'x19 doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
...
def run_doctor(args):
    ...
    os.environ.setdefault("X19_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from x19_cli.doctor_live import maybe_run_live_checks
