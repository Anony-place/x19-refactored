...
``x19 gateway migrate --multiplex`` runs and multiplexes only when the fold would have been
...
default flipped, and the log names the blocker plus ``x19 gateway migrate --multiplex``.
"""
...
def explicit_multiplex_flag(default_home: Path) -> Optional[bool]:
    ...
    other processes (``x19 -p X ...`` has X's config loaded) asking about the default's file."""
...
def default_gateway_multiplexes(default_home: Optional[Path] = None) -> bool:
    ...
    root = Path(default_home) if default_home is not None else get_default_x19_root()
...
def implicit_multiplex_blocker() -> Optional[str]:
    ...
    Mirrors what makes ``x19 gateway migrate --multiplex`` refuse or leave a per-profile gateway
    ...
    if active != "default":
        return (f"this is profile '{active}'s own gateway; only the default profile's gateway "
                f"multiplexes (x19 gateway migrate --multiplex folds the fleet onto it)")
    ...
    from x19_cli.gateway_migrate import MIGRATE_COMMAND, _host_supports_migration, build_migration_plan
...
def record_multiplex_decision(decision: MultiplexDecision) -> None:
    """Persist a guard refusal into ``gateway_state.json`` so `x19 gateway status` can show why this
...
def log_multiplex_decision(decision: MultiplexDecision) -> None:
    ...
    if decision.source == "config" and not decision.enabled:
        logger.info("gateway.multiplex_profiles is false: serving this profile only "
                    "(x19 gateway migrate --multiplex folds every profile onto the default gateway).")
