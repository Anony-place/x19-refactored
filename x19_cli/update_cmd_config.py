"""Post-``x19 update`` config-schema migration for the active profile and every sibling.
Names are re-imported by ``update_cmd`` (``x19_cli.update_cmd.<name>`` resolves/monkeypatches);
...
def _run_config_check_fresh() -> tuple:
    ...
    from x19_cli.config import check_config_version
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    sibling (active skipped): scope via the context-local X19_HOME override (never ``os.environ``)
    ...
    profile, but ``x19 update`` historically migrated only the active profile's config — siblings drifted
    versions until their gateway hit a config the new code couldn't read.
    """
    from x19_cli.update_cmd import _run_config_check_fresh, _run_migrate_config_fresh
    migrated: list[tuple[str, int, int]] = []
    with _best_effort('Sibling profile enumeration failed: %s'):
        from x19_constants import (
            get_process_x19_home, reset_x19_home_override, set_x19_home_override)
        from x19_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
        active_home = get_process_x19_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_x19_home_override(entry)
            try:
...
def _restore_snapshot_safety_nets(pre_update_snapshot_id) -> None:
    ...
    *pre_update_snapshot_id*) and every sibling profile (against ITS OWN snapshot)."""
    ...
    with _best_effort('Sibling cron auto-restore check failed: %s'):
        from x19_cli.backup import restore_cron_jobs_all_profiles
    ...
    with _best_effort('Sibling config auto-restore check failed: %s'):
        from x19_cli.backup import restore_config_model_settings_all_profiles
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from x19_cli.update_cmd import _gateway_prompt
    if assume_yes:
    ...
    try:
        ...
        print(
            ...
            "Run 'x19 config migrate' manually to configure.")
        return "n"
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    try:
        from x19_cli.config import get_missing_env_vars, get_missing_config_fields
        ...
        print("     Run 'x19 config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'x19 config migrate' to retry.")
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if unattended and missing_env:
                print("  ℹ API keys require manual entry: x19 config migrate")
        else:
            print()
            print("Skipped. Run 'x19 config migrate' later to configure.")
    else:
        print("  ✓ Configuration is up to date")

    # The migration above touched only the active profile; run the same NON-INTERACTIVE
    # migration per sibling home via the context-local X19_HOME override (never os.environ).
