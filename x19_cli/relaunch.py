"""Unified self-relaunch for X19 CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``x19 sessions browse`` / post-setup relaunch keep the user's mode."""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched x19."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_x19_bin() -> Optional[str]:
    """X19 entry point: ``sys.argv[0]`` if a real executable, else ``which x19``, else ``None``
    (caller falls back to ``python -m x19_cli.main``).
    ...
    lose the venv. Falling through to ``sys.executable -m x19_cli.main`` keeps the venv.
    """
    argv0 = sys.argv[0]
    ...
    def _is_unsafe_python_launcher(p: str) -> bool:
        ...
        from x19_cli.linux_desktop_entry import _needs_interpreter
...
def build_relaunch_argv(
    ...
    """Construct an argv list for replacing the current process with x19."""
    bin_path = resolve_x19_bin()
    argv = [bin_path] if bin_path else [sys.executable, "-m", "x19_cli.main"]
...
def relaunch(
    ...
    """Replace the current process with a fresh x19 invocation.
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``x19`` not on
            # PATH yet (install hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nX19 relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run x19.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
