"""``x19 gateway migrate --multiplex`` / ``--standalone``: move a per-profile-gateway install onto one
...
what the multiplexer would do at startup. ``x19 update`` calls :func:`maybe_auto_migrate_after_update`.
"""
...
MIGRATE_COMMAND = "x19 gateway migrate --multiplex"
...
class ProfileGateway:
    ...
    runtime_home: Optional[Path] = None  # X19_HOME the installed unit pins, when it differs from ``home``
...
class MigrationPlan:
    ...
    def eligible_for_migration(self) -> bool:
        ...
        This is the AUTO-migration (``x19 update``) bar; the explicit command also proceeds with
...
def _home_env(home: Path) -> Iterator[None]:
    """Run service-manager helpers as if ``home`` were the active X19_HOME. Both the contextvar
    override (``get_x19_home``) and ``os.environ`` (``gateway.status`` identity files, unit
    ...
    previous = os.environ.get("X19_HOME")
    token = set_x19_home_override(str(home))
    os.environ["X19_HOME"] = str(home)
    x19_constants._default_x19_root_memo = None
    try:
        yield
    finally:
...
def _gateway_identity(home: Path, pid: Optional[int], services: list[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    from x19_cli.gateway_migrate_guards import gateway_identity
...
def _installed_services(home: Path) -> list[tuple[str, bool]]:
    ...
    from x19_cli import gateway as gw
...
def _stop_gateway_process(home: Path) -> None:
    from x19_cli.profiles import _stop_gateway_process
...
def _write_multiplex_flag(default_home: Path, value: bool) -> None:
    ...
    (same read-guard + nested-set + atomic write ``x19 config set`` uses; no raw YAML edits)."""
    from x19_cli.config import _set_nested, _write_user_config, require_readable_config_before_write
        ...
        for platform, platform_config in cfg.platforms.items():
            ...
            if platform_serves_profile_prefix(platform.value):
                ...
                plan.blockers.append(
                    ...
                    f"keep '{profile.name}' on a standalone gateway (x19 -p {profile.name} gateway start --force)."
                )
...
def build_migration_plan() -> MigrationPlan:
    ...
    for check in _PREFLIGHT_CHECKS:
        check(plan, configs)
    from x19_cli.gateway_migrate_guards import auto_migration_blockers
    ...
    plan.notices.extend(f"Not migrated automatically by `x19 update`: {b}" for b in auto_migration_blockers(plan))
    plan.notices.append(
...
def format_plan(plan: MigrationPlan, *, dry_run: bool) -> list[str]:
    ...
    lines.append(f"  - record the previous state in {plan.default_home / MANIFEST_NAME} (rollback: x19 gateway migrate --standalone)")
    return lines + _plan_tail(plan)
...
def _no_manifest_lines(default_home: Path) -> list[str]:
    return [f"✗ No migration manifest at {_manifest_path(default_home)}; nothing to roll back.",
            "  To leave multiplex mode by hand: x19 config set gateway.multiplex_profiles false && x19 gateway restart"]
...
def format_update_warning(plan: MigrationPlan, auto_blockers: list[str]) -> list[str]:
    return [
        ...
        *[f"    • {b}" for b in (*plan.blockers, *auto_blockers)],
        ...
        "  (`x19 update` will migrate automatically once nothing blocks it.)",
    ]
...
def apply_migration(plan: MigrationPlan, *, served_wait: float = _SERVED_WAIT_SECONDS) -> bool:
    ...
    if plan.interrupted:
        ...
        _print([f"✗ A previous migration's manifest is still at {_manifest_path(plan.default_home)} (its rollback did not finish).",
                "  Finish it with: x19 gateway migrate --standalone   (or delete the manifest to start over)"])
        return False
    else:
    ...
    try:
        ...
        if not rolled_back:
            print(f"  Re-run {MIGRATE_COMMAND} to resume, or x19 gateway migrate --standalone to roll back.")
        return False
    ...
    if served is not None and expected <= set(served):
        _print(["", f"✓ Migrated: the default gateway now serves {len(served)} profiles: {', '.join(served)}",
                "  Rollback any time with: x19 gateway migrate --standalone",
    ...
    _print(["", f"⚠ Migration applied, but the default gateway has not confirmed serving: {', '.join(missing)}",
            "  Check `x19 gateway status` and the gateway log; the flag and manifest are in place.",
            "  Rollback: x19 gateway migrate --standalone"])
    return False
...
def rollback_migration(default_home: Optional[Path] = None) -> bool:
    ...
    if default_gw.has_gateway:
        try:
            ...
            print("    The default gateway is still multiplexing; stop it by hand (x19 gateway stop) and re-run.")
    if ok:
...
def _host_supports_migration() -> Optional[str]:
    ...
    if gw.is_windows():
        return "Windows Scheduled Tasks are not migrated automatically; set gateway.multiplex_profiles true, stop the per-profile tasks, and `x19 gateway restart`."
    return None


def cmd_migrate(args) -> None:
    """``x19 gateway migrate [--multiplex|--standalone] [--dry-run] [--yes]``."""
...
def maybe_auto_migrate_after_update() -> None:
    """``x19 update`` hook: with >= 2 profiles, per-profile gateways present and multiplex off,
    ...
    service-domain / UNIX-user / X19_HOME boundary blocks this path only (the explicit command decides)."""
    from x19_cli.gateway_migrate_guards import auto_migration_blockers, auto_migration_opted_out
