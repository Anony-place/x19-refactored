...
01-x19-setup (the stage2 hook) has chowned the volume and seeded $X19_HOME, but
...
def reconcile_profile_gateways(
    *, x19_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$X19_HOME``): ``x19_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``x19 gateway start`` (no ``-p``) targets.

    Without it, bare ``x19 gateway start`` inside the container would land on ``s6-svc -u
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        x19_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(x19_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(x19_home)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", x19_home, default_prior_state, default_should_start))

    profiles_root = x19_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            # SOUL.md (seeded by `x19 profile create`) is the "real profile" marker.
    ...
    if not dry_run:
        _write_reconcile_log(x19_home, actions)
    return actions


def _maybe_migrate_legacy_gateway_run_state(
    x19_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = x19_home / "gateway_state.json"
    if state_file.exists():
        return None
    if os.environ.get("X19_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the x19 args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "x19":  # the wrapper re-execs `x19 <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `x19 -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with x19 ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the x19 user won't EACCES.
...
def _write_reconcile_log(x19_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $X19_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted X19_HOME both
    ...
    actions = reconcile_profile_gateways(x19_home=x19_home, scandir=scandir)
    for a in actions:
