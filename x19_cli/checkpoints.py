"""`x19 checkpoints` CLI subcommand.
...
from x19_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: x19 checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``x19 checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `x19 checkpoints` → status
