...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "x19_cli.gateway",
        "x19_cli.auth",  # provider auth state → copilot_auth → subprocess
        "x19_cli.copilot_auth",
        "x19_cli.runtime_provider",
        "x19_cli.skin_engine",  # resolve_skin() config + skin engine init
        "x19_cli.inventory",  # provider catalogs + models.dev cache
        "x19_cli.model_switch",
    ):
        try:
            __import__(mod)
        except Exception:
...
def _eager_reconcile_own_session_db() -> None:
    ...
    :func:`x19_cli.web_server_sessions._open_session_db_at_path`. Never
    raises: an unfixable store still gets the per-poll read-probe heal.
    """
    try:
        from x19_cli.web_server_sessions import _open_session_db_for_profile
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``X19_DESKTOP_READY_FILE`` is set.
    ...
    """
    target = os.environ.get("X19_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``X19_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``x19.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    """
    raw_pid = os.environ.get("X19_PARENT_PID")
    # Empty inherited values mean "absent", not "marker present but blank".
    start_marker = os.environ.get("X19_PARENT_START_MARKER") or None
    nonce = os.environ.get("X19_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable X19_PARENT_START_MARKER=%r / nonce; "
    ...
    try:
        poll = max(0.5, float(os.environ.get("X19_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'x19 serve' / 'x19 dashboard' backend or the X19 gateway. "
