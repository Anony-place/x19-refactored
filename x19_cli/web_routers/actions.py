"""Gateway restart/drain, X19 update and background-action status dashboard routes.

Extracted from ``x19_cli.web_server``; helpers/state that tests monkeypatch on
...
from x19_cli.config import format_docker_update_message, recommended_update_command_for_method
from x19_cli.web_deps import LateState, late
from x19_cli.web_server_gateway import _ACTION_LOG_FILES
from x19_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "x19_cli.config")
get_x19_home = late("get_x19_home", "x19_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "x19_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "x19_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "x19_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "x19_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import x19_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== x19-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "X19 updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== x19 update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``x19 gateway restart`` in the background."""
...
async def gateway_migrate_plan():
    ...
    from x19_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    """Run ``x19 gateway migrate --multiplex --yes`` detached; the CLI re-runs the preflight and
    refuses (exit 1 into the action log) when blocked, so the UI should gate on the plan first."""
    from x19_cli.web_server_gateway import _spawn_x19_action
    with http_failure("Failed to spawn gateway migrate", 500, "Failed to start gateway migration"):
        proc = _spawn_x19_action(["gateway", "migrate", "--multiplex", "--yes"], "gateway-migrate")
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("x19-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "x19-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/x19/update")
async def update_x19():
    """Kick off ``x19 update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("x19-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "x19-update", "already_running": True}
        action_id = _ACTION_IDS.get("x19-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn x19 update", 500, "Failed to start update"):
        proc = _spawn_x19_action(["update"], "x19-update", env_overrides={"X19_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "x19-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "X19 is managed by Termux APT; run `pkg upgrade x19`.",
}
...
async def check_x19_update(force: bool = False):
    ...
    try:
        from x19_cli.banner import check_for_updates, upstream_commits_behind

        if force:
            with contextlib.suppress(OSError):
                (get_x19_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "x19-update":
        ...
        if durable_update_action_id:
            marker = f"=== x19-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``x19 update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``x19 update`` run since #91283, including
...
@status_router.get("/api/x19/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `x19 update` run recorded).")
