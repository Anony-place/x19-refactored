...
from x19_cli.web_server_sessions import _maybe_auto_archive_for_profile, _session_latest_descendant
from x19_cli.web_models import (
    BulkDeleteSessions, SessionImport, SessionOwnerBackfill, SessionPrune, SessionRename)
from x19_cli.web_routers._common import log as _log, http_failure
from x19_state import is_malformed_db_error
from x19_state_errors import is_transient_sqlite_error

list_router = APIRouter()
search_router = APIRouter()
manage_router = APIRouter()

_cron_default_profile = late("_cron_default_profile", "x19_cli.web_server_cron")
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `x19 sessions prune`)."""
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_x19_home()
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics (mirrors `x19 sessions stats`)."""
    def _stats(db):
        out = {
...
async def get_session_timeline(
    ...
    """
    from x19_state_timeline import get_session_timeline as read_timeline
...
async def get_session_messages_around(
    ...
    from x19_state_timeline import get_session_messages_around as read_around
