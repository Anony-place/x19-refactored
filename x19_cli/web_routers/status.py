...
Extracted from ``x19_cli.web_server``; app state and helpers are late-bound through
:mod:`x19_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from x19_cli.web_deps import LateState, late
from x19_cli.web_server_gateway import _display_system_platform
...
from x19_cli.config import get_config_path, get_env_path
from x19_constants import get_process_x19_home, profile_name_for_home
from x19_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from x19_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "x19_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "x19_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "x19_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "x19_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "x19_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "x19_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "x19_cli.web_server_lifecycle")
_spawn_x19_action = late("_spawn_x19_action", "x19_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "x19_cli.config")
get_x19_home = late("get_x19_home", "x19_cli.config")
...
load_config = late("load_config", "x19_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "x19_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another X19 process uses it."""
...
async def get_ssh_ownership(request: Request):
    from x19_cli.web_server import _SSH_OWNER_NONCE
...
async def get_health_idle(request: Request):
    ...
    from x19_cli.web_server_idle_proof import idle_proof
    _require_token(request)
...
# Profile segment mirrors x19_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level X19_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``x19 --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_x19_home())
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from x19_cli.dashboard_auth import (
...
def _nous_session_validity() -> str:
    ...
    try:
        from x19_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from x19_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_x19_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from x19_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from x19_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of x19_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_x19": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_x19_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (X19 Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "x19_home": str(get_x19_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond x19 home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "x19_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_x19_home()))
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``x19 <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_x19_action(argv, name)
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from x19_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from x19_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from x19_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_x19_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from x19_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
