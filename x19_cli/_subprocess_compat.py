...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_X19_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    X19's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    X19 shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_x19(text: str) -> bool:
    r"""True when *text* names X19 at a path-segment / token boundary.

    A bare ``"x19" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\myx19notes\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("x19", ".x19"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_x19(pid: int) -> bool:
    """Best-effort check that *pid* currently runs X19 code."""
    try:
        import psutil
        ...
        return _text_names_x19(f"{command} {executable}")
    except Exception:
        return False
...
def pid_is_x19(pid: int, *, expected_start_time: int | None = None) -> bool:
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_x19(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_x19`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``x19 update`` on slow-WMI
