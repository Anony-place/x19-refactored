"""``x19 gateway restart`` for a gateway whose supervisor X19 did not install.
...
def gateway_declares_external_supervisor(pid: int, home: Path | None = None) -> bool:
    ...
    from gateway.status import _get_process_x19_home, read_runtime_status
    from x19_cli.gateway import _capture_gateway_argv

    home = home or _get_process_x19_home()
...
def restart_externally_supervised_gateway(supervised_pid: int) -> None:
    ...
    """
    from x19_cli.gateway import _get_restart_exit_wait_budget, _graceful_restart_via_sigusr1, _print_lines
    ...
    _print_lines(
        ...
        "  then rerun once it is healthy: x19 gateway restart",
    )
    sys.exit(1)
