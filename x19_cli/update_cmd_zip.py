"""ZIP-download fallback for ``x19 update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``x19_cli.update_cmd.<name>`` keeps
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = ".x19-update-staging", ".x19-update-old"
...
_STASH_HINT = "  Stash or commit your changes, then rerun `x19 update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``x19_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.x19-update-staging"
    backup = f"{dst}.x19-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.x19-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``x19-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"x19-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, x19_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ X19.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="x19-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"x19-{branch}.zip")
        ...
        print("  Re-run `x19 update` to retry; if the agent won't start, reinstall from https://anony-place.github.io/x19-refactored")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False, _windows_gateway_resume=None) -> bool:
    ...
    from x19_cli.update_cmd import _hand_off_post_swap, _m, _read_project_version, _resolve_update_options, _sweep_bytecode_after_update
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `x19 update "
            f"--branch {branch}`, or update against main with `x19 update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    _download_and_swap_zip(branch, f"https://github.com/Anony-place/x19-refactored/archive/refs/heads/{branch}.zip")
...
def _finish_zip_update(
    *, active_tool_dependencies, pre_update_version, had_desktop_app_before_update: bool,
    ...
    if not import_ok:
        ...
        print("  Re-run `x19 update` to complete it.")
        _m().sys.exit(1)
