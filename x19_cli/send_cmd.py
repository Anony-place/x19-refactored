"""CLI subcommand: ``x19 send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"x19 send: {file_path} is not a text file. --file reads the "
                ...
                f'  x19 send --to telegram "MEDIA:{file_path}"\n'
                f'  x19 send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  x19 send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"x19 send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"x19 send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"x19 send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"x19 send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"x19 send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `x19 gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.x19/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_x19_env() -> None:
    """Populate the credential environment from ``~/.x19/.env`` AND bridge top-level ``config.yaml``
    ...
    if isinstance(scope, dict):
        ...
        if env_path.exists():
            try:
                from x19_cli.env_loader import _load_dotenv_with_fallback
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_x19_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `x19 send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "x19 send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  x19 send --to telegram \"hello\"\n"
            "  x19 send --to discord:#ops --file report.md\n"
            "  x19 send --list      # list available targets",
            _USAGE_EXIT)
    message = _read_message_body(getattr(args, "message", None), getattr(args, "file", None))
    if message is None or not message.strip():
        _fail(
            "x19 send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `x19 send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `x19 send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform X19 "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.x19/.env + ~/.x19/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  x19 send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
