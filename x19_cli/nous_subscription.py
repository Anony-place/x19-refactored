...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "x19-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from x19_constants import agent_browser_runnable
    ...
    try:
        ...
        # X19-managed Node dirs ($X19_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_x19_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("X19_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from x19_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# Inline Nous Portal login for the Tool Gateway picker (`x19 tools`)
