"""On-demand supply-chain audit for X19 installs.
...
def _discover_plugins(x19_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.x19/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, x19_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = x19_home or Path(get_x19_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `x19 security audit`."""
    ...
    components = _discover_components(x19_home=home, **skips)
    ...
    try:
        findings = run_audit(x19_home=home, components=components)
