"""``x19 checkpoints`` subcommand parser."""
...
def build_checkpoints_parser(subparsers) -> None:
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints", help="Inspect / prune / clear ~/.x19/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo x19 uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.")
    from x19_cli.checkpoints import register_cli as _register_checkpoints_cli
