...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description="Install, update, remove, list, or validate native X19 plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Bare plugin catalog entry name (see `x19 plugins search`), Git URL, or owner/repo "
            "shorthand (e.g. anpicasso/x19-plugin-chrome-profiles)")
    plugins_install.add_argument(
    ...
    _install_enable_group.add_argument(
        ...
        help="Install disabled (skip confirmation prompt); enable later with `x19 plugins enable <name>`",
    )

    plugins_search = plugins_subparsers.add_parser(
        "search", help="Search the curated X19 plugin catalog")
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack", help="Declarative, shareable plugin sets (x19-pack.yaml)",
    ...
    pack_install.add_argument("source", help="Path to a x19-pack.yaml file, or an https:// URL")
    ...
    pack_export.add_argument(
        "--name", default="my-x19-pack", help="Pack name to embed in the exported YAML")

    pack_show = pack_subparsers.add_parser(
        "show", help="Dry-run: parse and display a pack without installing")
    pack_show.add_argument("source", help="Path to a x19-pack.yaml file, or an https:// URL")
