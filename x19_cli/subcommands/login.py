...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``invalid choice``. Registered WITHOUT ``help=`` so it is omitted from ``x19 --help``
    ...
    login_parser = subparsers.add_parser(
        "login",
        description="Deprecated. Use `x19 auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider", default=None, help="(deprecated) Provider name; ignored — see `x19 model`")
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: x19-cli)")
