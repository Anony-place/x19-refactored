...
def build_migrate_parser(subparsers) -> None:
    ...
    from x19_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    migrate_relay = migrate_subparsers.add_parser(
        "relay", help="Convert legacy X19_NEMO_RELAY_ATIF_*/ATOF_* exporter vars into relay-plugins.toml",
        description="The NeMo Relay cutover stopped reading the legacy exporter variables; a .env that still "
            "carries them (and no X19_NEMO_RELAY_PLUGINS_TOML) exports nothing. Generate "
            "<x19 home>/relay-plugins.toml from them, point X19_NEMO_RELAY_PLUGINS_TOML at it, "
            "and comment the legacy lines out. `x19 update` runs this for every profile automatically.")
    migrate_relay.add_argument(
        "--all-profiles", action="store_true",
        help="Migrate the default home and every named profile (what `x19 update` does)")
    migrate_relay.add_argument(
...
def _cmd_migrate_relay(args) -> None:
    from x19_cli.relay_plugin_migrate import cmd_migrate_relay
