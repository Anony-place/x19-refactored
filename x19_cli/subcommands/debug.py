...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for X19. Use 'x19 debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    x19 debug share              Upload debug report (asks for confirmation)
    x19 debug share --yes        Skip confirmation (for scripts/CI)
    x19 debug share --lines 500  Include more log lines
    x19 debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    x19 debug share --local      Print report locally (no upload)
    x19 debug share --no-redact  Disable upload-time secret redaction
    x19 debug share --nous       Upload to Nous-internal storage (private)
    x19 debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'x19 debug share'")
