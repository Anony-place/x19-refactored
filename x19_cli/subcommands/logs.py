...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter X19 log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    x19 logs                    Show last 50 lines of agent.log
    x19 logs -f                 Follow agent.log in real time
    x19 logs errors             Show last 50 lines of errors.log
    x19 logs gateway -n 100     Show last 100 lines of gateway.log
    x19 logs gui -f             Follow gui.log in real time
    x19 logs desktop -f         Follow desktop.log (Electron app boot/backend)
    x19 logs --level WARNING    Only show WARNING and above
    x19 logs --session abc123   Filter by session ID
    x19 logs --component tools  Only show tool-related lines
    x19 logs --since 1h         Lines from the last hour
    x19 logs --since 30m -f     Follow, starting from 30 min ago
    x19 logs list               List available log files with sizes
""")
