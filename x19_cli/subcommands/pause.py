"""``x19 pause`` / ``x19 resume`` — the global emergency stop.

``pause`` writes the ESTOP sentinel at ``$X19_HOME/ESTOP``; cron, kanban and new gateway
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "X19 paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `x19 resume` to lift the pause.")
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  X19 resumed — dispatch picks up on the next tick.")
    else:
        print(f"X19 is not paused (no sentinel at {sentinel_path()}).")
    return 0
