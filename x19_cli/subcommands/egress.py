...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `x19 proxy` is the separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description="Manage iron-proxy, the optional TLS-intercepting egress firewall "
            ...
            "https://anony-place.github.io/x19-refactored/docs/user-guide/egress/iron-proxy")

    from x19_cli import proxy_cli as _proxy_cli
    _proxy_cli.register_cli(egress_parser)

    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``x19 proxy`` (dest='proxy_command').
