...
# Re-exported: callers/tests patch x19_cli.model_switch.<name>.
from x19_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix X19 wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by X19.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog X19 itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
_X19_MODEL_WARNING = (
    ...
    "for use with X19. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.).")

# Match only the real Nous Research chat families. The token below is the
# PROVIDER'S model name, not our product identity, so it must stay verbatim:
# renaming it silently disables the detector (a repo-wide brand rename did
# exactly that once, and nothing caught it because no model is named after us).
# A bare substring check on the family name also false-positived on tool-capable
# local Modelfiles that merely live under that tag namespace (a
# ``<family>-brain:qwen3-14b-ctx16k`` wrapper is agentic and must not warn).
#   match:    NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
#   no match: qwen3:14b, claude-opus-4-6, any *-brain:* Modelfile wrapper
_NOUS_CHAT_MODEL_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_nous_x19_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real Nous Hermes 3/4 chat model (single owner; cli.py uses it too)."""
    return bool(model_name and _NOUS_CHAT_MODEL_NON_AGENTIC_RE.search(model_name))


def _check_x19_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Nous Hermes 3/4 chat model, else ""."""
    return _X19_MODEL_WARNING if is_nous_x19_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from x19_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``X19CLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from x19_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
            if is_routing_aggregator(_norm_prov(current_provider)):
                from x19_cli.models import _find_openrouter_slug
    ...
    try:
        from x19_cli.models import normalize_provider
...
def parse_model_flags_detailed(raw_args: str) -> ModelFlagParseResult:
    ...
    is the raw level word (validated by :func:`x19_constants.parse_reasoning_effort` at apply
...
def _configured_provider_identity(slug: str, cfg: dict) -> tuple[str, str, str, str]:
    ...
    from x19_cli.config_providers import _canonical_api_mode, _normalize_custom_provider_entry
...
def _ollama_configured_base() -> tuple[dict, str]:
    from x19_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'x19 model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'x19 doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    from x19_cli.models_local import _same_ollama_native_root
...
def _convert_vendor_colon_slug(st: _Switch) -> None:
    ...
    ``left`` names a provider X19 knows, so ``/model alibaba:qwen3.6-plus`` routes like
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from x19_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from x19_cli.config import load_config
        from x19_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from x19_cli.providers import resolve_user_provider
    ...
    if user_pdef is not None and user_pdef.base_url:
        ...
        try:
            ...
            return st.fail_on_target(
                ...
                f"`x19 auth add {st.target_provider}`, or pick a connected provider in /model.\n"
                f"  Details: {e}")
    return None
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from x19_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
    keep_current_ollama_endpoint = False
    ollama_headers: dict[str, str] = {}
    if st.current_provider == "custom" and st.current_base_url:
        try:
            from x19_cli.models_local import should_use_ollama_native_catalog
...
def _validate_switch(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from x19_cli.models_local import _get_ollama_request_headers
    from x19_cli.models_validate import validate_requested_model
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from x19_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from x19_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from x19_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_x19_model_warning(st.new_model)) if w]
    ...
    try:
        from x19_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
def persist_model_selection(result: ModelSwitchResult, config_path: Any = None) -> None:
    ...
    ``X19_HOME/config.yaml`` — the context override or ``X19_HOME`` at call time).
    ...
    from x19_cli.config import get_config_path, read_user_config_raw, warn_unpinned_cron_jobs_after_model_config_change
...
def _scoped_key_env(name: str) -> str:
    ...
    ``$X19_HOME/.env`` authenticates the ``/model`` verification probe (#109315) and a rotated
