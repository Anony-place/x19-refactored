"""Plugin catalog — curated, Nous-approved X19 plugins shipped with the repo.
...
an installed X19 sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://anony-place.github.io/x19-refactored/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_x19": self.requires_x19,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_x19=str(data.get("requires_x19") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_x19_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``X19_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_x19_home(cache.parent)
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_x19:
        bits.append(f"Requires X19 {entry.requires_x19}.")
    return " ".join(bits)
