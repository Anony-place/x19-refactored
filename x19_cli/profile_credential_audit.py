...
"not connected to any AI provider" setup prompt instead of the owner's credentials. ``x19 update``
...
def _profile_has_own_credentials(profile_dir: Path) -> bool:
    ...
    from x19_cli.main import _dotenv_has_provider_key
...
def profiles_without_own_credentials() -> List[str]:
    ...
    from x19_cli.profiles import _iter_named_profile_dirs
    if not _auth_store_has_credentials(get_default_x19_root() / "auth.json"):
        return []
...
def print_profiles_without_credentials_notice() -> None:
    ...
    for name in names:
        print(f"     x19 -p {name} model        # or: x19 -p {name} auth add <provider>")
