"""Slash command registry for the X19 CLI and gateway.
...
from x19_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``x19_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of X19 config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal X19 works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .x19/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey X19' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while X19 is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.x19/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update X19 to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show X19 version", "Info", aliases=("v",),
