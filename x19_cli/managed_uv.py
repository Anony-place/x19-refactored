"""X19-managed uv and Python runtime repair.

The Python backing the install is shared by every X19 profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.x19-runtime/python``. A vulnerable interpreter is never reinstalled in place: a
...
from x19_cli.sqlite_runtime import (
    SQLiteRuntimeInfo, isolated_interpreter_env, probe_sqlite_runtime)

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".x19-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nousresearch.x19.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of X19' own uv binary (``$X19_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_x19_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def pip_install_hint(package: str) -> str:
    ...
    Names X19' own uv when it exists: the installer drops it in ``$X19_HOME/bin``
...
def managed_python_env(
    ...
    """Return a sanitized environment for X19-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: X19 keeps databases "
              "out of WAL mode on this SQLite build. The next `x19 update` "
              "will retry.")
        return
...
def _record_runtime_repair(repair: RuntimeRepairResult) -> None:
    """Put the repair outcome into the update receipt (no-op outside ``x19 update``).
    ...
    """
    from x19_cli.update_receipt import record_skip, record_step
...
def _uv_self_update_stamp() -> Path:
    from x19_constants import get_x19_home
    return get_x19_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``x19 update`` daily; a blocking network
...
def _reload_x19_constants():
    """Re-execute ``x19_constants`` from disk (the imported one may predate venv_python_path)."""
    import x19_constants
    return importlib.reload(x19_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the X19 generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `x19 update`. The
...
def _stream_sync(argv: list[str], *, cwd: Path, env: dict[str, str]) -> tuple[int, str]:
    ...
    scrollback only, and the rejection carried a bare exit code — "x19 update says the SQLite
...
def _install_uv(target: Path) -> None:
    ...
    into ``$X19_HOME/bin/`` instead of ``~/.local/bin/``.
    """
