...
from x19_cli.config import get_env_path, get_env_value, get_x19_home, load_config
from x19_cli.models import provider_label
from x19_cli.runtime_provider import resolve_requested_provider
from x19_cli.vercel_auth import describe_vercel_auth
from x19_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from x19_constants import OPENROUTER_MODELS_URL
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from x19_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from x19_cli.gateway_multiplex_served import served_profile_ingress_urls
...
def _render_cron(ctx):
    ...
    jobs_file = get_x19_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from x19_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_x19_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
