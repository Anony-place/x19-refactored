...
``x19_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from x19_cli.urllib_security import open_credentialed_url
from x19_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from x19_cli.models_reasoning_caps import (
...
_X19_USER_AGENT = f"x19-cli/{_X19_VERSION}"
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from x19_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_x19_home() / "cache" / "openrouter_curated_catalog.json"
...
def _nous_recommended_disk_path() -> "Path":
    from x19_constants import get_x19_home
    return get_x19_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from x19_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from x19_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    ...
    sign-in completion in ``x19_cli.anon_auth`` so both land on the same model.
    """
    from x19_cli import models_pricing as mp
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (x19 is tool-calling-first).
...
def fetch_openrouter_models(
    ...
    from x19_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    try:
        from x19_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — x19 requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from x19_cli.model_catalog import get_curated_nous_models
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from x19_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from x19_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``x19 model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from x19_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from x19_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``x19_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether X19 should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from x19_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from x19_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from x19_cli.model_search import model_alias_canonical
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from x19_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``x19 model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models X19 actually ships. (#46850)
    ...
    from x19_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $X19_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    inflight_key = cache_key if get_x19_home_override() is None else (x19_home_key(), cache_key)
...
def _provider_models_cache_path() -> Path:
    from x19_constants import get_x19_home
    return get_x19_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_x19_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``x19 model --refresh``."""
    try:
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _X19_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"x19/{_X19_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=x19"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _X19_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from x19_cli.model_switch_providers import _NativePickerModelList
