"""Image-authored deployment provenance for immutable X19 runtimes.

The image bakes ``/etc/x19/image-provenance.json`` outside ``$X19_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/x19/image-provenance.json")
