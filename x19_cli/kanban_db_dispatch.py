...
Split out of ``x19_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from x19_cli.kanban_db import Task
...
# ``X19_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``x19 kanban tail`` is the only place the
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|☤\s*X19")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``x19 kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``x19_cli.profiles.profile_exists``, or ``None`` when it cannot be
    ...
    """
    try:
        from x19_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
        return None
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several X19 homes),
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real X19 profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (X19 ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``X19_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``x19 -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``x19 kanban tail`` shows why the task looks stuck.
...
def _module_x19_argv() -> list[str]:
    """Interpreter-bound X19 CLI invocation (``x19_cli.main`` is the
    console-script target — there is no top-level ``x19`` package)."""
    return [sys.executable, "-m", "x19_cli.main"]


def _absolute_x19_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved X19 shim."""
...
def _x19_path_argv(path: str) -> list[str]:
    """argv for a resolved X19 executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_x19_argv()
    return [_absolute_x19_path(path)]


def _resolve_x19_argv() -> list[str]:
    """Resolve the ``x19`` invocation as argv for ``Popen``: ``$X19_BIN``
    ...
    x19_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("x19")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``x19_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``x19`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_x19_bin``; local because ``x19_cli``
    sits below ``gateway`` in the dependency order.
    """
    import importlib.util
    import shutil

    env_bin = os.environ.get("X19_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _x19_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _x19_path_argv(resolved_env_bin)
        return _module_x19_argv()
    ...
    x19_bin = _safe_which_no_cwd("x19") if _kb._IS_WINDOWS else shutil.which("x19")
    if x19_bin:
        return _x19_path_argv(x19_bin)
    return _module_x19_argv()
...
def _resolve_worker_cli_toolsets(x19_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``X19_KANBAN_TASK`` is set.
    """
    if not x19_home:
        return None
    try:
        ...
        token = set_x19_home_override(x19_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(x19_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for X19_HOME=%r (%s)",
            x19_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, x19_home: Optional[str]) -> list[str]:
    """Build the ``x19 -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_x19_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped X19_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(x19_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `x19 kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``x19 -p <profile> chat -q ...`` subprocess.
    ...
    ``X19_KANBAN_DB`` / ``X19_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from x19_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject X19_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_x19_home() falls back to the DEFAULT
    # profile root because `x19 -p` applies its override before
    # x19_constants is imported.
    if profile_home:
        env["X19_HOME"] = profile_home
    ...
    if task.tenant:
        env["X19_TENANT"] = task.tenant
    env["X19_KANBAN_TASK"] = task.id
    env["X19_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["X19_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["X19_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["X19_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["X19_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["X19_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `x19 -p` rewrites X19_HOME (symlink / Docker layouts).
    env["X19_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["X19_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["X19_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["X19_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads X19_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["X19_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping X19_TUI covers
    # older x19 builds on PATH that predate the flag's precedence.
    env.pop("X19_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("X19_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`x19` executable not found on PATH. "
            "Install X19 or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``x19 kanban dispatch`` — the standalone
...
from x19_cli import kanban_db as _kb  # noqa: E402
from x19_cli import kanban_db_connect as _kbc  # noqa: E402
from x19_cli import kanban_db_workspace as _kbw  # noqa: E402
