"""X19 update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``x19_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``x19_cli.main`` at call time.
"""
...
from x19_cli.config import get_x19_home  # noqa: F401  (re-exported; patched via update_cmd)
from x19_cli.update_cmd_common import _best_effort
from x19_constants import get_default_x19_root, project_venv_dir, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from x19_cli.update_abort_recovery import (  # noqa: F401
...
from x19_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _x19_holder_subcommand, _holder_value_flags,
...
from x19_cli.update_cmd_fleet import (  # noqa: F401
...
from x19_cli.update_cmd_zip import (  # noqa: F401
...
from x19_cli.update_cmd_stash import (  # noqa: F401
...
from x19_cli.update_cmd_config import (  # noqa: F401
...
from x19_cli.update_cmd_deps import (  # noqa: F401
...
from x19_cli.update_cmd_git import (  # noqa: F401
...
from x19_cli.update_cmd_maint import (  # noqa: F401
...
def _m():
    """Lazy ``x19_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from x19_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "x19_cli/main.py", "x19_cli/config.py", "x19_cli/__init__.py",
    "x19_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "x19_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from x19_cli.update_receipt import record_step
        record_step(step, ok, detail)


# A fetch whose transport dead-stalls (HTTP/2 to GitHub on some networks, a black-holed proxy)
# otherwise leaves `x19 update` on "Fetching updates..." forever (#93759, #95777). Five
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="x19-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from x19_constants import get_x19_home  # noqa: F811  (deliberate: constants variant)
    home = get_x19_home()
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live X19 launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["x19.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically X19")
    print("  Desktop, a gateway, or another x19 REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `x19` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_x19_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-x19.exe gate."""
    shim = scripts_dir / "x19.exe"
    lines = [
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from x19_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _log_only_write(text: str) -> None:
    ...
    with suppress(Exception):
        if log_file is None:
            log_path = get_x19_home() / "logs" / "update.log"
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from x19_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``x19 update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from x19_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from x19_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        from x19_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from x19_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    _write_update_incomplete_marker()
    from x19_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all X19 windows/gateways and re-run: x19 update")
        return False
...
def _pip_install_prefix(uv_bin) -> tuple[list[str], dict | None]:
    ...
    if uv_bin:
        ...
        # resolves here.
        # See #83914.
        from x19_cli.managed_uv import managed_python_env
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    from x19_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by x19.exe...")
    elif not healthy:
    ...
    if handed_off_sync or not healthy:
        ...
        if runtime_repaired is not None:
            ...
            # imports pass while lazily-installed backends and ``x19 tools`` dependencies
            ...
            # must leave the breadcrumb so the next `x19` run finishes the repair.
            ...
            if _m()._refresh_active_lazy_features(
                ...
                print("  ⚠ Lazy-refresh recovery incomplete — run `x19` again "
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running X19 gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/x19-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``x19 update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in x19_cli/config.py bricked
    # every user who ran ``x19 update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``x19 update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway x19.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from x19_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running X19 runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from x19_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another x19.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_x19_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.sh | bash")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``x19 update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``x19
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && x19 update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
...
def _handle_update_called_process_error(
    ...
    if _should_zip_fallback_on_update_error(e):
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            print()
            print("  X19 may not start until the dependencies are installed. Fix the error above")
            print("  (usually network or disk space), then run `x19 update` again.")
            if _m()._is_windows():
                print("  If `x19 update` itself will not start, retry through the venv interpreter:")
                print(
                    '    venv\\Scripts\\python.exe -c '
                    '"from x19_cli.main import main; main()" update --yes')
        else:
            print(f"✗ {stage}.")
            print(f"  Details: {e}")
            _print_called_process_error_tail(e)
...
def _post_swap_payload(
    *, swap: str, branch: str, opts, gateway_mode: bool, had_desktop_app_before_update: bool,
    ...
    from x19_cli.update_receipt import detach_update_receipt
...
def _sibling_snapshots_module():
    ...
    import x19_cli.update_cmd_config as _cfg
    return _cfg


def _hand_off_post_swap(args, **payload_kwargs) -> None:
    """Re-execute ``x19 update --post-swap`` on the pulled tree and exit with its code.

    The parent detaches from the receipt and its Windows resume hook — the child owns both —
    and only relays the exit code (``x19_cli/update_handoff.py``).
    """
    from x19_cli.update_handoff import continue_update_in_fresh_interpreter
    from x19_cli.update_receipt import resume_update_receipt
...
def _run_post_swap_phase(args, gateway_mode: bool) -> None:
    ...
    pulled code."""
    from x19_cli.update_handoff import read_handoff
    from x19_cli.update_receipt import resume_update_receipt
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    import x19_cli.update_cmd_config as _cfg
    from x19_cli.update_inventory import UpdatePlan
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `x19 serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `x19
