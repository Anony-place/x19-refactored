"""x19 webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _subscriptions_path() -> Path:
    ...
    return get_x19_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: x19 gateway run
"""


def webhook_command(args):
    """Entry point for 'x19 webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: x19 webhook {subscribe|list|remove|test}")
        print("Run 'x19 webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    if profile_arg is None:
        ...
        from x19_cli.profiles import normalize_profile_name, profile_exists, validate_profile_name
        try:
    ...
    if cron_job:
        ...
        try:
            ...
            return
        if job is None:
            print(f"Error: no cron job matches '{cron_job}'. List jobs with: x19 cron list")
            return
    ...
    print("  The gateway must be running to receive events (x19 gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: x19 webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from x19 webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (x19 gateway run)")
