...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`x19_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
...
def _append_unconfigured_rows(
    ...
    from x19_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `x19 model` to reactivate. {tail}"
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (X19 device flow or Claude Code
    ...
    try:
        ...
        readers = (read_x19_oauth_credentials, read_claude_code_credentials)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `x19 model` to configure ({auth_type})")
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from x19_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from x19_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from x19_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (x19_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="x19-picker-pricing-prewarm", daemon=True)
