...
_REASON_COPY: dict[str, str] = {
    "auth": "Your {provider} key was rejected. Run `x19 model` to re-enter it.",
    "auth_permanent": "Your {provider} key was rejected. Run `x19 model` to re-enter it.",
...
_UNKNOWN_COPY = "The model request failed. Run /model to switch or `x19 doctor` to check the setup."
...
def agent_init_failure_message(error: BaseException) -> str:
    ...
    return (
        f"X19 couldn't start the model connection: {_short(str(error)) or type(error).__name__}. "
        "Your message was not sent. Run `x19 doctor` to check the setup, "
