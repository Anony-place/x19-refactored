"""Argparse tree for ``x19 kanban …`` (``build_parser``).
...
def _triage_sweep_args(verb: str, Verb: str, noun: str):
    ...
    return (
        ...
        _arg("--author",
             help=f"Author name recorded on the audit comment (default: $X19_PROFILE or '{noun}')"),
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `x19 kanban boards export`. The board always lands as a "
...
# Top-level ``x19 kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--project",
             help="Link to a project (id or slug). Anchors the task's "
                  ...
                  "deterministic branch. See `x19 project list`."),
        _TENANT,
        _PRIORITY,
    ...
    _cmd("list", [
        _arg("--mine", action="store_true", help="Filter by $X19_PROFILE as assignee"),
        _arg("--assignee"),
    ...
    _cmd("comment", [
        ...
        _arg("--author", help="Author name (default: $X19_PROFILE or 'user')"),
    ...
    _cmd("attach", [
        ...
        _arg("--author", help="uploaded_by label (default: $X19_PROFILE or 'user')"),
    ...
    ], help="DEPRECATED — dispatcher now runs in the gateway. Use `x19 gateway start`."),
    _cmd("watch", [
    ...
    _cmd("assignees", [_json_flag()],
         help="List known profiles + per-profile task counts (union of ~/.x19/profiles/ and current assignees on the board)"),
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across X19 profiles. "
                    "Tasks are claimed atomically, can depend on other tasks, and "
                    "are executed by a named profile in an isolated workspace. "
                    "See https://anony-place.github.io/x19-refactored/docs/user-guide/features/kanban.",
    )
    # --board scopes every subcommand to one board's DB; when omitted the
    # resolution is X19_KANBAN_BOARD, then the persisted current-board
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `x19 kanban boards switch <slug>` or the "
                                    "X19_KANBAN_BOARD env var). Use `x19 kanban boards "
