...
Lives under the shared X19 root: ``default`` board DB at ``<root>/kanban.db`` (pre-boards
back-compat), other boards at ``<root>/kanban/boards/<slug>/``; a worker on one board never sees
another. Board resolution: ``board=`` arg > ``X19_KANBAN_BOARD`` > ``X19_KANBAN_DB`` (pins the
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    from x19_constants import VALID_REASONING_EFFORTS
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    try:
        from x19_cli.lifecycle import invoke_hook
...
# ``heartbeat_claim`` or raise it via X19_KANBAN_CLAIM_TTL_SECONDS.
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    """Explicit ``ttl_seconds`` > ``X19_KANBAN_CLAIM_TTL_SECONDS`` > default."""
    if ttl_seconds is not None:
        return max(1, int(ttl_seconds))

    return _env_int("X19_KANBAN_CLAIM_TTL_SECONDS", DEFAULT_CLAIM_TTL_SECONDS, minimum=1)
...
def _resolve_crash_grace_seconds() -> int:
    """``X19_KANBAN_CRASH_GRACE_SECONDS`` (0 = immediate, for tests) else default."""
    return _env_int("X19_KANBAN_CRASH_GRACE_SECONDS", DEFAULT_CRASH_GRACE_SECONDS)


def _resolve_rate_limit_cooldown_seconds() -> int:
    """``X19_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` (0 = next tick, for tests) else default."""
    return _env_int("X19_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "x19_kanban_current_board_override", default=None,
)
...
def kanban_home() -> Path:
    """``X19_KANBAN_HOME`` else ``get_default_x19_root()``. Shared across
    profiles BY DESIGN: resolving through the active profile's X19_HOME would
    fork the board per profile and break the dispatcher/worker handoff."""
    override = os.environ.get("X19_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    """Active slug: context override -> ``X19_KANBAN_BOARD`` -> ``<root>/kanban/current``
    ...
    for candidate in (
        ...
        os.environ.get("X19_KANBAN_BOARD", "").strip(),
    ):
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    """``kanban.db`` path: ``X19_KANBAN_DB`` pins it (injected into workers);
    ``default`` -> ``<root>/kanban.db`` (back-compat), else the board dir."""
    return _board_path("X19_KANBAN_DB", board, ("kanban.db",), "kanban.db")


def workspaces_root(board: Optional[str] = None) -> Path:
    """Per-board scratch workspace root (``X19_KANBAN_WORKSPACES_ROOT`` wins);
    ``default`` keeps the legacy ``<root>/kanban/workspaces/``."""
    return _board_path("X19_KANBAN_WORKSPACES_ROOT", board, ("kanban", "workspaces"), "workspaces")


def attachments_root(board: Optional[str] = None) -> Path:
    """Per-board attachments root (``X19_KANBAN_ATTACHMENTS_ROOT`` wins). Workers
    read attachments by absolute path, so remote terminal backends must mount it."""
    return _board_path("X19_KANBAN_ATTACHMENTS_ROOT", board, ("kanban", "attachments"), "attachments")
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    """Per-board worker log dir (logs follow the board so ``x19 kanban log``
...
class Task:
    ...
    session_id: Optional[str] = None         # originating X19_SESSION_ID; NULL from CLI/dashboard
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (x19_cli/projects_db). When set,
    ...
    -- inside an agent loop that propagated ``X19_SESSION_ID``. NULL
...
def _resolve_project_link(
    ...
    from x19_cli import projects_db as _pdb
...
def create_task(
    ...
    """
    from x19_cli.kanban_db_graph import initial_task_state, inherit_creator_origin
    from x19_cli.kanban_pr_acceptance import validate_contract
...
# Canonical sort-order mappings for ``x19 kanban list --sort``.
...
# dispatcher tick or a manual `x19 kanban recompute` (issue #22459).
...
def complete_task(
    ...
    if not _parents_satisfied(conn, task_id):
        return False
    from x19_cli.kanban_pr_acceptance_store import prepare_acceptance, record_acceptance
...
def promote_task(
    ...
    if unsatisfied:
        return False, (
            ...
            f"`x19 kanban unlink <parent_id> {task_id}`)"
        )
...
def _ctx_comments(lines: list[str], comments: list[Comment], now: int) -> None:
    ...
    worker" framing stops an operator-controlled X19_PROFILE like
    "x19-system" being read as a system directive above an
    ...
    if omitted_note:
        lines.append(omitted_note)
    for c in shown:
        # Render author with explicit "comment from worker" framing so operator-controlled X19_PROFILE
        # values like "x19-system" or "operator" can't be misread by the next worker as a system
...
def list_profiles_on_disk() -> list[str]:
    ...
    directly to avoid importing ``x19_cli.profiles`` at startup."""
    try:
        from x19_constants import get_default_x19_root
...
from x19_cli.kanban_db_connect import (  # noqa: E402
    _INITIALIZED_PATHS,
    init_db,
    write_txn,
)
from x19_cli.kanban_db_workspace import (  # noqa: E402
    _cleanup_workspace,
    _is_managed_scratch_path,
    _managed_scratch_path_info,
    _scratch_workspace,
)
from x19_cli.kanban_db_dispatch import (  # noqa: E402
