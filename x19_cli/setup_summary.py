...
resolved through the module object so test patches on ``x19_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from x19_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'x19 setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'x19 setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'x19 tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("x19 setup", "          Re-run the full wizard"), ("x19 setup model", "    Change model/provider"),
    ("x19 setup terminal", " Change terminal backend"), ("x19 setup gateway", "  Configure messaging"),
    ("x19 setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("x19 config", "         View current settings"), ("x19 config edit", "    Open config in your editor"),
    ("x19 config set <key> <value>", ""))
_READY_ROWS = (
    ("x19", "              Start chatting"), ("x19 gateway", "      Start messaging gateway"),
    ("x19 doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'x19 setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `x19 tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'x19 setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via x19 auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, x19_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — X19 cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    x19 model            (pick any provider/model)",
              "    x19 setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'x19 setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {x19_home}/cron/, sessions/, logs/")):
...
import x19_cli.setup as _setup  # noqa: E402  (bottom: x19_cli.setup imports this module)
