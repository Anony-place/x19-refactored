"""CLI handlers for ``x19 secrets onepassword ...``.

Unlike Bitwarden, the ``op`` binary is NOT auto-installed: 1Password publishes the CLI through OS
package managers and signed installers, so X19 expects an already-installed, already-
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "X19 resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]x19 secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]x19 secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]x19 secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]x19 secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "X19 will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]x19 secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]x19 secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    return rotate_token(
        ...
        disabled_note=None if op_cfg.get("enabled") else (
            ...
            "run `x19 secrets onepassword setup` to turn it on.[/yellow]"
        ),
    )
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not references:
        console.print(
            ...
            "`x19 secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
    ...
    console.print(
        ...
        "[cyan]x19[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    return disable_secret_source(
        ...
        "[cyan]x19 secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them.",
    )
