"""Install and remove the Linux desktop entry (``x19.desktop``).
...
DESKTOP_ENTRY_NAME = "x19.desktop"
...
READY_FD_ENV_VAR = "X19_DESKTOP_READY_FD"
...
def _can_import_x19_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``x19_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``x19_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``x19 desktop``.

    Prefer the real ``x19`` launcher; fall back to ``<python> -m x19_cli.main desktop``.
    """
    from x19_cli.relaunch import resolve_x19_bin

    bin_path = _resolve_x19_bin_for_desktop_entry(resolve_x19_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_x19_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import x19_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "x19_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `x19` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `x19` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m x19_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_x19_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`x19_cli.relaunch.resolve_x19_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from x19_cli.relaunch import resolve_x19_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/x19, a venv console script) wins
    ...
    # /opt/.../bin/x19 from another install method, or a venv console script). It must be evaluated
    ...
    # ``x19``) must NOT return None here: that skipped the durable-wrapper probe below and persisted
    # the module form, so the entry's bytes flipped on every alternating launch context — and
    # gnome-shell 50.x crashes when x19.desktop changes while its ShellApp is STARTING (#110885).
    ...
    # every flip rewrites x19.desktop, which arms the gnome-shell 50.x crash this function's
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``x19``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "x19")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/x19"))
    candidates.append(Path.home() / ".local" / "bin" / "x19")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=X19\n"
        "GenericName=X19 Desktop\n"
        "Comment=Launch X19 Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "x19.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=x19`` resolves without an
...
def _launcher_entry_management_enabled() -> bool:
    ...
    rewrite: a hand-edited ``x19.desktop`` is then left alone instead
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "x19"
...
class DeferredDesktopEntryInstall:
    ...
    The launcher hands Electron the write end of a pipe (``X19_DESKTOP_READY_FD``); Electron
