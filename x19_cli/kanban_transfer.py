...
Backs ``x19 kanban export|import``, the ``/boards/{slug}/export`` and
...
from x19_cli.archive_safe import (
    archive_root_dirs,
    copy_regular_files,
    make_targz,
    safe_extract_targz,
...
ARCHIVE_FORMAT = "x19-kanban-board"
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError("archive is not an X19 kanban board export (no manifest.json)")
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not an X19 kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this X19 "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update X19 and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "x19_version": manifest.get("x19_version"),
        },
        "counts": counts,
