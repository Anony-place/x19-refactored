"""Quiet ``x19 chat -Q`` helpers: bind this session's key and resume nested notifies.

Bot Mode delivers a local DM as ``x19 -p <bot> chat -Q --query-file``. Interactive
...
``X19_SESSION_KEY`` and never woke the recipient. Quiet also printed and exited
