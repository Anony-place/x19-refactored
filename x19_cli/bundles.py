"""Implementation of the ``x19 bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  x19 bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``x19 bundles`` argparse tree (called from x19_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `x19 bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``x19 bundles <subcommand>`` to the right handler."""
