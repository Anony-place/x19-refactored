"""Shell completion script generation for x19 CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_x19_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# X19 bash completion
# Add to ~/.bashrc:
#   eval "$(x19 completion bash)"

_x19_profiles() {{
    local profiles_dir="$HOME/.x19/profiles"
...
_x19_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_x19_profiles)" -- "$cur"))
        return
    fi
...
complete -F _x19_completion x19
"""
...
_x19_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.x19/profiles" ]]; then
        profiles+=($HOME/.x19/profiles/*(N/:t))
    fi
...
_x19() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_x19_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# X19 fish completion",
        "# Add to your config:",
        "#   x19 completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __x19_profiles",
        "    echo default",
        "    if test -d $HOME/.x19/profiles",
        "        for d in $HOME/.x19/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c x19 -f -s p -l profile"
        " -d 'Profile name' -xa '(__x19_profiles)'",
        "",
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__x19_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
