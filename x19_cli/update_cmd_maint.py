"""Post-update maintenance for ``x19 update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary.

Split out of ``update_cmd.py``, which re-imports every name so ``x19_cli.update_cmd.<name>``
...
def _sqlite_partial_completion_lines(sqlite_version: str) -> list[str]:
    ...
    return [
        ...
        "corruption bug. X19 works, but sessions could be damaged.",
        f"  Fix: run the installer again ({_REINSTALL_ONE_LINER[bool(_m()._is_windows())]}) "
        "which installs a safe Python, then run `x19 doctor` to confirm.",
    ]
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  x19 curator run --dry-run")
    print("  Pause it:     x19 curator pause")
    print("  Docs:         https://anony-place.github.io/x19-refactored/docs/user-guide/features/curator")
...
def _print_fts_optimize_available_notice() -> None:
    ...
    print("  Run when convenient:  x19 sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: x19 curator status)")
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    try:
        ...
        print("    x19 dashboard --port <port>   (or: systemctl --user restart x19-dashboard)")
        return
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from x19_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("X19_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== x19-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from x19_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next X19 runtime."""
    from x19_cli.update_cmd import _post_update_sqlite_runtime_status
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from x19_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `x19 desktop` to retry the desktop rebuild.")
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from x19_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from x19_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (x19 gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart X19) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from x19_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from x19_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: x19 skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v x19'`` resolves."""
    from x19_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/x19).
    fhs_link = Path("/usr/local/bin/x19")
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `x19 gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``x19-acp`` launcher next to ``x19`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``x19 acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$X19Home\bin``, never
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        x19_cmd = bin_dir / "x19"
        acp_cmd = bin_dir / "x19-acp"
        try:
            if not (x19_cmd.is_file() or x19_cmd.is_symlink()):
                continue
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# X19 — ACP launcher (written by `x19 update`).\n"
            ...
            continue
        print(f"  ✓ Installed x19-acp launcher → {acp_cmd}")
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_x19_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_x19_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from x19_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from x19_cli.backup import create_pre_update_snapshots_all_profiles
...
def _run_full_backup() -> None:
    """Zip X19_HOME under ``backups/`` (restorable via ``x19 import``). Never raises."""
    try:
    ...
    from x19_cli.sizefmt import format_bytes
    # display_x19_home so the user sees ~/.x19/...
    try:
        from x19_constants import get_x19_home, display_x19_home
        display_path = f"{display_x19_home()}/{out_path.relative_to(get_x19_home())}"
    except Exception:
        display_path = str(out_path)

    print(f"  Saved:    {display_path} ({format_bytes(size_bytes)}, {elapsed:.1f}s)")
    print(f"  Restore:  x19 import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of X19_HOME under ``backups/`` (``x19 import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in x19_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # X19_HOME, so sync_skills()'s module-level X19_HOME cache can't skew it.
    with suppress(Exception):
        from x19_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from x19_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from x19_cli.tools_config import install_cua_driver
        print()
...
def _print_profiles_without_credentials_notice() -> None:
    from x19_cli.profile_credential_audit import print_profiles_without_credentials_notice
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from x19_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from x19_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('x19-acp launcher self-heal failed: %s', _ensure_acp_launcher),
        ...
        # Legacy X19_NEMO_RELAY_ATIF_*/ATOF_* vars produce no traces since the Relay cutover;
...
def _migrate_relay_exporter_env() -> None:
    from x19_cli.relay_plugin_migrate import run_relay_migration_after_update
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from x19_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if X19 re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.nousresearch.x19` (repeat "
    ...
    try:
        # See #95596.
        from x19_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from x19_cli.model_catalog import seed_cache_from_checkout
