...
The provider list shown by ``x19 model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`x19_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `x19 model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``x19 model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("x19_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("x19_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    X19_OVERLAYS = _safe_import("x19_cli.providers", "X19_OVERLAYS", {})
    try:
