...
Split out of :mod:`x19_cli.plugins`; validation warns and never fails a load.
"""
...
from x19_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``x19``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "x19", "depends",
    "requires_x19", "python_runtime",
}

# Highest manifest schema version this X19 understands.
...
def _plugins_debug() -> bool:
    from x19_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this X19 "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `x19 plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `x19 plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # X19 version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_x19_error``.
    requires_x19: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — X19 never auto-installs these (isolation design for the install seam
...
# ── requires_x19 version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_x19_version() -> str:
    """Installed ``x19`` distribution version, else ``x19_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("x19")
    except Exception:
...
def requires_x19_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_x19`` rejects the running version."""
    if not manifest.requires_x19:
        return None
    current = running_x19_version()
    if version_satisfies(manifest.requires_x19, current):
        return None
    return f"requires x19 {manifest.requires_x19}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_x19=str(data.get("requires_x19") or "").strip(),
