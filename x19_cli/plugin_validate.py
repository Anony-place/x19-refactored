"""``x19 plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``X19_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "X19_VALIDATE_JSON:"
...
def _requires_x19_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_x19`` spec.

    Unlike :func:`x19_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from x19_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_x19(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_x19") or "").strip()
    if not spec:
        report.add("requires_x19", True, "not declared")
        return
    if _requires_x19_spec_valid(spec):
        report.add("requires_x19", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_x19",
            False,
            f"requires_x19 spec {spec!r} does not parse "
...
# from x19 so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "x19_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="x19-validate-") as scratch:
        env = dict(os.environ)
        env["X19_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_x19(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that X19 can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from x19_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from x19_cli.plugin_python_deps import applicable_specs, unsupported_specs
