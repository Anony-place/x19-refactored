...
from x19_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts

from x19_state_ids import SESSION_ID_PATTERN  # timestamp prefix: strongest sentinel for schema-less rows
...
# 3.50.7 and 3.44.6 — the same version gate x19_state applies to the
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="x19-recover-probe-")
...
def _cli_recover_attempts(source: Path, lf_path: Path, sqlite3_bin: str, *, timeout: float) -> list[dict[str, Any]]:
    ...
    for command in (".recover --ignore-freelist", ".recover"):
        ...
        with tempfile.TemporaryFile(prefix="x19-recover-dump-stderr-") as dump_stderr:
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but X19 never writes one, so
...
# The closed sets SessionDB writes (x19_state.py: request/claim/complete/
