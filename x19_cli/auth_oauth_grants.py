...
# Profiles must never receive a copy; each profile signs in on its own (``x19 -p <name> auth add``).
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``x19 profile create
    ...
    """
    from x19_cli.auth import _same_path, _save_auth_store
    ...
    try:
        ...
        root_auth_path = get_default_x19_root() / "auth.json"
