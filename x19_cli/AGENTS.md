# x19_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `X19CLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`x19_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`X19CLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.x19/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`x19_cli/curses_ui.py`; example
`x19_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`x19_cli/commands.py`)
...
`x19_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`x19_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `x19 config
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`x19 tools/setup`, most subcommands, `x19_cli/config.py`, merges `DEFAULT_CONFIG`);
  `x19_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `x19 send`, doctor,
  `x19_time`/`x19_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' x19_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`x19_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.x19/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`x19 update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `x19 update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `x19-gateway*` unit /
  `ai.x19.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.x19/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `x19 update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `x19 update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `x19_cli/main.py` sets `X19_HOME` before any module import for
single-profile commands (`x19 -p x <cmd>`), so there `get_x19_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["X19_HOME"]` keeps the launch
...
carries an identity marker (`x19_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_x19_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `x19_constants.x19_home_key()`, never a single flag.
...
Migration from per-profile gateways: `x19_cli/gateway_migrate.py` (`x19 gateway migrate
...
other service domain / UNIX user / X19_HOME outside `profiles/` — notices for the explicit command,
...
`x19 gateway restart` for a gateway X19 did not install (custom launchd agent / unit running
...
profile's `X19_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nous free tier (`x19_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `x19` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`x19 auth upgrade`), the CLI `/login` handler and the desktop
