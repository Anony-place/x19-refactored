"""CLI handlers for ``x19 migrate ...``.

Currently exposes only ``x19 migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``x19 migrate <subtype>``."""
    if getattr(args, "migrate_type", None) == "xai":
        return cmd_migrate_xai(args)

    print("usage: x19 migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `x19 migrate xai --apply` to rewrite "
    ...
    print()
    print(color("Run `x19 doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
