...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``x19_session_at``
(access token; Max-Age = token TTL), ``x19_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``x19_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``x19_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``x19_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "x19_session_at"
SESSION_RT_COOKIE = "x19_session_rt"
SESSION_PROVIDER_COOKIE = "x19_session_provider"
PKCE_COOKIE = "x19_session_pkce"
SSO_ATTEMPT_COOKIE = "x19_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/x19`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (x19 #37247); a provider that omits it returns
