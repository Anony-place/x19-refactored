...
def _iter_sync_files(agent: str, source_root: Path) -> Iterator[Path]:
    ...
    :class:`~x19_cli.agent_import.AgentImporter` reads for ``agent``."""
...
def sync_imported_agents(args) -> None:
    ...
    from x19_cli.agent_import import AgentImporter, print_import_report
    from x19_cli.setup import print_error, print_header, print_info, print_success
    from x19_constants import get_x19_home

    dry_run = bool(getattr(args, "dry_run", False))
    x19_home = get_x19_home().resolve()
    agents: Dict[str, Any] = load_sync_manifest(x19_home).get("agents", {})
    if not agents:
        print()
    ...
    for agent_name in sorted(agents):
        ...
        try:
            report = AgentImporter(agent_name, source_dir, x19_home, execute=not dry_run,
        ...
        if not dry_run:
            try:
                ...
                update_sync_manifest(agent_name, source_dir, x19_home, overwrite, report,
