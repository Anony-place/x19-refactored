"""Boundaries the AUTOMATIC multiplex migration (``x19 update``) must not cross, and the opt-out.

The multiplexer replaces a kernel-enforced boundary (separate UNIX users, separate service domains,
separate X19_HOME trees) with in-process isolation. An operator may choose that with
``x19 gateway migrate --multiplex``; an unattended update hook must not choose it for them.
...
if TYPE_CHECKING:
    from x19_cli.gateway_migrate import MigrationPlan, ProfileGateway
...
def _system_unit_uid(unit_path: Path) -> Optional[int]:
    ...
    from x19_cli.gateway import _read_systemd_user_from_unit
...
def gateway_identity(home: Path, pid: Optional[int], services: list[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    ...
    X19_HOME an installed unit pins, which is where the gateway really runs; ``home`` otherwise.
    """
    from x19_cli.gateway import _x19_home_pinned_by_unit, get_systemd_unit_path
    from x19_cli.gateway_migrate import _home_env
    ...
    for kind, system in services:
        ...
        pinned = _x19_home_pinned_by_unit(unit_path)
...
def _guard_home_tree(plan: MigrationPlan, profile: ProfileGateway) -> Optional[str]:
    ...
    if runtime_home.is_relative_to(profiles_root):
        return None
    return (f"Profile '{profile.name}' runs with X19_HOME={runtime_home}, outside {profiles_root}: "
...
def auto_migration_blockers(plan: MigrationPlan) -> list[str]:
    ...
    domain, one profiles/ tree — the only shape ``x19 update`` may fold on its own."""
    findings = [
        finding
