...
def _strip_x19_path_lines(content: str) -> str:
    """Drop the ``# X19`` marker (+ its PATH line) and any x19 PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# X19' in line or '# x19' in line:
            skip_next = True
            continue
        if skip_next and ('x19' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'x19' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove X19 PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_x19_path_lines(content)
...
def remove_wrapper_script():
    """Remove the x19 wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a x19_cli / x19 reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('x19_cli' in content or 'x19' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("x19", "x19-acp", "x19-agent"))
...
def remove_node_symlinks(x19_home: Path) -> list:
    ...
    node_dir = (x19_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from x19_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from x19_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from x19_cli import gateway_windows as gw
...
# X19_HOME / X19_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\x19\git\{cmd,bin,usr\bin}, ...\x19\node), PortableGit + Node copies
...
def _x19_path_markers(x19_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying X19-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(x19_home).rstrip("\\/")
    subs = ("x19", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(x19_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip X19-owned entries from User-scope PATH in the registry (see ``_x19_path_markers``)."""
    markers = tuple(m.lower() for m in _x19_path_markers(x19_home, include_managed_bin=include_managed_bin))
...
def remove_x19_env_vars_windows() -> list[str]:
    """Delete X19_HOME and X19_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("X19_HOME", "X19_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(x19_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``x19_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (x19_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``x19`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from x19_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from x19_constants import get_default_x19_root
        bin_dir = get_default_x19_root() / "bin"
    except Exception as e:
...
def _is_default_x19_home(x19_home: Path) -> bool:
    """Return True when ``x19_home`` points at the default (non-profile) root."""
    try:
        from x19_constants import get_default_x19_root
        return x19_home.resolve() == get_default_x19_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``x19 -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current X19_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m x19_cli.main` (the `x19` wrapper may be half-gone).
    x19_invocation = [sys.executable, "-m", "x19_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                x19_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's X19_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``x19 uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from x19_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    x19_home = get_x19_home()
    summary = gui_install_summary(x19_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ X19 Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No X19 Chat GUI installation was found.")
        print(f"  Checked: {x19_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The X19 stays installed.", Colors.CYAN))
    print()
    ...
    print()
    uninstall_gui(x19_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The X19 is still installed. Run 'x19' to use the CLI,")
    print("or 'x19 uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.x19/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, x19_home=x19_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie X19_HOMEs and systemd units behind.
    is_default_profile = _is_default_x19_home(x19_home)
    ...
    print()
    _print_box("│            ☤ X19 Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {x19_home / 'config.yaml'}")
    print(f"  Secrets: {x19_home / '.env'}")
    print(f"  Data:    {x19_home / 'cron/'}, {x19_home / 'sessions/'}, {x19_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, X19_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL X19 data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the X19 code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, x19_home=x19_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, x19_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • X19 PATH entries from shell configs / Windows User PATH")
    print("  • X19 wrapper scripts and X19-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep X19 config/data: {x19_home}")
    else:
        print(f"  • X19 config/data: {x19_home}")
        profiles = _discover_named_profiles() if _is_default_x19_home(x19_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$X19_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: x19_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; x19\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_x19_home(x19_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(x19_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No X19-owned PATH entries in User environment"),
        (windows, "Removing X19_HOME / X19_GIT_BASH_PATH User env vars...",
         remove_x19_env_vars_windows, "Removed User env var: {}", "No X19-set User env vars to remove"),
        (True, "Removing x19 command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows x19 launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows x19 launchers found"),
        (True, "Removing X19-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(x19_home), "Removed {}", "No X19-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE X19_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from x19_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(x19_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(x19_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.x19/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(x19_home)
    else:
        log_info(f"Keeping configuration and data in {x19_home}")
    ...
    print()
    print("Thank you for using X19! ☤")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.ps1)",
    False: "  curl -fsSL https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m x19_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``x19_constants`` + ``x19_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m x19_cli.uninstall")
    parser.add_argument(
