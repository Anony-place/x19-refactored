"""Top-level argparse construction for the x19 CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``x19_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``X19_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/x19_cli/
...
def _cfg_path() -> str:
    """``~/.x19/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_x19_home()`` would emit the
    "[X19_HOME fallback] ... wrong profile" warning on every ``x19`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from x19_constants import display_x19_home, get_process_x19_home

    return f"{display_x19_home(get_process_x19_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``x19 --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``x19_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    x19                        Start interactive chat
    x19 chat -q "Hello"        Single query mode
    x19 --tui                  Launch the modern TUI (or set display.interface: tui)
    x19 --cli                  Force the classic REPL (overrides display.interface: tui)
    x19 -c                     Resume the most recent session
    x19 -c "my project"        Resume a session by name (latest in lineage)
    x19 --resume <session_id>  Resume a specific session by ID
    x19 --resume latest        Resume the most recent session (same as -c)
    x19 --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    x19 setup                  Run setup wizard
    x19 logout                 Clear stored authentication
    x19 auth add <provider>    Add a pooled credential
    x19 auth list              List pooled credentials
    x19 auth remove <p> <t>    Remove pooled credential by index, id, or label
    x19 auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    x19 auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    x19 auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    x19 model                  Select default model
    x19 fallback [list]        Show fallback provider chain
    x19 fallback add           Add a fallback provider (same picker as `x19 model`)
    x19 fallback remove        Remove a fallback provider from the chain
    x19 config                 View configuration
    x19 config edit            Edit config in $EDITOR
    x19 config set model gpt-4 Set a config value
    x19 gateway                Run messaging gateway
    x19 -s x19-dev,github-auth
    x19 -w                     Start in isolated git worktree
    x19 gateway install        Install gateway background service
    x19 sessions list          List past sessions
    x19 sessions browse        Interactive session picker
    x19 sessions rename ID T   Rename/title a session
    x19 logs                   View agent.log (last 50 lines)
    x19 logs -f                Follow agent.log in real time
    x19 logs errors            View errors.log
    x19 logs --since 1h        Lines from the last hour
    x19 debug share             Upload debug report for support
    x19 console                Open the safe X19 command console
    x19 update                 Update to latest version
    x19 dashboard              Start web UI dashboard (port 9119)
    x19 dashboard --stop       Stop running dashboard processes
    x19 dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via X19_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `x19 setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to X19_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``x19 -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/x19_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with X19")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also X19_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from X19 itself.")
...
class X19ArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "x19" at the top level and "x19 gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = X19ArgumentParser(
        prog="x19", description="X19 - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``x19 [...] <command>`` instead of the brace list of
    # every subcommand name; ``x19 --help`` still lists each command with its help row.
