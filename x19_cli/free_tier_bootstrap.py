...
Every X19 process that may need the free tier runs this once at boot (``x19 serve`` on a
...
(:func:`x19_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
def start_background_bootstrap() -> threading.Thread:
    """``x19 serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
