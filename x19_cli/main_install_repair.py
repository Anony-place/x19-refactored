...
# Install-scoped breadcrumbs live next to the venv (not under $X19_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``x19 acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``x19 update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from x19_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `x19 update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``x19.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_x19_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from x19.exe; applying package-only first aid, "
    ...
    try:
        from x19_cli import _install_repair as _ir
        ...
        manual = (
            "  X19 is still running from the launcher that needs "
            "replacing. Close other X19 windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\x19.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``x19.exe`` never matches.
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _x19_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_x19_launcher_locked() -> bool:
    """True when a venv ``x19*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "X19_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``x19 update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\x19.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    cmd = [str(python_exe), "-m", "x19_cli.main", *sys.argv[1:]]
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `x19 update` starts, so an update running
...
def _venv_scripts_dir() -> Path | None:
    ...
    from x19_cli.main import PROJECT_ROOT
    from x19_constants import project_venv_dir, venv_bin_dir
...
def _x19_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_x19_exe(
    scripts_dir: Path, *, max_attempts: int = 4, failed_out: list[str] | None = None
) -> list[tuple[Path, Path]]:
    """Pre-empt the Windows file lock on the running ``x19.exe``.
    ...
    transient; X19 Desktop backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _x19_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close X19 Desktop, exit other `x19` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `x19 update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older X19 versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _x19_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_x19_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``x19``
...
class ShimQuarantineError(RuntimeError):
    """A live ``x19*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``x19.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``x19.exe``
    can be neither deleted nor overwritten, so without quarantine ``x19`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_x19_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `x19` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``x19.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``x19.exe`` missing while ``x19.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``x19.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``x19.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `x19 update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from x19_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``x19*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves x19.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``x19.exe`` in the wheel RECORD while the
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `x19 update --force` after "
            "closing other x19 processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m x19_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``x19 update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `x19 update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `x19 update --force` after closing other x19 processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `x19 update --force` after closing other x19 processes.",
