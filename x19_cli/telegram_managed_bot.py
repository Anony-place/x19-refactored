...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "X19SetupBot",
    ...
    print(f"\n  Contacting X19 Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the X19 Telegram onboarding service.\n"
