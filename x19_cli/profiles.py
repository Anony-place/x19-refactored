"""Profile management for multiple isolated X19 instances."""
...
from x19_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
...
# stay visible; containers persist HOME state here). See x19_constants.get_subprocess_home().
...
# (``~/.x19``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# ``x19_cli.backup._EXCLUDED_ROOT_DIRS`` so the two lists cannot drift.
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "x19", ".worktrees", "profiles", "bin", "node_modules",
}) | LOCAL_RUNTIME_ROOT_DIRS

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `x19 backup` archives, quick-backup
...
# Marker written by `x19 profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `x19 update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this X19 profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_x19_home().resolve():
...
# Allow-list for ``export_profile("default")``: when X19_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known X19 profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``x19_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"x19", "default", "test", "tmp", "root", "sudo"})

# X19 subcommands that cannot be used as profile names/aliases
_X19_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the x19 root (NOT the current X19_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_x19_home() / "profiles"


def _get_default_x19_home() -> Path:
    """Default (pre-profile) X19_HOME: ``~/.x19``, or X19_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from x19_constants import get_default_x19_root
    return get_default_x19_root()


def _get_active_profile_path() -> Path:
    return _get_default_x19_home() / "active_profile"
...
# The wrapper body always ends the launcher token with `-p <profile>`, but the
# launcher itself is spelled per platform: a quoted absolute path with an
# `.exe` suffix on Windows, a shell-quoted path in the POSIX `exec` line, or a
# bare `x19` when nothing was resolvable on PATH. Matching only the literal
# `x19 -p` missed every quoted spelling, which left real wrappers unremovable
# (`remove_wrapper_script`) and made alias collisions refuse to reuse them.
# Still anchored on the `x19 … -p` shape so an arbitrary file cannot match.
_OUR_WRAPPER_RE = re.compile(r"""\bx19(?:\.(?:exe|cmd|bat))?['"]?\s+-p\s""", re.IGNORECASE)
...
def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: x19 profile create {canon}")
...
def _unknown_profile_error(canon: str) -> FileNotFoundError:
    ...
    return FileNotFoundError(f"No profile named '{canon}'. See your profiles with: x19 profile list")


def _profile_exists_error(canon: str) -> FileExistsError:
    return FileExistsError(
        f"A profile named '{canon}' already exists. Switch to it with `x19 profile use {canon}`, "
        "see all profiles with `x19 profile list`, or choose a different name."
    )
...
def _invalid_profile_name_error(name: str) -> ValueError:
    ...
    return ValueError(
        ...
        f"Then run `x19 profile create {suggestion}`."
    )
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.x19
    if not _PROFILE_ID_RE.match(name):
        raise _invalid_profile_name_error(name)
    if name in _RESERVED_NAMES:
        raise ValueError(
            f"Profile name {name!r} is reserved — it collides with either "
            f"the X19 installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its X19_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_x19_home()
    ...
    # profiles/x19 may still exist and must keep resolving.
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    shell into a "real" profile on the next ``x19 update`` (#95188, #94823, #99392)."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _X19_SUBCOMMANDS:
        return f"'{canon}' conflicts with a x19 subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        # Both platforms resolve the real installed launcher: the wrapper must
        # call `x19`, and must keep working when it is installed somewhere not
        # on PATH at wrapper-creation time (fall back to the bare name).
        x19_exe = shutil.which("x19") or "x19"
        if sys.platform == "win32":
            wrapper_path.write_text(f'@echo off\r\n"{x19_exe}" -p {profile} %*\r\n', encoding="utf-8")
        else:
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(x19_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `x19 doctor --fix` surfaces
    # the detailed error in the target profile.
    with contextlib.suppress(Exception):
        from x19_constants import reset_x19_home_override, set_x19_home_override
        from x19_cli.config import check_config_version, migrate_config
        token = set_x19_home_override(str(profile_dir))
        try:
...
# wrappers are a few hundred bytes with the ``x19 -p X`` needle near the top; the wrapper
...
class ProfileInfo:
    ...
    display_name: str = ""
    # Bot Mode title (``profile.yaml`` ``ui_meta['x19-bots'].title``) — the name
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `x19 model` sets it later
        import yaml
        from x19_constants import get_x19_home
        from x19_cli.config import read_user_config_raw
        source = get_x19_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating X19_HOME.
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing X19 config, ~5000
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    break ``x19 profile list``."""
    data = _load_yaml_dict(profile_dir / "profile.yaml") or {}
    ui_meta = data.get("ui_meta")
    bot_title = ""
    if isinstance(ui_meta, dict):
        x19_bots = ui_meta.get("x19-bots")
        if isinstance(x19_bots, dict):
            bot_title = str(x19_bots.get("title") or "").strip()
    return {
...
def list_profiles() -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_x19_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """``(profile_name, x19_home)`` pairs a gateway should serve — the single chokepoint
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_x19_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    if any(stripped.values()):
        logger.info(
            ...
            "(run `x19 -p %s auth add <provider>` to sign in)", canon, stripped, canon,
        )
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    ``sync_imports`` also copies the source's ``import-sync.json`` (the ``x19 import-agent``
    manifest) so the clone stays registered against the same external Claude Code / Codex trees
    and ``x19 -p <clone> import-agent --sync`` keeps pulling from them. The link is to the
    ...
    for relpath in _CLONE_SUBDIR_FILES:
        _clone_file(source_dir, profile_dir, relpath)
    if sync_imports:
        from x19_cli.agent_import_sync import SYNC_MANIFEST_NAME  # lazy: keeps yaml/utils off the hot startup path
...
def create_profile(
    ...
    makes two gateways fight over one bot (``x19_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``x19 update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.x19).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from x19_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from x19_cli.profile_channels import strip_channel_settings
    ...
    # `x19 -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _finish_profile_layout(profile_dir: Path, *, no_skills: bool, clone_all: bool,
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `x19 -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from x19_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `x19 update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`x19 profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `x19 profile describe` works later
...
def _notify_multiplexer(canon: str) -> None:
    from x19_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _purge_identity(canon: str) -> bool:
    ...
    from x19_cli.profile_identity import purge_profile_identity
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from x19_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches X19_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "X19_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_x19_home() / ".env"
...
_X19_ARGV_MARKERS = ("x19_cli.main", "x19-gateway", "tui_gateway")
...
# `findOnPath('x19')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "x19".
...
# argv[1] is matched against exact names, not ``startswith("x19")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("x19-notes.py").
_X19_CONSOLE_SCRIPT_NAMES = frozenset({"x19", "x19-agent", "x19-acp"})
...
def _is_x19_argv(argv: list) -> bool:
    ...
    if any(marker in joined for marker in _X19_ARGV_MARKERS) or exe_name.startswith("x19"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _X19_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running X19 *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`x19 -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_x19_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by X19_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("X19_HOME", "")
...
class ProfileIdentitySettlementPending(RuntimeError):
    ...
    settled (``x19_cli.profile_identity.purge_profile_identity`` returned False).

    Subclasses ``RuntimeError`` so delete-failure handling that treats the error as fatal (the
    CLI's ``x19 profile delete``) keeps working unchanged; surfaces that can report a partial
    ...
    def __init__(self, profile: str, path: Path):
        ...
        self.path = path
        self.retry_command = f"x19 profile purge-identity {profile}"
        super().__init__(
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.x19).\nTo remove everything, use: x19 uninstall")
    ...
    shutdown_mcp_servers(scope=x19_home_key(profile_dir))
    ...
    with contextlib.suppress(Exception):
        from x19_state_registry import close_all_under as _close_session_dbs_under
    ...
    with contextlib.suppress(Exception):
        from x19_logging import release_profile_log_handlers
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``X19_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # X19_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("X19_HOME")
    try:
        os.environ["X19_HOME"] = str(profile_dir)
        from x19_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("X19_HOME", None)
        if old_home is not None:
            os.environ["X19_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's x19_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded X19_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from X19_HOME: ``"default"`` when unset or ``~/.x19``, the
    name under ``~/.x19/profiles/<name>``, ``"custom"`` for any other path."""
    from x19_constants import get_x19_home
    resolved = get_x19_home().resolve()
    if resolved == _get_default_x19_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when X19_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_x19_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point X19_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/x19-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".x19-profile-exports", Path(tempfile.gettempdir()) / f"x19-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    X19_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-X19 file the user may have lying alongside X19_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``x19
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.x19 (dir name ".x19"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: x19 profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.x19). "
            "Specify a different name: x19 profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise _profile_exists_error(canon)
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="x19_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"x19_{old_name}"
    legacy_old_host = f"x19.{old_name}"
    new_host = f"x19_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_x19_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``x19_<old>`` or legacy ``x19.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_x19_home`` does not refuse it) and the periodic reconcile re-adopts the
    ...
    from x19_constants import x19_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=x19_home_key(old_dir))
    ...
    from x19_cli.profile_identity import _migrate_profile_identity
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a X19_HOME path string. Called early in the CLI entry
    point, before x19 modules are imported, to set X19_HOME.

    When X19_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match X19-owned PYTHONPATH
