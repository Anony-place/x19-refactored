...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or x19 -c.",
    ...
    "Ctrl+Z suspends X19 to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "x19 -c resumes your most recent CLI session. x19 -c \"project name\" resumes by title.",
    "x19 -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "x19 -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "x19 chat -t web,terminal enables only specific toolsets for a focused session.",
    "x19 chat -s github-pr-workflow preloads a skill at launch.",
    "x19 chat -q \"query\" runs a single non-interactive query and exits.",
    "x19 chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "x19 chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "x19 --yolo bypasses all dangerous command approval prompts for the entire session.",
    "x19 chat --source telegram tags the session for filtering in x19 sessions list.",
    "x19 -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "x19 doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "x19 dump outputs a compact setup summary — great for bug reports.",
    "x19 config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "x19 config edit opens config.yaml in your default editor.",
    "x19 config check scans for missing or stale configuration options.",
    "x19 sessions browse opens an interactive session picker with search.",
    "x19 sessions stats shows session counts by platform and database size.",
    "x19 sessions prune --older-than 30 cleans up old sessions.",
    "x19 skills search react --source skills-sh searches the skills.sh public directory.",
    "x19 skills check scans installed hub skills for upstream updates.",
    "x19 skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "x19 skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "x19 mcp add github --command npx adds MCP servers from the command line.",
    "x19 mcp serve runs X19 itself as an MCP server for other agents.",
    "x19 auth add lets you add multiple API keys for credential pool rotation.",
    "x19 completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "x19 logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "x19 backup creates a zip backup of your entire X19 home directory.",
    "x19 profile create coder creates an isolated profile that becomes its own command.",
    "x19 profile create work --clone copies your current config and keys to a new profile.",
    "x19 update syncs new bundled skills to ALL profiles automatically.",
    "x19 gateway install sets up X19 as a system service (systemd/launchd).",
    "x19 memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "x19 webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: x19 tools disables unused tools, x19 skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "x19 models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as x19 -w).",
    ...
    "X19_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call X19 tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'x19 profile create coder' creates the 'coder' command.",
    "x19 profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, X19 prints a resume command with session ID and stats.",
    "x19 sessions export backup.jsonl exports all sessions for backup or analysis.",
    "x19 -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "x19 skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .x19/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: x19 cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "X19 runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "x19 gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to X19 via a local macOS server.",
    ...
    "Context files (.x19.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "X19 loads project context from .x19.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "x19 mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "x19 mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "x19 mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — x19 tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.x19/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "x19 chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "x19 plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "x19 acp runs X19 as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "X19_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "x19 auth add nous or x19 auth add openai-codex sets up OAuth-based providers.",
    ...
    "x19 status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.x19/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make X19 your own.",
    ...
    "Before a gateway session reset, X19 auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from x19_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.x19/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope X19 state via X19_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "X19_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in X19_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.x19/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'x19 profile create ls' would be rejected.",
    "x19 profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "x19 claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "X19_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via x19 tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.x19/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.x19/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — X19 auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of X19 config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while X19 is working.',
    ...
    '/reload reloads ~/.x19/.env into the running session — pick up new API keys without restarting.',
    ...
    "X19_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.x19/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.x19/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'x19 curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "x19 curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'x19 curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'x19 auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'X19_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "X19_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'x19 dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'x19 dashboard embeds the full X19 TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.x19/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.x19/dashboard-plugins/ — no npm build required.',
    ...
    'X19_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'X19_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'X19_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'X19_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.x19/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.x19/skills/ so drop-in skills appear without restarting the session.',
    ...
    'x19 -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'x19 chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'x19 chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'x19 chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "x19 chat --source tool tags programmatic chats so they don't clutter x19 sessions list.",
    'x19 dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'x19 sessions rename <ID> "new title" renames any past session; x19 sessions delete <ID> removes one.',
    'x19 import restores a full X19 backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'x19 fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'x19 pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'x19 setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'x19 status --deep runs the full health sweep across every component; plain x19 status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'X19_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'X19_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.x19/plugins/ — trust-gated by design.',
    "X19_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'X19_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'X19_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'X19_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'X19_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'X19_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'X19_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'X19_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'X19_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'X19_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'X19_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes X19 block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from x19 sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.x19/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — X19 is
