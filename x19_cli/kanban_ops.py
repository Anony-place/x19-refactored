"""Dispatcher and maintenance verbs for ``x19 kanban``: ``dispatch``,
...
from x19_cli.kanban_output import _err, _fmt_ts, _print_json
...
_DAEMON_DEPRECATED = (
    "x19 kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    x19 gateway start       # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"successfully.{held} Check profile health (venv, PATH, credentials) and `x19 "
                    f"kanban list --status ready` / `x19 kanban list --status blocked` for "
