"""Default configuration data for X19: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from x19_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["x19-cli"],
    # journal_mode: SQLite journal mode for every X19 DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `x19 -c`: each CLI session writes a breadcrumb under
        # $X19_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `x19
        ...
        # X19-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping X19 (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env X19_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `x19 doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`x19 status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # X19_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else X19_HOME/cache/terminal (auto-pruned
        ...
        # X19_HOME/home; "real" = force real HOME; "profile" = force X19_HOME/home when it
        ...
        # ["/home/user/.x19/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled X19 image,
        # which drops to `x19` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `x19 tools`, not a ring
        # member). Set by the `x19 tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on an X19-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by X19' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.x19/browser-profile/ so credentials
        ...
        # runs `x19 browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `x19 chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.x19/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `x19 checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .x19.md,
    ...
    # Same bound for single-query mode (``x19 -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # X19' summarizer cannot shrink it. native = codex decides; x19 = X19' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `x19 model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `x19`/`x19 chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / X19_TUI=1 forces the TUI.
        "interface": "cli",
        # `x19 --tui` auto-resumes the most recent human-facing session (like `x19 -c`).
        # X19_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = X19_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/x19`.
        ...
        # CLI/TUI/desktop, managed with `x19 pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_x19_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: X19_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than X19_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env X19_DASHBOARD_OAUTH_CLIENT_ID / X19_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env X19_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # X19_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env X19_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/x19) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.x19/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → X19 turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to X19 (any model / provider you have
    ...
    # "Hey X19" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_x19" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.x19/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.x19/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <x19_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.x19/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # X19_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.x19/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `x19 skills trust` / `untrust`.
        ...
        # skip; X19_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${X19_SKILL_DIR} / ${X19_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `x19 skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.x19/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.x19/.curator_backups/blobs/); powers `x19
    ...
    # daemon. `x19 curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `x19 curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `x19 update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `x19 curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.x19/skills/ to
        # ~/.x19/skills/.curator_backups/<utc-iso>/skills.tar.gz (`x19 curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); x19-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *X19*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (X19_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export X19_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. X19_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `x19 plugins enable|disable` and
    ...
    # approvals persist in ~/.x19/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # X19_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .x19 config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `x19 doctor --ack <id>`; remove by editing the list. Catalog:
        # x19_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $X19_HOME/plugins/ <name>/),
        ...
        # Env override: X19_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: X19_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: X19_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: X19_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `x19 -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across X19 homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `x19 kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with x19's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core X19 tools
    ...
    "logging": {  # File logging to ~/.x19/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://anony-place.github.io/x19-refactored/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `x19 model` once the cache is older. Network
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: X19
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "x19-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to X19_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to X19_STARTUP_WATCHDOG / X19_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.x19/sessions/sessions.json mirror of the routing index
        ...
        # `false` keeps per-profile gateways for good. `x19 gateway migrate --multiplex` folds a
        ...
        # adapter is parked; `x19 profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `x19 update` fold this install onto a multiplexed default gateway by itself?
        ...
        # `x19 gateway migrate --multiplex` is an explicit request and always proceeds.
        ...
        # (X19_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # X19_GATEWAY_MAX_STARTS / X19_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.x19/.env, auth.json). True:
        # files must be under the X19 cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to X19_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the X19 cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to X19_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to X19_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # X19_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.x19/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `x19 sessions prune`).
        ...
        # stay until `x19 sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `x19 update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.x19/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to X19_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`x19 sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `x19 doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `x19 update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <X19_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``x19 backup`` zip in
        # <X19_HOME>/backups/ (``x19 import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `x19 update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `x19 update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <X19_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `x19 tools`.
    "x_search": {
    ...
    # encrypted vault (`x19 vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.x19/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.x19/cache/,
            ...
            # Auto-download bws into ~/.x19/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; X19 sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; X19 also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.x19/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # X19 Desktop (Electron) launch options; only affect `x19 desktop`.
        ...
        # Bridged to X19_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to X19_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        "anthropic_wire": "chat",
        # Nous free tier: with no other provider configured, X19 sets up a free Nous identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `x19 auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (X19 still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by X19 releases after validation.
        "tag": "b10964",
    ...
    # X19 organization runtime — how the organization executes.
    #
    # The roster is NOT configured here. Roles are registered in code
    # (x19/org/roles.py) and can be added at runtime, so a team list in config
    # would drift from the registry that actually runs the work. Everything the
    # UI, status and audit layers report is read from that registry.
    #
    # Every key below is read by x19/org/config.py::load_org_settings and has a
    # real consumer; out-of-range values are clamped, and an unreadable config
    # falls back to these defaults rather than stopping the organization.
    "x19": {
        "org": {
            # Where the operating record lives (task graph, event stream, run,
            # approvals). Empty = X19_HOME/org.
            "state_dir": "",
            # Persist the operating record across restarts. Turn off only for
            # ephemeral runs; the task graph is then lost on exit.
            "persist": True,
            # Seconds with no observed activity before an active role is marked
            # degraded rather than healthy.
            "stale_after_seconds": 900,
        },
        "execution": {
            # Retry budget per worker task. Once spent, the guards refuse to
            # re-dispatch and the failure escalates to X19 instead of looping.
            "max_attempts": 3,
            # Tasks dispatched in one pass over the ready set.
            "max_parallel": 4,
        },
        "authority": {
            # Role ids whose tasks must be approved by the operator before
            # dispatch. Empty = no role is pre-gated. There is deliberately no
            # switch to auto-approve a pending request: human authority cannot
            # be configured away.
            "approval_roles": [],
        },
    },
    "_config_version": 47,  # Config schema version - bump this when adding new required fields (47: x19 section is real org config)
}
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials X19 mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'x19 model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the X19 auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey X19' wake word engine (optional; "
    ...
    "X19_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "X19_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "X19_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use X19, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @x19:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. X19_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #x19)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: x19-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'x19' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote X19 API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote X19 API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote X19 API server (proxy mode). Must "
    ...
    # X19_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. X19_TOOL_PROGRESS is unsupported.
    "X19_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "X19_EPHEMERAL_SYSTEM_PROMPT": _setting(
        "Ephemeral system prompt injected at API-call time (never persisted to sessions)",
        "Ephemeral system prompt", None),
    # ── X19 organization ──
    "X19_ORG_DIR": _setting(
        "Directory for the X19 organization operating record (task graph, event "
        "stream, run, approvals). Empty = X19_HOME/org.",
        "X19 organization state directory", None),
}
