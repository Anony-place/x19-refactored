...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: x19 gateway install\n"
                "                    sudo x19 gateway install --system  # Linux servers\n"
                "     Check status:  x19 cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'x19 cron create ...' "
...
def _delivery_fix_hint(job: Dict[str, Any]) -> str:
    return (f"Check the target with `x19 cron status` or change it with "
            f"`x19 cron edit {job.get('id', '<id>')} --deliver <target>`.")
...
def _missed_fire_line(job: Dict[str, Any], fire_err: Dict[str, Any]) -> str:
    ...
    return (f"{color('⚠ A scheduled run was skipped', Colors.RED)} at {fire_err.get('at', '?')}: the messaging "
            f"gateway was unreachable. Run `x19 gateway restart`, then `x19 cron run {job.get('id', '<id>')}` "
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `x19 cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: x19 cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: x19 cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec x19 x19 cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `x19 cron status`.\n"
              "  If heartbeat never appears, restart: x19 gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: x19 gateway restart")
...
def cron_status():
    ...
    from x19_cli.gateway import find_gateway_pids, named_profile_served_by_running_multiplexer
    print()
    ...
    if provider != "builtin":
        ...
        print(color("✓ Gateway is running via the default-profile multiplexer — it ticks this profile's jobs.",
                    Colors.GREEN))
        print(color("  Ticker health is reported by `x19 cron status` on the default profile.", Colors.DIM))
    else:
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `x19 cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits X19_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (X19_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _x19_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _x19_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``x19 cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: x19 cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: x19 cron [list|create|edit|pause|resume|run|remove|resnap|status|runs|doctor|tick]")
    sys.exit(1)


def _cron_resnap(args) -> int:
    """Handle `x19 cron resnap [job_id] [--all]`."""
    ...
    if not job_id:
        ...
        print("Usage: x19 cron resnap <job_id> | x19 cron resnap --all")
        return 1
