"""Multi-provider authentication system for X19.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.x19/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``x19_cli.auth.<name>`` stays the public/patchable surface."""
...
from x19_cli.config import (
    get_x19_home, get_config_path, read_raw_config, require_readable_config_before_write)
from x19_constants import OPENROUTER_BASE_URL, x19_home_key, secure_parent_dir
...
from x19_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from x19_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from x19_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from x19_cli.auth_oauth_grants import (  # noqa: F401  re-exported
    SINGLE_USE_REFRESH_POOL_PROVIDERS, strip_cloned_single_use_oauth_grants)
from x19_cli.auth_nous import (  # noqa: F401  re-exported
...
from x19_cli.auth_minimax import (  # noqa: F401  re-exported
...
from x19_cli.auth_xai import (  # noqa: F401  re-exported
...
from x19_cli.auth_codex import (  # noqa: F401  re-exported
...
from x19_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from x19_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from x19_cli.auth_qwen import (  # noqa: F401  re-exported
...
from x19_cli.auth_constants import (  # noqa: F401  re-exported
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.x19/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from x19_cli.config import get_env_value_prefer_dotenv
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$X19_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from x19_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.x19/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $X19_HOME/.env (e.g. X19_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``x19 auth``."""
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `x19 model` to re-authenticate."
...
# ── Auth Store — persistence layer for ~/.x19/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_x19_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch X19_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".x19" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set X19_HOME to a tmp_path in your test fixture, or run "
...
def _save_private_json(target: Path, data: Any, *, fsync_dir: bool = False, **dump_kwargs: Any) -> None:
    ...
    mkdir_under_x19_home(target.parent)
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``x19 auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    not inherited (#111724) — ``x19 -p <name> auth add <provider>`` gives the profile its own."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``x19 auth reset``) and skip the
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT X19 flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in X19.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "x19_pkce", "manual"})
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit X19 flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when X19-scoped routing config is present.
    ...
    X19-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    X19-initiated flow, or X19-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``x19 logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    if str(provider_name or "").strip().lower() in {"opencode-free", "free", "opencode_free"}:
        return ("OpenCode discontinued anonymous free-tier access outside its own client "
                ...
                "($10/mo subscription, OPENCODE_GO_API_KEY) via 'x19 model'.")
    try:
        from x19_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'x19 doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from x19_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run x19 setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `x19 auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `x19 auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `x19 auth list` showing the credential while requests go out
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    dashboard's Ink chat parked every session on Setup Required while ``x19 chat`` worked
    (#108383)."""
    try:
        from x19_cli.config import load_config
        model_cfg = (load_config() or {}).get("model")
        ...
        if base_url:
            from x19_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.x19/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.x19/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'x19 model' for available providers, "
                "or run 'x19 doctor' to diagnose config issues.")
    ...
    from x19_constants import display_x19_home, profile_cli_selector, profile_name_for_home
    ...
    profile_name = profile_name_for_home(get_x19_home()) if sel else None
    where = f"Profile '{profile_name}' is" if profile_name else "X19 is"
    raise AuthError(
        f"{where} not connected to any AI provider yet. Run `x19 {sel}model` to pick one (the free "
        f"Nous tier needs no API key), type `/login` in chat, or add a key with "
        f"`x19 {sel}auth add <provider>`. (Advanced: put an API key such as OPENROUTER_API_KEY in "
        f"{display_x19_home()}/.env.)",
...
# Keyed by x19_home_key(): the resolution itself is profile-scoped (_auth_file_path reads the
# per-turn X19_HOME override a multiplex gateway sets), so a single slot would hand profile A's
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """X19_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("X19 is not logged into Nous Portal.", relogin=True)
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            from x19_cli.anon_auth import is_guest_state, refresh_guest_state
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("x19_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``x19 logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from x19_cli.providers import normalize_provider
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    X19 can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from x19_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: X19_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / X19_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``x19_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``x19 doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from x19_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `x19 doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on X19' "
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from x19_cli.config import clear_model_endpoint_credentials
...
def login_command(args) -> None:
    """Deprecated: use 'x19 model' or 'x19 setup' instead."""
    print("The 'x19 login' command has been removed.\nUse 'x19 auth' to manage credentials,\n"
          "'x19 model' to select a provider, or 'x19 setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "nous":
        from x19_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
    ...
    if target == "nous":
        ...
        from x19_cli.auth_nous import _clear_shared_nous_state
    ...
    if not should_reset_config:
        ...
        print("X19 will use OpenRouter for inference.")
    else:
        print("Run `x19 model` or configure an API key to use X19.")
