...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from x19_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"x19 {candidate}", f"x19_cli.main {candidate}", f"x19_cli/main.py {candidate}")
...
_DASHBOARD_SYSTEMD_UNIT = "x19-dashboard.service"
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (X19 installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``x19-serve.service``), or None.
...
# launchd plist directories that can supervise a ``x19 dashboard`` / ``x19 serve`` backend on
...
def _loaded_launchd_backend_jobs(
    ...
    ``ProgramArguments`` is a ``x19 dashboard`` / ``x19 serve`` backend. macOS only (empty
    ...
    if sys.platform != "darwin":
        return []
    import plistlib
    from x19_cli.gateway import _launchd_print_service_pid
...
def _restart_launchd_job(domain: str, label: str, old_pid: int | None, *, timeout: float = 15.0) -> bool:
    ...
    from x19_cli.gateway import _wait_for_launchd_service_pid
    try:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``x19 update``, detached, logging to
    ...
    log_path = get_x19_home() / "logs" / "dashboard-restart.log"
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``x19 update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # x19_cli.config.get_x19_home to simulate setup failure.
        from x19_cli.config import get_x19_home as _get_x19_home
        logs_dir = _get_x19_home() / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file = open(logs_dir / "update.log", "a", buffering=1, encoding="utf-8")

        import datetime as _dt

        stage = "continued on the pulled code" if os.environ.get("X19_UPDATE_POST_SWAP") == "1" else "started"
        log_file.write(f"\n=== x19 update {stage} {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No x19 dashboard or serve processes running.")
        return 0

    print(f"{len(live)} x19 dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from x19_cli.web_server import should_require_dashboard_auth
    ...
    print("    [2] OAuth via Nous Portal (run `x19 dashboard register`)\n    [3] Cancel\n")
    ...
    if choice == "2":
        ...
        print(
            ...
            "X19_DASHBOARD_OAUTH_CLIENT_ID into ~/.x19/.env for you.\n"
    ...
    try:
        ...
        from x19_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from x19_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.x19/
    # desktop-ssh, independent of X19_HOME and the active profile. Anchor
    # validation there, NOT get_x19_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_x19_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".x19" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``x19 dashboard`` inheriting that ``X19_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (X19_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("X19_DESKTOP") == "1"
    ):
        return
    ...
    # X19_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping X19_HOME: in the Docker layout the root is /opt/data, and an
    # unset X19_HOME would fall back to $HOME/.x19 = /opt/data/.x19 — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_x19_root() strips a trailing profiles/<name> for both layouts.
    try:
        from x19_constants import get_default_x19_root
        env["X19_HOME"] = str(get_default_x19_root())
    except Exception:
        env.pop("X19_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets X19_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless X19_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed X19_WEB_DIST can't be populated.
    """
    from x19_cli.main import PROJECT_ROOT
    from x19_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["X19_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "X19_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["X19_WEB_DIST"])
            if "X19_WEB_DIST" in os.environ
            else PROJECT_ROOT / "x19_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom X19_WEB_DIST
            ...
            _recoverable = "X19_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # X19_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # X19_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["X19_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ X19_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset X19_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads X19_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["X19_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from X19_WEB_DIST: {_dist_root}")
