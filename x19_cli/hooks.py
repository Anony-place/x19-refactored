"""x19 hooks — inspect and manage shell-script hooks."""
...
def hooks_command(args) -> None:
    """Entry point for ``x19 hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)
    if not sub:
        print("Usage: x19 hooks {list|test|revoke|doctor}")
        print("Run 'x19 hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.x19/config.yaml.")
        print("See `x19 hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.x19/config.yaml.")
    else:
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if drift is True:
                        print(
                            ...
                            f"run `x19 hooks doctor` to re-validate"
                        )
            print()
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (X19 wire shape): {json.dumps(parsed)}")
    else:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if drift is True:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `x19 hooks revoke` + re-approve to refresh")
        elif drift is False:
