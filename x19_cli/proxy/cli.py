"""CLI handlers for the ``x19 proxy`` subcommand."""
...
from x19_cli.proxy.server import (
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err("x19 proxy requires aiohttp. Run `x19 setup` to install it.")
        return 1
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"x19 auth add {adapter.name}")
        _err(f"Not logged into {adapter.display_name}. Run `{auth_hint}` first.")
        return 2
    host = getattr(args, "host", None) or DEFAULT_HOST
    port = getattr(args, "port", None) or DEFAULT_PORT
    _err(
        f"Starting X19 proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("X19 proxy upstream adapters\n")
    ...
    print("\nStart the proxy with: x19 proxy start [--provider <name>]")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``x19 proxy <subcommand>``; no/unknown subcommand prints the short help."""
    handler = _SUBCOMMANDS.get(getattr(args, "proxy_command", None))
    if handler is not None:
        return handler(args)
    _err(
        "x19 proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  x19 proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
