"""Helpers for loading X19 .env files consistently across entrypoints."""
...
# Once-per-process guards: load_x19_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `x19
...
# X19_HOME paths already pulled external secrets for: load_x19_dotenv() runs at import time from
...
# Behavioral routing keys a parent X19 process injects into child env that silently redirect a profile
...
def get_secret_source_values(x19_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``x19_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(x19_home).resolve()), {}))


def hydrate_profile_secret_sources(x19_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(x19_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_x19_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def reset_secret_source_cache(x19_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``x19_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    if x19_home is None:
        ...
        return
    home_key = str(Path(x19_home).resolve())
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `x19 setup` (or edit the "
...
def _load_dotenv_with_fallback(path: Path, *, override: bool, load_pass: int | None = None) -> None:
    ...
    ``load_pass`` groups the layered files of one ``load_x19_dotenv`` call: within a pass a later layer
    (project, managed) still sees the earlier layer's output, as it always did; only OTHER passes' output
    is peeled. A bare call (``x19 send``'s direct reload) is its own pass."""
    raw = path.read_bytes()
    try:
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from x19_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_x19_dotenv(
    *,
    x19_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load X19 env files: ``~/.x19/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(x19_home) if x19_home else get_process_x19_home()
    ...
    if is_multiplex_active() and get_x19_home_override() is not None:
        ...
        if load_external_secrets:
            from x19_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``x19 update`` retry just completed a deferred dependency
    ...
    # ~/.x19/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.x19/.env (e.g. written by an older
    # `x19 setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_x19_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_x19_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE X19 reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and x19_logging reuse one parse instead of 3-4.
    if home_path == _process_x19_home():
        try:
...
def _process_x19_home() -> Path:
    """The X19_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_x19_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``x19_constants.get_process_x19_home()`` is the override-immune
