...
_MANAGED_FILES_ROOT_ENV = "X19_DASHBOARD_FILES_ROOT"
...
def _default_x19_root_is_opt_data() -> bool:
    raw = os.environ.get("X19_HOME", "").strip()
    if not raw:
        return False
    try:
        from x19_constants import get_default_x19_root

        root = get_default_x19_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``x19 update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the x19-webui image), where
    ...
    if _default_x19_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the X19
    # root actually IS /opt/data or X19_DASHBOARD_FILES_ROOT is set.
    if _default_x19_root_is_opt_data():
