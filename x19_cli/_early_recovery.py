"""Dependency-light venv recovery that runs BEFORE x19_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``x19_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``x19_cli.main`` to become importable again (force-reinstall of
...
# ``x19 update`` renames the live ``x19*.exe`` shims aside (``x19.exe.old.<unix-ms>``) so
# uv can write replacements. Putting them BACK is the safety-critical direction: losing that rename
# leaves no ``x19`` on PATH, and the command that would repair it IS ``x19 update``. The
...
def _should_skip_external_secret_sources() -> bool:
    """True inside any ``x19 update`` process (and its import probes).

    Every dotenv load in the process — ``x19_cli.main``, ``run_agent``, ``cli`` — consults
    ...
    Profile flags are stripped before ``x19_cli.main`` loads dotenv, so ``argv[1]`` is
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which X19
    vendors (``~/.x19/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".x19", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``x19 acp`` speaks JSON-RPC on stdout). uv is preferred when the
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``x19_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    """Repair wiped core packages so ``x19_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls. Only acts when a recovery marker from a
    prior ``x19 update`` exists AND an import probe confirms a core package is actually broken.
    ...
    try:
        ...
        # recovered even when this launch is itself `x19 update`: CLI and Desktop retries keep
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``recover_if_needed`` invokes this when ``.update-incomplete`` exists — a prior ``x19 update`` (or
    ...
    venv extension mapped; this process, running before ``x19_cli.main``'s third-party imports, maps
    nothing yet, so the installer can replace ``.pyd`` files without hitting the lock.
    """
    try:
        from x19_cli import _install_repair as ir
        ...
        if not _claim_recovery_lock(root):
            return False
        try:
            print("⚠ A previous `x19 update` was interrupted mid-install — "
            ...
            print("  The next launch will retry; x19 will keep working from "
