...
def bang_shell_enabled() -> bool:
    ...
    return not (env_var_enabled("X19_GATEWAY_SESSION") or env_var_enabled("X19_CRON_SESSION")
                or (os.getenv("X19_SESSION_PLATFORM") or "").strip())
...
def _bang_env() -> dict:
    """Environment for a bang command with X19-managed secrets filtered.
...
def run_bang_command(command: str, *, cwd: Optional[str] = None, timeout: int = DEFAULT_TIMEOUT, writer=None) -> int:
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the X19 session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
