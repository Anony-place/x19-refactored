...
description: "Run X19 directly on an Android phone with Termux"
---

# X19 on Android with Termux
...
X19 can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop X19 from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
For users who prefer a native package-manager install rather than building Python/Rust dependencies on the phone, a community-maintained APT repository is available. The repository bootstrap and packaging sources are published in [`adybag14-cyber/termux-python`](https://github.com/adybag14-cyber/termux-python), with the X19 package build in [`adybag14-cyber/termux-x19`](https://github.com/adybag14-cyber/termux-x19).

Install the repository key/source and X19 with:
...
APT-managed X19 installs are marked with install method `apt`. X19 therefore does not run its Git self-updater against package-owned files; use the package manager instead:
...
Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. X19 runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

X19 now ships a Termux-aware installer path:
...
- links `x19` into `$PREFIX/bin` so it stays on your Termux PATH
...
X19 requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `x19` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/x19" "$PREFIX/bin/x19"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `x19` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.x19/.env`.
...
### `x19 doctor` says ripgrep or Node is missing
