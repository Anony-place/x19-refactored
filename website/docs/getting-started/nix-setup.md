...
description: "Install and deploy X19 with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
X19 ships a Nix flake, a NixOS module, and a Home Manager module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`x19 setup`, `x19 gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage x19 the same way you manage any other NixOS service.
:::
...
```bash
# Run the desktop app
nix run github:Anony-place/x19-refactored#desktop

# Or install persistently
nix profile install github:Anony-place/x19-refactored#desktop

# run the tui
nix run github:Anony-place/x19-refactored -- setup
nix run github:Anony-place/x19-refactored -- --tui

# or install it in your profile
nix profile install github:Anony-place/x19-refactored
x19 setup
x19 --tui
```
...
:::note
This module needs NixOS. X19 is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    x19.url = "github:Anony-place/x19-refactored";
  };
...
{ config, ... }: {
  services.x19 = {
    ...
    environmentFiles = [ config.sops.secrets."x19-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `x19` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o x19 /dev/stdin /var/lib/x19/env
```

```nix
services.x19.environmentFiles = [ "/var/lib/x19/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `x19` CLI on your system PATH **and** sets `X19_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `x19` in your shell creates a separate `~/.x19/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `x19` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `x19 chat`, `x19 sessions list`, `x19 --version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the x19 codebase, set `X19_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.x19` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `x19` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo x19 chat` manually.
:::
...
x19 --version
x19 config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, X19 defaults to OpenRouter.
:::
...
{ config, ... }: {
  services.x19 = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    # USER.md is memory, so it goes to X19_HOME. Workspace files use
...
```nix
services.x19.configFile = /etc/x19/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$X19_HOME/config.yaml` on each activation.
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."x19-env".path ]` |
| Give the agent an identity | `x19HomeFiles."SOUL.md"` | `"You are a terse ops assistant."` |
...
| Override the x19 package | `package` | `inputs.x19.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/x19"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$X19_HOME/.env` at activation time (`nixos-rebuild switch`). X19 reads this file on every startup, so changes take effect with a `systemctl restart x19` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/x19.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."x19-env" = { format = "yaml"; };
  };

  services.x19.environmentFiles = [
    config.sops.secrets."x19-env".path
  ];
}
```
...
```yaml
# secrets/x19.yaml (encrypted with sops)
x19-env: |
...
```nix
{
  age.secrets.x19-env.file = ./secrets/x19-env.age;

  services.x19.environmentFiles = [
    config.age.secrets.x19-env.path
  ];
}
```
...
{
  services.x19 = {
    authFile = config.sops.secrets."x19/auth.json".path;
...
X19 reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
```nix
{
  services.x19 = {
    ...
    workingDirectory = "/var/lib/x19/workspace";
    documents = {
...
`x19HomeFiles` installs into **`X19_HOME`**. X19 reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. X19 does not load that file as the identity:

```nix
{
  services.x19.x19HomeFiles = {
...
`x19HomeFiles` needs no `workingDirectory`, because the module owns the `X19_HOME` directory. Most users want `x19HomeFiles`.
...
:::tip
Environment variables in `env` values are resolved from `$X19_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.x19.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. X19 implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.x19.mcpServers.my-oauth-server = {
...
Tokens are stored in `$X19_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, X19 prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u x19` (native):

```bash
# Container mode
docker exec -it x19 \
  x19 mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u x19 X19_HOME=/var/lib/x19/.x19 \
  x19 mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
x19 mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.x19/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/x19/.x19/mcp-tokens/
# Ensure: chown x19:x19, chmod 0600
```
...
```nix
{
  services.x19.mcpServers.analysis = {
    command = "npx";
...
When x19 runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `x19 setup` | Config is declarative — edit `settings` in your Nix config |
| `x19 config edit` | Config is generated from `settings` |
| `x19 config set <key> <value>` | Config is generated from `settings` |
| `x19 gateway install` | The systemd service is managed by NixOS |
| `x19 gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **The `X19_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `X19_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it x19 x19 config set ...`.
...
The flake also exports `homeManagerModules.default`. X19 is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.x19`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:

| | NixOS module | Home Manager module |
|---|---|---|
| Runs as | a system user that you declare, with `user`, `group` and `createUser` | you |
| State directory | `stateDir` and `/.x19` | `x19Home`, set directly. The default is `~/.x19`. |
| Service | `systemd.services` | `systemd.user.services` on Linux, `launchd.agents` on macOS |
| CLI on the PATH | `addToSystemPackages`, which exports `X19_HOME` for the full system | `programs.x19.enable`, which exports it for your session only |
| Desktop application | not supported, because a system service cannot own a user session | `programs.x19.desktop.enable` |
...
```nix
{
  imports = [ x19.homeManagerModules.default ];
...
`home-manager switch` makes `~/.x19`, writes `config.yaml`, builds `.env` and starts the gateway as a user service.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. X19 Desktop and the web dashboard connect to a *different* process, which is `x19 serve` or `x19 dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that X19 Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `X19_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
```bash
# Linux
systemctl --user status x19
journalctl --user -u x19 -f

# macOS
launchctl list | grep x19
tail -f ~/Library/Logs/x19.log

x19 --version
x19 config     # shows the configuration that Nix wrote
```
...
When container mode is enabled, x19 runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-x19-0.1.0  ──►  /nix/store/... (ro)
~/.x19 -> /var/lib/x19/.x19       (symlink bridge, per hostUsers)
/var/lib/x19/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/x19 gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/x19` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart x19` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the x19 package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/x19` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/x19-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current x19 package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `x19 plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [x19-lcm](https://github.com/stephenschoettler/x19-lcm)):

```nix
services.x19.extraPlugins = [
...
Plugins are symlinked into `$X19_HOME/plugins/` at activation time. X19 discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."x19_agent.plugins"]` (e.g., [rtk-x19](https://github.com/ogallotti/rtk-x19)):

```nix
services.x19.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the x19 wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in x19's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.x19.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  inputs.x19.url = "github:Anony-place/x19-refactored";
  outputs = { x19, nixpkgs, ... }: {
    nixpkgs.overlays = [ x19.overlays.default ];
    # Then:
...
```nix
services.x19.settings.plugins.enabled = [
  "x19-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core x19 dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # X19_MANAGED blocks mutation
...
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `X19_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the x19 service |
| `package` | `package` | `x19` | The x19 package to use |
| `user` | `str` | `"x19"` | System user |
| `group` | `str` | `"x19"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/x19"` | State directory (`X19_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `x19` CLI to system PATH and set `X19_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$X19_HOME/.env` at activation time |
...
| `x19HomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `X19_HOME`. `SOUL.md` and `memories/` must be here, or X19 does not load them. |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `x19 gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the x19 user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$X19_HOME/plugins/`. Each must contain `plugin.yaml` |
...
### Backend (`x19 serve` / `x19 dashboard`)

This option runs the process that X19 Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
| Option | Type | Default | Description |
|---|---|---|---|
| `x19Home` | `str` | `"${config.home.homeDirectory}/.x19"` | `X19_HOME` directly. The NixOS module builds it from `stateDir`. |
...
### `programs.x19` (Home Manager only)

Home Manager separates "install this application for me" from "run this
daemon". `services.x19` keeps the state, the configuration and the
daemons. `programs.x19` installs what you use, and reads
`x19Home` and the backend address from the services.

| Option | Type | Default | Description |
|---|---|---|---|
| `enable` | `bool` | `false` | Add the `x19` CLI to `home.packages`, and export `X19_HOME` for your shells |
| `package` | `package` | `services.x19.package` | The package to install. The default applies `extraPythonPackages` and `extraDependencyGroups` from the services, so both are one build. |
| `desktop.enable` | `bool` | `false` | Add the X19 Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.x19Desktop` | The desktop package. The default follows `package`, so the application and the services run one X19 runtime. |
...
services.x19 = {
  enable = true;
  backend.mode = "serve";
  backend.sessionTokenFile = config.sops.secrets."x19/desktop-token".path;
};
```

The launcher carries `X19_HOME` itself. A desktop menu reads no shell
profile, so the value that `programs.x19.enable` exports with
`home.sessionVariables` reaches an interactive shell only. Without the
value in the launcher, the application opens `~/.x19` while the
services use `x19Home`, and you see no sessions and no keys.
...
`services.x19.installPackage` was removed by this split. A
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.x19` symlink to the service stateDir and are auto-added to the `x19` group |
...
```
/var/lib/x19/                     # stateDir (owned by x19:x19, 0750)
├── .x19/                         # X19_HOME
│   ├── SOUL.md                      # from x19HomeFiles: the agent identity
...
```
~/.x19/                           # x19Home (X19_HOME), 0700
├── SOUL.md                          # from x19HomeFiles
...
├── auth.json                        # OAuth credentials: seeded, then X19 owns it
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | X19 binary + all Nix deps |
| `/data` | `/var/lib/x19` | `rw` | All state, config, workspace |
| `/home/x19` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update x19
...
```bash
systemctl status x19
docker ps -a --filter name=x19
docker inspect x19 --format='{{.State.Status}}'
docker exec -it x19 bash
docker exec x19 readlink /data/current-package
docker exec x19 cat /data/.container-identity
```
...
```bash
sudo systemctl stop x19
docker rm -f x19
sudo rm /var/lib/x19/.container-identity
sudo systemctl start x19
```
...
```bash
# Native mode
sudo -u x19 cat /var/lib/x19/.x19/.env

# Container mode
docker exec x19 cat /data/.x19/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec x19 readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u x19` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `x19 --version` shows old version | Container not restarted | `systemctl restart x19` |
| Permission denied on `/var/lib/x19` | State dir is `0750 x19:x19` | Use `docker exec` or `sudo -u x19` |
| `nix-collect-garbage` removed x19 | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "x19"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user x19` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart x19` |
