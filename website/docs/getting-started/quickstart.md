---
sidebar_position: 1
title: "X19 Quickstart"
description: "Your first conversation with X19 — from install to chatting in under 5 minutes"
---

# X19 Quickstart

This guide gets you from zero to a working X19 setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [X19 Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="X19 Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up X19 for a team, bot, or always-on workflow
...
|---|---|---|
| I just want X19 working on my machine | `x19 setup` | Run a real chat and verify it responds |
| I already know my provider | `x19 model` | Save the config, then start chatting |
| I want a bot or always-on setup | `x19 gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `x19 model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `x19 model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if X19 cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install X19
### With the X19 Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the X19 Desktop installer](https://anony-place.github.io/x19-refactored/) from our website and run it.

### Without X19 Desktop:
For a command-line only install without X19 Desktop, run:
...
The single most important setup step. Use `x19 model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `x19 setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `x19 update`. Re-enable anything later with `x19 tools`, seed skills with `x19 skills opt-in --sync`, or tune settings with `x19 setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `x19 model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `x19 model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `x19 model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `x19_cli/models.py` may change between releases) | `x19 model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `x19 model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `x19 model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `x19 model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `x19 model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `x19 model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

X19 separates secrets from normal config:

- **Secrets and tokens** → `~/.x19/.env`
- **Non-secret settings** → `~/.x19/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
x19 config set model anthropic/claude-opus-4.6
...
```bash
x19            # classic CLI
x19 --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
X19 ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `x19` vs `x19 --tui`.
:::
...
```bash
x19 --continue    # Resume the most recent session
x19 -c            # Short form
```
...
```bash
x19 gateway setup    # Interactive platform configuration
```
...
- `x19 tools` — tune tool access per platform
- `x19 skills` — browse and install reusable workflows
...
```bash
x19 config set terminal.backend docker    # Docker isolation
x19 config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `x19 egress setup && x19 egress start`; `x19 setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the X19 install directory (the curl installer placed it at
# ~/.x19/x19 on Linux/macOS or %LOCALAPPDATA%\x19\x19 on Windows):
cd ~/.x19/x19
...
Skills are on-demand instruction documents that teach X19 how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

X19 ships with a catalog of bundled skills already installed in `~/.x19/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
x19 skills browse                      # list everything available
x19 skills search kubernetes           # find skills by keyword
x19 skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `x19 skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let X19 ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.x19/x19 && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| X19 opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `x19 model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `x19 gateway setup` and check `x19 gateway status` |
| `x19 --continue` can't find old session | Switched profiles or session never saved | Check `x19 sessions list` and confirm you're in the right profile |
...
| `x19 doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
|---------|-------------|
| `x19` | Start chatting |
| `x19 model` | Choose your LLM provider and model |
| `x19 tools` | Configure which tools are enabled per platform |
| `x19 setup` | Full setup wizard (configures everything at once) |
| `x19 doctor` | Diagnose issues |
| `x19 update` | Update to latest version |
| `x19 gateway` | Start the messaging gateway |
| `x19 --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-x19-to-another-machine)** — `x19 backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
