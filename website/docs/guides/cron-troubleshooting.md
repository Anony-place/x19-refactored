...
description: "Diagnose and fix common X19 cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`x19 gateway` for foreground, or `x19 gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `x19 cron tick`.
...
```bash
date
x19 cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.x19/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.x19/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.x19/.env` |
...
| `local` | Write access to `~/.x19/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `x19 cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (X19 Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `x19 skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `x19 skills list`.
...
2. `~/.x19/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `x19 cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the X19 config directory). Verify:
```bash
ls ~/.x19/scripts/your-script.py   # Must exist
x19 cron edit <job_id> --script ~/.x19/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `x19 skills install <skill-name>`.
...
Jobs are stored in `~/.x19/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.x19/cron/jobs.json
chmod 600 ~/.x19/cron/jobs.json   # Your user should own it
```
...
```bash
x19 cron list                    # Show all jobs, states, next_run times
x19 cron run <job_id>            # Schedule for next tick (for testing)
x19 cron edit <job_id>           # Fix configuration issues
x19 logs                         # View recent X19 logs
x19 skills list                  # Verify installed skills
```
...
1. Run the job with `x19 cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.x19/logs/agent.log` for scheduler messages and `~/.x19/logs/errors.log` for warnings
3. Open an issue at [github.com/Anony-place/x19-refactored](https://github.com/Anony-place/x19-refactored) with:
