...
Copy-paste blueprints for common automation patterns. Each blueprint uses X19's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`x19 webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `x19 webhook subscribe`) |
...
```bash
x19 cron create "0 2 * * *" \
  "You are a project manager triaging the Anony-place/x19-refactored GitHub repo.

1. Run: gh issue list --repo Anony-place/x19-refactored --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
x19 webhook subscribe github-pr-review \
  --events "pull_request" \
...
x19 cron create "0 9 * * 1" \
  "Scan the Anony-place/x19-refactored repo for documentation drift.

1. Run: gh pr list --repo Anony-place/x19-refactored --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (x19_cli/commands.py, x19_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (x19_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
x19 cron create "0 6 * * *" \
  "Run a dependency security audit on the x19 project.

1. cd ~/.x19/x19 && source .venv/bin/activate
...
```bash
x19 webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
x19 webhook subscribe alert-triage \
...
```python title="~/.x19/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "X19-Monitor/1.0"})
...
x19 cron create "every 30m" \
  ...
  --script ~/.x19/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
x19 cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to X19 development." \
...
```bash
x19 webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
x19 webhook subscribe stripe-payments \
...
```bash
x19 cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the x19 codebase.
