...
This tutorial walks you through setting up a Telegram bot powered by X19 that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **X19 installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.x19/.env`

:::tip
A $5/month VPS is plenty for running the gateway. X19 itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team X19 Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_x19_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by X19. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u x19-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `x19 gateway install` to update the plist.
:::
...
```bash
x19 gateway stop && x19 gateway start
```
...
x19 pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.x19/.env`:
...
Control how much detail the bot shows when using tools. In `~/.x19/config.yaml`:
...
Customize how the bot communicates by editing `~/.x19/SOUL.md`:

For a full guide, see [Use SOUL.md with X19](/guides/use-soul-with-x19).
...
```bash
# From the CLI
x19 cron list          # View all scheduled jobs
x19 cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u x19-gateway` (Linux) or `~/.x19/logs/gateway.log` (macOS) |
| Cron job output | `~/.x19/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.x19/cron/jobs.json` |
| Pairing data | `~/.x19/pairing/` |
| Session history | `~/.x19/sessions/` |
