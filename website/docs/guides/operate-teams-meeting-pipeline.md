...
```bash
x19 teams-pipeline token-health
x19 teams-pipeline token-health --force-refresh
```
...
```bash
x19 teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
x19 teams-pipeline maintain-subscriptions
x19 teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: X19 cron (recommended if you already run the X19 gateway)

X19 ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.x19/scripts/`. First create the script:

```bash
mkdir -p ~/.x19/scripts
cat > ~/.x19/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec x19 teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.x19/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
x19 cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/x19-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=X19 Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=x19
EnvironmentFile=/etc/x19/env
ExecStart=/usr/local/bin/x19 teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/x19-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run X19 Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now x19-teams-pipeline-maintain.timer
systemctl list-timers x19-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/x19 teams-pipeline maintain-subscriptions >> /var/log/x19/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.x19/.env` at the top of a wrapper script that crontab calls.
...
```bash
x19 teams-pipeline subscriptions   # should show expirationDateTime advanced
x19 teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
x19 teams-pipeline list
x19 teams-pipeline list --status failed
x19 teams-pipeline show <job-id>
```

### Replay a stored job

```bash
x19 teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
x19 teams-pipeline fetch --meeting-id <meeting-id>
x19 teams-pipeline fetch --join-web-url "<join-url>"
x19 teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `x19 teams-pipeline maintain-subscriptions --dry-run`
- inspect `x19 teams-pipeline list --status failed`
...
- run `x19 teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `x19 teams-pipeline run`
...
- [ ] `x19 teams-pipeline validate` returns an OK snapshot
- [ ] `x19 teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (X19 cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
