---
sidebar_position: 8
title: "Use Voice Mode with X19"
description: "A practical guide to setting up and using X19 voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with X19
...
- you want X19 sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in X19.
...
## Step 1: make sure normal X19 works first
...
X19 supports both local and cloud speech stacks.
...
### If you use `x19 setup`

If you choose NeuTTS in the setup wizard, X19 checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. X19 transcribes and responds
...
- asking X19 to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full X19 loop.

## Tuning CLI behavior

### Silence threshold

If X19 starts/stops too aggressively, tune:
...
X19 stays a normal chat bot, but can speak replies.
...
- you want X19 to function like a portable research or ops assistant
...
X19 joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- X19 detects speech boundaries
- transcripts are posted in the associated text channel
- X19 responds in text and audio
...
1. get text X19 working
2. run `x19 setup tts` to enable voice support
