---
sidebar_position: 6
title: "Use MCP with X19"
description: "A practical guide to connecting MCP servers to X19, filtering their tools, and using them safely in real workflows"
---

# Use MCP with X19

This guide shows how to actually use MCP with X19 in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native X19 tool
- you want X19 to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect X19 to internal APIs, databases, or company systems without modifying X19 core

Do not use MCP when:
- a built-in X19 tool already solves the job well
...
- X19 remains the agent
- MCP servers contribute tools
- X19 discovers those tools at startup or reload time
...
If you installed X19 with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- X19 banner/status should show MCP integration when configured
- ask X19 what tools it has available
...
## WSL2: bridge X19 in WSL to Windows Chrome

This is the practical setup when:

- X19 runs inside WSL2
...
In this setup, X19 does **not** connect to Chrome directly. Instead:

- X19 runs in WSL
- X19 starts a local stdio MCP server
...
```text
X19 (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- X19 stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on X19 core browser transport
...
```bash
x19 mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
x19 mcp test chrome-devtools-win
```

Then start a fresh X19 session or run:
...
Once loaded, X19 can use the MCP-prefixed browser tools directly. For example:
...
If X19 runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start X19 from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start X19 from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in X19:
...
So X19 will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want X19 to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want X19 to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. X19 remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
x19 mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
x19 mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `x19 mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want X19 to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize X19 to merge, publish, deploy, or spawn runtimes.
...
Now X19 can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing X19 core.
...
Because X19 now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
