...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `x19 send`."
---

# Pipe Script Output to Messaging Platforms

`x19 send` is a small, scriptable CLI that pushes a message to any
messaging platform X19 is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | x19 send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `x19
...
x19 send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
x19 send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
x19 send --list

# Filter by platform
x19 send --list telegram
```
...
Any platform X19 ships adapters for works as a target:
...
`x19 send` resolves the message body in this order:

1. **Positional argument** — `x19 send --to telegram "hi"`
2. **`--file PATH`** — `x19 send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | x19 send --to telegram`

When stdin is a TTY (no pipe), X19 does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  x19 send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `x19 send` reuses your X19 config, the same script works on
any host where X19 is installed — no need to export bot tokens into
...
`x19 send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  x19 send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | x19 send \
...
```bash
./train.py --epochs 200 && \
  x19 send --to telegram "training done" || \
  x19 send --to telegram "training failed (exit $?)"
```
...
x19 send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(x19 send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `x19 send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `x19 send` calls
the platform's REST endpoint directly using credentials from
`~/.x19/.env` and `~/.x19/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `x19 gateway start` and retry.
...
The listing is built from `~/.x19/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`x19 gateway
...
| Approach | Multi-platform | Reuses X19 creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `x19 send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`x19 send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `x19 send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `x19 send` shares with cron delivery.
