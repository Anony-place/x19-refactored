...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.x19/.env` at the end.
...
4. Fill in:
   - **Name:** `X19 Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `x19-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.x19/.env`:
...
X19 ships a Graph auth smoke-test. From your X19 install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.x19/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `x19 gateway restart`.
