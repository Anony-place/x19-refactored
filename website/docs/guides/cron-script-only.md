...
X19 calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, X19 writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **X19:** *(writes `~/.x19/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.x19/scripts/memory-watchdog.sh",
...
>
> **X19:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `x19 cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **X19:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.x19/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.x19/scripts/memory-watchdog.sh
...
# 3. Verify
x19 cron list
x19 cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.x19/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
x19 cron create "every 5m"        # interval
x19 cron create "every 2h"
x19 cron create "0 9 * * *"       # standard cron: 9am daily
x19 cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.x19/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.x19/.env` / `~/.x19/config.yaml`.

## Editing and Lifecycle

```bash
x19 cron list                                    # see all jobs
x19 cron pause <job_id>                          # stop firing, keep definition
x19 cron resume <job_id>
x19 cron edit <job_id> --schedule "every 10m"    # adjust cadence
x19 cron edit <job_id> --agent                   # flip to LLM mode
x19 cron edit <job_id> --no-agent --script …     # flip back
x19 cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.x19/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.x19/scripts/disk-alert.sh
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on X19' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When X19 might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to an X19 webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on X19 being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
