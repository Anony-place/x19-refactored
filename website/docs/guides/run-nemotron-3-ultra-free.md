...
[Download the X19 Desktop installer](https://anony-place.github.io/x19-refactored/) for macOS or Windows, then open it. On first launch it finishes setting itself up (usually under a minute).

### 2. Connect Nous Portal

When the app opens, you'll see a "Let's get you set up" screen. Click **Nous Portal** (marked **Recommended**). Your browser opens — create a [Nous Portal](https://portal.nousresearch.com) account (or sign in), choose the **Free** plan, and authorize X19. The app connects automatically.
...
Prefer to review first? Download [`install.sh`](https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.sh), inspect it, then run it.
...
Select **Quick Setup**. X19 opens a browser tab and waits for you to finish the next steps.
...
When prompted to connect your account to X19, click **Connect**. You'll see a confirmation once it's linked.
...
- **Don't see the model in the list?** Make sure you finished the Nous Portal connection and that you're on the **Free** plan. In the CLI, `x19 portal info` confirms you're logged in and routing through Nous.
...
- **[Run X19 with Nous Portal](/guides/run-x19-with-nous-portal)** — Full Portal walkthrough: models, Tool Gateway, and verification
