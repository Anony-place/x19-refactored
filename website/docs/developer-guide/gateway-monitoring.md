...
X19 gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by X19's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `x19.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `x19.platform.up/degraded` with bounded `error_code` attributes |
...
`x19.gateway.active_agents`, `x19.gateway.background_work`, and
`x19.gateway.background_delegations` are complementary. `active_agents`
...
```bash
x19 monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'x19[otlp]'`),
...
a plain `x19 gateway run`: the exporter lives in the gateway process, so
...
`x19.gateway.up`, `x19.platform.up`, and `x19.platform.degraded`.
...
absent_over_time(x19_gateway_up[5m])

# Locally owned bridge is explicitly down.
x19_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
x19_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
x19_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
x19_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(x19_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `x19.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
x19.status = failed|unknown
x19.delivery_outcome = failed|not_configured
x19.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `x19.job_key`;
...
X19-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/x19_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/x19_otel_capture.jsonl --wait 8
...
`x19.error_class = ...` list in this file's alert section and the enum's unit
...
The `x19 monitoring` CLI intentionally exposes `status` only. This first
release covers only X19-owned service-health and operational-diagnostic
signals, including X19-owned Relay transport health. Team Gateway's
