...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to X19 — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to X19! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying X19 core? Start with [Build an X19 Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for X19 itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated X19 instance (throwaway X19_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m x19_cli.main
scripts/dev-sandbox.sh --persistent python -m x19_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.x19/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.x19` — use `get_x19_home()` from `x19_constants` for code paths and `display_x19_home()` for user-facing messages. See [AGENTS.md](https://github.com/Anony-place/x19-refactored/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) X19 can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask X19 to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/Anony-place/x19-refactored/blob/main/LICENSE).
