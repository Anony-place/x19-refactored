...
title: "Desktop Plugin SDK (@x19/plugin-sdk)"
description: "Extend the native X19 Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [X19 Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `X19Plugin`.
It imports one module — `@x19/plugin-sdk` — and gets everything: the app's
...
`$X19_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across X19. This page is the **native
desktop app** (`x19 desktop`) SDK — the `@x19/plugin-sdk` module and
`$X19_HOME/desktop-plugins/`. The **web dashboard** (`x19 dashboard`) has
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$X19_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$X19_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `X19Plugin` contract, appear in **Capabilities → Plugins**,
...
Create `$X19_HOME/desktop-plugins/hello/plugin.js` (that's `~/.x19/...`
...
```javascript
// ~/.x19/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@x19/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, X19' }),
      jsx('div', {
...
The only importable specifiers are `@x19/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `X19Plugin`:
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `x19.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@x19/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@x19/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@x19/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@x19/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@x19/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@x19/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@x19/plugin-sdk'
...
`window.x19.send('get-price eth')` (or a declarative
`<button data-x19-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@x19/plugin-sdk'
...
for requests; `targetProfile` is the backend X19 profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from X19
...
ctx.os.notify({
  ...
  // Body click → focus X19 + navigate. Same vocabulary as OS deep links:
  activate: 'x19://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'x19://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `x19://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `x19://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@x19/plugin-sdk'
...
```javascript
import { queryClient } from '@x19/plugin-sdk'
...
copies that half into `$X19_HOME/desktop-plugins/<id>/` beside a
`.x19-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.x19/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`x19 plugins update`, or
...
a package into `~/.x19/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.x19/plugins` is not reachable as a filesystem —
...
the `x19://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular X19 plugin and declare it in a
`manifest.json`:
...
x19 codebase directly (`x19_state`, `x19_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.x19/`)
...
(`x19.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `X19Plugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@x19/plugin-sdk` alias.
2. It's still lint-fenced to `@x19/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@x19/plugin-sdk`, `react`,
...
| Plugin contract | `X19Plugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `X19OpenTarget`, `Contribution` |
...
### Agents: the `x19-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`x19-desktop-plugins`** skill — it carries the same contract as this page in
...
`$X19_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `x19 logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@x19/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.x19/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.x19/logs/errors.log` for
