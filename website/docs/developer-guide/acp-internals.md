...
The ACP adapter wraps X19' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
x19 acp / x19-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.x19/.env
  -> configure stderr logging
  -> construct X19ACPAgent
...
- `allow_once` -> X19 `once`
- `allow_always` -> X19 `always`
- reject options -> X19 `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps X19 tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["x19-acp"])
...
Instead it reuses X19' runtime resolver:

- `acp_adapter/auth.py`
- `x19_cli/runtime_provider.py`

So ACP advertises and uses the currently configured X19 provider/credentials. It also always advertises a terminal setup auth method (`x19-setup`, args `--setup`) so first-run ACP clients can open X19' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.x19/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `x19-acp` toolset definition
- `x19_cli/main.py` — `x19 acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `x19-acp` script
