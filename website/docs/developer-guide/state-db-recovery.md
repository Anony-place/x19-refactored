...
description: "How X19 recovers state.db when the FTS index or the file itself is corrupt"
---
...
`x19 doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `x19 doctor --fix` while the gateway
is still up. Next steps:

```bash
x19 gateway stop
X19_HOME="$HOME/.x19" x19 sessions recover --source "$HOME/.x19/state.db" --inspect-only
# if recoverable:
X19_HOME="$HOME/.x19" x19 sessions recover --source "$HOME/.x19/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
x19 gateway stop
X19_HOME="$HOME/.x19" x19 sessions repair --check-only
X19_HOME="$HOME/.x19" x19 sessions repair
```
