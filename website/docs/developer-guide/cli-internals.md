...
sidebar_position: 15
title: "CLI Internals"
description: "How x19_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `x19_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `x19_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `x19_cli.update_cmd._x19_holder_subcommand` (top-level subcommand of any X19 argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `x19_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.x19/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

X19 supports profiles — fully isolated instances, each with its own `X19_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`x19 -p x <cmd>`),
`_apply_profile_override()` in `x19_cli/main.py` sets `X19_HOME` before any module imports, so
every `get_x19_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["X19_HOME"]` stays the launch
...
`Path.home() / ".x19" / "profiles"`, not `get_x19_home() / "profiles"`) so
`x19 -p coder profile list` sees all profiles regardless of which one is active — intentional.
