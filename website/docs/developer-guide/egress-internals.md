...
description: "How the iron-proxy egress firewall integrates with X19 — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`x19 egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
x19_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `x19 egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

x19_cli/subcommands/egress.py:_dispatch_egress
                                       Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `x19 proxy` subparser, which uses
                                       dest='proxy_command').

x19_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _X19_EGRESS_NODE_OPTIONS_APPEND
...
tests/x19_cli/test_iron_proxy_cli.py          CLI handler unit tests (~20).  Argparse
                                       wiring, fail-loud paths, BWS refresh
                                       wire-up, dest='egress_command'
                                       regression guard.

tests/agent/test_iron_proxy_e2e.py          Live E2E (gated on X19_RUN_E2E=1).
...
x19 egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.x19/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.x19/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

x19 egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.x19/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `x19 egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `x19 egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `x19 egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, X19_EGRESS_PROXY) and the per-mapping `X19_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/x19-egress-ca.crt`).
...
- User-facing setup + troubleshooting: [Egress proxy](https://anony-place.github.io/x19-refactored/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://anony-place.github.io/x19-refactored/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`x19 secrets bitwarden`](https://anony-place.github.io/x19-refactored/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`x19 egress`](https://anony-place.github.io/x19-refactored/docs/reference/cli-commands#x19-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://anony-place.github.io/x19-refactored/docs/reference/environment-variables#egress-proxy-sandbox-injected)
