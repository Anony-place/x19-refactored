...
and subscribers are unavailable. The `x19[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the X19 `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `X19_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `X19_NEMO_RELAY_ATOF_*` and `X19_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, X19 does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

X19 requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts X19
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

X19 installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the X19 process so configured interceptors can
...
Collection remains off unless X19 policy enables it:
...
opt-in. Set `X19_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, X19 does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, X19 reports the error and does not invoke
...
retain the same `session_id` and add `x19.session.segment` plus
`x19.session.segment_reason` (`compaction` or `max_turns`).
...
policy. After initialization succeeds, X19 logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the X19
process and share the same policy boundary.

Run profiles in separate X19 processes when they require different trust
...
X19 core owns one Relay host and one isolated Relay session scope per X19
...
marks do not require X19 plugin registration. Shared-metrics marks must
...
```text
X19 turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> X19 shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

X19 sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that X19 used for the logical call, such as
...
New calls use `x19.model_route.count`. The previous
`x19.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `x19.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; X19
version, OS family, architecture, and install method remain bounded package
resources. Concurrent X19 processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `x19.task_run`, parented to
the owning X19 session. The start counter contains only bounded execution
...
provider attempts for the same X19 API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their X19 tool-call
...
cancellations. Active task ownership follows the task ID if X19 rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`x19.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. X19
...
tool or plugin names. X19 does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `x19.tool_approval` marks and
...
Successful skill mutations emit `x19.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `x19.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. X19 derives reuse and patch-generation
...
```text
$X19_HOME/telemetry/shared_metrics/metrics.sqlite3
$X19_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the X19 version,
...
schema field name, its current scope is one `X19_HOME`, so it is more
...
Deleting `$X19_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `X19_HOME`. To reset it, stop X19
processes and remove `$X19_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real X19 CLI turn against the deterministic local model server:
...
`$X19_HOME/telemetry/shared_metrics/outbox/` to the X19 telemetry ingest
...
Removing `$X19_HOME/telemetry/shared_metrics` still resets local identity,
