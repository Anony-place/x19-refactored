...
Plugins can launch and supervise fresh X19 child sessions without importing
...
active agent turn fails closed with `No active X19 parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until X19 can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; X19's existing unsafe-tool block remains enforced.
