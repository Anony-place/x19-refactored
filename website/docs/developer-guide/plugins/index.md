...
X19 has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `X19_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.x19/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `x19 skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `Anony-place/x19-refactored`. Users install them into `~/.x19/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Nous Research Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/Anony-place/x19-refactored/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

X19 can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components X19 already owns. It does not replace native `plugin.yaml` plus
...
```bash
x19 plugins install owner/repository --no-enable
x19 plugins list
x19 plugins enable <plugin-name>
```
...
X19 validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by X19.
...
Portable `streamable-http` entries are routed through X19' existing native
...
same full-trust posture as other installed X19 plugins.
...
records it as Published. X19 keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. X19 does not expose a
...
continue to work after a normal X19 upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  X19 inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older X19 releases can therefore load a plugin whose manifest contains
...
X19 enforces this contract with frozen external-plugin fixtures discovered
from an isolated `X19_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.x19/plugins/calculator
cd ~/.x19/plugins/calculator
```

### Validate with Plugin Doctor

`x19 plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by X19 itself. It reports invalid hook names, callbacks that do
...
```bash
x19 plugins doctor . --ci
```

Doctor uses a temporary `X19_HOME`, restores plugin registration state after
...
This tells X19: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `x19_cli/plugin_capabilities.py` for the
...
`x19_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`x19_agent.plugins` entry point:

```toml
[project.entry-points."x19_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."x19_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

X19 reads these from installed metadata without importing your code, so
`x19 plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this X19 understands still loads with a warning.
...
| `python_dependencies` | list of str | PEP 508 requirements (e.g. `"requests>=2.0,<3"`). Installed into X19' venv on `x19 plugins install` / `enable` and **re-applied after every `x19 update`** (see [Python dependencies](#python-dependencies)). A `pyproject.toml` beside `plugin.yaml` with `[project].dependencies` is the equivalent, preferred form. |
| `python_runtime` | str | `external` — the plugin manages its own interpreter/venv (sidecar pattern); X19 installs nothing and leaves any `pyproject.toml` alone. |
...
requires_plugins:
  - id: other-plugin
    version_range: ">=1.0,<2"
python_dependencies:
  - "somepkg>=1.0,<2"     # installed on install/enable, re-applied after x19 update
config_schema:
...
When both exist the `pyproject.toml` wins. What X19 does with them:

- **Install / enable** — the declared packages are installed into X19' venv with
  `uv pip install` (pip fallback) under a **constraints file built from X19' own pinned
  dependencies**, so a plugin can never move a core package (httpx, pydantic, …) off the version
  X19 was tested with. Environment markers (`; sys_platform == "win32"`) are honoured.
...
- **`x19 update` re-applies them** — the update's `uv sync` rebuilds the venv from X19' lock
...
- **Opt out with `python_runtime: external`** — plugins that keep a heavy runtime (torch, native
  ...
  `plugin.yaml`; X19 then installs nothing and the plugin never joins the shared resolution.
- **Nothing to load is an error** — `x19 plugins validate` (and the catalog CI) fail a
...
`X19_HOME/plugins/` survives `x19 update` and Desktop updates: the updater only rebuilds the
...
4. **Accept `**kwargs`:** X19 may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `x19 my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but X19 continues fine
...
Use plugin-relative config keys for user-visible behavior. X19 resolves them
...
`<X19_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `X19_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
X19_PLUGINS_DEBUG=1 x19 plugins list
```
...
The same logs are always written to `~/.x19/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
x19 logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `x19 plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.x19/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `x19 plugins update` / `remove` git-pull or delete it — your
...
# <x19 home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.x19/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.x19/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `x19 plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing X19 into installing arbitrary packages — only specs X19 itself ships are eligible |
...
auditable in `~/.x19/logs/agent.log`. Plugins load after built-in
...
does not ship with X19 core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `x19 plugins list`
...
loader, the plugin is disabled and X19 continues. The gate exists
...
You normally never edit this key by hand. `x19 plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), X19 injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$X19_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $X19_HOME/hook_outputs
```
...
- **The system prompt is X19's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `x19_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before X19 sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`x19.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer X19 still loads on an older one.

### Register CLI commands

Plugins can add their own `x19 <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for x19 my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: x19 my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for x19 my-plugin."""
...
After registration, users can run `x19 my-plugin status`, `x19 my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `x19 name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `x19 chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `X19_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `x19 <subcommand>` (e.g. `x19 kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "x19 kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive X19 from a hook.
...
X19 has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.x19/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `x19 gateway status` without SDK instantiation.
...
X19 also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into X19 without any Python plugin. Declare them in `~/.x19/config.yaml`:
...
X19 connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.x19/hooks/<name>/`:

```yaml
# ~/.x19/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.x19/hooks/long-task-alert/handler.py
...
x19 skills search my-workflow --source myorg/skills-repo
x19 skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install x19-plugin-calculator
# Plugin auto-discovered on next x19 startup
```
...
```python
# Wrong — will break if X19 passes extra context
def handler(args):
    ...
