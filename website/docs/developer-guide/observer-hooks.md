...
X19 observer hooks are the read-only telemetry contract for plugins that
...
They do not replace X19' planner, model providers, memory, tool registry,
...
X19 also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "x19.observer.v1"
```

Hook callbacks are fail-open. X19 catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so X19 only builds
...
The native NeMo Relay SDK integration maps X19 session, turn, LLM, and tool
