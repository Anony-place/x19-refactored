...
X19 tools are self-registering functions grouped into toolsets and executed through a central registry/dispatch system.
...
2. **Plugin tools** — `x19_cli.plugins.discover_plugins()` loads user/project/pip plugins that may register additional tools.
...
Toolsets are named bundles of tools. X19 resolves them through:

- explicit enabled/disabled toolset lists
- platform presets (`x19-cli`, `x19-telegram`, etc.)
- dynamic MCP toolsets
- curated special-purpose sets like `x19-acp`
