...
description: "How X19 detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in X19' browser tooling:
...
(`x19-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per X19 `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `x19-acp`, `x19-api-server`, and core toolsets (gated on CDP reachability)
- `x19_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
