...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept X19 already has applies. The plugin never sees a
...
x19 itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — X19 provider id (e.g. "openrouter")
...
using X19 built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different X19 provider
        # (must be one X19 already knows about — same names as
        # `x19 model` and config.yaml model.provider).
        ...
        # plugin sends — X19 does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.x19/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through X19' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing X19 subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Anony-place/x19-refactored/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Anony-place/x19-refactored/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/Anony-place/x19-refactored-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/Anony-place/x19-refactored-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
