...
Guided onboarding suggests outcomes from the user's app preferences and observed capabilities. It does not grant access or implement a second connection lifecycle. The feature remains inside the existing `X19_GUEST_ONBOARDING` flow.
...
It was checked against the current integration and Sid's pending [connection-operation PR](https://github.com/Anony-place/x19-refactored/pull/111008). That PR moves MCP execution into the backend; recommendations do not depend on which side currently executes the operation. Optional catalog fields preserve older clients, and missing fields preserve older backends. Sid's cancelled portal toolkit-metadata endpoint is not a dependency.
