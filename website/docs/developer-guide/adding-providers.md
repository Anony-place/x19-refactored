...
description: "How to add a new inference provider to X19 — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

X19 can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `x19 model` menu entries
...
1. `x19_cli/auth.py` decides how credentials are found.
2. `x19_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `x19_cli/models.py` and `x19_cli/main.py` make the provider show up in the CLI. (`x19_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`x19_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `x19 setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$X19_HOME/plugins/model-providers/` both get picked up.
...
7. `x19 model` menu includes the provider
8. `x19 setup` wizard delegates to `main.py` automatically
...
User plugins at `$X19_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `x19 model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `x19_cli/auth.py`
- `_PROVIDER_LABELS` in `x19_cli/models_catalog_static.py` (re-exported by `x19_cli/models.py`)
- `_PROVIDER_ALIASES` in both `x19_cli/auth.py` and `x19_cli/models_catalog_static.py`
- CLI `--provider` choices in `x19_cli/main.py`
...
## Step 2: Add auth metadata in `x19_cli/auth.py`
...
- What env vars should X19 check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `x19_cli/models.py`
...
## Step 4: Resolve runtime data in `x19_cli/runtime_provider.py`
...
Be careful with API-key precedence. X19 already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `x19_cli/main.py`

A provider is not discoverable until it shows up in the interactive `x19 model` flow.

Update these in `x19_cli/main.py`:
...
:::tip
`x19_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `x19 model` and `x19 setup` automatically.
:::
...
When you add a native provider, double-check that X19 is only sending fields that provider actually understands.
...
- `tests/x19_cli/test_runtime_provider_resolution.py`
- `tests/x19_cli/test_cli_provider_resolution.py`
- `tests/x19_cli/test_model_switch_custom_providers.py` (and adjacent `tests/x19_cli/test_model_switch_*.py`)
- `tests/x19_cli/test_setup_model_provider.py`
...
python -m pytest tests/x19_cli/test_runtime_provider_resolution.py tests/x19_cli/test_cli_provider_resolution.py tests/x19_cli/test_setup_model_provider.py tests/agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m x19_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `x19_cli/auth.py`
- [ ] aliases added in `x19_cli/auth.py` and `x19_cli/models.py`
- [ ] model catalog added in `x19_cli/models.py`
- [ ] runtime branch added in `x19_cli/runtime_provider.py`
- [ ] CLI wiring added in `x19_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `x19 model` but not `x19 setup`
