...
has to be thrown away or summarized. X19 has always done this in one batch:
...
X19 folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. X19's normal session flush is
...
With `micro_compact` unset or `false` X19 behaves exactly as it always has:
...
Defaults to `$X19_HOME/logs/agent.log`. It reports passes, outcome mix, net
