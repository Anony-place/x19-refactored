...
description: "Wire contract between the X19 gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **X19 gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `x19 gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`x19 gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `x19` CLI.
