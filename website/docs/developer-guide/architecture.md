...
sidebar_position: 1
title: "Architecture"
description: "X19 internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of X19 internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ x19_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # X19CLI facade — mixins in x19_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── x19_state.py           # SQLite session/state database facade (+ x19_state_*.py siblings)
├── x19_constants.py       # X19_HOME, profile-aware paths
...
│
├── x19_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `x19` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # x19 skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # x19 tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # x19 gateway start/stop
│
...
```text
User input → X19CLI.process_input()
...
Three discovery sources: `~/.x19/plugins/` (user), `.x19/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `x19 plugins` or `config.yaml`.
...
Exposes X19 as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`x19 -p <name>`) gets its own X19_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
