...
description: "Frequently asked questions and solutions to common issues with X19"
---
...
### What LLM providers work with X19?

X19 works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `x19 auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `x19 model` or by editing `~/.x19/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run X19 in WSL2. What's the best way to control my normal Windows Chrome?
...
- let X19 use the resulting MCP browser tools

This is more reliable than trying to force X19 core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with X19](../guides/use-mcp-with-x19.md#wsl2-bridge-x19-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). X19 does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.x19/`.

### Can I use it offline / with local models?

Yes. Run `x19 model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← X19 minimum; set this to match your server's actual context window
```
...
X19 persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in X19 — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
X19 auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `X19_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

X19 itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same X19 instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use X19 programmatically:
...
#### `x19: command not found` after installation
...
**Cause:** X19 requires Python 3.11 or newer.
...
**Cause:** X19 builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when X19 runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** X19 auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.x19/config.yaml`:
...
Independently of the init files, every terminal command's `PATH` is completed with the standard system directories (`/usr/local/bin`, `/opt/homebrew/bin`, …), the X19-managed runtime dirs, and `~/.local/bin` when it exists (the `pip --user` / `pipx` / `uv tool` install target) — appended after your own entries, so precedence is unchanged. This covers backends started with a thin non-interactive PATH (systemd, GUI launchers, the Desktop SSH remote backend) without any configuration.
...
#### The agent says "X19 policy" or "X19 guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden X19 runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

X19 enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. X19 does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.
...
**Solution:** Exit your session and use `x19 model` from your terminal to add new providers:

```bash
# Exit the X19 chat session first (Ctrl+C or /quit)
...
After adding a new provider via `x19 model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `x19 model` (from terminal) |
| Enter/change API keys | `x19 model` (from terminal) |
...
# Or set directly
x19 config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.x19/.env` for conflicting entries.
:::
...
# Set a valid model
x19 config set model.default anthropic/claude-opus-4.7

# Or specify per-session
x19 chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `x19 chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or X19 detected the wrong context length for your model.
...
x19 chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, X19 may have the wrong context length for your model. Check what it detected:
...
**Cause:** X19 detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — X19 never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `x19 chat`
...
cat ~/.x19/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `x19 gateway setup`
- Check gateway logs: `cat ~/.x19/logs/gateway.log | tail -50`
...
Configure in `~/.x19/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.x19/x19 && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `x19 gateway start` fails
...
tmux new -s x19 'x19 gateway run'
# Reattach later: tmux attach -t x19

# Option 3: Background via nohup
nohup x19 gateway run > ~/.x19/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'x19 gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `x19 gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
x19 gateway install    # Re-snapshots your current PATH
x19 gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.x19.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `x19 chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `x19 chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`x19 prompt-size`](/reference/cli-commands#x19-prompt-size) to measure it, then trim: disable toolsets you don't use (`x19 tools`) and uninstall or disable skills you don't need (`x19 skills`).
...
Verify your `~/.x19/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
x19 config show | grep -A 12 mcp_servers

# Restart X19 or reload MCP after config changes
x19 chat
```
...
:::warning
If an MCP server crashes mid-request, X19 will report a timeout. Check the server's own logs (not just X19 logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `x19 profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or X19 home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `x19 update`?

`x19 update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `x19 update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is a directory under `~/.x19/profiles/` that carries at least one identity file (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json` or `state.db`); a bare directory without one (a leftover from a log rotation or cron tick) is not a profile — it is not listed or served, `-p <name>` reports it as missing, and `x19 profile create <name>` refuses to overwrite it until you move or remove it. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** X19 can route subagents to a different model automatically. Set this in `~/.x19/config.yaml`:
...
Now when you tell X19 "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can X19 do this?

**Current limitation:** X19 profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, X19 reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `x19 skills config` settings don't seem to take effect.

**Solution:** Use `x19 skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`x19 gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** X19 creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting X19 to another machine
...
1. Install X19 on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/Anony-place/x19-refactored/main/scripts/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   x19 backup
   ```
   This creates a zip of your entire `~/.x19/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/x19-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/x19-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   x19 import ~/x19-backup-<timestamp>.zip
   ```

4. On the new machine, run `x19 setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
x19 profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
x19 profile import ./work-backup.tar.gz work
```
...
### `x19 backup` vs `x19 profile export`

| Feature | `x19 backup` | `x19 profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.x19` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='x19' ~/.x19/ newmachine:~/.x19/
```

:::tip
`x19 backup` produces a consistent snapshot even while X19 is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
x19 config show | head -20

# Re-run model selection
x19 model

# Or test with a known-good model
x19 chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/Anony-place/x19-refactored/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), X19 version (`x19 --version`), and the full error message
