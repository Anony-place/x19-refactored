...
of fields, and X19 schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and X19 asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.x19.blueprint` block in its
