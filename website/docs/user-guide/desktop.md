...
sidebar_position: 3
title: "X19 Desktop"
description: "The native X19 desktop app — a polished experience for chatting with X19, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# X19 Desktop

The X19 desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same X19 core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `x19` in a terminal, everything you set up there is already here, and anything you do here shows up there.
...
X19 has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`x19`) and **[TUI](./tui.md)** (`x19 --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`x19 dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [X19 Desktop product page](https://anony-place.github.io/x19-refactored/desktop), or follow the [installation instructions for X19 Desktop](../getting-started/installation.md).

If you already have X19 installed, simply run
...
- **The same conversation history** as every other X19 surface — sessions started here resume in the CLI/TUI and vice versa.
...
X19 Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional X19 sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `x19 desktop --cwd <path>` (or the `X19_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask X19 to open PR**. You can also create and switch branches from here.
...
Under local WSLg, X19 launches with `--ozone-platform=wayland` to avoid the XWayland maximized-window offset and shifted mouse hit-testing ([microsoft/wslg#1015](https://github.com/microsoft/wslg/issues/1015)). The platform must be selected at process launch, before Electron loads application JavaScript. Explicit `--ozone-platform=x11` and `desktop.ozone_platform_hint: x11` remain available. The app draws its own minimize, maximize and close controls on WSLg.

When `x19 gui` runs inside WSL2 with `/dev/dxg` present and Mesa's `d3d12_dri.so` installed, the launcher sets `GALLIUM_DRIVER=d3d12` for Electron so rendering uses the Windows GPU instead of the llvmpipe software rasterizer; an explicit `GALLIUM_DRIVER`, `MESA_LOADER_DRIVER_OVERRIDE`, `LIBGL_ALWAYS_SOFTWARE`, or `LIBGL_DRIVERS_PATH` in your environment is left untouched (for example `GALLIUM_DRIVER=llvmpipe x19 gui` keeps software rendering).
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `x19 model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader X19 management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#x19-cron).
- **Profiles** — switch between [X19 profiles](./profiles.md) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels. Telegram has a **Quick setup** card: click **Create with QR**, scan the code (or open the link) in Telegram, and X19 creates the bot, detects your user ID for the allowlist, saves the credentials, and restarts the gateway for you. Any credential save, clear, or enable toggle keeps a **Restart now** banner on the page until the gateway has actually restarted; if a restart fails, the banner stays so you can retry or restart manually.
...
|---|---|---|
| Skills | [`/docs/api/skills.json`](https://anony-place.github.io/x19-refactored/docs/api/skills.json) | `https://nousresearch.github.io/x19/docs/api/skills.json` |
| Plugins | [`/docs/api/plugins.json`](https://anony-place.github.io/x19-refactored/docs/api/plugins.json) | `https://nousresearch.github.io/x19/docs/api/plugins.json` |
...
The public hubs' **Install in X19** buttons open `x19://skill/install`
or `x19://plugin/install` links and require confirmation in Desktop. Use
...
roster where every [X19 profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by X19 cron). The roster lives in the left
...
each other's Bot Chats directly (`x19 -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update X19, and more.
...
The desktop app and the X19 backend it talks to update on separate clocks — the app package on your machine, the backend wherever it runs. When more than one update target exists (a remote gateway, or several registered gateways), the update affordances (**Update now** on the About panel, the ⌘K **Update X19** row, and the update-ready toast) update **everything**: the connected backend first, then every other eligible registered gateway (X19 Cloud entries are platform-managed and skipped), and the desktop app itself last, since applying the client update relaunches the app. Single-machine installs keep the one-button experience.
...
The [manual update process](https://anony-place.github.io/x19-refactored/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the X19, your config, and your chats stay. (Same as `x19 uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `x19 uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `x19 uninstall --full`.)
...
You can do the same from the terminal — `x19 uninstall --gui` for the GUI alone, or `x19 uninstall` / `x19 uninstall --full` for the agent too.

:::note
Running `x19 uninstall --gui` from a **source checkout** (a `x19 desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `x19 desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `x19 desktop`

To launch via the CLI, simply run `x19 desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.

On Linux, launches refresh `$XDG_DATA_HOME/applications/x19.desktop` (by default `~/.local/share/applications/x19.desktop`) so X19 appears in the application menu. To preserve a hand-edited entry, disable refreshes:

```bash
x19 config set desktop.manage_launcher_entry false
```

A missing entry is still created; the flag only stops `x19 desktop` from rewriting an entry that already exists.

When you start X19 from the application grid or menu (the launcher sets `DESKTOP_STARTUP_ID`), the entry is written only after the window is on screen. If the app exits before a window appears, nothing is written that time; the next terminal launch, updater relaunch, or grid launch that shows a window installs it. Some GNOME Shell versions lose track of an app whose `.desktop` file changes while it is still starting (they keep it in that state until the startup notification completes or times out, not until the process exits), and that can crash the whole session later; waiting for the window avoids it. Terminal launches and the updater's relaunch still write the entry immediately.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `x19 update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `X19_DESKTOP_CWD`)           |
| `--x19-root PATH` | Override the X19 source root the app uses (sets `X19_DESKTOP_X19_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `x19` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the X19 runtime into `X19_HOME` (`~/.x19`, or `%LOCALAPPDATA%\x19` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `X19_DESKTOP_X19_ROOT`, then a completed managed install, then a probed `x19` on `PATH` (unless `--ignore-existing` / `X19_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `X19_DESKTOP_X19` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `x19 serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `x19 --tui`. The desktop app is **self-contained**: it runs its own `x19 serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at an X19 backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `x19 serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **X19 Cloud** — sign in once to X19 Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every X19 gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), X19 Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another X19 gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many X19 Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the X19 Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `x19 update` to every eligible gateway at once — X19 Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `x19 serve` process
"Remote backend" means a **`x19 serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `x19 serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.x19/.env` (the secrets file, mode 0600):

```bash
# 1. Set the dashboard login credentials.
cat >> ~/.x19/.env <<'EOF'
X19_DASHBOARD_BASIC_AUTH_USERNAME=admin
X19_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
# Recommended: a stable signing secret so sessions survive restarts.
# Without it a random key is generated per boot and you'll be logged out
# on every restart.
X19_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.x19/.env

# 2. Run the backend bound to a reachable address. The non-loopback bind
#    engages the auth gate; the username/password provider handles login.
x19 serve --host 0.0.0.0 --port 9119
```

Keep that `x19 serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `x19 serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `X19_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.x19/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/x19` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `X19_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `X19_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `X19_DASHBOARD_BASIC_AUTH_USERNAME` / `X19_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.x19/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `X19_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting X19 Desktop to a remote backend](./features/web-dashboard.md#connecting-x19-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & X19 Desktop](../reference/environment-variables.md#web-dashboard--x19-desktop).
...
`$X19_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
- **Desktop column** — the half loaded into this app. It is app-level: the
  ...
  `~/.x19/desktop-plugins/`; the desktop half of a unified agent+desktop
...
If the local X19 backend process exits after it was ready, Desktop restarts it on its own and shows a **X19 stopped working in the background** notice; the chat reconnects once the replacement is up. `X19_HOME/logs/desktop.log` records the exit code together with the backend's last output lines (`X19 backend exited (1)` followed by `Recent backend output:`), so the reason it died is in the log even when the app recovered by itself. A backend that keeps dying within seconds of every restart points at the backend itself — look at the traceback in that tail. After three such restarts within two minutes Desktop stops respawning and shows a **keeps crashing** notice instead of cycling; relaunch the app once the cause is fixed.
...
- **Open logs** — opens `X19_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `x19 debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `X19_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.x19/x19/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```

**Other native downloads (e.g. the `get-windows` prebuilt on Windows) that need a mirror:** put the npm keys in `$X19_HOME/npmrc` (`%LOCALAPPDATA%\x19\npmrc` on Windows, `~/.x19/npmrc` elsewhere) — for example `node_get_windows_binary_host_mirror=https://<mirror>/sindresorhus/get-windows/releases/download/`. Every `npm ci`/`npm run` the updater spawns (desktop, web and TUI builds) points `NPM_CONFIG_USERCONFIG` at that file when it exists, so the config survives `x19 update`; the repo-root `.npmrc` is git-tracked and gets autostashed on every update, and `~/.npmrc` may be missed because the desktop hand-off inherits the GUI's environment. An `NPM_CONFIG_USERCONFIG` you set yourself is never overridden.
...
(Desktop, then Downloads, then Documents, ...) as X19 touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with X19' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **X19.app** if you use Desktop.
3. Fully quit and relaunch them once.

`x19 doctor` reports whether the current terminal context already has the
grant, and `x19 setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.nousresearch.x19   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
X19. Grants are stable from then on.
...
certificate once and tell X19 to use it. The one-shot command does
...
```bash
x19 desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `X19 Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `x19 config set desktop.macos_signing_identity "X19 Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `X19 Local Signing`). The command is idempotent —
...
`tccutil reset All com.nousresearch.x19` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `x19 --tui` and the dashboard chat tab
