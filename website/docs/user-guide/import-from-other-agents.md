...
description: "One-command import of a Claude Code (~/.claude) or OpenAI Codex CLI (~/.codex) setup into X19 — instructions, allowlists, MCP servers, skills, and memories."
---

# Import from Other Agents

`x19 import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into X19 with one command. It follows the same preview-first pattern as [`x19 claw migrate`](../guides/migrate-from-openclaw.md): you always see a per-item plan before anything is written, and `--dry-run` never touches disk.

```bash
x19 import-agent                    # auto-detect ~/.claude or ~/.codex
x19 import-agent claude-code        # import from ~/.claude
x19 import-agent codex              # import from ~/.codex
x19 import-agent claude-code --dry-run          # preview only
x19 import-agent codex --source /path/to/.codex # custom location
x19 import-agent claude-code --overwrite --yes  # replace conflicts, skip prompts
```
...
| Claude Code | X19 |
|---|---|
| `CLAUDE.md` (global instructions) | Memory entries in `~/.x19/memories/MEMORY.md` |
...
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.x19/skills/claude-code-imports/<name>/` |
...
| Codex CLI | X19 |
|---|---|
| `AGENTS.md` (global instructions) | Memory entries in `~/.x19/memories/MEMORY.md` |
| `config.toml` → `[mcp_servers.*]` | `mcp_servers` in `config.yaml` |
| `memories/*.md` | Memory entries in `~/.x19/memories/MEMORY.md` |
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.x19/skills/codex-imports/<name>/` |

## What is never imported

**API keys and credentials.** Credential files (`~/.claude/.credentials.json`, `~/.codex/auth.json`) are never read, and MCP server environment variables or headers with secret-looking names (`*_TOKEN`, `*_API_KEY`, `Authorization`, ...) are stripped and listed in the report so you can re-add them deliberately. Run `x19 setup` to configure providers, or add secrets to `~/.x19/.env`.
...
- **Conflicts are skipped by default.** An MCP server or skill that already exists in X19 is reported as a conflict; pass `--overwrite` to replace it.
...
- Coming from OpenClaw instead? Use [`x19 claw migrate`](../guides/migrate-from-openclaw.md).

## Keeping imports in sync

Every successful import registers its source (and the digest of everything it read) in `~/.x19/import-sync.json`. When the other agent's setup changes later — new skills, edited `CLAUDE.md`/`AGENTS.md`, added MCP servers — pull the changes in with:

```bash
x19 import-agent --sync            # re-import every changed source
x19 import-agent --sync --dry-run  # preview what a sync would do
```
...
The manifest is per profile. A profile created with `x19 profile create <name> --clone --sync-imports` carries it over, so `x19 -p <name> import-agent --sync` keeps pulling from the same external trees (see [Profiles](./profiles.md#keep-a-clones-imported-agent-setups-synced---sync-imports)).
