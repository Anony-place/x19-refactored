...
When running X19 inside Docker, the default `network_mode: host` gives the
...
The X19 [SECURITY.md](https://github.com/Anony-place/x19-refactored/blob/main/SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   X19_UID=$(id -u) X19_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=x19,x19-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://x19-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/Anony-place/x19-refactored/blob/main/SECURITY.md) — X19 trust model and vulnerability reporting
- [Docker](/user-guide/docker) — running X19 in a container
- [Egress proxy](iron-proxy.md) — credential-injection firewall for the sandbox
- [docker-compose.yml](https://github.com/Anony-place/x19-refactored/blob/main/docker-compose.yml) — default compose configuration
