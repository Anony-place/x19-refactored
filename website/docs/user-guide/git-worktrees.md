...
title: "Git Worktrees"
description: "Run multiple X19s safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

X19 is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with X19 so each session has a clean, isolated working directory.

## Why Use Worktrees with X19?

X19 treats the **current working directory** as the project root:

- CLI: the directory where you run `x19` or `x19 chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.x19/config.yaml`
...
X19 creates `.worktrees/my-experiment/` inside the repo (branch
`x19/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `x19-<id>`
...
git worktree add ../repo-feature feature/x19-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/x19-experiment` checked out in that directory

Now you can `cd` into the new worktree and run X19 there:

```bash
cd ../repo-feature

# Start X19 in the worktree
x19
```
...
git worktree add ../repo-experiment-a feature/x19-a
git worktree add ../repo-experiment-b feature/x19-b
```
...
- Works on its own branch (`feature/x19-a` vs `feature/x19-b`).
...
- X19 checkpoint data under `~/.x19/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per X19 experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/x19-checkpoints-docs`, `feature/x19-refactor-tests`.
- **Commit frequently**
...
- **Avoid running X19 from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `x19 -w` (Automatic Worktree Mode)

X19 has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `x19/x19-<hash>`).
...
```bash
x19 -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `x19 -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each X19 session its own clean checkout.
