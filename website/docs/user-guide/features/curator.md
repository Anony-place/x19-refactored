...
It exists so that skills created via the [self-improvement loop](/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.x19/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

By default (`prune_builtins: true`) the curator can archive **unused bundled built-in skills** (shipped with the repo) after `archive_after_days` of non-use, alongside the agent-created skills it primarily manages. Hub-installed skills (from [agentskills.io](https://agentskills.io)) are always off-limits. Set `curator.prune_builtins: false` to restore the old agent-created-only behavior, where bundled skills are never touched. The curator also **never auto-deletes** — the worst outcome is archival into `~/.x19/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/Anony-place/x19-refactored/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron job. On CLI session start, during gateway housekeeping, and on the Desktop/`x19 serve` maintenance timer, X19 checks whether:
...
Desktop and other `x19 serve` backends share the existing hourly maintenance timer (first poll after 90 seconds), independently of cron jobs. They measure inactivity from process startup and the most recent chat activity in the same profile, retaining that activity timestamp after a session closes or is reaped, and skip curator while a turn in that profile is running. A connected but inactive window does not block maintenance. This timer also polls personal and organization Skill Sync, subject to those features' own opt-in gates. A running messaging gateway for the same profile owns these chores instead.
...
On a brand-new install (or the first time a pre-curator install ticks after `x19 update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `x19 curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (14) become `stale`; skills unused for `archive_after_days` (30) are moved to `~/.x19/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
```bash
x19 model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`x19 model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
x19 curator status         # last run, counts, pinned list, LRU top 5
x19 curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
x19 curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
x19 curator run --background  # fire-and-forget: start the run in a background thread
x19 curator run --dry-run  # preview only — report without any mutations
x19 curator backup         # take a manual snapshot of ~/.x19/skills/
x19 curator rollback       # restore from the newest snapshot
x19 curator rollback --list     # list available snapshots
x19 curator rollback --id <ts>  # restore a specific snapshot
x19 curator rollback -y         # skip the confirmation prompt
x19 curator pause          # stop runs until resumed
x19 curator resume
x19 curator pin <skill>    # never auto-transition this skill
x19 curator unpin <skill>
x19 curator adopt <skill>    # hand an unmanaged skill to the curator
x19 curator adopt --all-unmanaged   # hand over every unmanaged skill
x19 curator list-unmanaged   # itemize skills with no provenance marker
x19 curator restore <skill>  # move an archived skill back to active
x19 curator list-archived    # list skills currently in ~/.x19/skills/.archive/
x19 curator archive <skill>  # manually archive a single skill now
x19 curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default: `archive_after_days`, 30)
x19 curator ledger           # list the per-mutation audit ledger (all actors)
x19 curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
x19 curator rollback <entry-id>  # undo a single mutation from the ledger
x19 curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```

## Backups and rollback

Before every real curator pass, X19 takes a tar.gz snapshot of `~/.x19/skills/` at `~/.x19/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
x19 curator rollback        # restore newest snapshot (with confirmation)
x19 curator rollback -y     # skip the prompt
x19 curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, X19 takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `x19 curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `x19 curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`x19 curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.x19/skills/.curator_ledger.jsonl`:
...
- **before/after** — per-file `{path, sha256}` manifests. File contents are stored content-addressed (deduped by hash) under `~/.x19/.curator_backups/blobs/`, so a hundred entries touching the same unchanged file cost one blob.

```bash
x19 curator ledger                  # newest 20 entries
x19 curator ledger --skill my-skill --limit 50
x19 curator rollback <entry-id>     # restore that one mutation's before-state
```

Single-entry rollback restores exactly the files that mutation touched (and removes files it created) from the blob store — nothing else in the skills tree moves. Like whole-tree rollback, it takes a safety ledger entry of the current state first and **fails closed**: if the safety capture can't be written, nothing is changed. Because foreground deletes are ledgered too, `x19 curator rollback <entry-id>` can resurrect a hard-deleted skill.
...
Archived skills are kept forever by default. If you want `~/.x19/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
x19 curator purge --dry-run   # preview what would be deleted
x19 curator purge             # delete archives older than the TTL (with confirmation)
x19 curator purge --days 90   # one-off TTL override
```

## What "agent-created" means

The curator only manages skills explicitly marked as **agent-created** in
`~/.x19/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.x19/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.x19/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed X19 at an external skill
...
**To see which skills the curator actually manages**, run `x19 curator status`.
...
`x19 curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `x19 curator adopt <name>` hands one over
```
...
```bash
x19 curator list-unmanaged                    # itemize them, with reasons
x19 curator adopt <name> [<name> ...]         # hand specific skills over
x19 curator adopt --all-unmanaged --dry-run   # preview the full list
x19 curator adopt --all-unmanaged             # hand over everything (prompts)
x19 curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `x19 curator adopt` changes the policy, and
...
**wrote** it — X19 edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `x19 curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `x19 curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `x19 curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
x19 curator pin <skill>
x19 curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.x19/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.x19/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.x19/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.x19/logs/curator/`:
...
```bash
x19 curator restore <skill-name>
```

This moves the skill back from `~/.x19/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.x19/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `x19 curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/Anony-place/x19-refactored/issues/7816) — original proposal and design discussion
