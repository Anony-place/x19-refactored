...
Type `@` followed by a reference to inject content directly into your message. X19 expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- X19 env: `$X19_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$X19_HOME/skills/.hub/`
