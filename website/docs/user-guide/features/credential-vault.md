...
fills the password through X19. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): X19 generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `x19 vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, X19
...
in, X19 picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). X19 hands the master password to the manager's
...
Prefer not to use a detected manager? `x19 vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `x19 vault add`), bound to the checkout site,
...
- **CLI**: `x19 vault list`, `x19 vault add`, `x19 vault rm <handle>`,
  `x19 vault sources`.

Items live encrypted under `~/.x19/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `x19 chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through X19: not in
