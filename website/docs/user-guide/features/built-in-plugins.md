...
description: "Plugins shipped with X19 that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
2. **User** — `~/.x19/plugins/<name>/`
3. **Project** — `./.x19/plugins/<name>/` (requires `X19_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `x19_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `x19 memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `x19 plugins list` and the interactive `x19 plugins` UI), but none load until you explicitly enable them:

```bash
x19 plugins enable disk-cleanup
```

Or via `~/.x19/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer X19. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
x19 plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `x19 plugins enable <name>`.
...
| `x19-achievements` | dashboard tab | Steam-style collectible badges generated from your real X19 session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `x19 memory` and `x19 plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `X19_HOME` or `/tmp/x19-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under X19_HOME | always | Never |
...
**State** — everything lives at `$X19_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `X19_HOME` or `/tmp/x19-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end. User project trees (`workspace/`, `projects/`, `plans/`, `home/`) are never tracked or swept at all: a `test_*.py` or `tmp_*` file inside your project is source code, not scratch.

**Enabling:** `x19 plugins enable disk-cleanup` (or check the box in `x19 plugins`).

**Disabling again:** `x19 plugins disable disk-cleanup`.
...
**Enabling:** `x19 plugins enable security-guidance` (or check the box in `x19 plugins`).

**Disabling again:** `x19 plugins disable security-guidance`.
...
Traces X19 turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of X19' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `x19 logs`.
...
```bash
x19 tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart X19 and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
x19 plugins enable observability/langfuse
```

Then put the credentials in `~/.x19/.env`:

```bash
X19_LANGFUSE_PUBLIC_KEY=pk-lf-...
X19_LANGFUSE_SECRET_KEY=sk-lf-...
X19_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "X19 turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `X19_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the X19 session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `x19 chat` session lives under one Langfuse session.

**Verify:**

```bash
x19 plugins list                 # observability/langfuse should show "enabled"
x19 chat -q "hello"              # check the Langfuse UI for a "X19 turn" trace
```
...
| `X19_LANGFUSE_RELEASE` | — | Release/version tag |
| `X19_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `X19_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `X19_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

X19-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — X19-prefixed wins when both are set.
...
**Disabling:** `x19 plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled X19 plugin. Do not run `x19 plugins enable observability/nemo_relay`; X19 core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `X19_NEMO_RELAY_PLUGINS_TOML` to that file before starting X19. The policy is process-wide for every profile hosted by that X19 process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `X19_NEMO_RELAY_ATOF_*` and `X19_NEMO_RELAY_ATIF_*` settings no longer activate exporters — a `.env` that still carries them (and no `X19_NEMO_RELAY_PLUGINS_TOML`) exports **nothing**, and the gateway logs one warning saying so. `x19 doctor` reports these stale settings when no replacement `plugins.toml` is selected.

**Automatic migration.** `x19 update` (and `x19 migrate relay`, or `x19 migrate relay --all-profiles` for every profile home) converts the legacy variables into `<x19 home>/relay-plugins.toml`, sets `X19_NEMO_RELAY_PLUGINS_TOML` in that profile's `.env`, and comments the legacy lines out (nothing is deleted). Under a multiplexed gateway every profile home gets its own file. The generated file is validated through Relay before it is written; this is the shape it produces (note the `type = "file"` sink discriminator — a sink without it is rejected):
...
type = "file"
output_directory = "/home/you/.x19/telemetry/nemo-relay/atof"
filename = "x19-atof.jsonl"
mode = "append"
...
agent_name = "X19"
model_name = "unknown"
output_directory = "/home/you/.x19/telemetry/nemo-relay/atif"
...
Then add `X19_NEMO_RELAY_PLUGINS_TOML=/home/you/.x19/relay-plugins.toml` to `.env` and restart the gateway.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `x19.session.segment` plus `x19.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.x19/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
x19 plugins enable google_meet
x19 meet setup   # preflight: playwright, chromium, auth file
x19 meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `x19 plugins disable google_meet`. Any saved transcripts stay in `~/.x19/workspace/meetings/` until you remove them.

### x19-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real X19 session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with X19 feature changes.

**How it works:**

- Scans your entire `~/.x19/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$X19_HOME/plugins/x19-achievements/state.json`
...
| Secret | Hidden until X19 detects the first related signal in your history |

**API** — routes mount under `/api/plugins/x19-achievements/`:
...
**State files** — live under `$X19_HOME/plugins/x19-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across X19 updates. |
...
**Enabling:** Nothing to enable — `x19-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `x19 dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/x19-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.x19/plugins/x19-achievements/` that ships no dashboard. The plugin's state files under `$X19_HOME/plugins/x19-achievements/` survive — reinstalling preserves your unlock history.
