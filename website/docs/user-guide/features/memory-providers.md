...
X19 ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
x19 memory setup      # interactive picker + configuration
x19 memory status     # check what's active
x19 memory off        # disable external provider
```

You can also select the active memory provider via `x19 plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.x19/config.yaml`:
...
When a memory provider is active, X19 automatically:
...
**Setup Wizard:**
```bash
x19 memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `x19 honcho setup` command still works (it now redirects to `x19 memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$X19_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$X19_HOME/honcho.json` > `~/.x19/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Anony-place/x19-refactored/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `x19 honcho`
If you previously used `x19 honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per X19 profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All X19 profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per X19 profile. Host key `x19` → default; `x19.<profile>` for others. |
...
```bash
x19 profile create coder --clone
```

`--clone` creates a `x19.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every X19 profile, creates host blocks for any profile without one, inherits settings from the default `x19` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `x19 memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/Anony-place/x19-refactored/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
# Then configure X19
x19 memory setup    # select "openviking"
# Or manually:
x19 config set memory.provider openviking
```

`x19 memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.x19/.env`, and for named profiles use
`~/.x19/profiles/<profile>/.env`.
...
Peer identity is optional. By default, X19 sends no peer ID and writes
...
Set `memory.openviking.agent: x19` to restore the old peer-scoped writes.
...
X19 sends `User-Agent: openviking-memory-x19/<version>` on OpenViking
...
**Setup (Platform):**
```bash
x19 memory setup    # select "mem0" → "Platform"
# Or manually:
x19 config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.x19/.env
```

**Setup (OSS):**
```bash
x19 memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
x19 memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
x19 memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
x19 memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
x19 memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.x19/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.x19/.env
```
...
**Config:** `$X19_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.x19/.env`.
...
| `user_id` | `x19-user` | User identifier |
| `agent_id` | `x19` | Agent identifier |
...
**Switching modes:** Re-run `x19 memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
x19 memory setup    # select "hindsight"
# Or manually:
x19 config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.x19/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p x19 ui start`

**Config:** `$X19_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `x19` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between X19 and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Anony-place/x19-refactored/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
x19 memory setup    # select "holographic"
# Or manually:
x19 config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.x19-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$X19_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
x19 memory setup    # select "retaindb"
# Or manually:
x19 config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.x19/.env
```
...
# Then configure X19
x19 memory setup    # select "byterover"
# Or manually:
x19 config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$X19_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=x19), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
x19 memory setup    # select "supermemory"
# Or manually:
x19 config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.x19/.env
```
...
Before running `x19 memory setup`, set `base_url` in
`$X19_HOME/supermemory.json`:
...
Then run `x19 memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$X19_HOME/supermemory.json`
...
| `container_tag` | `x19` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `x19-{identity}` → `x19-coder`) to isolate memories per X19 profile
...
| **Requires** | `pip install x19-memori` + `x19-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install x19-memori
x19-memori install
x19 config set memory.provider memori
x19 memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `x19-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$X19_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$X19_HOME/` so each profile has its own credentials
- **Cloud providers** (RetainDB) auto-derive profile-scoped project names
- **Env var providers** (OpenViking) are configured via each profile's `.env` file

## Providers Moving to the Plugin Catalog

Memory providers are moving out of the X19 tree into their maintainers' own repositories,
...
When a provider you have configured stops shipping with X19, `x19 update` installs its
...
`false`, in which case it prints the `x19 plugins install <name>` one-liner instead).
