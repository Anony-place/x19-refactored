...
description: "Customize X19's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

X19's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `X19_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who X19 is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

X19 now seeds a default `SOUL.md` automatically in:

```text
~/.x19/SOUL.md
```

More precisely, it uses the current instance's `X19_HOME`, so if you run X19 with a custom home directory, it will use:
...
- X19 creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- X19 loads `SOUL.md` only from `X19_HOME`
- X19 does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, X19 falls back to a built-in default identity
...
If X19 loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `X19_HOME`, the personality belongs to the X19 instance itself.

That also makes it easier to teach users:
- "Edit `~/.x19/SOUL.md` to change X19' default personality."
...
- how X19 should handle uncertainty, disagreement, or ambiguity
...
## What X19 injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, X19 falls back to a built-in default identity ("You are X19, built by Nous Research. Be direct: match the length of your reply to the weight of the ask..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
X19 ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain X19, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives X19 its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.x19/config.yaml` under `agent.personalities` (a top-level `personalities:` block works too; `agent.personalities` wins if the same name appears in both).
...
1. Keep a thoughtful global `SOUL.md` in `~/.x19/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-x19)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how X19 speaks
- `display.skin` and `/skin` affect how X19 looks in the terminal
