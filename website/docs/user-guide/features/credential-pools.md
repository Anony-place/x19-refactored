...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, X19 automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, X19 auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
x19 auth add openrouter --api-key sk-or-v1-your-second-key

# ...or let a browser login mint one (OpenRouter OAuth PKCE; stored as a plain API key)
x19 auth add openrouter --type oauth

# Add a second Anthropic key
x19 auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
x19 auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  x19_pkce          oauth   id=ab12cd34 priority=0 x19_pkce ←
...
`x19 auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `x19 auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `x19 auth` | Interactive pool management wizard |
| `x19 auth list` | Show all pools and credentials |
| `x19 auth list <provider>` | Show a specific provider's pool |
| `x19 auth add <provider>` | Add a credential (prompts for type and key) |
| `x19 auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `x19 auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `x19 auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `x19 auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `x19 auth remove <provider> <index>` | Remove credential by 1-based index |
| `x19 auth reset <provider>` | Clear all cooldowns/exhaustion status |
| `x19 auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `x19 auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |
...
cooldowns are preserved. Reauthenticate with `x19 auth add nous --type oauth`
...
Configure via `x19 auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`x19 auth priority` changes it) |
...
command (`x19 auth add <provider>`), and the credential leaves rotation — marked `dead`, or dropped
...
When you set up a custom endpoint via `x19 model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via x19 model:
x19 auth list
# Shows:
...
x19 auth add Together.ai --api-key sk-together-second-key
```
...
X19 automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| X19 PKCE OAuth | `~/.x19/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `x19 auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `x19 auth add`) are never auto-pruned.
...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. X19 can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and X19-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`x19_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`x19_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.x19/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to X19' credential store, so its token remains persistable.

An `env:` row is re-hydrated from the environment on every load, and the variable name does not have to be one X19 declares for the provider: numbered siblings (`OPENROUTER_API_KEY_2`, see [Auto-Discovery](#auto-discovery)) appear here automatically, and a hand-written row pointing at any other variable is filled the same way, without the secret ever being written to `auth.json`.
