...
X19 can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<X19_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), X19 renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `x19 pets list [query] [--limit N]` |
| List installed pets | `x19 pets list --installed` |
| Install a pet | `x19 pets install <slug> [--select] [--force]` |
| Set the active pet | `x19 pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `x19 pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `x19 pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `x19 pets off` |
| Remove an installed pet | `x19 pets remove <slug>` |
| Diagnose setup | `x19 pets doctor` |
...
Beyond installing pre-made pets from the gallery, X19 can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `x19 pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `x19 tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `X19_PET_IMAGE_PROVIDER` env var (e.g. `X19_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while X19 is
...
Run `x19 pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; X19 uses its own
  profile-scoped `<X19_HOME>/pets/` instead — install through `x19 pets`.

## See also

- The [`x19` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-x19.md)
