...
When X19 runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.x19/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.x19/config.yaml` (switchable per session
via `/personality`).
...
If you use X19' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.x19/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. X19's deliverable
