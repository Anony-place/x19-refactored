...
description: "Durable SQLite-backed task board for coordinating multiple X19 profiles"
---
...
X19 Kanban is a durable task board, shared across all your X19 profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.x19/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
The board has two front doors, both backed by the same `~/.x19/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `x19 kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `x19 kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), reaps workers that outlived their finished run (a worker still alive after its own `kanban_complete`/`kanban_block` is terminated once its run has been closed for two minutes, leaving it time to finish its final turn — matched by PID *and* spawn-time fingerprint, so a recycled PID is never signalled; recorded as a `terminal_worker_reaped` event), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `X19_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
x19 kanban boards list

# Create a new board.
x19 kanban boards create atm10-server \
    --name "ATM10 Server" \
...
x19 kanban --board atm10-server list
x19 kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
x19 kanban boards switch atm10-server
x19 kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
x19 kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `X19_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.x19/kanban/current` — the slug persisted by `x19 kanban
   boards switch`.
4. `default`.
...
`x19 dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<x19-home>/kanban/attachments/<task_id>/` for the default board, or
  `<x19-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `X19_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
x19 kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `x19 kanban show t_abcd`.
...
Override the config flag at runtime via `X19_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `x19 gateway
...
until one comes up — `x19 kanban create` warns about this at creation
time.

Running `x19 kanban daemon` as a separate process is **deprecated**;
...
Mounting one `kanban.db` in several X19 homes (containers, fleet hosts) shares
...
x19 kanban create "nightly ops review" \
    --assignee ops \
...
```bash
x19 kanban complete t_abc t_def t_hij --result "batch wrap"
x19 kanban archive  t_abc t_def t_hij
x19 kanban unblock  t_abc t_def
x19 kanban block    t_abc "need input" --ids t_def t_hij
```
...
```bash
x19 -p planner tools enable kanban                      # CLI / TUI / Desktop chats
x19 -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `x19 tools` and the dashboard.
...
**Workers do not shell out to `x19 kanban`.** When the dispatcher spawns a worker it sets `X19_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `x19 kanban` CLI.
...
kanban_show()                                     # no args — uses X19_KANBAN_TASK
...
### Why tools instead of shelling to `x19 kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `x19 kanban complete` *inside* the container, where `x19` isn't installed and `~/.x19/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.x19/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `x19 chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `X19_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.

The auto-injected kanban guidance teaches the model which tool to call when and in what order. It is injected only for the dispatcher-spawned worker that owns the task: an interactive session that merely has the `kanban` toolset enabled keeps the tools but is not told it "has been assigned ONE task", and a `delegate_task` child or a cron run fired from inside a worker — which inherit the worker's `X19_KANBAN_TASK` — neither receives the worker protocol nor records a terminal outcome against the worker's card when its own turn budget runs out.
...
```json
{
  ...
  "verification": ["pytest tests/x19_cli/test_kanban_db.py -q"],
...
2. `cd $X19_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, X19 injects up to two
...
guard is active only for the dispatcher-spawned worker itself (`X19_KANBAN_TASK` is
...
and can be disabled with `X19_KANBAN_STOP_NUDGE=0`.
...
**Worker session names.** A worker's session is titled after its card (`Fix the swap modal`, or `Kanban task <id>` when the board row can't be read) at spawn, so `x19 sessions` and session search show the card, not a model's guess. Workers never make the auxiliary `title_generation` model call that names interactive sessions; a manual `/title` in a worker session still wins.
...
```bash
x19 kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

x19 kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `x19 skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
x19 kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
x19 kanban set-model t_abcd claude-opus-4.6 --provider anthropic
x19 kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.x19/profiles/<name>/`, and the dispatcher injects the profile-scoped `X19_HOME` when it spawns `x19 -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.x19/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.x19/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.x19/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `x19 kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
x19 kanban create "Translate the docs site to French" \
...
A goal-mode worker's **Worker log** (dashboard drawer, `x19 kanban log <id>`) carries the same live tool feed as any other worker's, plus one `kanban goal loop: turn N/M verdict=…` line per judged turn, so you can follow what the loop is doing while the card is running.
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `x19 kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `x19 kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `x19 profile create --description "..."`, `x19 profile describe <name> --text "..."`, `x19 profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.x19/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `x19 profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.x19/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `x19 dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.x19/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `x19 -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard X19 dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
x19 kanban init                                     # create kanban.db + print daemon hint
x19 kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
x19 kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
x19 kanban show <id> [--json]
x19 kanban assign <id> <profile>                    # or 'none' to unassign
x19 kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
x19 kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
x19 kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
x19 kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
x19 kanban diagnostics [--json]                     # board health snapshot (alias: diag)
x19 kanban link <parent_id> <child_id>
x19 kanban unlink <parent_id> <child_id>
x19 kanban claim <id> [--ttl SECONDS]
x19 kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
x19 kanban complete <id>... [--result "..."] [--force]
x19 kanban block <id> "<reason>" [--ids <id>...]
x19 kanban unblock <id>...
x19 kanban archive <id>...

x19 kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
x19 kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
x19 kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

x19 kanban tail <id>                                # follow a single task's event stream
x19 kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
x19 kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
x19 kanban runs <id> [--json]                       # attempt history (one row per run)
x19 kanban assignees [--json]                       # profiles on disk + per-assignee task counts
x19 kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
x19 kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `x19 gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
x19 kanban stats [--json]                           # per-status + per-assignee counts
x19 kanban log <id> [--tail BYTES]                  # worker log from ~/.x19/kanban/logs/
x19 kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
x19 kanban notify-list [<id>] [--json]
x19 kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
x19 kanban context <id>                             # what a worker sees
x19 kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
x19 kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
x19 kanban create "nightly backup audit" \
...
To see why a ready card is not spawning, run `x19 kanban dispatch --dry-run` — the output lists `Guarded (<reason>): <task id>` per held card (and `respawn_guarded`, `rate_limited`, `skipped_locked`, `memory_pressure` with `--json`). The gateway's and the standalone daemon's "dispatcher stuck" warning also names what the last tick held back, e.g. `Last tick held back: active_pr=1`.

`recent_success` and `active_pr` hold the **ready** lane only — they are the inputs to a review handoff, not signals against one. To have a reviewer, closer or other recovery profile pick up a card whose PR is already open, either move it to the review lane with `x19 kanban request-review <id>` (accepted from `ready` as well as `running`; the review-lane spawn is not subject to either guard) or hand the ready card to that profile with `x19 kanban assign <id> <profile>`: a handoff recorded *after* the PR comment — an operator reassign, a reviewer's changes-requested verdict, or a review reopen — lifts `active_pr` for the profile now named on the card, because that PR is exactly what it must work on. Only a change to a *different* profile counts: re-assigning the same profile, unassigning, or the dispatcher's own `kanban.default_assignee` fill-in does not lift the guard, so the assignee that opened the PR is still not re-spawned against it after a crash, reclaim or no-op reassign, and a newer PR comment posted after the handoff guards again. A deliberate re-queue after a success (drag `done→ready`, `unblock`, re-promotion) also lifts `recent_success`, so a manual re-run is never silently held for the whole window.
...
```bash
x19 kanban swarm "Design a multi-region failover plan" \
...
Every `x19 kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `x19 chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `x19_cli.kanban.run_slash()` entry point that reuses the `x19 kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `x19 kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.x19/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Dispatcher workers creating tasks through `kanban_create` or `x19 kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`x19 kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `x19 kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
_metadata_: `{"changed_files": ["x19_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
x19 kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
[`agent-merge-conflict-arbiter` optional skill](https://github.com/Anony-place/x19-refactored/blob/main/optional-skills/autonomous-ai-agents/agent-merge-conflict-arbiter/SKILL.md)
...
```
hotspot: x19_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
x19 kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
x19 kanban notify-list
x19 kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
x19 kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
x19 kanban runs t_abcd
...
**Bulk close caveat.** `x19 kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.

**Live-claim guard on complete.** A `running` task whose worker holds a live claim is only completed by that worker (`kanban_complete` from inside the run) or by an explicit operator override: `x19 kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `x19 kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `x19 kanban reclaim`, so a second session can no longer close a live worker's run underneath it. Completing `ready`, `blocked` or `review` cards without a claim is unchanged.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`x19 kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `x19 kanban heartbeat $TASK` to signal liveness during long operations. |
...
`x19 kanban tail <id>` shows these for a single task. `x19 kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.x19/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
