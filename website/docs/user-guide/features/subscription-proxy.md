...
chat completions — use your X19-managed provider subscription as their
...
| Use case | "Use X19 as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. X19 stores
the refresh token in `~/.x19/auth.json` — the same place all X19
...
interface in `x19_cli/proxy/adapters/`.
...
If you see `not logged in`, run `x19 portal`. If you see
...
`x19_cli/proxy/adapters/<provider>.py` and registering it in
