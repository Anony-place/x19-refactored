...
The Tool Gateway is included with every paid [Nous Portal](https://portal.nousresearch.com) subscription. It routes X19' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let X19 default to FLUX 2 Klein. |
...
```bash
x19 setup --portal     # Fresh install: Nous OAuth + set Nous as provider + turn on the Tool Gateway in one go
```

```bash
x19 model              # Switch your inference provider to Nous Portal — X19 then offers to turn on the gateway for all tools
```

```bash
x19 tools              # Enable the gateway per-tool — pick "Nous Subscription" for any tool you want
```

`x19 setup --portal` and `x19 model` are the all-at-once paths: log in once, optionally flip every tool to the gateway. `x19 tools` is the à la carte path — turn on just the tools you want, one at a time.

**You don't have to log in first.** With `x19 tools`, the Nous-managed backends (Web search, Image, Video, TTS, Browser) are always listed, even if you've never signed into Nous Portal. Select one and X19 runs the Portal login right there if you aren't already authenticated — no need to run `x19 model` beforehand. If your Nous OAuth is already active, selecting the backend enables it immediately with no extra prompt. This path only logs you in and turns on the one tool you picked — it does **not** switch your inference provider, and it does **not** prompt you to enable the gateway for every other tool.

Check what's active at any time:

```bash
x19 portal info        # Portal auth + Tool Gateway routing summary
x19 portal tools       # Gateway catalog with current routing per tool
x19 status             # Full system status (Tool Gateway is one section)
```

`x19 portal info` shows a section like:
...
Picking a Nous model (`x19 model`) offers a per-tool checklist of gateway backends. Its behavior respects your existing setup:
...
```bash
x19 tools          # Interactive picker for each tool category
```

Select the tool, pick **Nous Subscription** as the provider (or any direct provider you prefer). No config editing required. If you aren't logged into Nous Portal yet, picking **Nous Subscription** kicks off the Portal login inline — you don't need to authenticate through `x19 model` first.
...
The set evolves — `x19 tools` → Image Generation shows the current live list.
...
```
image_gen is configured to use fal (set via x19 tools), but FAL_KEY is not set. Run 'x19 tools' to change it.
```

Categories you have **never configured** (no selection key ever written) autodetect from available credentials, same as before. But once a selection exists, adding a key to `.env` does not change the route — only `x19 tools` (or editing the selection key) does.

### Switching back to your own keys

```bash
x19 tools    # pick the tool → choose a direct provider (e.g. Firecrawl)
```
...
```yaml
web:
  backend: firecrawl   # X19 now uses FIRECRAWL_API_KEY from .env
```

### Legacy `use_gateway` flag (deprecated)

Older X19 versions used a per-tool `use_gateway: true` boolean to route through the gateway. That flag is **legacy**: it is never written anymore, and the `x19 tools` picker removes it from a category's config when it rewrites the selection. Old configs that still contain `use_gateway: true` are interpreted at read time as the `nous` selection, so existing setups keep working. Don't set `use_gateway` in new configs — select the provider in `x19 tools` instead.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.x19/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `x19 tools`. X19 shows a clear error pointing at the portal.
...
Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `x19 setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. While a tool's selection is **Nous Subscription**, direct keys for that tool are simply ignored. Pick the direct provider again in `x19 tools` and your keys become the source again. The gateway isn't a lock-in.
