...
MoA presets are selectable on **every X19 surface**, because MoA is a normal provider in the model system:
...
- **`x19 model`** and the **Dashboard model picker** — a `Mixture of Agents` provider row appears with your preset names as its models.
...
X19 temporarily switches to the default MoA preset for that one turn, sends the prompt, then restores your previous model afterward. The whole argument is the prompt — `/moa` no longer interprets it as a preset name.
...
For each main model call when provider `moa` is selected, X19:

1. resolves the selected preset by name;
2. runs the configured reference models without tool schemas (they receive only the conversation's user/assistant text — not the X19 system prompt or tool-call transcript — so reference calls stay cheap and avoid strict-provider rejections);
3. appends the reference outputs as private context for the aggregator;
4. calls the configured aggregator with the normal X19 tool schema;
5. treats the aggregator response as the real model response;
6. if the aggregator calls tools, X19 executes those tools normally;
...
moa:
  ...
  presets:
    default:
      ...
      # the same behavior as a single-model X19.
...
receive an internal value from X19.
...
strings) are masked by X19' central secret redactor; the MoA filter adds
...
X19' normal reasoning controls: `none`, `minimal`, `low`, `medium`, `high`,
`xhigh`, `max`, and `ultra`.
...
Omit `reasoning_effort` to use the provider/X19 default for that slot.

## Terminal preset management

```bash
x19 moa list
x19 moa configure              # update the default preset
x19 moa configure review       # create or update a named preset
x19 moa delete review
```

`x19 moa list` marks the aggregator as the acting model that carries almost all of the cost and lists references as advising once per user turn (by default). When the aggregator's provider differs from your main `model.provider`, both `list` and `configure` add:
...
On X19Bench, a two-model MoA preset — `claude-opus-4.8` aggregating over a `gpt-5.5` reference — outscores either model run on its own:

| Model | X19Bench score |
|---|---|
...
So MoA does not sacrifice prompt caching on either call type. Its only real cost is the extra reference calls (once per user turn with the default `fanout`) — you pay for multiple model perspectives, not for broken caches. The long-lived conversation prefix shared with the rest of X19 is fully intact.

## Notes

- MoA is no longer listed under `x19 tools`; there is no `moa` toolset to enable.
