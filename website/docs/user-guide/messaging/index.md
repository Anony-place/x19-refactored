...
description: "Chat with X19 from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with X19 from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with X19](/guides/use-voice-mode-with-x19).
...
saved by `x19 gateway setup` can enable a credential-based platform without a
...
For group chats, hooks, and automation flows, X19 supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. X19 keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; X19 does not hide failures just because the text resembles a silence token.
...
```bash
x19 gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
x19 gateway              # Run in foreground
x19 gateway setup        # Configure messaging platforms interactively
x19 gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo x19 gateway install --system   # Linux only: install a boot-time system service
x19 gateway start        # Start the default service
x19 gateway stop         # Stop the default service
x19 gateway status       # Check default service status
x19 gateway status --system         # Linux only: inspect the system service explicitly
```

### Stack dump on demand (`SIGUSR2`)

On Linux and macOS, `kill -USR2 <gateway pid>` appends a dump of every thread's
stack to `~/.x19/logs/gateway_faulthandler.log` and the gateway keeps
...
```bash
x19 gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. X19 sends heartbeats
...
| `/update` | Update X19 to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.x19/config.yaml`:
...
x19 pairing approve telegram XKGH5N7P

# Other pairing commands:
x19 pairing list          # View pending + approved users
x19 pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, X19 appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.x19/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.x19/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `X19_HOME`:

- `~/.x19/status_phrases.yaml` or any `*.yaml` in `~/.x19/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to X19_HOME
...
Off by default. When enabled, X19 prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.x19/config.yaml`:
...
```bash
X19_BACKGROUND_NOTIFICATIONS=result
```
...
x19 gateway start                 # Start the service
x19 gateway stop                  # Stop the service
x19 gateway status                # Check status
journalctl --user -u x19-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo x19 gateway install --system
sudo x19 gateway start --system
sudo x19 gateway status --system
journalctl -u x19-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit X19 installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit x19-gateway` (or `sudo systemctl edit x19-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `x19 update`. When `x19 update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart x19-gateway` command (it never blocks on an interactive password prompt).
...
```bash
x19 gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `x19 update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo x19 update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/x19-gateway`:

```
x19 ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed x19-gateway*, /usr/bin/systemctl --no-ask-password start x19-gateway*, /usr/bin/systemctl --no-ask-password restart x19-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. X19 will warn if it detects both because start/stop/status behavior gets ambiguous.

:::note Inside a container, only the system scope is offered
`x19 gateway install` (and the `x19 gateway setup` wizard) refuse to install a **user** service when X19 detects it is running inside a container. A user unit lands in `~/.config/systemd/user`, and when that home is bind-mounted from the host (podman/distrobox), the host's own `systemd --user` enables and starts the same unit — a second gateway polling the same bot token. Run the gateway as the container's main process (`x19 gateway run`, with a container restart policy), or in a systemd container (systemd as PID 1) install the isolated system scope: `sudo x19 gateway install --system --run-as-user <user>`.
:::

:::info Multiple installations
If you run multiple X19 installations on the same machine (with different `X19_HOME` directories), each gets its own systemd service name. The default `~/.x19` uses `x19-gateway`; other installations use `x19-gateway-<hash>`. The `x19 gateway` commands automatically target the correct service for your current `X19_HOME`.
:::

### macOS (launchd)

```bash
x19 gateway install               # Install as launchd agent
x19 gateway start                 # Start the service
x19 gateway stop                  # Stop the service
x19 gateway status                # Check status
tail -f ~/.x19/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.x19.gateway.plist`. It includes three environment variables:
...
- **X19_HOME** — scopes the gateway to your X19 installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `x19 gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `X19_HOME` directory gets its own launchd label. The default `~/.x19` uses `ai.x19.gateway`; other installations use `ai.x19.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `x19-cli` | Full access |
| Telegram | `x19-telegram` | Full tools including terminal |
| Discord | `x19-discord` | Full tools including terminal |
| WhatsApp | `x19-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `x19-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `x19-slack` | Full tools including terminal |
| Google Chat | `x19-google_chat` | Full tools including terminal |
| Signal | `x19-signal` | Full tools including terminal |
| SMS | `x19-sms` | Full tools including terminal |
| Email | `x19-email` | Full tools including terminal |
| Home Assistant | `x19-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `x19-mattermost` | Full tools including terminal |
| Matrix | `x19-matrix` | Full tools including terminal |
| DingTalk | `x19-dingtalk` | Full tools including terminal |
| Feishu/Lark | `x19-feishu` | Full tools including terminal |
| WeCom | `x19-wecom` | Full tools including terminal |
| WeCom Callback | `x19-wecom-callback` | Full tools including terminal |
| Weixin | `x19-weixin` | Full tools including terminal |
| BlueBubbles | `x19-bluebubbles` | Full tools including terminal |
| QQBot | `x19-qqbot` | Full tools including terminal |
| Yuanbao | `x19-yuanbao` | Full tools including terminal |
| Microsoft Teams | `x19-teams` | Full tools including terminal |
| API Server | `x19-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `x19-webhook` | Full tools including terminal |
| Raft | `x19-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.x19/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.x19/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
