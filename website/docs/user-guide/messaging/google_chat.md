...
sidebar_position: 12
title: "Google Chat"
description: "Set up X19 as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect X19 to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your X19
...
> Run `x19 gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only X19 can read (e.g.,
`~/.x19/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a x19 restart)
...
- Principal: `x19-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — X19 calls
...
- **App name**: whatever you want users to see ("X19" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/x19-chat-events`.
...
`ADDED_TO_SPACE` event that X19 uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure X19

Add the Google Chat section to `~/.x19/.env`:
...
On Docker / hosted images `/opt/x19/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `X19_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "X19 is thinking…" marker, then
...
`~/.x19/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "X19 is thinking…").
...
Thread support: when a user replies inside a thread, X19 detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate X19 session.
...
3. Download the JSON. Move it onto the host that runs X19.
4. Register the client with X19 (run under the profile you want it scoped to):
...
x19 -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's X19 home (e.g.
`~/.x19/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.x19/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.x19/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, X19 isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `x19 gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: X19 will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.x19/google_chat_user_tokens/<sanitized_email>.json` (filesystem
