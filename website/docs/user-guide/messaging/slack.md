...
sidebar_position: 4
title: "Slack"
description: "Set up X19 as a Slack bot using Socket Mode"
---

# Slack Setup

Connect X19 to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your X19 instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. X19 uses the modern
...
The fastest path is to paste a manifest X19 generates for you. It
...
### Option A: From an X19-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   x19 slack manifest --agent-view --write
   ```
   This writes `~/.x19/slack-manifest.json` and prints paste-in
...
4. Enter an app name (e.g., "X19") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, X19 can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `x19-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not an X19 configuration issue.
:::
...
X19 uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
x19 gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
x19 gateway              # Foreground
x19 gateway install      # Install as a user service
sudo x19 gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; X19 now
...
Every X19 command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
X19 command with its description.

Under the hood: X19 ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Anony-place/x19-refactored/blob/main/x19_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing X19
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so X19 can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. X19 only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When X19 adds new commands (e.g. after `x19 update`), regenerate
the manifest and update your Slack app:

```bash
x19 slack manifest --write
```
...
3. Paste the new contents of `~/.x19/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/x19 <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/x19 bg run the tests` — X19 routes it the same way as `/bg
run the tests`. Free-form questions also work: `/x19 what's the
...
Slack never delivers them to X19.

As a workaround, X19 recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
X19 treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@X19 !stop`) and with
...
X19 falls back to a text prompt, the prompt instructs you to reply
...
fails, X19 retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@X19 /cmd` — reply as normal visible
messages instead.)
...
```bash
x19 slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how X19 behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@X19 what time is it?`). In channels, X19 replies in a thread attached to that message. |
| **Threads** | If you @mention X19 inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.x19/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions X19, and "all" accepts every
      # other bot. X19 always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.native_task_cards` | `false` | When `true`, renders live tool calls as Slack-native plan/task cards. Cards work with Slack's built-in default `tool_progress: off`; an explicitly configured `display.tool_progress: off` (global or `display.platforms.slack`; `/verbose` writes the same key) disables cards too. Cards need a thread: when the card lane is active and the chat has no thread to anchor on (a top-level DM with `reply_in_thread: false`), X19 shows no tool progress instead of text bubbles, unless you explicitly set `tool_progress: new`/`all`, which falls back to editable text progress there. Recoverable native API failures fall back to one continuously edited text update. |
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions X19, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default X19 sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"X19 is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), X19 uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — X19 seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and X19 falls back
...
- X19 checks thread eligibility before attempting publication, so a
...
- On a supported threaded destination, if the native stream fails for a
  recoverable reason (API error, rate limit), X19 falls back to a single continuously edited text message so
...
- If Slack closes a stream during a long turn, X19 opens a fresh card in
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to X19 in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before X19 will respond.
...
By default X19 ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several X19 instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- X19 always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer X19s are filtered consistently across workspaces.
...
`client_msg_id` — the same signature X19 uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to X19 *as*
the logged-in user.
...
manifest — regenerate with `x19 slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where X19 will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @X19`).
...
Delivery works even when the cron process isn't co-located with the gateway — X19 falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
X19 can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, X19 also loads tokens from an **OAuth token file** at:

```
~/.x19/slack_tokens.json
```
...
- When a message arrives, X19 uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @X19` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @X19` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. X19 now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @X19`)
...
- Tokens should be stored in `~/.x19/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your X19 config directory
