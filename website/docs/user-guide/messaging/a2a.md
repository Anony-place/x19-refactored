...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The X19 A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your X19 over HTTP.

It interoperates with any A2A-compliant peer — another X19, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.
...
```bash
x19 tools enable a2a --platform cli        # CLI/TUI sessions
x19 tools enable a2a --platform telegram   # or any messaging platform
x19 tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, X19 serves:
...
- **Audit log** — every exchange appends to `~/.x19/a2a_audit.jsonl`.
