...
Connect X19 to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
X19 supports two WeCom integration modes:
...
> Run `x19 gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. X19 decrypts the message, queues it for the agent
...
(Use `x19 gateway start` only after `x19 gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in X19 and the token the
...
in `~/.x19/.env` values around `=` will also break signature checks. After
fixing, restart `x19 gateway run`.
...
Check `x19 gateway run` logs for the bound host/port. If the adapter bound to
