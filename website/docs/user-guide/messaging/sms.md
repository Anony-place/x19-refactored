...
title: "SMS (Twilio)"
description: "Set up X19 as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

X19 connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `cd ~/.x19/x19 && uv pip install -e ".[sms]"`
...
If you're running X19 locally, use a tunnel to expose the webhook:
...
Text your Twilio number — X19 will respond via SMS.
...
X19 validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check X19 gateway logs for Twilio API errors
