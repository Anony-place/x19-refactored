...
ntfy makes a great lightweight push channel for X19: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `x19 gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `x19-myname-2026` works fine)
...
Add these to `~/.x19/.env`:

```
NTFY_TOPIC=x19-myname-2026
NTFY_ALLOWED_USERS=x19-myname-2026
NTFY_HOME_CHANNEL=x19-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The X19 adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
- Or **pick a long, unguessable topic name** (`x19-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `x19-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `x19-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=x19-myname-2026' >> ~/.x19/.env
   echo 'NTFY_ALLOWED_USERS=x19-myname-2026' >> ~/.x19/.env
   x19 gateway restart
   ```
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`x19 send` CLI](/guides/pipe-script-output):

```bash
x19 send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing X19 replies:
