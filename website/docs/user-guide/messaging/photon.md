...
Connect X19 to **iMessage** through [Photon][photon], a managed
...
your first iMessage from X19 — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, X19 runs it in a
...
x19 photon setup --phone +15551234567
```
...
2. **Finds or creates** the `X19` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.x19/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.x19/.env`
...
(device token, dashboard project id) lives in `~/.x19/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other X19
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, X19 replies with a pairing code. Approve it with:

```bash
x19 pairing approve photon <CODE>
```

Use `x19 pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.x19/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.x19/.env`):
...
By default X19 responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `X19` and
`@X19` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and X19 will reply.
...
assigned iMessage line X19 uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run x19 photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `x19 photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `x19 photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `x19 photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** X19 sends images, voice
  ...
  media.
- **Native polls are supported.** X19 sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to X19, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for X19-sent messages
...
- **Message effects are supported.** X19 sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `x19 send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<x19-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when X19 is set
...
| `PHOTON_MENTION_PATTERNS` | X19 wake words  | JSON list / comma / newline regex patterns for group mentions |
