...
sidebar_position: 30
title: "X19 Relay"
description: "Connect X19 to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---
...
X19 Relay is not a chat platform itself — it is a **connector system** that
...
secret. `x19 gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.x19/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.x19/.env`:
...
`x19 gateway enroll` refuses to run in managed/hosted installs — there the
...
`x19 gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.x19/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `x19 auth add nous` (or `x19 setup`) and retry.
...
startup. Restart it (`x19 gateway restart`).
