...
sidebar_position: 16
title: "Yuanbao"
description: "Connect X19 to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect X19 to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.x19/.env`:
...
All standard X19 commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.x19/logs/gateway.log`
...
4. Consider enabling verbose logging: `x19 gateway run -vv`
...
Yuanbao has a maximum message size. X19 automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
x19 chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
