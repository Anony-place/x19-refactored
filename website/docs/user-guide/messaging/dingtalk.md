...
sidebar_position: 10
title: "DingTalk"
description: "Set up X19 as a DingTalk chatbot"
---
...
Before setup, here's the part most people want to know: how X19 behaves once it's in your DingTalk workspace.

## How X19 Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | X19 responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | X19 responds when you `@mention` it. Without a mention, X19 ignores the message. |
| **Shared groups with multiple users** | By default, X19 isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
```bash
cd ~/.x19/x19 && uv pip install -e ".[dingtalk]"
```
...
X19 uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
- **QR-code device flow (recommended).** Scan the QR that prints in your terminal with the DingTalk mobile app — your Client ID and Client Secret are returned automatically and written to `~/.x19/.env`. No developer-console trip needed.
...
X19 can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
X19 automatically adds emoji reactions to your messages to show processing status:
...
**Fix**: Verify `DINGTALK_CLIENT_ID` and `DINGTALK_CLIENT_SECRET` are set correctly in `~/.x19/.env`. The Client ID is your AppKey, and the Client Secret is your AppSecret from the DingTalk Developer Console.
...
**Fix**: Check that `x19 gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
