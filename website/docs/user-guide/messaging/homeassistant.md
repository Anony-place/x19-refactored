...
description: Control your smart home with X19 via Home Assistant integration.
...
X19 integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "X19"
5. Copy the token
...
X19 registers four tools for smart home control:
...
Configure which events the agent sees in `~/.x19/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "X19".
...
The adapter reads credentials from `~/.x19/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active X19 profile
...
running X19 — `curl -H "Authorization: Bearer <token>" <url>/api/` should
