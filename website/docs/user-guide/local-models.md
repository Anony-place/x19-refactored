...
X19 can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; X19 does the rest.
...
2. Click **Install runtime**. X19 downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with X19, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How X19 picks what to download
...
Models ship in several quality grades (quantizations). X19 picks the
...
guarantees. Below 4-bit the quality loss is too severe, so X19 never
...
Local models live or die by memory placement, so X19 manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, X19 deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), X19 uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  X19 compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — X19
...
If a llama-server is already running on your machine, X19 detects it
...
[Run X19 Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with X19.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; X19 updates it with
                     # each release after re-validation
```

Models and runtime builds live under the X19 home directory
