...
X19 can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
x19 chat --checkpoints
```

Or enable globally in `~/.x19/config.yaml`:
...
| Command | Description |
|---------|-------------|
| `x19 checkpoints` | Show total size, project count, per-project breakdown |
| `x19 checkpoints status` | Same as bare `checkpoints` |
| `x19 checkpoints list` | Alias for `status` |
| `x19 checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `x19 checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `x19 checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- X19 detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.x19/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/x19/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.x19/checkpoints/ in the
  ...
  # cleanup is only ever done via the explicit `x19 checkpoints prune`
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `x19 checkpoints prune` manually.
...
X19 responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.x19/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/x19                                        20       2h ago  live
...
Clear with: x19 checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
x19 checkpoints prune --retention-days 3 --max-size-mb 200
```
...
`/rollback <N>` restores only the files X19 itself changed. Every successful
...
X19 last wrote (you edited it afterwards, or X19 never touched it) is
...
If the ledger is empty (a store created before this feature, or X19 hasn't
...
- **Directory scope** — X19 skips overly broad directories (root `/`, home `$HOME`).
...
~/.x19/checkpoints/
  ...
  │   ├── refs/x19/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `x19 checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.x19/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.x19/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
x19 checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `x19 chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `x19 checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each X19 session in its own worktree/branch, with checkpoints as an extra layer.
