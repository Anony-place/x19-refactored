...
`~/.x19/config.yaml`, `~/.x19/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/x19`:

```text
/etc/x19/
├── config.yaml     # managed config layer (wins over ~/.x19/config.yaml)
└── .env            # managed env layer (wins over ~/.x19/.env + shell)
```
...
The location can be relocated with the `X19_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `X19_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by X19.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export X19_MANAGED_DIR=/opt/org/x19-policy
```

:::warning
A user who can set `X19_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `x19 doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/x19/config.yaml` (managed) | `/etc/x19/.env` (managed) |
| 2 | `~/.x19/config.yaml` (user) | `~/.x19/.env` (user) |
...
```bash
x19 config        # shows a header naming the managed source + the pinned keys
x19 doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, X19 refuses and names the source:

```bash
$ x19 config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/x19/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `x19 config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/x19

# Pin some config values for every user on this machine
sudo tee /etc/x19/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/x19/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/x19
sudo chmod 0644 /etc/x19/config.yaml /etc/x19/.env
```

Changes take effect on the next X19 start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`x19 doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs X19 as `root`), managed scope is advisory.
