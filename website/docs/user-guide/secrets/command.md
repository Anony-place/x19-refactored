...
secrets:
  command:
    enabled: true
    command: "cat /run/user/1000/x19-secrets.env"
    # or any vault CLI that dumps KEY=VALUE lines:
    # command: "pass show x19/env"
    # command: "secret-tool lookup service x19-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach X19' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (X19 discards its stderr on purpose) |
