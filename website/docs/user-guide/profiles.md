...
Run multiple independent X19s on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate X19 home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. X19 recognises a directory under `~/.x19/profiles/` as a profile only when it carries one of those identity files (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json`, `state.db`); a bare directory left behind by logging or cron is ignored by `profile list`, gateways and `-p`. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up X19 state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same X19 home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
These terms describe different parts of X19:
...
- **Agent** is the running X19 assistant that uses that configuration and
  state. "X19" also names the product.
...
- **Messaging bot** is an account on a platform such as Telegram, Discord, or
  Slack, connected to X19 through the gateway. Its
...
```bash
x19 profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own X19 profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `x19 setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
x19 profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `x19 profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies **everything** — config, API keys, personality, all memories, skills, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. When cloning from the default profile, the local-model runtime trees (`models/`, `runtimes/`, `node/` — downloaded weights and managed binaries, re-fetched on demand) are skipped too, as `x19 backup` already does. **Cron jobs are not cloned** either: they are scheduled work bound to the source profile and its delivery channel, and a clone that inherited them would run every job twice (two gateways, same job ids). The new profile starts with an empty `cron/`. For a full backup including history and cron jobs, use `x19 profile export` or `x19 backup` instead.

:::note OAuth logins are never copied
Anthropic (Claude Pro/Max), OpenAI Codex, and xAI OAuth logins use **single-use refresh tokens** — a copy of one is not a second credential, it is the same credential with two owners, and the first profile to refresh it revokes it for every other copy. `--clone-all` (and the dashboard's credential mirroring) therefore drops those OAuth rows from the clone. Static API keys are copied as usual. Sign the new profile into the OAuth provider itself: `x19 -p <name> auth add <provider>` (or `x19 -p <name> model`).
:::

### Every profile owns its credentials

A named profile resolves providers from **its own** `auth.json` and `.env` only. It never inherits the root profile's logins or API keys, and a token refresh inside a profile never writes to the root store. A profile with no provider of its own is asked to set one up (`x19 -p <name> model`, or `x19 -p <name> auth add <provider>`) — it does not silently act as the owner. `x19 update` lists every profile that has no provider of its own so a bot never goes quiet unannounced.

### Clone from a specific profile

```bash
x19 profile create work --clone-from coder
```
...
```bash
x19 profile create work-backup --clone-from coder --clone-all
```
...
parks the duplicate adapter (and `x19 gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`x19 -p <name> setup` or the dashboard Messaging page.

```bash
x19 profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `x19 profile list` prints the same warning for any existing
...
The alias works with every x19 subcommand — it's just `x19 -p <name>` under the hood.
...
```bash
x19 -p coder chat
x19 --profile=coder doctor
x19 chat -p coder -q "hello"    # works in any position
```

### Sticky default (`x19 profile use`)

```bash
x19 profile use coder
x19 chat                   # now targets coder
x19 tools                  # configures coder's tools
x19 profile use default    # switch back
```

Sets a default so plain `x19` commands target that profile. Like `kubectl config use-context`.
...
- **`x19 profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives X19 its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory X19 was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.x19/profiles/coder/.env

# Edit assistant's tokens
nano ~/.x19/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates x19-gateway-coder systemd/launchd service
assistant gateway install     # creates x19-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `x19 profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `x19 -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.x19/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `x19 profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`x19 update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
x19 profile list           # show all profiles with status
x19 profile show coder     # detailed info for one profile
x19 profile rename coder dev-bot   # rename (updates alias + service)
x19 profile migrate-identity coder dev-bot   # retry a rename's identity migration
x19 profile purge-identity dev-bot   # retry a delete's identity purge
x19 profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
x19 profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.x19` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
x19 profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `x19 profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.x19/profile.yaml`; remove that line to
...
```bash
x19 profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `x19 profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.x19`). To remove everything, use `x19 uninstall`.
:::
...
```bash
# Bash
eval "$(x19 completion bash)"

# Zsh
eval "$(x19 completion zsh)"
```
...
Profiles use the `X19_HOME` environment variable. When you run `coder chat`, the wrapper script sets `X19_HOME=~/.x19/profiles/coder` before launching x19. Since 119+ files in the codebase resolve paths via `get_x19_home()`, X19 state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `X19_HOME`.
...
isolated by `X19_HOME`, not by changing `HOME`. Container backends still use
`{X19_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `X19_HOME` is the profile boundary. It controls X19 config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other X19
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, X19 keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode X19 launches tool
subprocesses with `HOME={X19_HOME}/home`; you then need to initialize or link
...
X19 also exposes `X19_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.x19` itself. No migration needed — existing installs work identically.
...
x19 profile export coder
x19 profile import ./coder.tar.gz --name coder
```
...
x19 profile install github.com/you/research-bot --alias

# Update later when the author ships a new version (keeps your memories + .env)
x19 profile update research-bot
```
