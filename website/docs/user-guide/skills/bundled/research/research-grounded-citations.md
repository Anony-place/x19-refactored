...
Ledger location: `$X19_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `X19_CITATION_LEDGER`.

## How to Run

```bash
S=~/.x19/skills/research/grounded-citations/scripts/sources.py
...
`x19 skills install official/social-media/reddit-reading` or
`x19 skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `X19_CITATION_LEDGER`) if their
