...
This skill is separate from the X19 Email gateway adapter. The gateway
adapter lets people email the agent and uses X19' built-in IMAP/SMTP
...
To reply non-interactively from X19, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from X19)** — pipe the message via stdin:
