...
| List/filter by metadata (age, source, cost, tokens, workspace) | `x19 sessions list` / `stats` via terminal |
| Rename | `x19 sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `x19 sessions archive <filters>` |
| Delete (destructive) | `x19 sessions delete` / `x19 sessions prune <filters>` |
| Export before deleting anything valuable | `x19 sessions export --session-id <id> --format md` |
...
`x19 sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `x19 sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `x19 sessions list`
