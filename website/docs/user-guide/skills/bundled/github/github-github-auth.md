...
- Give it a title like "x19-&lt;machine-name>"
...
uv run python3 "${X19_HOME:-$HOME/.x19}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _x19_env="${X19_HOME:-$HOME/.x19}/.env"; [ -f "$_x19_env" ] && grep -q "^GITHUB_TOKEN=" "$_x19_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_x19_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${X19_HOME:-$HOME/.x19}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
