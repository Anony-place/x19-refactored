...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/finance/dcf-model` |
...
**If you encounter errors or unreasonable results, read [TROUBLESHOOTING.md](https://github.com/Anony-place/x19-refactored/blob/main/optional-skills/finance\dcf-model/TROUBLESHOOTING.md) for detailed debugging guidance.**
...
3. **Check output**:
   ...
   - If `status` is `"errors_found"` → Check `error_summary` and read [TROUBLESHOOTING.md](https://github.com/Anony-place/x19-refactored/blob/main/optional-skills/finance\dcf-model/TROUBLESHOOTING.md) for debugging guidance
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In X19:

- **If you have any structured financial-data MCP configured** (X19 supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
