...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not an X19 core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with X19 native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer X19 native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer X19 native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended X19 usage patterns
...
- Do not silently choose Parallel when X19 native tools are already sufficient.
