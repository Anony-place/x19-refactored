...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by X19 |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during X19 terminal calls (desktop app flow only)
...
### Service Account (recommended for X19)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${X19_HOME:-~/.x19}/.env` (the skill will prompt for this on first load).
...
## X19 Execution Pattern (desktop app flow)

X19 terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/x19-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/x19-op.sock"
