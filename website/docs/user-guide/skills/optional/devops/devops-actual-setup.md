---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in X19"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in X19"
---
...
Set up Actual Computer (actual.inc) inference in X19.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `x19 skills install official/devops/actual-setup` |
...
- User has an `ac_` key and wants X19 routed through their Actual cluster.
...
- X19 has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current X19 — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.x19/.env`.
...
3. Select provider + model:
   ```bash
   x19 config set model.provider actual
   x19 config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   x19 chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point X19 at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.x19/.env`, then:
   ```bash
   x19 config set model.provider actual
   x19 config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   x19 chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by X19 since the first-class provider).**
   ...
   request still 400s this way on an old X19, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** X19' default toolset
   ...
   servers emit a bare `data: [DONE]` — X19 reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current X19 the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
x19 chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
x19 chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
