...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/autonomous-ai-agents/dynamic-workflow` |
...
| Related skills | [`x19`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-x19), [`simplify-code`](/docs/user-guide/skills/bundled/software-development/software-development-simplify-code) |
...
for serial chains. For a refactor or fix campaign on x19 itself, load
`x19` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `X19_HOME`,
