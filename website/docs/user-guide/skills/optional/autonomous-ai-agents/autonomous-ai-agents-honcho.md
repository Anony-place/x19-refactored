---
title: "Honcho — Configure and troubleshoot Honcho memory for X19"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for X19"
---
...
Configure and troubleshoot Honcho memory for X19.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `x19 skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for X19

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every X19 profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hermes#running-honcho-locally-with-x19

### Verify

```bash
x19 honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this X19 profile's AI peer
...
Honcho models conversations as interactions between **peers**. X19 creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this X19 instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each X19 run |
| `global` | Single session across all directories |

Manual override: `x19 honcho map my-project-name`
...
Each X19 profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block x19.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `x19.coder` host block in `honcho.json`
...
```bash
x19 honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for X19 when Honcho memory is active.
...
Config file: `$X19_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `x19 honcho setup`. Ensure `memory.provider: honcho` is in `~/.x19/config.yaml`.

### Memory not persisting across sessions
Check `x19 honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `x19 profile create <name> --clone`. For existing profiles: `x19 honcho sync`.
...
| Command | Description |
|---------|-------------|
| `x19 honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `x19 honcho status` | Show resolved config, connection test, peer info for active profile |
| `x19 honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `x19 honcho disable` | Disable Honcho for the active profile |
| `x19 honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `x19 honcho peers` | Show peer identities across all profiles |
| `x19 honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `x19 honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `x19 honcho sessions` | List known directory-to-session-name mappings |
| `x19 honcho map <name>` | Map current working directory to a Honcho session name |
| `x19 honcho identity` | Seed AI peer identity or show both peer representations |
| `x19 honcho sync` | Create host blocks for all X19 profiles that don't have one yet |
| `x19 honcho migrate` | Step-by-step migration guide from OpenClaw native memory to X19 + Honcho |
| `x19 memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `x19 memory status` | Show active memory provider and config |
| `x19 memory off` | Disable external memory provider |
