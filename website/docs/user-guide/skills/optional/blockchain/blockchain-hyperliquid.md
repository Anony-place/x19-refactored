...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), X19 |
| License | MIT |
...
The script reads `${X19_HOME:-~/.x19}/.env` for two optional defaults:
...
Helper script: `~/.x19/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.x19/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${X19_HOME:-~/.x19}/.env`.
...
```bash
python ~/.x19/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.x19/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
