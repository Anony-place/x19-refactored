...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the X19 MCP catalog. The
...
Two halves, in this order: the editor side must be up before X19 connects.
...
`~/.x19/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for X19 — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. X19 connects
...
2. Start the X19 session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through X19
they appear as:
...
then happens at `x19 mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — X19 runs batched calls
...
- **Start order matters.** Editor + server up first, then the X19
...
- **Timeouts on long operations.** X19' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.x19/config.yaml` for
