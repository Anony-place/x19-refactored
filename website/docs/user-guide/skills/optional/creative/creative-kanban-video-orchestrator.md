...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/creative/kanban-video-orchestrator` |
...
3. **Generates** a setup script that creates X19 profiles, project workspace, and the initial kanban task
...
For mapping role → which X19 skills + toolsets it loads, see
...
3. Creates each X19 profile via `x19 profile create --clone`
...
7. Fires the initial `x19 kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/Anony-place/x19-refactored/blob/main/optional-skills/creative\kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
x19 kanban watch --tenant <project-tenant>     # live events
x19 kanban list  --tenant <project-tenant>     # board snapshot
x19 dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/Anony-place/x19-refactored/blob/main/optional-skills/creative\kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/Anony-place/x19-refactored/blob/main/optional-skills/creative\kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `x19 kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${X19_HOME:-~/.x19}/.env` or the user's secret store.
