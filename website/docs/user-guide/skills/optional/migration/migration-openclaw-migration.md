---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into X19"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into X19"
---
...
Import an OpenClaw setup (memories, skills) into X19.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `x19 skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | X19 (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `X19`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> X19 Migration

Use this skill when a user wants to move their OpenClaw setup into X19 with minimal manual cleanup.
...
```bash
x19 claw migrate              # Full interactive migration
x19 claw migrate --dry-run    # Preview what would be migrated
x19 claw migrate --preset user-data   # Migrate without secrets
x19 claw migrate --overwrite  # Overwrite existing conflicts
x19 claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `x19 setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_x19.py` to:

- import `SOUL.md` into the X19 home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into X19 memory entries
- merge OpenClaw command approval patterns into X19 `command_allowlist`
- migrate X19-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to X19 working-directory configuration
- copy OpenClaw skills into `~/.x19/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen X19 workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.x19/tts/`
- archive non-secret docs that do not have a direct X19 destination
...
- `scripts/openclaw_to_x19.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py`

Do not guess a shorter path like `~/.x19/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.x19/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of X19-compatible secrets, currently:
...
X19 CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in X19. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into an X19 workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing X19 skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.x19/skills/migration/openclaw-migration/scripts/openclaw_to_x19.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of X19 unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty X19 targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean X19 destination. Unsupported auth blobs must still be reported as skipped.
...
- X19 persona state imported
- X19 memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.x19/skills/openclaw-imports/`
