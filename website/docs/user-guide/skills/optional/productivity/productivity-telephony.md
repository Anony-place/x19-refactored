...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/productivity/telephony` |
...
This optional skill gives X19 practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${X19_HOME:-~/.x19}/.env`
...
It does **not** turn X19 into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to X19 memory
...
### 1) "I want X19 to own a real phone number"
Use **Twilio**.
...
- pairs well with X19 `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.x19/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
x19 skills search telephony
x19 skills install official/productivity/telephony
```
...
Then save credentials into X19:
...
3. Buy it and save it into `${X19_HOME:-~/.x19}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is X19 calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing X19's existing `text_to_speech` support.

Use this when:
- you want the call to use X19's configured TTS voice rather than Twilio `<Say>`
...
Recommended X19 TTS -> Twilio Play workflow:

1. Generate the audio with X19 `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask X19 to create the message audio with `text_to_speech`
...
Important note:
- X19 TTS is great for prerecorded outbound messages
...
- X19 STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to X19 memory.
...
- Do not store arbitrary third-party phone numbers in X19 memory.
...
3. persist that number to `${X19_HOME:-~/.x19}/.env`
