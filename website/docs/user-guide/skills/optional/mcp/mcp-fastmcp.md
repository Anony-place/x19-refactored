...
| | |
|---|---|
| Source | Optional — install with `x19 skills install official/mcp/fastmcp` |
...
| Related skills | [`x19`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-x19), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into X19 or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to X19. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.x19/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.x19/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is X19 integration, either:

- configure the server in `~/.x19/config.yaml` using the `native-mcp` skill, or
...
### X19 cannot see the deployed server

The server-building part may be correct while the X19 config is not. Load the `native-mcp` skill and configure the server in `~/.x19/config.yaml`, then restart X19.
