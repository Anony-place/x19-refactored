...
# Connecting Desktop to Many X19 Instances

Register every X19 backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and X19 Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another X19 gateway…"**) deep-links straight to
...
list of X19 gateways. Its intro says it plainly: *"Manage this device and
every X19 gateway it can reach through remote, SSH, or Cloud connections."*
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the X19 Cloud sign-in/discovery flow at
  the top of the Gateways page — the **X19 Cloud** kind in the add-connection
...
The X19 Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
4. Fill the fields:
   ...
   - *SSH only:*
     ...
     - **X19 path (optional)** — full path to the `x19` executable on
       the remote (for example `/opt/x19/bin/x19`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `x19` on its `PATH` and **Test** reports *"X19 is not
...
Gateways are machines or hosted backends; profiles are isolated X19s
...
connection is registered) dispatches `x19 update` to every eligible
...
- **X19 Cloud** instances are skipped with a *"Managed by X19 Cloud"*
...
panel, ⌘K **Update X19**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `x19 serve` is running on the remote host, the
...
- **"Update X19 Desktop to chat with agents on other connections"** — the
