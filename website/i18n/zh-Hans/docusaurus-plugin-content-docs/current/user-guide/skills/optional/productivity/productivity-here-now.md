...
| | |
|---|---|
| 来源 | 可选 — 通过 `x19 skills install official/productivity/here-now` 安装 |
...
- Skill 辅助脚本路径：
  - `${X19_SKILL_DIR}/scripts/publish.sh` 用于发布站点
  - `${X19_SKILL_DIR}/scripts/drive.sh` 用于私有 Drive 存储

## 创建站点

```bash
PUBLISH="${X19_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client x19
```
...
```bash
PUBLISH="${X19_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client x19
```
...
```bash
DRIVE="${X19_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}` | 用于归因的 agent 名称（如 `x19`） |
