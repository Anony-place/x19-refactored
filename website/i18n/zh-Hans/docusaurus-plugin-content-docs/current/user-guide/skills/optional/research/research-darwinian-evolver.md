...
| | |
|---|---|
| 来源 | 可选 — 通过 `x19 skills install official/research/darwinian-evolver` 安装 |
| 路径 | `optional-skills/research/darwinian-evolver` |
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), X19 |
| 许可证 | MIT |
| 平台 | linux, macos |
...
```bash
mkdir -p ~/.x19/cache/darwinian-evolver && cd ~/.x19/cache/darwinian-evolver
...
```bash
cd ~/.x19/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.x19/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.x19/skills/research/darwinian-evolver
DE_DIR=~/.x19/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 X19 核心中使用 `from darwinian_evolver import ...`。`~/.x19/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
