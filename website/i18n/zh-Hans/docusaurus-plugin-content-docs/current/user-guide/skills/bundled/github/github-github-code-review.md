...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.x19/.env ] && grep -q "^GITHUB_TOKEN=" ~/.x19/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.x19/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${X19_HOME:-$HOME/.x19}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${X19_HOME:-$HOME/.x19}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by X19. Code looks clean — good test coverage, no security concerns."
