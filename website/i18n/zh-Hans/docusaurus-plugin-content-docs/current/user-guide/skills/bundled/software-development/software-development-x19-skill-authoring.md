---
title: "X19 Skill 编写——在仓库中编写 SKILL"
sidebar_label: "X19 Skill 编写"
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/software-development/x19-skill-authoring` |
| 版本 | `1.0.0` |
| 作者 | X19 |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `skills`, `authoring`, `x19`, `conventions`, `skill-md` |
...
# 编写 X19-Agent Skills（仓库内）

## 概述

SKILL.md 可以存放在两个位置：

1. **用户本地：** `~/.x19/skills/<maybe-category>/<name>/SKILL.md` — 个人使用，不共享。通过 `skill_manage(action='create')` 创建。
2. **仓库内（本 skill 讨论此情况）：** `/home/bb/x19/skills/<category>/<name>/SKILL.md` — 已提交，随包一起发布。使用 `write_file` + `git add`。`skill_manage(action='create')` **不**针对此目录树。

## 使用时机

- 用户要求你"在此分支 / 仓库 / 提交中"添加一个 skill
- 你正在提交一个应随 x19 一起发布的可复用工作流
- 你正在编辑 `/home/bb/x19/skills/` 下的现有 skill（小改动用 `patch`，重写用 `write_file`；`skill_manage` 对仓库内 skill 的 `patch` 仍有效，但 `create` 无效）
...
- X19-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
...
1. **对仓库内 skill 使用 `skill_manage(action='create')`。** 它会写入 `~/.x19/skills/`，而非仓库目录树。仓库内创建请使用 `write_file`。
