...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Anony-place/x19-refactored/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    x19_meta = (fm.get("metadata") or {}).get("x19") or {}
    tags = x19_meta.get("tags") or []
    related = x19_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `x19 skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with X19"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "X19 ships with a large built-in skill library copied into `~/.x19/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "X19 also syncs bundled skills on `x19 update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.x19/skills/` tree, it is still shipped with X19; restore it with `x19 skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with x19 — install via x19 skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with x19 under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
