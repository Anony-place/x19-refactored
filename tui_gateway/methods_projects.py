...
def _is_repo_junk(root: str) -> bool:
    ...
    X19_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from x19_constants import get_x19_home
    real = os.path.realpath(root)
    x19_home = os.path.realpath(str(get_x19_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == x19_home
        or real.startswith(x19_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of X19_HOME may be an
    intentional prose/data workspace, so only X19_HOME itself is excluded here."""
    if not cwd:
        return True
    from x19_constants import get_x19_home
    real = os.path.normcase(os.path.realpath(cwd))
    x19_home = os.path.normcase(os.path.realpath(str(get_x19_home())))
    return real in _non_workspace_dirs() or real == x19_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero X19 sessions still appear in the sidebar (#81723).
