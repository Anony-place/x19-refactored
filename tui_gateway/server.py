...
from x19_constants import (
    get_x19_home, get_x19_home_override, get_process_x19_home, profile_name_for_home,
    reset_x19_home_override, set_x19_home_override)
...
from tools.environments.local import x19_subprocess_env
...
_x19_home = _X19_HOME_AT_IMPORT = get_x19_home()
load_x19_dotenv(x19_home=_x19_home, project_env=Path(__file__).parent.parent / ".env")
...
_CRASH_LOG = os.path.join(_x19_home, "logs", "tui_gateway_crash.log")
...
with contextlib.suppress(Exception):
    from x19_cli.banner import prefetch_update_check
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("X19_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("X19_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("X19_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("X19_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "x19_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("x19_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve X19-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
        managed_bin = str(Path(get_x19_home()) / "bin")
...
class _SlashWorker:
    """Persistent X19CLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the X19 → needs provider credentials. Tier-1 secrets
        ...
        # Prepend the X19 venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve X19-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("X19_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("X19_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("X19_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
def _launch_state_db_path() -> Path:
    """Launch profile's ``state.db`` at call time: the patched ``_x19_home`` when a test changed
    it, else the live process home — resolved through :func:`get_process_x19_home`, which honours
    ``X19_HOME`` but ignores the context-local override. The desktop multiplex cron ticker sets
    ...
    import time lets a harness that redirects ``X19_HOME`` after import be honoured (#112692)."""
    home = _x19_home if _x19_home != _X19_HOME_AT_IMPORT else get_process_x19_home()
...
def _profile_db(params: dict | None = None, *, writer: bool = False):
    ...
    x19_cli.web_routers.profiles._read_profile_db."""
    ...
    if (profile_home := _profile_home(profile)) is None:
        ...
        try:
            if writer:
                ...
                from x19_cli.web_server_sessions import _open_session_db_at_path
...
def _canonical_profile_request(name: str) -> str:
    ...
    installation details — unless a real named profile of that name exists (``x19`` is a legal
    id), in which case it wins; other unknown names keep failing closed in ``_profile_home``.
    """
    if name.casefold() in {".x19", "x19"}:
        from x19_cli import profiles as profiles_mod
        # Check the profiles root directly: get_profile_dir rejects "x19" as a
...
# backend, so calls carry ``profile`` → open that profile's db and bind its X19_HOME (ContextVar
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_x19_home).resolve():
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s full runtime scope (X19_HOME + secrets + terminal policy) around a
...
def _bind_build_profile_scopes(profile_home: "str | None") -> "_TurnScopes | None":
    """Bind a session profile's X19_HOME / secret / terminal scopes for an agent build. ``None`` is the
    ...
    if profile_home:  # secret/terminal helper failed: keep at least the home + terminal refusal scope
        scopes.home = set_x19_home_override(profile_home)
        with contextlib.suppress(Exception):
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's X19_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _x19_home) / "config.yaml"
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess X19_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(X19_GATEWAY_SESSION="1", X19_EXEC_ASK="1", X19_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No X19 Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the X19 Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``x19 --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("X19_MODEL", "") or os.environ.get("X19_INFERENCE_MODEL", "")).strip()
...
def _resolve_model() -> str:
    ...
    with contextlib.suppress(Exception):
        from x19_cli.models import get_preferred_silent_default_model
        return get_preferred_silent_default_model()
    return "z-ai/glm-5.2"


def _resolve_session_platform() -> str:
    """``X19_DESKTOP=1`` without ``X19_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``x19 --tui``)."""
    desktop = is_truthy_value(os.environ.get("X19_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("X19_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the X19_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("X19_TUI_PROVIDER", "").strip():
        return model, explicit_provider
    if not (explicit_model := _env_model_seed()):
        return model, None
    with contextlib.suppress(Exception):
        from x19_cli.models import detect_static_provider_for_model
        cfg = _load_cfg().get("model") or {}
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("X19_INFERENCE_PROVIDER", "").strip().lower() or "auto")
...
from x19_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from x19_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider and not _is_routable_provider(provider):
        healed = None
        try:
            from x19_cli.runtime_provider import canonical_custom_identity
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # persist). The full scope, not X19_HOME alone: the external memory provider's system_prompt_block()
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`x19_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("X19_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_X19_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``X19_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a X19_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] X19_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    try:  # (enabled, disabled) MCP server names from raw config; both empty on any failure
        ...
        from x19_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown X19_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in X19_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit X19_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("X19_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        if resolved is not False:
            return resolved
        fallback_notice = "[tui] no valid X19_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from x19_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid X19_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on X19_DEV_CREDITS so the payload stays clean otherwise.
    if is_truthy_value(os.environ.get("X19_DEV_CREDITS")):
        with contextlib.suppress(Exception):
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            with contextlib.suppress(Exception):
                from x19_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from x19_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if provider == "custom" and "provider" not in mirror and agent is not None:
        ...
        with _profile_build_scope(sess.get("profile_home") or _x19_home):
    ...
    if live_agent:
        ...
        with contextlib.suppress(Exception):
            from x19_cli.banner import get_available_skills
    ...
    with contextlib.suppress(Exception):
        from x19_cli.banner import get_update_result
        from x19_cli.config import recommended_update_command
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + X19_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    skill is missing (cli.py parity): a typo'd name must not auto-block the Kanban task."""
    from x19_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `x19 skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("x19_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("X19_IGNORE_RULES"))
    with _sessions_lock:
        session = _sessions.get(sid)
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("X19_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("X19_TUI_PASS_SESSION_ID")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped X19_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str, *, model_history_only: bool = False) -> tuple[list, list, list]:
    ...
    from x19_state import SessionResumeTooLargeError
...
def _pet_gen_root():
    ...
    root = get_x19_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("X19_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`x19 setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`x19 gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`x19 sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`x19 config edit` needs $EDITOR in a real terminal",
}
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `x19` is interactive — use `/x19 chat -q …` or run `x19` in another terminal"
