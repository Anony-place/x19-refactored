...
_SESSION_OWNERSHIP_UNAVAILABLE = "X19 could not safely reserve this session. Try again."
...
def _install_borrowed_lease(sid: str, session: dict, frame: dict) -> None:
    ...
    if not key or str(vouch.get("session_id") or "") != key:
        return
    from x19_cli.active_sessions import ActiveSessionLease
...
def _ensure_active_session_slot(sid: str, session: dict) -> str | None:
    ...
    if limit_message is None:
        ...
        return None
    from x19_cli.active_sessions import SESSION_NOT_OWNED
...
def _take_over_detached_runtime_lease(sid: str, session: dict, key: str) -> bool:
    ...
    """
    from x19_cli.active_sessions import transfer_active_session
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    try:
        from x19_cli.active_sessions import active_session_liveness_guard, release_active_session_liveness_guard
    except Exception as exc:
...
def _pop_session_by_id(sid: str) -> dict | None:
    ...
    with _sessions_lock:
        ...
        if session is not None:
            ...
            home = str(Path(session.get("profile_home") or get_x19_home()).resolve())
...
def _interrupt_session_turn(sid: str, session: dict, *, request_id: str | None = None) -> bool:
    ...
    if should_interrupt:
        ...
        try:
            from x19_cli.plugins import invoke_hook as _invoke_hook
            _invoke_hook(
    ...
    for sid, session in clientless:
        ...
        with _session_resume_lock, _sessions_lock:
            ...
            if current.get("close_on_disconnect"):
                ...
                # `x19 --tui` keeps real _stdio. UNLESS another window (pop-out viewer) still shows the session:
