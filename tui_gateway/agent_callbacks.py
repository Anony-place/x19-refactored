...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from x19_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via x19_cli.personality (single owner)."""
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    from x19_cli.personality import normalize_personality_name, render_personality_prompt
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML for AIAgent (x19_cli.personality owns this)."""
    from x19_cli.personality import prompt_text
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    if env_val := os.environ.get("X19_TUI_MAX_TURNS"):
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("X19_TUI_SKILLS", "")
...
def _load_fallback_model():
    ...
    X19CLI/gateway: ``fallback_providers`` first, legacy ``fallback_model`` merged after)."""
