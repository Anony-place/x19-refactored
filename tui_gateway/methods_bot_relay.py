...
def _relay_root() -> Path:
    ...
    every X19_HOME, including non-``profiles/`` subdirs of ``~/.x19``."""
    from tools.bot_mode_probe import _default_home, _x19_root
    return _x19_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``x19 -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "x19" else profile
        ...
        owner_home = live_home if live_home is not None else Path(_x19_home)
        ...
        fd, tmp = tempfile.mkstemp(prefix="x19-relay-dm-", suffix=".txt", text=True)
        try:
