...
activate with `x19 --tui` or `X19_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `X19_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.x19`. The first non-launch home hosted flips
...
**A method that sets only `get_x19_home_override()` is half-bound**: config paths resolve to the
...
(`x19 -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `x19_cli/commands.py` (`x19_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds x19-ink + tsx --watch)
npm start         # production
npm run build     # full build (x19-ink + tsc)
