...
from x19_constants import display_x19_home as _display_x19_home
...
def _cfg_get_provider(params):
    from x19_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _x19_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_x19_home), "display": _display_x19_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's X19_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``X19_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from x19_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No X19 provider is configured.", source)
            ...
            from x19_cli.anon_auth import route_is_welcome_host
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``x19 debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from x19_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
