# Política de Seguridad de X19

Este documento describe el modelo de confianza de X19, identifica el
...
- Detalles del entorno (`x19 --version`, SHA del commit, SO, versión de Python).
...
X19 es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta X19,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a X19 al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de X19
...
X19 admite dos posturas de aislamiento a nivel de SO. Abordan
...
X19 admite esto de dos maneras:

- **La propia imagen Docker de X19 y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de X19
...
X19 filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en X19
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de X19
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   X19 no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de X19 o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  X19 ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de x19-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de X19
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en X19. Los errores en la ruta de
  instalación de habilidades o plugins de X19 que impidan al
...
- X19 incluye guardias de cadena de suministro para lanzamientos de servidores
