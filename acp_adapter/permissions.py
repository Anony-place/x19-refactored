"""ACP permission bridging for X19 dangerous-command approvals."""
...
# ACP permission option id -> X19 approval result. Ids are stable across the
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True, smart_denied: bool = False,
    ...
    """Return ACP options that match X19 approval semantics."""
    ...
    # DENY override — offering a scope X19 discards would re-prompt every write.
    # See #81887.
    once_only = smart_denied or not allow_session
    options = [PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once")]
    if not once_only:
        # ACP has no session-scoped kind: closest persistent hint, X19 semantics in the id.
...
def _map_outcome_to_x19(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into X19 approval strings."""
    ...
    if outcome.option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_X19.get(outcome.option_id, "deny")
...
def make_approval_callback(request_permission_fn: Callable, loop: asyncio.AbstractEventLoop,
    ...
    def _callback(command: str, description: str, *, allow_permanent: bool = True,
        ...
        if response is None:
            return "deny"
        return _map_outcome_to_x19(response.outcome, allowed_option_ids={option.option_id for option in options})
