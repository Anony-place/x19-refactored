...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"X19 — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from x19_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str, *, deferred: Optional[dict] = None,
    ...
    try:
        source_home = get_x19_home().resolve()
    ...
    # The running install first (same trust order as gateway.run._resolve_x19_bin): the
    # scheduler lives in the long-running gateway, so a PATH-first lookup would hand delivery
    # to whatever `x19` PATH names — another install, or a planted one — instead of this one.
    try:
        import importlib.util as _ilu
        found = _ilu.find_spec("x19_cli") is not None
    except Exception:
        found = False
    if found:
        argv = [sys.executable, "-m", "x19_cli.main"]
    else:
        x19_bin = shutil.which("x19")
        if not x19_bin:
            return ("X19 could not deliver this result to Bot Chat: the `x19` command was not found. "
                    "The result is saved; run `x19 cron runs` to see it, or `x19 doctor` if this keeps happening")
        argv = [x19_bin]
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="x19-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
        ...
        if result.returncode != 0:
            ...
            return (
                f"X19 could not deliver this result to Bot Chat (profile '{profile_label}'). "
                "The result is saved; run `x19 cron runs` to see it, or `x19 doctor` if this keeps happening"
                f". Details: {tail}")
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`x19 cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_X19_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `x19 cron list` shows it.
    ...
    # at boot; standalone runs (`x19 cron run`) did not, silently dropping files. Idempotent.
