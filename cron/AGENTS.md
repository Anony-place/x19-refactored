...
schedule via the `cronjob` tool; users via `x19 cron list|add|edit|pause|resume|run|remove` or
...
- **Inactivity watchdog** on cron agent sessions (`_cron_inactivity_seconds()`): default 600s idle,
  `X19_CRON_TIMEOUT` overrides, `0` = unlimited. It is idle time, not wall-clock — a stalled
...
- Per-home tick lock `<home>/cron/.tick.lock` prevents duplicate ticks across processes for
  that profile's store; never a `~/.x19/...` literal.
...
- The cron ticker runs in the desktop-spawned backend when `X19_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `x19 kanban
...
- **CLI:** `x19_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`x19 tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/x19-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `X19_KANBAN_BOARD` pinned in their env and
...
- **Notifications leave through the task's owning profile.** `x19_cli/kanban_db_notify.py`
  ...
  Dispatched workers get `X19_KANBAN_BOARD` and the assignee's `X19_HOME` pinned in a
...
- **Prompt injection sites gate on ownership, not tool access.** Tool access (`kanban_show` visible
  via a profile's toolset) and an inherited `X19_KANBAN_TASK` (delegate children, cron runs beside
...
- **Descendant fence is a path, not a flag.** A delegated child's Kanban marker
  ...
  `kanban_path_is_fenced(path)` denies mutations only on the dispatcher-pinned `X19_KANBAN_DB`
  or under that root, so a child working against a scratch `X19_HOME` keeps a writable board.

## Tests

`tests/cron/`, `tests/x19_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
