...
``cron.scheduler``, not ``x19_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips X19-owned PYTHONPATH entries because user
children must not see our tree. This child IS X19, so the pin is applied *after* the
...
def pin_x19_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
