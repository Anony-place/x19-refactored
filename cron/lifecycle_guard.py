...
A cron job that restarts/stops the gateway from inside the gateway (``x19 gateway restart``,
``launchctl kickstart ai.x19.gateway``, ``systemctl restart x19-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `x19 gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `x19` from being a path component or word tail (`/docs/x19 gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])x19\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a x19-gateway label so unrelated x19 services stay
    ...
    # -l ai.x19.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a x19-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.x19.gateway-... \` / `  -- ...`,
...
# Branch A2: `x19 -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `x19 -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the x19-gateway label anchor
...
_X19_GATEWAY_LABEL_RE = re.compile(r"(?i)\bx19[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``X19_PROFILE_NAME``/``X19_PROFILE`` env first, then
    ``x19_cli.profiles.get_active_profile_name`` (from ``X19_HOME``); ``None`` if neither."""
    for env_name in ("X19_PROFILE_NAME", "X19_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _X19_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``x19``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: x19 gateway
    ...
    # Profile-flag form (#78028): `x19 -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `x19` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<X19_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_x19_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_x19_home() falls back to Path.home(), which raises when neither X19_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.x19/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `x19 gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `x19 gateway restart` from a shell outside "
