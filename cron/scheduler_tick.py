...
def _tick_admitted(
    ...
    try:
        # `x19 pause` ESTOP: skip dispatch, never touch in-flight runs; check_paused logs once.
        ...
        if not due_jobs:
            ...
            if verbose:
                ...
                # due.
                _sched.logger.info("%s - No jobs due", _sched._x19_now().strftime('%H:%M:%S'))
            _sched._sweep_mcp_orphans()
            return 0

        if verbose:
            _sched.logger.info("%s - %s job(s) due", _sched._x19_now().strftime('%H:%M:%S'), len(due_jobs))
