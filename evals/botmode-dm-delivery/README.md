...
Real Electron, production Python backend and tool execution, disposable HOME/X19_HOME,
...
(cd apps/desktop && DISPLAY=:0 XAUTHORITY=/run/user/1000/xauth_cnpsqU \
  ...
  X19_DESKTOP_CDP_PORT=off npx playwright test e2e/probe-dm-delivery.spec.ts --reporter=list)
...
fixture's generated x19 shim pins every child to this checkout, not an installed launcher.
...
custom X19 root, and holds the real quiet CLI child at loopback inference.
...
creates a second `.x19` root under HOME.
...
actual Desktop label is X19. No production failure is claimed for that attempt.
