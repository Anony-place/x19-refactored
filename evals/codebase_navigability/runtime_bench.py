...
Each probe runs in a FRESH subprocess with an isolated X19_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "X19_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "X19_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("X19_SESSION", "X19_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "x19_cli.main", "gateway.run", "tools.registry", "x19_state", "tui_gateway.server", "x19_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "x19" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `x19 --version`, `x19 --help`, `x19 doctor --help`, `x19 config get model` (no network)
CLI = {"version": ["x19_cli/main.py", "--version"], "help": ["x19_cli/main.py", "--help"], "config_get": ["x19_cli/main.py", "config", "get", "model"], "tools_list": ["x19_cli/main.py", "tools", "--help"], "skills_help": ["x19_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_x19_default_ms", lambda: resolve_toolset("x19-default"), 200)
...
try:
    import x19_state, tempfile, uuid
    from pathlib import Path
    db=x19_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(x19_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
