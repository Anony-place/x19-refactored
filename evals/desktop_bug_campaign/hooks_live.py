...
env = {k: v for k, v in os.environ.items() if not (k.startswith('X19_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), X19_HOME=str(home), X19_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', X19_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'x19_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
