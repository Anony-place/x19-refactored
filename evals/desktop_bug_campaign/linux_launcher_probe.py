...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "X19_HOME": str(root / "home/.x19"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "X19_NONINTERACTIVE": "1"}
    Path(env["X19_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from x19_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'x19')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,x19_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,x19_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=X19\n", b"Name=X19 custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["X19_HOME"]) / "config.yaml"
