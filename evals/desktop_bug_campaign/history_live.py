...
env.update(HOME=str(home), X19_HOME=str(home), X19_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, X19_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'x19_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-X19-Token':'history-fixture-token'})
