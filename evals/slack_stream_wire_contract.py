...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "x19", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="X19 is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("X19_HOME", os.path.join(os.path.dirname(a.out), "x19-home-probe"))
    os.makedirs(os.environ["X19_HOME"], exist_ok=True)
