...
measured in the production session DB) through `x19 chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    X19_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured X19 home with credentials for the
...
(tracker: Anony-place/x19-refactored#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "X19_HOME": str(HOME),
                "X19_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "x19_cli.main", "chat", "--query", q,
