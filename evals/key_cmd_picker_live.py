...
Run with the X19 venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by X19 children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "x19_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="x19-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".x19"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "X19_HOME": str(state),
