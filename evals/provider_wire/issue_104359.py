...
home = tempfile.mkdtemp(prefix="x19-104359-")
os.environ.clear()
os.environ.update(
    HOME=home,
    X19_HOME=home + "/.x19",
...
Path(os.environ["X19_HOME"], "config.yaml").write_text(yaml.safe_dump(config), encoding="utf-8")
...
db = SessionDB(Path(os.environ["X19_HOME"]) / "state.db")
