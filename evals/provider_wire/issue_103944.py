...
os.environ.clear()
os.environ.update(
    HOME=home, X19_HOME=home + "/.x19", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["X19_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["X19_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["X19_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from x19_cli.config import get_compatible_custom_providers, load_config
from x19_cli.config_providers import get_custom_provider_context_length
