...
def main():
    ...
    if not args.child:
        with tempfile.TemporaryDirectory(prefix="kanban-identity-") as home:
            env = {"HOME": home, "X19_HOME": home + "/x19", "PATH": os.defpath,
    ...
    from x19_cli import kanban_db as kb, kanban_db_connect as kbc
    from x19_cli.kanban_db_dispatch import dispatch_once
    from tools.kanban_tools import _handle_create
    from x19_cli.kanban_decompose import _apply_fanout, _Routing
    graph = importlib.import_module("x19_cli.kanban_db_graph") if (args.repo / "x19_cli/kanban_db_graph.py").exists() else kb
