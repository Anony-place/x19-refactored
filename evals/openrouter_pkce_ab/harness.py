#!/usr/bin/env python3
"""Local A/B harness for `x19 auth add openrouter --type oauth` against a FAKE OpenRouter.

Runs the REAL entry point (``x19_cli.auth_commands.auth_add_command``) with a temp X19_HOME.
...
Scenarios (each prints PASS/FAIL, exit code = number of failures):
  ...
  api_key_path   `x19 auth add openrouter --api-key` still works with no --type (regression)

Usage: X19_PYTHON=<venv python> python3 evals/openrouter_pkce_ab/harness.py [--json OUT]
...
def run(scenario: str, fake: FakeOpenRouter, home: str) -> dict:
    ...
    for m in [m for m in sys.modules if m.startswith(("x19_cli", "agent", "x19_constants"))]:
        del sys.modules[m]
    ...
    try:  # BEFORE (origin/main) has no auth_openrouter sibling; the flow itself must then fail.
        ...
        import x19_cli.auth_device_flow as dfl
