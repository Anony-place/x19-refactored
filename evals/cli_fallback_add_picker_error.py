"""Drive the real ``x19 fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from x19_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="x19_test_fallback_") as home:
        hh = Path(home) / ".x19"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "X19_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "x19_cli.main", "fallback", "add"],
