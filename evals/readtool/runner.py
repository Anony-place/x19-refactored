"""Run the read-tool eval through the REAL X19 AIAgent.

For each task: fresh temp X19_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    x19_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".x19"
    ...
    try:
        ...
        shutil.rmtree(x19_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.x19/.env; set +a  — then relaunch."
        )
