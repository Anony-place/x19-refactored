...
Run with a fresh HOME and X19_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" X19_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `X19_EVAL_REPO` selects another checkout for A/B.
