...
def child(out: Path) -> None:
    ...
    runner.session_store = SessionStore(Path(os.environ["X19_HOME"]) / "sessions", runner.config)
    ...
    cli = X19CLI(model="fixture", provider="openai-compat", api_key="fixture", base_url=base_url)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="context-display-") as home:
        ...
        if pid == 0:
            os.environ.clear()
            os.environ.update(HOME=home, X19_HOME=home + "/.x19", PATH="/usr/bin:/bin", TERM="dumb", NO_COLOR="1")
            try:
                child(out)
            finally:
                sys.stdout.flush()
