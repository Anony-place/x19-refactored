#!/usr/bin/env bash
# Live A/B for the post-swap hand-off (x19_cli/update_handoff.py).
#
# Installs a real clone at <installed-sha>, publishes a target commit on its origin that
# (a) adds a symbol to x19_logging (purge-protected on the old updater),
# (b) imports it at module level from x19_cli/config.py (re-imported by the old updater's
...
# then runs `x19 update` from the installed clone and reads the receipt + output.
#
...
# Needs: uv on PATH, a Python for the venv. Runs with a disposable HOME/X19_HOME, no npm
# on PATH (skips Node/web/Desktop work) and --no-gateway-restart (never touches your fleet).
set -euo pipefail
REPO=$1; SHA=$2; LABEL=$3
ROOT=$(mktemp -d /tmp/x19-post-swap-ab.XXXXXX)
...
(
  ...
  python3 - <<'PY'
import re
p = "x19_cli/config.py"; s = open(p).read()
s = s.replace("import logging\n", "import logging\nfrom x19_logging import post_swap_probe  # noqa: F401  (A/B probe: new symbol on a cached module)\n", 1)
open(p, "w").write(s)
p = "x19_cli/update_cmd_maint.py"; s = open(p).read()
needle = 'def _update_complete_message('
i = s.index(needle); j = s.index('\n', s.index('"""', s.index('"""', i) + 3)) + 1  # end of docstring
s = s[:j] + '    from x19_logging import post_swap_probe\n    print(f"  [completion printed by: {post_swap_probe()}]")\n' + s[j:]
open(p, "w").write(s)
PY
  git -c user.name=ab -c user.email=ab@example.invalid commit -qam "probe: new x19_logging symbol used by config.py and the completion line"
  git push -q origin main
)

# Venv for the installed clone (deps unchanged between the two commits → the updater skips the sync).
uv venv -q --python "${X19_AB_PYTHON:-3.11}" "$ROOT/install/.venv"
uv pip install -q --python "$ROOT/install/.venv/bin/python" -e "$ROOT/install"

export HOME="$ROOT/home" X19_HOME="$ROOT/home/.x19"
mkdir -p "$X19_HOME"
export PATH="$ROOT/install/.venv/bin:/usr/bin:/bin"
set +e
( cd "$ROOT/install" && python -m x19_cli.main update --yes --no-backup --no-gateway-restart ) > "$ROOT/update.out" 2>&1
CODE=$?
set -e
...
python3 - "$X19_HOME/logs/update_receipts/latest.json" <<'PY'
import json, sys
try:
