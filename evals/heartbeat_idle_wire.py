...
Run from the repo with a clean environment and a temporary X19_HOME:
...
from x19_cli import heartbeat  # noqa: E402
...
async def main(base_poller):
    assert os.environ.get("X19_HOME"), "Use a temporary X19_HOME"
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["X19_HOME"]) / "sessions", config=runner.config)
