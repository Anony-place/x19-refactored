...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='x19-acp-empty-'))
    x19 = home / '.x19'
    x19.mkdir()
    (x19 / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), X19_HOME=str(x19), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               X19_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(x19 / 'state.db') as db:
