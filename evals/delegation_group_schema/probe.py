...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, X19_HOME=home, PATH="/usr/bin:/bin", X19_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["x19-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["x19-cli"], quiet_mode=True)
