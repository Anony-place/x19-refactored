...
# --- hermetic env BEFORE any x19 import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("X19_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
x19_home = os.path.join(tmp_root, ".x19")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(x19_home)
os.makedirs(workspace)
with open(os.path.join(x19_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the X19 desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "x19_home": x19_home,
