...
ctx keys: workspace, x19_home, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire X19_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["x19_home"]):
        for fn in files:
            try:
