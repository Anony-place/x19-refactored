...
sandbox = tempfile.TemporaryDirectory(prefix="x19-104260-")
os.environ.clear()
os.environ.update(
    HOME=sandbox.name,
    X19_HOME=sandbox.name + "/x19",
    PATH="/usr/bin:/bin",
    PYTHONDONTWRITEBYTECODE="1",
    X19_DISABLE_MODEL_METADATA_FETCH="1",
)
Path(os.environ["X19_HOME"]).mkdir()
...
# JSON is valid YAML; only the temporary X19 home is written.
Path(os.environ["X19_HOME"], "config.yaml").write_text(
