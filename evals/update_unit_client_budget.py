...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), X19_HOME=str(home / "x19"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("x19-gateway*", "x19-serve*")] + [unit + ".service"]
...
unit = f"x19-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
