...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["X19_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(X19TokenStorage("positive", x19_home=owner).get_tokens())
        ...
        override = set_x19_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from x19_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="x19-oauth-http-") as temp:
            home = Path(temp)
            (home / ".x19").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "X19_HOME": str(home / ".x19"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
