...
- For `--live` probes: real credentials in `X19_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.x19/logs/agent.log*"
...
X19 recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp X19_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel X19 subagents (2026-09-04), rewritten here to take `--db`/`--root`
