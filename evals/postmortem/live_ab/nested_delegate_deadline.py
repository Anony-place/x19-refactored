...
# Temp X19_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
home = tempfile.mkdtemp(prefix="dl_home_"); os.environ["X19_HOME"] = home
real_home = os.environ.get("X19_HOME_SOURCE", os.path.expanduser("~/.x19"))  # credentials are copied from here into a temp home
