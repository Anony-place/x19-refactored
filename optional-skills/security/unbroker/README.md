...
The design is **X19-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
x19 skills install official/security/unbroker
```

Then start a new X19 session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the X19 home directory (`0600`, optionally
