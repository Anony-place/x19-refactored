...
All per-subject data lives under PDD_DATA_DIR (default: $X19_HOME/unbroker),
which is the same trust boundary X19 uses for .env and OAuth tokens.
"""
...
def x19_home() -> Path:
    return Path(os.environ.get("X19_HOME") or (Path.home() / ".x19"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else x19_home() / "unbroker"
