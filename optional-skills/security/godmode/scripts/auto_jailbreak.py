...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("X19_HOME", os.path.expanduser("~/.x19")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("X19_HOME", Path.home() / ".x19")) / "skills" / "red-teaming" / "godmode"
...
PREFILL_PATH = X19_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/X19 models — already uncensored, just needs clean prompt
    "x19": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "x19" in model_lower or "nous" in model_lower:
        return "x19"
...
def _get_current_model() -> tuple:
    """Read current model and provider from X19 config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.x19/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart X19 for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart X19 for changes to take effect.")
