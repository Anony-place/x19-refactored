...
is typically only in `~/.x19/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike X19,
...
`x19-custom-providers` skill, pitfall 2). X19 hits this because it forwards
