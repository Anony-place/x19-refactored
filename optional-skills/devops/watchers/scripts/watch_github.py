...
Usage (via cron with --no-agent):
    ...
    x19 cron create x19-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$X19_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name x19-issues --repo Anony-place/x19-refactored --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the X19 .env file
(``${X19_HOME:-~/.x19}/.env``) to avoid the 60 req/hr
