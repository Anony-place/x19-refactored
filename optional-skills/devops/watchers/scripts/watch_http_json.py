...
Usage (via cron with --no-agent):
    ...
    x19 cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$X19_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "X19-Watcher/1.0"})
