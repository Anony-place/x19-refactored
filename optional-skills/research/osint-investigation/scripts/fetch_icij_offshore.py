...
$X19_OSINT_CACHE/icij/ (default: ~/.cache/x19-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("X19_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "x19-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "x19 osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $X19_OSINT_CACHE/icij or ~/.cache/x19-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
