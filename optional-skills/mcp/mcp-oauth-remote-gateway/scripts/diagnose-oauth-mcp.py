...
Usage:
  ...
  <server_name> matches the files in $X19_HOME/mcp-tokens/<server>.json etc.
...
def _x19_home():
    # Prefer X19' own resolver (profile-safe); fall back to env then ~/.x19.
    try:
        from x19_constants import get_x19_home
        return str(get_x19_home())
    except Exception:
        return os.environ.get("X19_HOME") or os.path.expanduser("~/.x19")


def _tokens_dir():
    return os.path.join(_x19_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "x19-diag", "version": "1.0"}},
        },
        headers={
