#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `x19 kanban list` and
...
def x19_available() -> bool:
    return shutil.which("x19") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed x19 CLI."""
    try:
        out = subprocess.run(
            ["x19", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `x19 kanban list`
    out = subprocess.run(
        ["x19", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not x19_available():
        print("ERROR: 'x19' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
