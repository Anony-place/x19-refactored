...
Maps each role archetype to the X19 skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public x19
...
## X19 skills relevant to video production

### Visual / rendering skills (`x19/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `x19 plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`x19/skills/creative/`)
...
### Audio / media skills (`x19/skills/creative/` + `skills/media/`)
...
These are **not** X19 skills but external CLIs / APIs that profiles invoke.
...
## Built-in X19 tools for media review

These are native X19 tools — not invoked via terminal but through their own
...
key is present in `${X19_HOME:-~/.x19}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every X19 profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${X19_HOME:-~/.x19}/.env` → environment variable.
...
When a new X19-public video skill ships:
