...
the actual `setup.sh` that creates the project workspace, configures X19
...
2. **Create profiles** — `x19 profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.x19/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `x19 kanban create` assigned to the director
...
```bash
x19 profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.x19/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.x19/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most X19
...
Each profile gets a `SOUL.md` at `~/.x19/profiles/<name>/SOUL.md` that
...
```bash
x19 kanban create "Direct production of <video title>" \
    --assignee director \
...
the X19 `.env` (`${X19_HOME:-$HOME/.x19}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _x19_env="${X19_HOME:-$HOME/.x19}/.env"
    if grep -q "^${var}=" "$_x19_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_x19_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_x19_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY x19 ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY x19 OPENROUTER_API_KEY || exit 1
# ...
```
