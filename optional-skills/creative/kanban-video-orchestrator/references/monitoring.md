...
x19 kanban watch --tenant <project-slug>

# Snapshot of the board
x19 kanban list --tenant <project-slug>
x19 kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
x19 kanban stats --tenant <project-slug>

# Visual dashboard (browser)
x19 dashboard

# Inspect a specific task (includes comments + events)
x19 kanban show <task-id>

# Follow a single task's event stream
x19 kanban tail <task-id>
```

Verify available subcommands with `x19 kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `x19 kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `x19 kanban block <id>` then `x19 kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `x19 kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `x19 kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `x19 profile list` (profile exists?), `x19 status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(x19 kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
x19 kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
x19 kanban block <task-id>
x19 kanban archive <task-id>

# Diagnose what happened
x19 kanban show <task-id>      # task body, comments, recent events
x19 kanban tail <task-id>      # follow the live event stream
x19 kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `x19 kanban create` for the director
...
while true; do
    clear
    x19 kanban list --tenant <slug>
    echo "---"
    x19 kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `x19 kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `x19 profile list` and start
