...
### 1. Start order: editor first, X19 second

X19 probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW X19 session.
...
restart the editor, restart the X19 session.
...
server fails to bind (Output Log) or X19' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.x19/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for X19

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. X19' connection
lives in `~/.x19/config.yaml` via `x19 mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms X19 — just don't
mistake it for the X19 setup step.
...
explicitly warns against overlapping calls. X19 executes same-turn tool
...
### 9. Timeouts: X19 gives up before Unreal does

X19' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If X19 runs elsewhere (SSH backend,
