...
Under X19 that means each tool becomes `mcp_unreal_engine_<tool_name>` at
session start, and `x19 mcp configure unreal-engine` can prune the list.
...
| Server Port Number | `8000` | Change on conflict; mirror in X19 config url |
...
| `ModelContextProtocol.GenerateClientConfig <Client\|All>` | Write client config files (ClaudeCode/Cursor/VSCode/Gemini/Codex) — NOT used for X19 |
...
- **After Live Coding / authoring:** connected clients can hold stale
  schemas. `ModelContextProtocol.RefreshTools`, then reconnect (new X19
...
then re-`list_toolsets` from X19. Users on Claude Code can scaffold with
