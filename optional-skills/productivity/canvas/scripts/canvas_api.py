...
def _check_config():
    ...
    if missing:
        x19_env = os.path.join(
            os.environ.get("X19_HOME", os.path.expanduser("~/.x19")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {x19_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for X19"
    )
