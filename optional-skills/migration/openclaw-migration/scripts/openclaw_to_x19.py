#!/usr/bin/env python3
"""OpenClaw -> X19 migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into X19, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into X19.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into X19 memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into X19 memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import X19-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of X19-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into X19 command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.x19/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.x19/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into X19 .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into X19 .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into X19 .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into X19 .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into X19 .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into X19 config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into X19 config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into X19.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into X19 MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into X19 config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'x19 cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into X19 config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "full-providers": {
        ...
        "description": "Import custom model providers (baseUrl, apiType, headers) into X19 custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into X19 config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into X19 config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into X19 config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `x19 config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`x19 config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.x19/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update X19 config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with X19 in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``X19`` (prose), but lowercase matches
# like ``openclaw`` → ``x19`` (so filesystem paths like ``~/.openclaw``
# become ``~/.x19`` — the real X19 home — not the broken ``~/.X19``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'X19'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'X19'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'X19'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``X19`` but maps ``openclaw`` → ``x19`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.x19/config.yaml`` (the real X19 home) instead of the broken
    ``~/.X19/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with X19.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION X19 memory store (memories/MEMORY.md, USER.md).
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-x19/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the X19 config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-x19/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new X19 session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "X19 config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the X19
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No X19-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted X19-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: x19 config set",
                        )
                    continue
        ...
        for oc_key, x19_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and x19_key not in secret_additions:
                secret_additions[x19_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, x19_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and x19_key not in secret_additions:
                        secret_additions[x19_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        x19_config = load_yaml_file(destination)
        current_model = x19_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = x19_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                x19_config["model"] = {"default": model_str}
            dump_yaml_file(destination, x19_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; X19 still uses "edge"
        ...
        x19_config = load_yaml_file(destination)
        existing_tts = x19_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct X19 destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected X19-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected X19-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        x19_cfg_path = self.target_root / "config.yaml"
        x19_cfg = load_yaml_file(x19_cfg_path)
        existing_mcp = x19_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in X19 config")
                continue

            x19_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                x19_srv["command"] = srv["command"]
                if srv.get("args"):
                    x19_srv["args"] = srv["args"]
                if srv.get("env"):
                    x19_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    x19_srv["cwd"] = srv["cwd"]
            ...
            if srv.get("url"):
                x19_srv["url"] = srv["url"]
                if srv.get("headers"):
                    x19_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    x19_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                x19_srv["enabled"] = False
            if srv.get("timeout"):
                x19_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                x19_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                x19_srv["tools"] = {}
                if tools_cfg.get("include"):
                    x19_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    x19_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(x19_cfg_path)
            x19_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(x19_cfg_path, x19_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'x19 cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = x19_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> X19 reasoning_effort
        ...
        if compaction:
            compression = x19_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'x19 gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("X19_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if session.get("reset") or session.get("resetTriggers") or session.get("reset_triggers"):
            self.record("session-config", "session reset timers", None, "skipped",
                        "X19 conversations do not reset on idle or daily timers")
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        x19_cfg_path = self.target_root / "config.yaml"
        x19_cfg = load_yaml_file(x19_cfg_path)
        custom_providers = x19_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(x19_cfg_path)
            x19_cfg["custom_providers"] = custom_providers
            dump_yaml_file(x19_cfg_path, x19_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to X19 config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            x19_cfg_path = self.target_root / "config.yaml"
            x19_cfg = load_yaml_file(x19_cfg_path)
            discord_x19 = x19_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_x19["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_x19["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                x19_cfg["discord"] = discord_x19
                dump_yaml_file(x19_cfg_path, x19_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        x19_cfg_path = self.target_root / "config.yaml"
        x19_cfg = load_yaml_file(x19_cfg_path)
        browser_x19 = x19_cfg.get("browser") or {}
        changed = False

        # Map fields that have X19 equivalents
        if browser.get("cdpUrl"):
            browser_x19["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_x19["headless"] = browser["headless"]
            changed = True

        if changed:
            x19_cfg["browser"] = browser_x19
            if self.execute:
                self.maybe_backup(x19_cfg_path)
                dump_yaml_file(x19_cfg_path, x19_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            x19_mode = mode_map.get(mode, "manual")
            x19_cfg.setdefault("approvals", {})["mode"] = x19_mode
            if self.execute:
                self.maybe_backup(x19_cfg_path)
                dump_yaml_file(x19_cfg_path, x19_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{x19_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in X19. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing X19 Config Not Overwritten)",
                "",
                "These items already existed in your X19 config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the X19 discovers these",
            "directories, it may read/write to them instead of the X19 state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `x19 claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `x19 claw cleanup` to fix it.",
            "",
            "## X19-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `x19 claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `x19 setup` to configure any remaining settings",
            "- Run `x19 mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `x19 cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `x19 cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `x19 gateway install` if you need the gateway service",
            "- Review `~/.x19/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into X19.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("X19_HOME") or str(Path.home() / ".x19"), help="X19 home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing X19 targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of X19-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> X19 Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.x19/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.x19/config.yaml")
        print("    2. Run: x19 mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: x19 cron")
